# `haproxy:3.2-dev5-alpine`

## Docker Metadata

- Image ID: `sha256:ee6734e372f953c5328a903c65c2b54ca8c00b42b9c2148cc7fbd38ad63aebc5`
- Created: `2025-02-08T06:13:37Z`
- Virtual Size: ~ 36.08 Mb  
  (total size of all layers on-disk)
- Arch: `linux`/`amd64`
- Entrypoint: `["docker-entrypoint.sh"]`
- Command: `["haproxy","-f","/usr/local/etc/haproxy/haproxy.cfg"]`
- Environment:
  - `PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin`
  - `HAPROXY_VERSION=3.2-dev5`
  - `HAPROXY_URL=https://www.haproxy.org/download/3.2/src/devel/haproxy-3.2-dev5.tar.gz`
  - `HAPROXY_SHA256=c47007b577bee3ccbd7658dae4bd1e1aabb925e43c3a009e0f2c3660b9130fba`

## `apk` (`.apk`-based packages)

### `apk` package: `alpine-baselayout`

```console
alpine-baselayout-3.6.8-r1 description:
Alpine base dir structure and init scripts

alpine-baselayout-3.6.8-r1 webpage:
https://git.alpinelinux.org/cgit/aports/tree/main/alpine-baselayout

alpine-baselayout-3.6.8-r1 installed size:
6532 B

alpine-baselayout-3.6.8-r1 license:
GPL-2.0-only

```

### `apk` package: `alpine-baselayout-data`

```console
alpine-baselayout-data-3.6.8-r1 description:
Alpine base dir structure and init scripts

alpine-baselayout-data-3.6.8-r1 webpage:
https://git.alpinelinux.org/cgit/aports/tree/main/alpine-baselayout

alpine-baselayout-data-3.6.8-r1 installed size:
18 KiB

alpine-baselayout-data-3.6.8-r1 license:
GPL-2.0-only

```

### `apk` package: `alpine-keys`

```console
alpine-keys-2.5-r0 description:
Public keys for Alpine Linux packages

alpine-keys-2.5-r0 webpage:
https://alpinelinux.org

alpine-keys-2.5-r0 installed size:
13 KiB

alpine-keys-2.5-r0 license:
MIT

```

### `apk` package: `alpine-release`

```console
alpine-release-3.21.2-r0 description:
Alpine release data

alpine-release-3.21.2-r0 webpage:
https://alpinelinux.org

alpine-release-3.21.2-r0 installed size:
346 B

alpine-release-3.21.2-r0 license:
MIT

```

### `apk` package: `apk-tools`

```console
apk-tools-2.14.6-r2 description:
Alpine Package Keeper - package manager for alpine

apk-tools-2.14.6-r2 webpage:
https://gitlab.alpinelinux.org/alpine/apk-tools

apk-tools-2.14.6-r2 installed size:
247 KiB

apk-tools-2.14.6-r2 license:
GPL-2.0-only

```

### `apk` package: `busybox`

```console
busybox-1.37.0-r9 description:
Size optimized toolbox of many common UNIX utilities

busybox-1.37.0-r9 webpage:
https://busybox.net/

busybox-1.37.0-r9 installed size:
798 KiB

busybox-1.37.0-r9 license:
GPL-2.0-only

```

### `apk` package: `busybox-binsh`

```console
busybox-binsh-1.37.0-r9 description:
busybox ash /bin/sh

busybox-binsh-1.37.0-r9 webpage:
https://busybox.net/

busybox-binsh-1.37.0-r9 installed size:
1 B

busybox-binsh-1.37.0-r9 license:
GPL-2.0-only

```

### `apk` package: `ca-certificates`

```console
ca-certificates-20241121-r1 description:
Common CA certificates PEM files from Mozilla

ca-certificates-20241121-r1 webpage:
https://www.mozilla.org/en-US/about/governance/policies/security-group/certs/

ca-certificates-20241121-r1 installed size:
251 KiB

ca-certificates-20241121-r1 license:
MPL-2.0 AND MIT

```

### `apk` package: `ca-certificates-bundle`

```console
ca-certificates-bundle-20241121-r1 description:
Pre generated bundle of Mozilla certificates

ca-certificates-bundle-20241121-r1 webpage:
https://www.mozilla.org/en-US/about/governance/policies/security-group/certs/

ca-certificates-bundle-20241121-r1 installed size:
217 KiB

ca-certificates-bundle-20241121-r1 license:
MPL-2.0 AND MIT

```

### `apk` package: `libcrypto3`

```console
libcrypto3-3.3.2-r6 description:
Crypto library from openssl

libcrypto3-3.3.2-r6 webpage:
https://www.openssl.org/

libcrypto3-3.3.2-r6 installed size:
4607 KiB

libcrypto3-3.3.2-r6 license:
Apache-2.0

```

### `apk` package: `libssl3`

```console
libssl3-3.3.2-r6 description:
SSL shared libraries

libssl3-3.3.2-r6 webpage:
https://www.openssl.org/

libssl3-3.3.2-r6 installed size:
779 KiB

libssl3-3.3.2-r6 license:
Apache-2.0

```

### `apk` package: `lua5.4-libs`

```console
lua5.4-libs-5.4.7-r0 description:
Lua dynamic library runtime

lua5.4-libs-5.4.7-r0 webpage:
https://www.lua.org/

lua5.4-libs-5.4.7-r0 installed size:
214 KiB

lua5.4-libs-5.4.7-r0 license:
MIT

```

### `apk` package: `musl`

```console
musl-1.2.5-r8 description:
the musl c library (libc) implementation

musl-1.2.5-r8 webpage:
https://musl.libc.org/

musl-1.2.5-r8 installed size:
646 KiB

musl-1.2.5-r8 license:
MIT

```

### `apk` package: `musl-utils`

```console
musl-utils-1.2.5-r8 description:
the musl c library (libc) implementation

musl-utils-1.2.5-r8 webpage:
https://musl.libc.org/

musl-utils-1.2.5-r8 installed size:
102 KiB

musl-utils-1.2.5-r8 license:
MIT AND BSD-2-Clause AND GPL-2.0-or-later

```

### `apk` package: `pcre2`

```console
pcre2-10.43-r0 description:
Perl-compatible regular expression library

pcre2-10.43-r0 webpage:
https://pcre.org/

pcre2-10.43-r0 installed size:
707 KiB

pcre2-10.43-r0 license:
BSD-3-Clause

```

### `apk` package: `scanelf`

```console
scanelf-1.3.8-r1 description:
Scan ELF binaries for stuff

scanelf-1.3.8-r1 webpage:
https://wiki.gentoo.org/wiki/Hardened/PaX_Utilities

scanelf-1.3.8-r1 installed size:
65 KiB

scanelf-1.3.8-r1 license:
GPL-2.0-only

```

### `apk` package: `ssl_client`

```console
ssl_client-1.37.0-r9 description:
External ssl_client for busybox wget

ssl_client-1.37.0-r9 webpage:
https://busybox.net/

ssl_client-1.37.0-r9 installed size:
14 KiB

ssl_client-1.37.0-r9 license:
GPL-2.0-only

```

### `apk` package: `zlib`

```console
zlib-1.3.1-r2 description:
A compression/decompression Library

zlib-1.3.1-r2 webpage:
https://zlib.net/

zlib-1.3.1-r2 installed size:
101 KiB

zlib-1.3.1-r2 license:
Zlib

```
