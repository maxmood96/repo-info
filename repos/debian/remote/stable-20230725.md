## `debian:stable-20230725`

```console
$ docker pull debian@sha256:38933224bd9bed101d6abdc23f17e4f85e6da4a8c504bb6b294074d4a9b0a0b7
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:stable-20230725` - linux; amd64

```console
$ docker pull debian@sha256:15a7a83dce5973f382aa2e5eaba7e01f1cfe28e45303b3b258e30582877c0685
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49557366 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f0f73f12d5450e32c6e7845b313ea561942bba0638831d6c62f36cc8e34680f1`
-	Default Command: `["bash"]`

```dockerfile
# Thu, 27 Jul 2023 23:26:44 GMT
ADD file:467d54fcdcc4a4fc484eb81623a50891c72242f1da2748a7616413dc449fad22 in / 
# Thu, 27 Jul 2023 23:26:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1924af148565f8d4a8be4dab08aa865788130cd4461a64b42f5470404fb3f613`  
		Last Modified: Thu, 27 Jul 2023 23:32:41 GMT  
		Size: 49.6 MB (49557366 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20230725` - linux; arm variant v5

```console
$ docker pull debian@sha256:7fa6612989a597e2cf91f5e8948d0b77456b2387c50eb49f9050aec0a2bfe61d
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.4 MB (47414900 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:46e71de62e3f4887ca67517924f6987c4bf0dbecd42e0cb3799dcecbee93da40`
-	Default Command: `["bash"]`

```dockerfile
# Thu, 27 Jul 2023 23:49:22 GMT
ADD file:8f568232a57d693c9607a4bfb4953e58e90ddf6a40fd78ebb1fd6e6d41ef24c6 in / 
# Thu, 27 Jul 2023 23:49:23 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9688d95a3b850e726b035b41a8770f830ce4b26f8b903aff1450c1aa87c9bb6f`  
		Last Modified: Thu, 27 Jul 2023 23:53:34 GMT  
		Size: 47.4 MB (47414900 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20230725` - linux; arm variant v7

```console
$ docker pull debian@sha256:37f5cf689bd16eb5b1a81e992df833f8a54befc3336f950e2317f371ab8c972e
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.2 MB (45232988 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a10f129b0ba027d1ce2ec5ad68f45e056c060686f153323bcb778e1b7d94afe6`
-	Default Command: `["bash"]`

```dockerfile
# Fri, 28 Jul 2023 00:00:19 GMT
ADD file:20e9751a1ac7b1a4351d33fb8f1d9eafbfa0418af09f0f4dfac07e45c2dd03f1 in / 
# Fri, 28 Jul 2023 00:00:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:994d9244915c9e6508d8815d01ebde033ea2886f7b95198654dfdd8af2526b7c`  
		Last Modified: Fri, 28 Jul 2023 00:06:35 GMT  
		Size: 45.2 MB (45232988 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20230725` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:bfebf3f1864ed78e48bbdcb40516998311a07b16d620ab82335a99832d4016e6
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49591274 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8dc39630b362a322e82d31a77a60d7467ba3bfc51e8f57604fae6eb379a5e897`
-	Default Command: `["bash"]`

```dockerfile
# Thu, 27 Jul 2023 23:44:16 GMT
ADD file:1685e9bd99067394420a349fe9fafbf6f0730ca2beeb3cd7f3f82c05ed20622c in / 
# Thu, 27 Jul 2023 23:44:17 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:41ef695b0dcdf6513df3775010b6de9dd5cf146356e0b22a0c8fb668f77acbce`  
		Last Modified: Thu, 27 Jul 2023 23:49:32 GMT  
		Size: 49.6 MB (49591274 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20230725` - linux; 386

```console
$ docker pull debian@sha256:251318cd9c4e04d80c3f5eb567cb6c4a2af6713a37b9696beca6931d720d24a9
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50567827 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2208edac3c3178c70f4c4162b79365540260d33d7193e07acabe75e179535b77`
-	Default Command: `["bash"]`

```dockerfile
# Thu, 27 Jul 2023 23:40:47 GMT
ADD file:c3f4390279546266e26e039791de6cfd53e464f4a08e3c352003596745bcd329 in / 
# Thu, 27 Jul 2023 23:40:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:fb5312cfa75e3ad057b199cec5ef30dc0dc37843c2bba772372a61445f7e37c7`  
		Last Modified: Thu, 27 Jul 2023 23:47:08 GMT  
		Size: 50.6 MB (50567827 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20230725` - linux; mips64le

```console
$ docker pull debian@sha256:e2d6f22084eb264fda7bff602466e79c3b120e1128532f72d812497f5afc2135
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.5 MB (49542048 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:65b6ed334facd986bb9c292723b3c290ed897279da911e35330f80fb515b37e9`
-	Default Command: `["bash"]`

```dockerfile
# Thu, 27 Jul 2023 23:15:34 GMT
ADD file:224ff2c2330e0b11d4a23e2914ed2a70e26830febc235424ef7e0f8010c3b882 in / 
# Thu, 27 Jul 2023 23:15:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9f78c9f83bb7be271c6dcfb95642a820bcdbb0fe6c6d7131806209e41f2c3dbf`  
		Last Modified: Thu, 27 Jul 2023 23:27:01 GMT  
		Size: 49.5 MB (49542048 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20230725` - linux; ppc64le

```console
$ docker pull debian@sha256:43af724a36cb963525ca3efec97181ae5a2ba9599b7ecff7b941b51c68cd2148
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.5 MB (53543345 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9d0b278ff68f405cbbeaf722c08cce26c242308611ceb2db76150a0c196650d6`
-	Default Command: `["bash"]`

```dockerfile
# Thu, 27 Jul 2023 23:25:09 GMT
ADD file:09c45999929f09514adf2cb9d6d0f0a57f4aacd62c4bd5ef96405b1588914a05 in / 
# Thu, 27 Jul 2023 23:25:12 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1d1be89ccee2abf523e50d302d1b9d4b49c822db5e4182c8f25dba9e0e2f1519`  
		Last Modified: Thu, 27 Jul 2023 23:32:43 GMT  
		Size: 53.5 MB (53543345 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20230725` - linux; s390x

```console
$ docker pull debian@sha256:5d82963da6df7f9c2c620dfb66416dd212c0a34bc19c389ec5187efab49d39b7
```

-	Docker Version: 20.10.23
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47922019 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:768a9f2dbbcf4a9dc45e5634e686e31999c3b28b6de76b9b16b7ca1b6611bd29`
-	Default Command: `["bash"]`

```dockerfile
# Thu, 27 Jul 2023 23:48:50 GMT
ADD file:9aafe828c01aed42f615a8cd6a088f7fd6616d528e4d91135828263f14d05eea in / 
# Thu, 27 Jul 2023 23:48:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3542e74447a7814dd3151a0c260f296bafad0b05dcd5d5812bf936180f6e9dbf`  
		Last Modified: Thu, 27 Jul 2023 23:53:48 GMT  
		Size: 47.9 MB (47922019 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
