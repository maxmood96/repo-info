## `debian:oldstable`

```console
$ docker pull debian@sha256:2e07ae8d8d8c032548fc8c18aa7fb02f7569d3b153b0af9b670f2b981b8b9c72
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:oldstable` - linux; amd64

```console
$ docker pull debian@sha256:6762c4bee2cf3cc26781120a0d2895d53ca23608835c1a3f17b18a2aa5764d23
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50440424 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ee62235ee8c05615726e483c520e22d996bfd1cf4d722d50f291d271d10744fc`
-	Default Command: `["bash"]`

```dockerfile
# Sat, 28 May 2022 01:21:12 GMT
ADD file:6c6dd727b4a86330f0a001d48ee1e61c9cf1cccdedab9943b04dcb76d6cf737a in / 
# Sat, 28 May 2022 01:21:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:dd8dbe98482f3149a0de07fe52d533c0231718f894c974fdb1a19247dbe58145`  
		Last Modified: Sat, 28 May 2022 01:26:59 GMT  
		Size: 50.4 MB (50440424 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; arm variant v5

```console
$ docker pull debian@sha256:45988dffaf7b00da03d72ddba109d6020d1e74f5fd0fb984364865e6d74eb2da
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **48.2 MB (48160620 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3c4f59ab0d58c4112c2324c8ce19b01cfbc5c5d8466be64ee933bf4928d11ee1`
-	Default Command: `["bash"]`

```dockerfile
# Sat, 28 May 2022 02:05:27 GMT
ADD file:af261fe6e1fa05d64afb0e50492c00004953783bb5110559ea0db85b56accc5b in / 
# Sat, 28 May 2022 02:05:29 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:fdc83092ab774d1a9a0eff531fcaa95670f71d02803980fe1d298aebc26fd423`  
		Last Modified: Sat, 28 May 2022 02:22:05 GMT  
		Size: 48.2 MB (48160620 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; arm variant v7

```console
$ docker pull debian@sha256:cef8b373bde56bcea7188d0b5ddfa77e4928fb6a4b98dbd5a86d70ad159e3a15
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45915603 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:23c403ca1925288c85d217256e40c1e94793cdfd2626f27c3b7dbd8b1f63f21b`
-	Default Command: `["bash"]`

```dockerfile
# Sat, 28 May 2022 01:02:23 GMT
ADD file:93e45631d7edd7e30839cc3137e870dfdc4bc0fc5a060e953578681692b58d9b in / 
# Sat, 28 May 2022 01:02:24 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:46186334d02ab25c1cf8d13de854c4896919f55590bd823ebfe91bb84ceaae12`  
		Last Modified: Sat, 28 May 2022 01:18:47 GMT  
		Size: 45.9 MB (45915603 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:38f8618291afb4f398a5a3ccb2f9efd55b96d9eda52e65e9f63e3956b51aefd0
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49229037 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1fa8abc6fda0ce08f8c4e6d9b520a84193835eae5583c33a3fec3632e804ee8a`
-	Default Command: `["bash"]`

```dockerfile
# Sat, 28 May 2022 00:41:39 GMT
ADD file:80d27c6c157fd32ac9b69d8d7115259ba021ed0d7447c50aec7251a3899b4b9f in / 
# Sat, 28 May 2022 00:41:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:08bc7b3927bd8e671ea9952e215bb10105ddea98cf2b83651a3cf922566aaee0`  
		Last Modified: Sat, 28 May 2022 00:49:28 GMT  
		Size: 49.2 MB (49229037 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; 386

```console
$ docker pull debian@sha256:d45e7e050818cf36ad7d3e9f22ce46dc4a926e991431b13fc09eb0a0c9764cf5
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51204223 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:37f30b5734d92ef834ded1390c7d0732b04814de63b071548930924e5be2aa8a`
-	Default Command: `["bash"]`

```dockerfile
# Sat, 28 May 2022 00:40:37 GMT
ADD file:1db3a01981cc79d71fa5a24eba6c14a72990ecc081ed01b6d41e493b63e95ebe in / 
# Sat, 28 May 2022 00:40:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5559dbed49d2bf17fd15cac89e8b285b3d786020b52e88da8688c6b99f993d24`  
		Last Modified: Sat, 28 May 2022 00:48:36 GMT  
		Size: 51.2 MB (51204223 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; mips64le

```console
$ docker pull debian@sha256:747104f6b26c39195b778387b245f1e1e13500d5f8d0f1f42d5174e33af80077
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.1 MB (49073364 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8c2c8e2bd5fde0dac5469d7365b028df8c3e7db6aab2e6119cde316b4a7bc62e`
-	Default Command: `["bash"]`

```dockerfile
# Sat, 28 May 2022 01:12:54 GMT
ADD file:eb19dea96494052de4c2ea2ee51deb090eb008a2a39eaa9e1a623f1e7626e3ed in / 
# Sat, 28 May 2022 01:12:59 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d31bc4793744d8e909ed41c309a0633dea29816d12adde39b5f4b34fcafd2334`  
		Last Modified: Sat, 28 May 2022 01:23:40 GMT  
		Size: 49.1 MB (49073364 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; ppc64le

```console
$ docker pull debian@sha256:049860dc78216f4f19346f7bf4656d64756cb51f483190d6502e03253b09baea
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.2 MB (54177017 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a03e0000c5b1bce273fe78113c62c107eb5890b5e25535c66b88129946970780`
-	Default Command: `["bash"]`

```dockerfile
# Sat, 28 May 2022 01:24:21 GMT
ADD file:f0f6279b8fd985287b5f3df9480f1840f5d0b4a3dc72e3f373b89757ef9a9077 in / 
# Sat, 28 May 2022 01:24:26 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6a049936bd2803fc7c63adc8dfe2c2a7792bb32515dc427cf69787f3b0d352df`  
		Last Modified: Sat, 28 May 2022 01:33:59 GMT  
		Size: 54.2 MB (54177017 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; s390x

```console
$ docker pull debian@sha256:89104a86f194a70b87717e0b46bec303dd1157339c642852e320d5478f0ff631
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.0 MB (49006790 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e6f8cab30b13805b76a4a15594add3f5e53fb32221e5bf1cc627afc5dd2f0f03`
-	Default Command: `["bash"]`

```dockerfile
# Sat, 28 May 2022 00:44:00 GMT
ADD file:bd5cfd4dc8d90cc91b330c8c19624771c01f1ad03a3a3ab3cd71cfb76d2dd66c in / 
# Sat, 28 May 2022 00:44:04 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:97c6438ef1df635b92390f341cba00358af65b7c27e2bd11a4ef6fff81d5a77c`  
		Last Modified: Sat, 28 May 2022 00:50:36 GMT  
		Size: 49.0 MB (49006790 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
