## `debian:testing`

```console
$ docker pull debian@sha256:a0a3465d5f7858e2a1829bd8d373c4fef46d1fb87a8b7a220736f4b0a3ce60af
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:testing` - linux; amd64

```console
$ docker pull debian@sha256:e855d6b7a67490fc16fb57cd85501094392435183eb89613c7ad655cc2fa4bba
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.9 MB (52947617 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:46d3de8d9f7ebe35ac514dc87f6749a33ae5b6df51598d78f26b223521f4021f`
-	Default Command: `["bash"]`

```dockerfile
# Sat, 28 May 2022 01:22:29 GMT
ADD file:272efeab320060153c6fd77f52b9a9922149739b8f010d93cde516421fde6ce9 in / 
# Sat, 28 May 2022 01:22:30 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:344633e2dec142d10b6e072c2fded693fe7441d6af83d21be68657a30ac0ceaa`  
		Last Modified: Sat, 28 May 2022 01:29:25 GMT  
		Size: 52.9 MB (52947617 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; arm variant v5

```console
$ docker pull debian@sha256:af36f799648076a8d87784a52ffa26f637c93984ce52c3956a2ee6bc4cc85b19
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.7 MB (50735306 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:079899d6848e4ac1e204cebe49b5f1c26e5b885be75e5c51df89479cf6db03f8`
-	Default Command: `["bash"]`

```dockerfile
# Sat, 28 May 2022 02:09:23 GMT
ADD file:7f964e2808ba0559b6a5faeaf3f42317d9a52fbc41a1e6518966a2208a4c8bca in / 
# Sat, 28 May 2022 02:09:24 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7f085037f69424eeca21945518c8aa02c67c161b2d8557ccbed5817fd62ab34a`  
		Last Modified: Sat, 28 May 2022 02:26:45 GMT  
		Size: 50.7 MB (50735306 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; arm variant v7

```console
$ docker pull debian@sha256:517596634bb4cfa5841b38f0d646899dc82fe9601def6b7cc0d613ded3c42442
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **48.5 MB (48476410 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:355a597db35b913e54a3c6eee48582c2e7e27d28624800b188a5e61464b0138a`
-	Default Command: `["bash"]`

```dockerfile
# Sat, 28 May 2022 01:06:23 GMT
ADD file:ec90a19450a4b3174d662c6df3d034488e52773bda79988366e79bf6ac179803 in / 
# Sat, 28 May 2022 01:06:23 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8e10f53774ef90fce803a05457ec21c9be85884019be903eeed2eba672058603`  
		Last Modified: Sat, 28 May 2022 01:22:58 GMT  
		Size: 48.5 MB (48476410 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:ffcfd6fc573619b4ad433fa59365702ddaeb1930872b1c545e91d26dfb253caf
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.0 MB (52042837 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8b9a238dce786673afd81f94a67516b33b378f1c59eeb203d80b81d5fb71433f`
-	Default Command: `["bash"]`

```dockerfile
# Sat, 28 May 2022 00:43:11 GMT
ADD file:936ad327dc97009e77bbb7d1d1874d55f3d251b8a641269356e5caabe860a8cf in / 
# Sat, 28 May 2022 00:43:12 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:75fd77c03641c04273179a599ef54dea600d22da8b30160003b50fa90a397f43`  
		Last Modified: Sat, 28 May 2022 00:52:12 GMT  
		Size: 52.0 MB (52042837 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; 386

```console
$ docker pull debian@sha256:d0b3fbb1b13bdf479f1560ff370681bc3476bde45849abf41c8994f39458857d
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.9 MB (53948561 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ebf6d9f8caee875f005b8be1af37b3921fb1fa2ce662ba57a6fc521f9ff2e542`
-	Default Command: `["bash"]`

```dockerfile
# Sat, 28 May 2022 00:42:18 GMT
ADD file:ab2acf453e0f28f32503a483bbda0e5e5d09513add50fe39f591bf33810a4be7 in / 
# Sat, 28 May 2022 00:42:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:fd2231baceb33fbf6629c80839b9f9e3d4d4c20d0adb42b699791bd018f52c4c`  
		Last Modified: Sat, 28 May 2022 00:51:24 GMT  
		Size: 53.9 MB (53948561 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; mips64le

```console
$ docker pull debian@sha256:af6918e2a1a709ade61d03c7cd7530af6de082de33967c4f50af62cad1d25861
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.1 MB (52061526 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:53ba14371e2b24827cacb5fb6c4721d0fe5f491763bcfb302bfde781c50b80f9`
-	Default Command: `["bash"]`

```dockerfile
# Sat, 28 May 2022 01:16:20 GMT
ADD file:750180a6aa4a39bf911865e098608b764a75b2c50c00426efecdab074d6108b8 in / 
# Sat, 28 May 2022 01:16:25 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7cb5df6daf04a8d6061a3059f38de0f503140d437089f54459799cd3d1d80736`  
		Last Modified: Sat, 28 May 2022 01:27:18 GMT  
		Size: 52.1 MB (52061526 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; ppc64le

```console
$ docker pull debian@sha256:8d3d10acc4e10a3675c86d6f69c0e8d3f7bbfd14b365b07041bb7a29335ea012
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **57.2 MB (57161585 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:77dee559b9d2a032493598ddeac23f117479b94d00175a186fe46168a1385090`
-	Default Command: `["bash"]`

```dockerfile
# Sat, 28 May 2022 01:26:41 GMT
ADD file:c839faac39e30677565fb631dd4f40114392a7a667b8db7f85ca4d8aaeb32ad9 in / 
# Sat, 28 May 2022 01:26:46 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f5e0403edf3ce1164c65241f62272268585e383702736c781adae79230ef3422`  
		Last Modified: Sat, 28 May 2022 01:36:21 GMT  
		Size: 57.2 MB (57161585 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; s390x

```console
$ docker pull debian@sha256:c83600e58c1c3e089e73ff7ea70aa85708229109afcbff307ae03159aaca0203
```

-	Docker Version: 20.10.12
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.5 MB (51490267 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f451ccedc1812263acae906af219938794ac888f652cdba38ecb5758e9004e6c`
-	Default Command: `["bash"]`

```dockerfile
# Sat, 28 May 2022 00:45:48 GMT
ADD file:430aae00218790907570f298b5f5c4e47453d9297090e8d006c5169e5e9ee56f in / 
# Sat, 28 May 2022 00:45:52 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:33ada038def02ed9f3f9762c43feeaf3a66329f979154e6d3dca5e1ac4f04893`  
		Last Modified: Sat, 28 May 2022 00:51:52 GMT  
		Size: 51.5 MB (51490267 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
