<!-- THIS FILE IS GENERATED VIA './update-remote.sh' -->

# Tags of `debian`

-	[`debian:11`](#debian11)
-	[`debian:11-slim`](#debian11-slim)
-	[`debian:11.11`](#debian1111)
-	[`debian:11.11-slim`](#debian1111-slim)
-	[`debian:12`](#debian12)
-	[`debian:12-slim`](#debian12-slim)
-	[`debian:12.7`](#debian127)
-	[`debian:12.7-slim`](#debian127-slim)
-	[`debian:bookworm`](#debianbookworm)
-	[`debian:bookworm-20240926`](#debianbookworm-20240926)
-	[`debian:bookworm-20240926-slim`](#debianbookworm-20240926-slim)
-	[`debian:bookworm-backports`](#debianbookworm-backports)
-	[`debian:bookworm-slim`](#debianbookworm-slim)
-	[`debian:bullseye`](#debianbullseye)
-	[`debian:bullseye-20240926`](#debianbullseye-20240926)
-	[`debian:bullseye-20240926-slim`](#debianbullseye-20240926-slim)
-	[`debian:bullseye-backports`](#debianbullseye-backports)
-	[`debian:bullseye-slim`](#debianbullseye-slim)
-	[`debian:experimental`](#debianexperimental)
-	[`debian:experimental-20240926`](#debianexperimental-20240926)
-	[`debian:latest`](#debianlatest)
-	[`debian:oldstable`](#debianoldstable)
-	[`debian:oldstable-20240926`](#debianoldstable-20240926)
-	[`debian:oldstable-20240926-slim`](#debianoldstable-20240926-slim)
-	[`debian:oldstable-backports`](#debianoldstable-backports)
-	[`debian:oldstable-slim`](#debianoldstable-slim)
-	[`debian:rc-buggy`](#debianrc-buggy)
-	[`debian:rc-buggy-20240926`](#debianrc-buggy-20240926)
-	[`debian:sid`](#debiansid)
-	[`debian:sid-20240926`](#debiansid-20240926)
-	[`debian:sid-20240926-slim`](#debiansid-20240926-slim)
-	[`debian:sid-slim`](#debiansid-slim)
-	[`debian:stable`](#debianstable)
-	[`debian:stable-20240926`](#debianstable-20240926)
-	[`debian:stable-20240926-slim`](#debianstable-20240926-slim)
-	[`debian:stable-backports`](#debianstable-backports)
-	[`debian:stable-slim`](#debianstable-slim)
-	[`debian:testing`](#debiantesting)
-	[`debian:testing-20240926`](#debiantesting-20240926)
-	[`debian:testing-20240926-slim`](#debiantesting-20240926-slim)
-	[`debian:testing-backports`](#debiantesting-backports)
-	[`debian:testing-slim`](#debiantesting-slim)
-	[`debian:trixie`](#debiantrixie)
-	[`debian:trixie-20240926`](#debiantrixie-20240926)
-	[`debian:trixie-20240926-slim`](#debiantrixie-20240926-slim)
-	[`debian:trixie-backports`](#debiantrixie-backports)
-	[`debian:trixie-slim`](#debiantrixie-slim)
-	[`debian:unstable`](#debianunstable)
-	[`debian:unstable-20240926`](#debianunstable-20240926)
-	[`debian:unstable-20240926-slim`](#debianunstable-20240926-slim)
-	[`debian:unstable-slim`](#debianunstable-slim)

## `debian:11`

```console
$ docker pull debian@sha256:dbd11e0e32e2455e709b986df085b4ed011698bedc0fcb4bf59673782f1941b6
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:11` - linux; amd64

```console
$ docker pull debian@sha256:2a1c9a0e7aa4fcad4d6319547bfcc0d8a7a290fada438d6fabc416f137cf12b4
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.1 MB (55081329 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:34662f5448c46bb16e56edc0a8224cc0c0fb0a6bc4713061857761e8684ff388`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:30:56 GMT
ADD file:e1bdcceaa316a43ad58ce8cf054a8e89ecf5a0dbae8125eb85e9b26fdb2fca2b in / 
# Wed, 04 Sep 2024 22:30:57 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ba83bbfca9443648a883d1404b33faa0f5e096a99a2b683e3bbaee8912bca845`  
		Last Modified: Wed, 04 Sep 2024 22:34:34 GMT  
		Size: 55.1 MB (55081329 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; arm variant v7

```console
$ docker pull debian@sha256:ab7cd777687e690cc1359a7e2bff3a99db0aa904202cac004a52e4e4037836b9
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.2 MB (50240273 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:54a19847bde3692980284a498987764b1ecded03ca5b8442d26550bad64d64e4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:58:17 GMT
ADD file:e3043c245364d9fe593e24bbcac51334dca659e8ec7cd42b6368ba7ea83ea087 in / 
# Wed, 04 Sep 2024 21:58:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3e424c6173543918aac0bf0552ca8359d23f7b7baa2d77fc8e2c9deb6756f39d`  
		Last Modified: Wed, 04 Sep 2024 22:02:04 GMT  
		Size: 50.2 MB (50240273 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:501192a3d2c16fbe3d64e597b526b73f6ef610662d4f66368083627387f9cbc0
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.7 MB (53731619 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:740cfcca2f3e596ffacaa040c60d44c4fbf8af4b9da7dcca2dc28970965ac54b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:39:59 GMT
ADD file:aad8b86b3a958bc07504985acedcc819faa4f1ed12ca8b46d8d94c4d564cbdfa in / 
# Wed, 04 Sep 2024 21:40:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d82c4492ee91810a42e0c53e955a661e9a092364bc474c9db559ea5b24b7047f`  
		Last Modified: Wed, 04 Sep 2024 21:42:52 GMT  
		Size: 53.7 MB (53731619 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; 386

```console
$ docker pull debian@sha256:c55bb3bb54930a1347bdda91e178632a649e5ea3ecffc6850c21abecbba282fd
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.1 MB (56076167 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9f21a5cf38fa014bb6b26f5caa28743a9db29ef146fcc72a73cd0017915e6e08`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:44:44 GMT
ADD file:1d8ae4e7067486ce0279b3d90aaecbc5973872ad64266d252bfa3fd5e4fc2e5f in / 
# Wed, 04 Sep 2024 22:44:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c6549675a9e3cbb8e77f08089828d3b4f24a06d332dc86c4d140aa273748ba57`  
		Last Modified: Wed, 04 Sep 2024 22:48:29 GMT  
		Size: 56.1 MB (56076167 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:11-slim`

```console
$ docker pull debian@sha256:a3fd77cb2575476ac86ce2077ced3b48d8f7cf22b5444dd1cbad7c1f3fbf22e4
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:11-slim` - linux; amd64

```console
$ docker pull debian@sha256:f94d0d6faaec313eed26c957e7aef75225f0bf8b8376ef83082ed0d8792255f7
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.4 MB (31428677 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a1d53ccb2f5dc360cfb3e2ccc424d8461aab46b32bf47175ad438ba165c18df9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:31:09 GMT
ADD file:1b8ac8ec2b2cbdfc9682f076e7e579579d6df6235ed23b2e63f8eec671c52e92 in / 
# Wed, 04 Sep 2024 22:31:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6533c3eba3f3cd4c840877f9245b26929fc8c22a39f42c872aa314c32c6d654b`  
		Last Modified: Wed, 04 Sep 2024 22:34:54 GMT  
		Size: 31.4 MB (31428677 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:ac122a7511a9d58028712a4c28707d15053f3c011c431056c8858a93af0cd906
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.6 MB (26589615 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:67266cf04539b5b6c6b6f49f38aba2da8202906b210a537f264d0865d4a3be40`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:58:29 GMT
ADD file:c451ece1244c14bce110ffbe6906867ce12f8e88234988b0edba91009a9dbbf8 in / 
# Wed, 04 Sep 2024 21:58:30 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f572303598915f7fb61cc4b47f7207c9ee64d52b9db5fc03ee133ec2dd679347`  
		Last Modified: Wed, 04 Sep 2024 22:02:25 GMT  
		Size: 26.6 MB (26589615 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:fd83fda9b857c7a0dac79eb937029e8554e2553bce0c7590af73813e86f71a5a
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30074365 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:53385f8e66d26a884c129a95e8d1653dbb5197ec25c8a28073f37f74c5b0909d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:40:08 GMT
ADD file:0802ca323252c154049da951c7da8572fb921f823341726dd7664f05c009e50c in / 
# Wed, 04 Sep 2024 21:40:08 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:172514850d5c3a8b3bc66fd0f1f345729d1b0b249cc3f053febcb64a066835da`  
		Last Modified: Wed, 04 Sep 2024 21:43:10 GMT  
		Size: 30.1 MB (30074365 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; 386

```console
$ docker pull debian@sha256:0f810568366d5e9730afa52e94e0ad3855265e44f9ff13b7545fbd16c9b4e029
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32413314 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:59b7adde6af5eb650f77cd5b31bdcbbf9ee9a6424fa7a7a119795c76ce9145da`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:44:56 GMT
ADD file:9177ff00c3abd0afc64f577295a060d88f4daed1042f024f7cfcfcd8cb1da9b8 in / 
# Wed, 04 Sep 2024 22:44:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2e34c6adf259f6e5265d64b5a01b92c3cc93548f22be8c1ccc578b7e9babb30c`  
		Last Modified: Wed, 04 Sep 2024 22:48:51 GMT  
		Size: 32.4 MB (32413314 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:11.11`

```console
$ docker pull debian@sha256:dbd11e0e32e2455e709b986df085b4ed011698bedc0fcb4bf59673782f1941b6
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:11.11` - linux; amd64

```console
$ docker pull debian@sha256:2a1c9a0e7aa4fcad4d6319547bfcc0d8a7a290fada438d6fabc416f137cf12b4
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.1 MB (55081329 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:34662f5448c46bb16e56edc0a8224cc0c0fb0a6bc4713061857761e8684ff388`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:30:56 GMT
ADD file:e1bdcceaa316a43ad58ce8cf054a8e89ecf5a0dbae8125eb85e9b26fdb2fca2b in / 
# Wed, 04 Sep 2024 22:30:57 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ba83bbfca9443648a883d1404b33faa0f5e096a99a2b683e3bbaee8912bca845`  
		Last Modified: Wed, 04 Sep 2024 22:34:34 GMT  
		Size: 55.1 MB (55081329 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.11` - linux; arm variant v7

```console
$ docker pull debian@sha256:ab7cd777687e690cc1359a7e2bff3a99db0aa904202cac004a52e4e4037836b9
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.2 MB (50240273 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:54a19847bde3692980284a498987764b1ecded03ca5b8442d26550bad64d64e4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:58:17 GMT
ADD file:e3043c245364d9fe593e24bbcac51334dca659e8ec7cd42b6368ba7ea83ea087 in / 
# Wed, 04 Sep 2024 21:58:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3e424c6173543918aac0bf0552ca8359d23f7b7baa2d77fc8e2c9deb6756f39d`  
		Last Modified: Wed, 04 Sep 2024 22:02:04 GMT  
		Size: 50.2 MB (50240273 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.11` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:501192a3d2c16fbe3d64e597b526b73f6ef610662d4f66368083627387f9cbc0
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.7 MB (53731619 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:740cfcca2f3e596ffacaa040c60d44c4fbf8af4b9da7dcca2dc28970965ac54b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:39:59 GMT
ADD file:aad8b86b3a958bc07504985acedcc819faa4f1ed12ca8b46d8d94c4d564cbdfa in / 
# Wed, 04 Sep 2024 21:40:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d82c4492ee91810a42e0c53e955a661e9a092364bc474c9db559ea5b24b7047f`  
		Last Modified: Wed, 04 Sep 2024 21:42:52 GMT  
		Size: 53.7 MB (53731619 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.11` - linux; 386

```console
$ docker pull debian@sha256:c55bb3bb54930a1347bdda91e178632a649e5ea3ecffc6850c21abecbba282fd
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.1 MB (56076167 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9f21a5cf38fa014bb6b26f5caa28743a9db29ef146fcc72a73cd0017915e6e08`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:44:44 GMT
ADD file:1d8ae4e7067486ce0279b3d90aaecbc5973872ad64266d252bfa3fd5e4fc2e5f in / 
# Wed, 04 Sep 2024 22:44:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c6549675a9e3cbb8e77f08089828d3b4f24a06d332dc86c4d140aa273748ba57`  
		Last Modified: Wed, 04 Sep 2024 22:48:29 GMT  
		Size: 56.1 MB (56076167 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:11.11-slim`

```console
$ docker pull debian@sha256:a3fd77cb2575476ac86ce2077ced3b48d8f7cf22b5444dd1cbad7c1f3fbf22e4
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:11.11-slim` - linux; amd64

```console
$ docker pull debian@sha256:f94d0d6faaec313eed26c957e7aef75225f0bf8b8376ef83082ed0d8792255f7
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.4 MB (31428677 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a1d53ccb2f5dc360cfb3e2ccc424d8461aab46b32bf47175ad438ba165c18df9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:31:09 GMT
ADD file:1b8ac8ec2b2cbdfc9682f076e7e579579d6df6235ed23b2e63f8eec671c52e92 in / 
# Wed, 04 Sep 2024 22:31:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6533c3eba3f3cd4c840877f9245b26929fc8c22a39f42c872aa314c32c6d654b`  
		Last Modified: Wed, 04 Sep 2024 22:34:54 GMT  
		Size: 31.4 MB (31428677 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.11-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:ac122a7511a9d58028712a4c28707d15053f3c011c431056c8858a93af0cd906
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.6 MB (26589615 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:67266cf04539b5b6c6b6f49f38aba2da8202906b210a537f264d0865d4a3be40`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:58:29 GMT
ADD file:c451ece1244c14bce110ffbe6906867ce12f8e88234988b0edba91009a9dbbf8 in / 
# Wed, 04 Sep 2024 21:58:30 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f572303598915f7fb61cc4b47f7207c9ee64d52b9db5fc03ee133ec2dd679347`  
		Last Modified: Wed, 04 Sep 2024 22:02:25 GMT  
		Size: 26.6 MB (26589615 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.11-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:fd83fda9b857c7a0dac79eb937029e8554e2553bce0c7590af73813e86f71a5a
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30074365 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:53385f8e66d26a884c129a95e8d1653dbb5197ec25c8a28073f37f74c5b0909d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:40:08 GMT
ADD file:0802ca323252c154049da951c7da8572fb921f823341726dd7664f05c009e50c in / 
# Wed, 04 Sep 2024 21:40:08 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:172514850d5c3a8b3bc66fd0f1f345729d1b0b249cc3f053febcb64a066835da`  
		Last Modified: Wed, 04 Sep 2024 21:43:10 GMT  
		Size: 30.1 MB (30074365 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.11-slim` - linux; 386

```console
$ docker pull debian@sha256:0f810568366d5e9730afa52e94e0ad3855265e44f9ff13b7545fbd16c9b4e029
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32413314 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:59b7adde6af5eb650f77cd5b31bdcbbf9ee9a6424fa7a7a119795c76ce9145da`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:44:56 GMT
ADD file:9177ff00c3abd0afc64f577295a060d88f4daed1042f024f7cfcfcd8cb1da9b8 in / 
# Wed, 04 Sep 2024 22:44:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2e34c6adf259f6e5265d64b5a01b92c3cc93548f22be8c1ccc578b7e9babb30c`  
		Last Modified: Wed, 04 Sep 2024 22:48:51 GMT  
		Size: 32.4 MB (32413314 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:12`

```console
$ docker pull debian@sha256:b8084b1a576c5504a031936e1132574f4ce1d6cc7130bbcc25a28f074539ae6b
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:12` - linux; amd64

```console
$ docker pull debian@sha256:3521cd844df5a6f3fd71217af1fc8222db5a7138a753eef86a0550d153184cdf
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49556702 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4fd3f4b75df372cc4fd60fd5c133c666cf4738ca0973523513f564076395f8ad`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:30:36 GMT
ADD file:1129dcf71f67461f4730620f8148cc9ebc7641966fa683cdf84807219ad288b2 in / 
# Wed, 04 Sep 2024 22:30:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8cd46d290033f265db57fd808ac81c444ec5a5b3f189c3d6d85043b647336913`  
		Last Modified: Wed, 04 Sep 2024 22:33:56 GMT  
		Size: 49.6 MB (49556702 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12` - linux; arm variant v5

```console
$ docker pull debian@sha256:2ce53ff4f45fc8317955a0b614e4ca7f417bd70174beaecd1640b0e7bdc55419
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.3 MB (47333195 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bfda8cf85a5f6f22975fa8d3875b3f44371cb40cf5c8d47954f088b739694890`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:48:22 GMT
ADD file:7ffef6fd95a7091f8058ff29f285ea1ff980523eddf5b8cbcd5ced08753075c5 in / 
# Wed, 04 Sep 2024 21:48:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c9500290eb8f2377c9935666a66a6f2f71959169f4c4c62904bd950e7f1cb1cf`  
		Last Modified: Wed, 04 Sep 2024 21:51:08 GMT  
		Size: 47.3 MB (47333195 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12` - linux; arm variant v7

```console
$ docker pull debian@sha256:2da114e57f05dc19378e4bf2577293982f238bae9d789233368673b0f5bc96af
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.1 MB (45148448 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d88d3c81ffdd6746cb90e070661481003f8e70ff8a8aa7ec0afdf309841edbb9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:57:56 GMT
ADD file:ce9ce875a73b1b4b1e1c1d3a25f43061406d0cc45595b603c5aaf2eb033490e1 in / 
# Wed, 04 Sep 2024 21:57:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:40a80c95f31d4a590ac5cfb88f8380e036f60bcffc5a805946b43ba82dc5f6d7`  
		Last Modified: Wed, 04 Sep 2024 22:01:19 GMT  
		Size: 45.1 MB (45148448 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:21887a619d762d236e7c66666ca657622ed8749e788322400b86ab09283d8fba
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49585623 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b442aaf370465d5d37b13e37276edad101d308551c0ad5ead5f6bc83a68b1855`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:39:42 GMT
ADD file:7f28c8fde9feb67359cbf19f7d77d3f757490b5f586520257cf92d233b4bfaa4 in / 
# Wed, 04 Sep 2024 21:39:43 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:56c9b9253ff98351db158cb6789848656b8d54f411c0037347bf2358efb18f39`  
		Last Modified: Wed, 04 Sep 2024 21:42:16 GMT  
		Size: 49.6 MB (49585623 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12` - linux; 386

```console
$ docker pull debian@sha256:16d461b083307155826e009dbfb0fffbd87b4ce3ae09fe03e72513a5abd6ffe7
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50574606 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:40d7f23c7210c00b2412083d922fc0c9d7b8ac3dcd6cfd4a4645383a0555e57e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:44:24 GMT
ADD file:c4052556120bbd9469f83c0cbc2abed04e22bd316491de6954bbbee12ae8b9bf in / 
# Wed, 04 Sep 2024 22:44:25 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e9d39ee40700085571f0a8e492b9b3a1fc65d55e5816aeed53fa9575b0013a98`  
		Last Modified: Wed, 04 Sep 2024 22:47:45 GMT  
		Size: 50.6 MB (50574606 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12` - linux; mips64le

```console
$ docker pull debian@sha256:00d4c78b0732cac0bdbb0bca5839f5be9be45340affb46c062d5c3343f8f4b93
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49562002 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bc8ff2260a08e41ebba8716a9cbcb79e01bb33ee2e44e3c420881ad0ad45ed28`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:14:59 GMT
ADD file:7a12db48c47f9e4ffb57061ade21f94ff0eb0791886ef60b56a9820f096b39a2 in / 
# Wed, 04 Sep 2024 22:15:05 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:15bebfb1ce6517765581a89c1270a3039857497f5393a3a53eb55a6cf6c3b9e9`  
		Last Modified: Wed, 04 Sep 2024 22:23:29 GMT  
		Size: 49.6 MB (49562002 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12` - linux; ppc64le

```console
$ docker pull debian@sha256:1899286cd107336e2f2c56acd3b4d0ce4f3e5d86cbf25b8f237fdc9df299115f
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53553949 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6187b3675bb104952a839816bf29d1316e31faac658923562fc7ce58d6470be5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:25:38 GMT
ADD file:c5d3aa6402ede77c4a443935fc6572b655c0144f8f41a2967e2e2b55b4c3343e in / 
# Wed, 04 Sep 2024 22:25:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d0634fe6314ffdff41b21ba805323138b719229ae2c5a32bda44147f688ed49c`  
		Last Modified: Wed, 04 Sep 2024 22:29:47 GMT  
		Size: 53.6 MB (53553949 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12` - linux; s390x

```console
$ docker pull debian@sha256:909298a718fd7e56ef61d43e80b3c8348c5abdecdcc01a93db51ff4782f25d53
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47939277 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:568a2a1876461b89c4e77c8ddf65168b77968c9233a467d7b57a96816f6fec3c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:42:25 GMT
ADD file:61b2bf58d4b2b852e6903b24131c8a331d7a1afe2cc99c4dd08ddffc91545071 in / 
# Wed, 04 Sep 2024 21:42:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1f3378e3b07f08dbf0b6ce13220a2997d788ba77fb8aa9394ce275771f49ec7a`  
		Last Modified: Wed, 04 Sep 2024 21:47:26 GMT  
		Size: 47.9 MB (47939277 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:12-slim`

```console
$ docker pull debian@sha256:a629e796d77a7b2ff82186ed15d01a493801c020eed5ce6adaa2704356f15a1c
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:12-slim` - linux; amd64

```console
$ docker pull debian@sha256:903d3225acecaa272bbdd7273c6c312c2af8b73644058838d23a8c9e6e5c82cf
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29126484 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:96d8cccd8a539589c2fd0d5a7a4009618d9a72051cf85eb05a5aae4f6ef55044`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:30:47 GMT
ADD file:d13afefcc2b0b02b598a3ac2598fe2187db41de1e17820e5b600a955b1429d59 in / 
# Wed, 04 Sep 2024 22:30:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a2318d6c47ec9cac5acc500c47c79602bcf953cec711a18bc898911a0984365b`  
		Last Modified: Wed, 04 Sep 2024 22:34:17 GMT  
		Size: 29.1 MB (29126484 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:6f6c0bb0dcf3ecf5b7f7e30858fc796e79dcab3efccd8c409a2fc878a136820e
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.9 MB (26887411 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fe8320634926b1a53d5aaf15d433e1ae8e23c959ba272f6da8a12177d56e7bd9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:48:32 GMT
ADD file:c162e60b24ef6aed489ba1986f407fd9ab4ace659e0e3e6759ffac7eb0b7f770 in / 
# Wed, 04 Sep 2024 21:48:32 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:44990bd21e0c5db65674bd030d12ea2461a14f2bb4007469bc2667c8535a8357`  
		Last Modified: Wed, 04 Sep 2024 21:51:32 GMT  
		Size: 26.9 MB (26887411 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:756f2240fee673990dccc7bdd288792e149ce50072d5cd29d328e4cd80aa6979
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.7 MB (24718265 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:25031d64c4d2f2b0619110bb800197003b9c646bd4eb60e256aa97d5f7964a1b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:58:08 GMT
ADD file:90772cdf7913d0ef1bf41e513a6205fa3195a1583476a536ec770e8381f77ac1 in / 
# Wed, 04 Sep 2024 21:58:08 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:670631d7a00e03a2701d6b6aff29204afdeff4cd2da308462d92f5743156ede1`  
		Last Modified: Wed, 04 Sep 2024 22:01:44 GMT  
		Size: 24.7 MB (24718265 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:ace984fea4de21d4ad6d0047c3c6ab7ade9fcd9dee3e44c5fe6349d7dd063d86
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.2 MB (29156545 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4d3492ddbe3a7e0be3e4b21881792a4b576b6f3dcba3878d1c05a5daa3cea23b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:39:52 GMT
ADD file:06a1877f1e100122a40ed52ce771bfa7e2ab3d28323780f58f1e5b57c1e576f9 in / 
# Wed, 04 Sep 2024 21:39:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:92c3b3500be621c72c7ac6432a9d8f731f145f4a1535361ffd3a304e55f7ccda`  
		Last Modified: Wed, 04 Sep 2024 21:42:36 GMT  
		Size: 29.2 MB (29156545 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12-slim` - linux; 386

```console
$ docker pull debian@sha256:a09e6f30093763451525093f6c1f425fcb751de9afc5c7944a28e4e48250f4c9
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30144294 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0a2814d4a9c9d839f4657a69021b49996d85c774faa7f9b17319ececd986efb8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:44:34 GMT
ADD file:1d96491aed0d3091aadb984e1dfe642343013348f4a1807a69e132efaed68030 in / 
# Wed, 04 Sep 2024 22:44:34 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c304282df09b425e8240f53c55110ac282ba801da157a805e4fdbdf8c832b376`  
		Last Modified: Wed, 04 Sep 2024 22:48:08 GMT  
		Size: 30.1 MB (30144294 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12-slim` - linux; mips64le

```console
$ docker pull debian@sha256:460b58fe46216c353ee64696b20a853678b1a08da92a64a1caf5e6faf997b329
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29125054 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:54cccd9985eae722d1101afa257bb68c23b77fc90bdd998bc2e61a23d36004d7`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:15:45 GMT
ADD file:9ffa4d53a074e91efecf5e26e3b67077f46165e0d042c1e6cf1b93c531ed2bc4 in / 
# Wed, 04 Sep 2024 22:15:50 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:89502a0eae32ab684bc9df3d9922ee2874839b178a693286c4b866f5ca8e93f3`  
		Last Modified: Wed, 04 Sep 2024 22:24:11 GMT  
		Size: 29.1 MB (29125054 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:f39559bf405679e55f74c361f383e0cfe19b01934a53eca2149ddd966604293b
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.1 MB (33122358 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6a89f4cbcbed534f48ad72545c90c39db96d5d9224115cc959ae4bc72635d169`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:25:52 GMT
ADD file:d83b2f8d4d3fd22a390140e3bebefb48e5f086d072ad6062f6446b4fc42ec7a8 in / 
# Wed, 04 Sep 2024 22:25:54 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f19b11698292330b7d980ed50b0141417eec298d865e0c1b305ce7a8b80b572d`  
		Last Modified: Wed, 04 Sep 2024 22:30:11 GMT  
		Size: 33.1 MB (33122358 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12-slim` - linux; s390x

```console
$ docker pull debian@sha256:d11dd86eb4857cdeac7b99af3884fa13e8a3d18ce550f77cee3cca64a735f233
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.5 MB (27490321 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7d7cfbba8ab244d5ce87ea82c4c77e997d7146325213e561f525ed7e681978ab`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:42:50 GMT
ADD file:728128617ba2b026c358eb266cd495be84354c4e5dc4ecc2953cb829190a4546 in / 
# Wed, 04 Sep 2024 21:42:52 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:95fe27c895a828dc681ee4a0cbea0264c47528dad525efdb9641a375666536bd`  
		Last Modified: Wed, 04 Sep 2024 21:47:41 GMT  
		Size: 27.5 MB (27490321 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:12.7`

```console
$ docker pull debian@sha256:b8084b1a576c5504a031936e1132574f4ce1d6cc7130bbcc25a28f074539ae6b
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:12.7` - linux; amd64

```console
$ docker pull debian@sha256:3521cd844df5a6f3fd71217af1fc8222db5a7138a753eef86a0550d153184cdf
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49556702 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4fd3f4b75df372cc4fd60fd5c133c666cf4738ca0973523513f564076395f8ad`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:30:36 GMT
ADD file:1129dcf71f67461f4730620f8148cc9ebc7641966fa683cdf84807219ad288b2 in / 
# Wed, 04 Sep 2024 22:30:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8cd46d290033f265db57fd808ac81c444ec5a5b3f189c3d6d85043b647336913`  
		Last Modified: Wed, 04 Sep 2024 22:33:56 GMT  
		Size: 49.6 MB (49556702 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12.7` - linux; arm variant v5

```console
$ docker pull debian@sha256:2ce53ff4f45fc8317955a0b614e4ca7f417bd70174beaecd1640b0e7bdc55419
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.3 MB (47333195 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bfda8cf85a5f6f22975fa8d3875b3f44371cb40cf5c8d47954f088b739694890`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:48:22 GMT
ADD file:7ffef6fd95a7091f8058ff29f285ea1ff980523eddf5b8cbcd5ced08753075c5 in / 
# Wed, 04 Sep 2024 21:48:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c9500290eb8f2377c9935666a66a6f2f71959169f4c4c62904bd950e7f1cb1cf`  
		Last Modified: Wed, 04 Sep 2024 21:51:08 GMT  
		Size: 47.3 MB (47333195 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12.7` - linux; arm variant v7

```console
$ docker pull debian@sha256:2da114e57f05dc19378e4bf2577293982f238bae9d789233368673b0f5bc96af
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.1 MB (45148448 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d88d3c81ffdd6746cb90e070661481003f8e70ff8a8aa7ec0afdf309841edbb9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:57:56 GMT
ADD file:ce9ce875a73b1b4b1e1c1d3a25f43061406d0cc45595b603c5aaf2eb033490e1 in / 
# Wed, 04 Sep 2024 21:57:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:40a80c95f31d4a590ac5cfb88f8380e036f60bcffc5a805946b43ba82dc5f6d7`  
		Last Modified: Wed, 04 Sep 2024 22:01:19 GMT  
		Size: 45.1 MB (45148448 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12.7` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:21887a619d762d236e7c66666ca657622ed8749e788322400b86ab09283d8fba
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49585623 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b442aaf370465d5d37b13e37276edad101d308551c0ad5ead5f6bc83a68b1855`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:39:42 GMT
ADD file:7f28c8fde9feb67359cbf19f7d77d3f757490b5f586520257cf92d233b4bfaa4 in / 
# Wed, 04 Sep 2024 21:39:43 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:56c9b9253ff98351db158cb6789848656b8d54f411c0037347bf2358efb18f39`  
		Last Modified: Wed, 04 Sep 2024 21:42:16 GMT  
		Size: 49.6 MB (49585623 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12.7` - linux; 386

```console
$ docker pull debian@sha256:16d461b083307155826e009dbfb0fffbd87b4ce3ae09fe03e72513a5abd6ffe7
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50574606 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:40d7f23c7210c00b2412083d922fc0c9d7b8ac3dcd6cfd4a4645383a0555e57e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:44:24 GMT
ADD file:c4052556120bbd9469f83c0cbc2abed04e22bd316491de6954bbbee12ae8b9bf in / 
# Wed, 04 Sep 2024 22:44:25 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e9d39ee40700085571f0a8e492b9b3a1fc65d55e5816aeed53fa9575b0013a98`  
		Last Modified: Wed, 04 Sep 2024 22:47:45 GMT  
		Size: 50.6 MB (50574606 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12.7` - linux; mips64le

```console
$ docker pull debian@sha256:00d4c78b0732cac0bdbb0bca5839f5be9be45340affb46c062d5c3343f8f4b93
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49562002 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bc8ff2260a08e41ebba8716a9cbcb79e01bb33ee2e44e3c420881ad0ad45ed28`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:14:59 GMT
ADD file:7a12db48c47f9e4ffb57061ade21f94ff0eb0791886ef60b56a9820f096b39a2 in / 
# Wed, 04 Sep 2024 22:15:05 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:15bebfb1ce6517765581a89c1270a3039857497f5393a3a53eb55a6cf6c3b9e9`  
		Last Modified: Wed, 04 Sep 2024 22:23:29 GMT  
		Size: 49.6 MB (49562002 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12.7` - linux; ppc64le

```console
$ docker pull debian@sha256:1899286cd107336e2f2c56acd3b4d0ce4f3e5d86cbf25b8f237fdc9df299115f
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53553949 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6187b3675bb104952a839816bf29d1316e31faac658923562fc7ce58d6470be5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:25:38 GMT
ADD file:c5d3aa6402ede77c4a443935fc6572b655c0144f8f41a2967e2e2b55b4c3343e in / 
# Wed, 04 Sep 2024 22:25:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d0634fe6314ffdff41b21ba805323138b719229ae2c5a32bda44147f688ed49c`  
		Last Modified: Wed, 04 Sep 2024 22:29:47 GMT  
		Size: 53.6 MB (53553949 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12.7` - linux; s390x

```console
$ docker pull debian@sha256:909298a718fd7e56ef61d43e80b3c8348c5abdecdcc01a93db51ff4782f25d53
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47939277 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:568a2a1876461b89c4e77c8ddf65168b77968c9233a467d7b57a96816f6fec3c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:42:25 GMT
ADD file:61b2bf58d4b2b852e6903b24131c8a331d7a1afe2cc99c4dd08ddffc91545071 in / 
# Wed, 04 Sep 2024 21:42:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1f3378e3b07f08dbf0b6ce13220a2997d788ba77fb8aa9394ce275771f49ec7a`  
		Last Modified: Wed, 04 Sep 2024 21:47:26 GMT  
		Size: 47.9 MB (47939277 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:12.7-slim`

```console
$ docker pull debian@sha256:a629e796d77a7b2ff82186ed15d01a493801c020eed5ce6adaa2704356f15a1c
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:12.7-slim` - linux; amd64

```console
$ docker pull debian@sha256:903d3225acecaa272bbdd7273c6c312c2af8b73644058838d23a8c9e6e5c82cf
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29126484 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:96d8cccd8a539589c2fd0d5a7a4009618d9a72051cf85eb05a5aae4f6ef55044`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:30:47 GMT
ADD file:d13afefcc2b0b02b598a3ac2598fe2187db41de1e17820e5b600a955b1429d59 in / 
# Wed, 04 Sep 2024 22:30:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a2318d6c47ec9cac5acc500c47c79602bcf953cec711a18bc898911a0984365b`  
		Last Modified: Wed, 04 Sep 2024 22:34:17 GMT  
		Size: 29.1 MB (29126484 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12.7-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:6f6c0bb0dcf3ecf5b7f7e30858fc796e79dcab3efccd8c409a2fc878a136820e
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.9 MB (26887411 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fe8320634926b1a53d5aaf15d433e1ae8e23c959ba272f6da8a12177d56e7bd9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:48:32 GMT
ADD file:c162e60b24ef6aed489ba1986f407fd9ab4ace659e0e3e6759ffac7eb0b7f770 in / 
# Wed, 04 Sep 2024 21:48:32 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:44990bd21e0c5db65674bd030d12ea2461a14f2bb4007469bc2667c8535a8357`  
		Last Modified: Wed, 04 Sep 2024 21:51:32 GMT  
		Size: 26.9 MB (26887411 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12.7-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:756f2240fee673990dccc7bdd288792e149ce50072d5cd29d328e4cd80aa6979
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.7 MB (24718265 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:25031d64c4d2f2b0619110bb800197003b9c646bd4eb60e256aa97d5f7964a1b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:58:08 GMT
ADD file:90772cdf7913d0ef1bf41e513a6205fa3195a1583476a536ec770e8381f77ac1 in / 
# Wed, 04 Sep 2024 21:58:08 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:670631d7a00e03a2701d6b6aff29204afdeff4cd2da308462d92f5743156ede1`  
		Last Modified: Wed, 04 Sep 2024 22:01:44 GMT  
		Size: 24.7 MB (24718265 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12.7-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:ace984fea4de21d4ad6d0047c3c6ab7ade9fcd9dee3e44c5fe6349d7dd063d86
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.2 MB (29156545 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4d3492ddbe3a7e0be3e4b21881792a4b576b6f3dcba3878d1c05a5daa3cea23b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:39:52 GMT
ADD file:06a1877f1e100122a40ed52ce771bfa7e2ab3d28323780f58f1e5b57c1e576f9 in / 
# Wed, 04 Sep 2024 21:39:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:92c3b3500be621c72c7ac6432a9d8f731f145f4a1535361ffd3a304e55f7ccda`  
		Last Modified: Wed, 04 Sep 2024 21:42:36 GMT  
		Size: 29.2 MB (29156545 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12.7-slim` - linux; 386

```console
$ docker pull debian@sha256:a09e6f30093763451525093f6c1f425fcb751de9afc5c7944a28e4e48250f4c9
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30144294 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0a2814d4a9c9d839f4657a69021b49996d85c774faa7f9b17319ececd986efb8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:44:34 GMT
ADD file:1d96491aed0d3091aadb984e1dfe642343013348f4a1807a69e132efaed68030 in / 
# Wed, 04 Sep 2024 22:44:34 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c304282df09b425e8240f53c55110ac282ba801da157a805e4fdbdf8c832b376`  
		Last Modified: Wed, 04 Sep 2024 22:48:08 GMT  
		Size: 30.1 MB (30144294 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12.7-slim` - linux; mips64le

```console
$ docker pull debian@sha256:460b58fe46216c353ee64696b20a853678b1a08da92a64a1caf5e6faf997b329
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29125054 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:54cccd9985eae722d1101afa257bb68c23b77fc90bdd998bc2e61a23d36004d7`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:15:45 GMT
ADD file:9ffa4d53a074e91efecf5e26e3b67077f46165e0d042c1e6cf1b93c531ed2bc4 in / 
# Wed, 04 Sep 2024 22:15:50 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:89502a0eae32ab684bc9df3d9922ee2874839b178a693286c4b866f5ca8e93f3`  
		Last Modified: Wed, 04 Sep 2024 22:24:11 GMT  
		Size: 29.1 MB (29125054 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12.7-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:f39559bf405679e55f74c361f383e0cfe19b01934a53eca2149ddd966604293b
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.1 MB (33122358 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6a89f4cbcbed534f48ad72545c90c39db96d5d9224115cc959ae4bc72635d169`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:25:52 GMT
ADD file:d83b2f8d4d3fd22a390140e3bebefb48e5f086d072ad6062f6446b4fc42ec7a8 in / 
# Wed, 04 Sep 2024 22:25:54 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f19b11698292330b7d980ed50b0141417eec298d865e0c1b305ce7a8b80b572d`  
		Last Modified: Wed, 04 Sep 2024 22:30:11 GMT  
		Size: 33.1 MB (33122358 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:12.7-slim` - linux; s390x

```console
$ docker pull debian@sha256:d11dd86eb4857cdeac7b99af3884fa13e8a3d18ce550f77cee3cca64a735f233
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.5 MB (27490321 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7d7cfbba8ab244d5ce87ea82c4c77e997d7146325213e561f525ed7e681978ab`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:42:50 GMT
ADD file:728128617ba2b026c358eb266cd495be84354c4e5dc4ecc2953cb829190a4546 in / 
# Wed, 04 Sep 2024 21:42:52 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:95fe27c895a828dc681ee4a0cbea0264c47528dad525efdb9641a375666536bd`  
		Last Modified: Wed, 04 Sep 2024 21:47:41 GMT  
		Size: 27.5 MB (27490321 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bookworm`

```console
$ docker pull debian@sha256:b8084b1a576c5504a031936e1132574f4ce1d6cc7130bbcc25a28f074539ae6b
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bookworm` - linux; amd64

```console
$ docker pull debian@sha256:3521cd844df5a6f3fd71217af1fc8222db5a7138a753eef86a0550d153184cdf
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49556702 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4fd3f4b75df372cc4fd60fd5c133c666cf4738ca0973523513f564076395f8ad`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:30:36 GMT
ADD file:1129dcf71f67461f4730620f8148cc9ebc7641966fa683cdf84807219ad288b2 in / 
# Wed, 04 Sep 2024 22:30:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8cd46d290033f265db57fd808ac81c444ec5a5b3f189c3d6d85043b647336913`  
		Last Modified: Wed, 04 Sep 2024 22:33:56 GMT  
		Size: 49.6 MB (49556702 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; arm variant v5

```console
$ docker pull debian@sha256:2ce53ff4f45fc8317955a0b614e4ca7f417bd70174beaecd1640b0e7bdc55419
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.3 MB (47333195 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bfda8cf85a5f6f22975fa8d3875b3f44371cb40cf5c8d47954f088b739694890`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:48:22 GMT
ADD file:7ffef6fd95a7091f8058ff29f285ea1ff980523eddf5b8cbcd5ced08753075c5 in / 
# Wed, 04 Sep 2024 21:48:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c9500290eb8f2377c9935666a66a6f2f71959169f4c4c62904bd950e7f1cb1cf`  
		Last Modified: Wed, 04 Sep 2024 21:51:08 GMT  
		Size: 47.3 MB (47333195 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; arm variant v7

```console
$ docker pull debian@sha256:2da114e57f05dc19378e4bf2577293982f238bae9d789233368673b0f5bc96af
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.1 MB (45148448 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d88d3c81ffdd6746cb90e070661481003f8e70ff8a8aa7ec0afdf309841edbb9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:57:56 GMT
ADD file:ce9ce875a73b1b4b1e1c1d3a25f43061406d0cc45595b603c5aaf2eb033490e1 in / 
# Wed, 04 Sep 2024 21:57:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:40a80c95f31d4a590ac5cfb88f8380e036f60bcffc5a805946b43ba82dc5f6d7`  
		Last Modified: Wed, 04 Sep 2024 22:01:19 GMT  
		Size: 45.1 MB (45148448 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:21887a619d762d236e7c66666ca657622ed8749e788322400b86ab09283d8fba
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49585623 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b442aaf370465d5d37b13e37276edad101d308551c0ad5ead5f6bc83a68b1855`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:39:42 GMT
ADD file:7f28c8fde9feb67359cbf19f7d77d3f757490b5f586520257cf92d233b4bfaa4 in / 
# Wed, 04 Sep 2024 21:39:43 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:56c9b9253ff98351db158cb6789848656b8d54f411c0037347bf2358efb18f39`  
		Last Modified: Wed, 04 Sep 2024 21:42:16 GMT  
		Size: 49.6 MB (49585623 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; 386

```console
$ docker pull debian@sha256:16d461b083307155826e009dbfb0fffbd87b4ce3ae09fe03e72513a5abd6ffe7
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50574606 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:40d7f23c7210c00b2412083d922fc0c9d7b8ac3dcd6cfd4a4645383a0555e57e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:44:24 GMT
ADD file:c4052556120bbd9469f83c0cbc2abed04e22bd316491de6954bbbee12ae8b9bf in / 
# Wed, 04 Sep 2024 22:44:25 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e9d39ee40700085571f0a8e492b9b3a1fc65d55e5816aeed53fa9575b0013a98`  
		Last Modified: Wed, 04 Sep 2024 22:47:45 GMT  
		Size: 50.6 MB (50574606 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; mips64le

```console
$ docker pull debian@sha256:00d4c78b0732cac0bdbb0bca5839f5be9be45340affb46c062d5c3343f8f4b93
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49562002 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bc8ff2260a08e41ebba8716a9cbcb79e01bb33ee2e44e3c420881ad0ad45ed28`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:14:59 GMT
ADD file:7a12db48c47f9e4ffb57061ade21f94ff0eb0791886ef60b56a9820f096b39a2 in / 
# Wed, 04 Sep 2024 22:15:05 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:15bebfb1ce6517765581a89c1270a3039857497f5393a3a53eb55a6cf6c3b9e9`  
		Last Modified: Wed, 04 Sep 2024 22:23:29 GMT  
		Size: 49.6 MB (49562002 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; ppc64le

```console
$ docker pull debian@sha256:1899286cd107336e2f2c56acd3b4d0ce4f3e5d86cbf25b8f237fdc9df299115f
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53553949 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6187b3675bb104952a839816bf29d1316e31faac658923562fc7ce58d6470be5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:25:38 GMT
ADD file:c5d3aa6402ede77c4a443935fc6572b655c0144f8f41a2967e2e2b55b4c3343e in / 
# Wed, 04 Sep 2024 22:25:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d0634fe6314ffdff41b21ba805323138b719229ae2c5a32bda44147f688ed49c`  
		Last Modified: Wed, 04 Sep 2024 22:29:47 GMT  
		Size: 53.6 MB (53553949 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; s390x

```console
$ docker pull debian@sha256:909298a718fd7e56ef61d43e80b3c8348c5abdecdcc01a93db51ff4782f25d53
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47939277 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:568a2a1876461b89c4e77c8ddf65168b77968c9233a467d7b57a96816f6fec3c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:42:25 GMT
ADD file:61b2bf58d4b2b852e6903b24131c8a331d7a1afe2cc99c4dd08ddffc91545071 in / 
# Wed, 04 Sep 2024 21:42:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1f3378e3b07f08dbf0b6ce13220a2997d788ba77fb8aa9394ce275771f49ec7a`  
		Last Modified: Wed, 04 Sep 2024 21:47:26 GMT  
		Size: 47.9 MB (47939277 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bookworm-20240926`

```console
$ docker pull debian@sha256:eb37f58646a901dc7727cf448cae36daaefaba79de33b5058dab79aa4c04aefb
```

-	Manifest MIME: `application/vnd.oci.image.index.v1+json`
-	Platforms: 0

## `debian:bookworm-20240926-slim`

```console
$ docker pull debian@sha256:eb37f58646a901dc7727cf448cae36daaefaba79de33b5058dab79aa4c04aefb
```

-	Manifest MIME: `application/vnd.oci.image.index.v1+json`
-	Platforms: 0

## `debian:bookworm-backports`

```console
$ docker pull debian@sha256:8bdab9c98547dc8cdabfe63036b92c7a80e988eb84c8cc7af10ec84265bbc6d7
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bookworm-backports` - linux; amd64

```console
$ docker pull debian@sha256:7df948cda922346a152453e8c5221bc485994a673d93bb9186aa5ae9dbc76f5d
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49556925 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b21f8d0691a2b62d3c7ede39f89fa69444757567b8b7537e2c7dc5b6cb580323`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:30:36 GMT
ADD file:1129dcf71f67461f4730620f8148cc9ebc7641966fa683cdf84807219ad288b2 in / 
# Wed, 04 Sep 2024 22:30:36 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:30:39 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:8cd46d290033f265db57fd808ac81c444ec5a5b3f189c3d6d85043b647336913`  
		Last Modified: Wed, 04 Sep 2024 22:33:56 GMT  
		Size: 49.6 MB (49556702 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:f4078a8d2540026e08753e643da1f12f1d734119d95c402e7ec9de2301957f8a`  
		Last Modified: Wed, 04 Sep 2024 22:34:08 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:0d76cdb02fbff56c5c96e594f5246d8d0dee1215c4b6447fc3183b57047a71c0
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.3 MB (47333419 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ef60ffb71324a69d2b6083c36aa1077ba665b67963ffc93d196d767ea4fbed63`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:48:22 GMT
ADD file:7ffef6fd95a7091f8058ff29f285ea1ff980523eddf5b8cbcd5ced08753075c5 in / 
# Wed, 04 Sep 2024 21:48:22 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 21:48:26 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:c9500290eb8f2377c9935666a66a6f2f71959169f4c4c62904bd950e7f1cb1cf`  
		Last Modified: Wed, 04 Sep 2024 21:51:08 GMT  
		Size: 47.3 MB (47333195 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:b5c24f84b1545bd355240fc6e859c4816363bf2ca615bad411164010580b31c2`  
		Last Modified: Wed, 04 Sep 2024 21:51:22 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:78bb92d413b4c7b8771b87bffb849db48944266d228ebd065fccab3af0281d57
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.1 MB (45148672 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:68a2de59a0bfedd462cba014bce645d8bab83b925a323965e908674e5c7a9a36`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:57:56 GMT
ADD file:ce9ce875a73b1b4b1e1c1d3a25f43061406d0cc45595b603c5aaf2eb033490e1 in / 
# Wed, 04 Sep 2024 21:57:56 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 21:58:02 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:40a80c95f31d4a590ac5cfb88f8380e036f60bcffc5a805946b43ba82dc5f6d7`  
		Last Modified: Wed, 04 Sep 2024 22:01:19 GMT  
		Size: 45.1 MB (45148448 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:904c52c73be85fe6f8e90287088a74d62e59c9b3e89a39d912b7e6d520e7182c`  
		Last Modified: Wed, 04 Sep 2024 22:01:32 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:b11bb7a6f3ad3ce46bf74876e5f2e611a8f489bf695cb98aa09881e66cf14a6e
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49585847 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bc2937618af9f59c27930764e9b405e5bf23c6ec3c067ba5f9c35ca3846ffac2`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:39:42 GMT
ADD file:7f28c8fde9feb67359cbf19f7d77d3f757490b5f586520257cf92d233b4bfaa4 in / 
# Wed, 04 Sep 2024 21:39:43 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 21:39:47 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:56c9b9253ff98351db158cb6789848656b8d54f411c0037347bf2358efb18f39`  
		Last Modified: Wed, 04 Sep 2024 21:42:16 GMT  
		Size: 49.6 MB (49585623 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:ac5e3ccc15c551a1ad31980971f380be7d600b14de827e8a22cf995ce2fecf5d`  
		Last Modified: Wed, 04 Sep 2024 21:42:29 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; 386

```console
$ docker pull debian@sha256:47844f87ff5aa70a34d6f164128ebd11bfeea16bcdcc1bed4da9fe4254a01b07
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50574832 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:28284a2d7d4f4d34fff009675d7bc8e1b94c6f4eb46d2e5048f927f567b8a111`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:44:24 GMT
ADD file:c4052556120bbd9469f83c0cbc2abed04e22bd316491de6954bbbee12ae8b9bf in / 
# Wed, 04 Sep 2024 22:44:25 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:44:27 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:e9d39ee40700085571f0a8e492b9b3a1fc65d55e5816aeed53fa9575b0013a98`  
		Last Modified: Wed, 04 Sep 2024 22:47:45 GMT  
		Size: 50.6 MB (50574606 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:b17af8ace8ee2d1d4d65f3b01ee84bc4ce6bd1723fe418adbdb010162ceb06d5`  
		Last Modified: Wed, 04 Sep 2024 22:47:57 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; mips64le

```console
$ docker pull debian@sha256:f9a1bf0b544e10c053c53036dd714cc325c0d1e1dd7a13e5163f4401b9ff8c83
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49562230 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:84caa71ba7cc852167a3f13ecb7ad573ca243790ffae2defd7b94c7fde14d5cb`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:14:59 GMT
ADD file:7a12db48c47f9e4ffb57061ade21f94ff0eb0791886ef60b56a9820f096b39a2 in / 
# Wed, 04 Sep 2024 22:15:05 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:15:22 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:15bebfb1ce6517765581a89c1270a3039857497f5393a3a53eb55a6cf6c3b9e9`  
		Last Modified: Wed, 04 Sep 2024 22:23:29 GMT  
		Size: 49.6 MB (49562002 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:0787c411c1074225173fc9c94b223733df1ba8e4348fefdcba9a1e268c99d9a0`  
		Last Modified: Wed, 04 Sep 2024 22:23:45 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; ppc64le

```console
$ docker pull debian@sha256:72cdaadf6234eb24c114eafc38e937a8c91798c71cfd03a36b6ba74b2d746c84
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53554171 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:019ac59c7b79b65161abf83b018bcb7f887b2b5a3f965aa4d14fca0c2bee9637`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:25:38 GMT
ADD file:c5d3aa6402ede77c4a443935fc6572b655c0144f8f41a2967e2e2b55b4c3343e in / 
# Wed, 04 Sep 2024 22:25:40 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:25:45 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:d0634fe6314ffdff41b21ba805323138b719229ae2c5a32bda44147f688ed49c`  
		Last Modified: Wed, 04 Sep 2024 22:29:47 GMT  
		Size: 53.6 MB (53553949 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:d9e31a0656c33be087f2d6e85d0cd87788216492f90ef677379d706bcb40ae7c`  
		Last Modified: Wed, 04 Sep 2024 22:30:00 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; s390x

```console
$ docker pull debian@sha256:1c3668c596498a2fc9cd0461fdea2a41469a9a71685f42f207060ea6858d8603
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47939500 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:483712f42cc6fbcb99641ccce36a34c7cb503e957ae49e527c6069a23ef8e222`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:42:25 GMT
ADD file:61b2bf58d4b2b852e6903b24131c8a331d7a1afe2cc99c4dd08ddffc91545071 in / 
# Wed, 04 Sep 2024 21:42:31 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 21:42:40 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:1f3378e3b07f08dbf0b6ce13220a2997d788ba77fb8aa9394ce275771f49ec7a`  
		Last Modified: Wed, 04 Sep 2024 21:47:26 GMT  
		Size: 47.9 MB (47939277 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:2c9c0b773b42d2d67bae9b502cdebce3cc01ae622889b0148e24329183b0b4af`  
		Last Modified: Wed, 04 Sep 2024 21:47:34 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bookworm-slim`

```console
$ docker pull debian@sha256:a629e796d77a7b2ff82186ed15d01a493801c020eed5ce6adaa2704356f15a1c
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bookworm-slim` - linux; amd64

```console
$ docker pull debian@sha256:903d3225acecaa272bbdd7273c6c312c2af8b73644058838d23a8c9e6e5c82cf
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29126484 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:96d8cccd8a539589c2fd0d5a7a4009618d9a72051cf85eb05a5aae4f6ef55044`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:30:47 GMT
ADD file:d13afefcc2b0b02b598a3ac2598fe2187db41de1e17820e5b600a955b1429d59 in / 
# Wed, 04 Sep 2024 22:30:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a2318d6c47ec9cac5acc500c47c79602bcf953cec711a18bc898911a0984365b`  
		Last Modified: Wed, 04 Sep 2024 22:34:17 GMT  
		Size: 29.1 MB (29126484 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:6f6c0bb0dcf3ecf5b7f7e30858fc796e79dcab3efccd8c409a2fc878a136820e
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.9 MB (26887411 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fe8320634926b1a53d5aaf15d433e1ae8e23c959ba272f6da8a12177d56e7bd9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:48:32 GMT
ADD file:c162e60b24ef6aed489ba1986f407fd9ab4ace659e0e3e6759ffac7eb0b7f770 in / 
# Wed, 04 Sep 2024 21:48:32 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:44990bd21e0c5db65674bd030d12ea2461a14f2bb4007469bc2667c8535a8357`  
		Last Modified: Wed, 04 Sep 2024 21:51:32 GMT  
		Size: 26.9 MB (26887411 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:756f2240fee673990dccc7bdd288792e149ce50072d5cd29d328e4cd80aa6979
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.7 MB (24718265 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:25031d64c4d2f2b0619110bb800197003b9c646bd4eb60e256aa97d5f7964a1b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:58:08 GMT
ADD file:90772cdf7913d0ef1bf41e513a6205fa3195a1583476a536ec770e8381f77ac1 in / 
# Wed, 04 Sep 2024 21:58:08 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:670631d7a00e03a2701d6b6aff29204afdeff4cd2da308462d92f5743156ede1`  
		Last Modified: Wed, 04 Sep 2024 22:01:44 GMT  
		Size: 24.7 MB (24718265 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:ace984fea4de21d4ad6d0047c3c6ab7ade9fcd9dee3e44c5fe6349d7dd063d86
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.2 MB (29156545 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4d3492ddbe3a7e0be3e4b21881792a4b576b6f3dcba3878d1c05a5daa3cea23b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:39:52 GMT
ADD file:06a1877f1e100122a40ed52ce771bfa7e2ab3d28323780f58f1e5b57c1e576f9 in / 
# Wed, 04 Sep 2024 21:39:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:92c3b3500be621c72c7ac6432a9d8f731f145f4a1535361ffd3a304e55f7ccda`  
		Last Modified: Wed, 04 Sep 2024 21:42:36 GMT  
		Size: 29.2 MB (29156545 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; 386

```console
$ docker pull debian@sha256:a09e6f30093763451525093f6c1f425fcb751de9afc5c7944a28e4e48250f4c9
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30144294 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0a2814d4a9c9d839f4657a69021b49996d85c774faa7f9b17319ececd986efb8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:44:34 GMT
ADD file:1d96491aed0d3091aadb984e1dfe642343013348f4a1807a69e132efaed68030 in / 
# Wed, 04 Sep 2024 22:44:34 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c304282df09b425e8240f53c55110ac282ba801da157a805e4fdbdf8c832b376`  
		Last Modified: Wed, 04 Sep 2024 22:48:08 GMT  
		Size: 30.1 MB (30144294 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; mips64le

```console
$ docker pull debian@sha256:460b58fe46216c353ee64696b20a853678b1a08da92a64a1caf5e6faf997b329
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29125054 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:54cccd9985eae722d1101afa257bb68c23b77fc90bdd998bc2e61a23d36004d7`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:15:45 GMT
ADD file:9ffa4d53a074e91efecf5e26e3b67077f46165e0d042c1e6cf1b93c531ed2bc4 in / 
# Wed, 04 Sep 2024 22:15:50 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:89502a0eae32ab684bc9df3d9922ee2874839b178a693286c4b866f5ca8e93f3`  
		Last Modified: Wed, 04 Sep 2024 22:24:11 GMT  
		Size: 29.1 MB (29125054 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:f39559bf405679e55f74c361f383e0cfe19b01934a53eca2149ddd966604293b
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.1 MB (33122358 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6a89f4cbcbed534f48ad72545c90c39db96d5d9224115cc959ae4bc72635d169`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:25:52 GMT
ADD file:d83b2f8d4d3fd22a390140e3bebefb48e5f086d072ad6062f6446b4fc42ec7a8 in / 
# Wed, 04 Sep 2024 22:25:54 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f19b11698292330b7d980ed50b0141417eec298d865e0c1b305ce7a8b80b572d`  
		Last Modified: Wed, 04 Sep 2024 22:30:11 GMT  
		Size: 33.1 MB (33122358 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; s390x

```console
$ docker pull debian@sha256:d11dd86eb4857cdeac7b99af3884fa13e8a3d18ce550f77cee3cca64a735f233
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.5 MB (27490321 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7d7cfbba8ab244d5ce87ea82c4c77e997d7146325213e561f525ed7e681978ab`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:42:50 GMT
ADD file:728128617ba2b026c358eb266cd495be84354c4e5dc4ecc2953cb829190a4546 in / 
# Wed, 04 Sep 2024 21:42:52 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:95fe27c895a828dc681ee4a0cbea0264c47528dad525efdb9641a375666536bd`  
		Last Modified: Wed, 04 Sep 2024 21:47:41 GMT  
		Size: 27.5 MB (27490321 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bullseye`

```console
$ docker pull debian@sha256:dbd11e0e32e2455e709b986df085b4ed011698bedc0fcb4bf59673782f1941b6
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:bullseye` - linux; amd64

```console
$ docker pull debian@sha256:2a1c9a0e7aa4fcad4d6319547bfcc0d8a7a290fada438d6fabc416f137cf12b4
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.1 MB (55081329 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:34662f5448c46bb16e56edc0a8224cc0c0fb0a6bc4713061857761e8684ff388`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:30:56 GMT
ADD file:e1bdcceaa316a43ad58ce8cf054a8e89ecf5a0dbae8125eb85e9b26fdb2fca2b in / 
# Wed, 04 Sep 2024 22:30:57 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ba83bbfca9443648a883d1404b33faa0f5e096a99a2b683e3bbaee8912bca845`  
		Last Modified: Wed, 04 Sep 2024 22:34:34 GMT  
		Size: 55.1 MB (55081329 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; arm variant v7

```console
$ docker pull debian@sha256:ab7cd777687e690cc1359a7e2bff3a99db0aa904202cac004a52e4e4037836b9
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.2 MB (50240273 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:54a19847bde3692980284a498987764b1ecded03ca5b8442d26550bad64d64e4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:58:17 GMT
ADD file:e3043c245364d9fe593e24bbcac51334dca659e8ec7cd42b6368ba7ea83ea087 in / 
# Wed, 04 Sep 2024 21:58:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3e424c6173543918aac0bf0552ca8359d23f7b7baa2d77fc8e2c9deb6756f39d`  
		Last Modified: Wed, 04 Sep 2024 22:02:04 GMT  
		Size: 50.2 MB (50240273 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:501192a3d2c16fbe3d64e597b526b73f6ef610662d4f66368083627387f9cbc0
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.7 MB (53731619 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:740cfcca2f3e596ffacaa040c60d44c4fbf8af4b9da7dcca2dc28970965ac54b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:39:59 GMT
ADD file:aad8b86b3a958bc07504985acedcc819faa4f1ed12ca8b46d8d94c4d564cbdfa in / 
# Wed, 04 Sep 2024 21:40:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d82c4492ee91810a42e0c53e955a661e9a092364bc474c9db559ea5b24b7047f`  
		Last Modified: Wed, 04 Sep 2024 21:42:52 GMT  
		Size: 53.7 MB (53731619 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; 386

```console
$ docker pull debian@sha256:c55bb3bb54930a1347bdda91e178632a649e5ea3ecffc6850c21abecbba282fd
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.1 MB (56076167 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9f21a5cf38fa014bb6b26f5caa28743a9db29ef146fcc72a73cd0017915e6e08`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:44:44 GMT
ADD file:1d8ae4e7067486ce0279b3d90aaecbc5973872ad64266d252bfa3fd5e4fc2e5f in / 
# Wed, 04 Sep 2024 22:44:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c6549675a9e3cbb8e77f08089828d3b4f24a06d332dc86c4d140aa273748ba57`  
		Last Modified: Wed, 04 Sep 2024 22:48:29 GMT  
		Size: 56.1 MB (56076167 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bullseye-20240926`

```console
$ docker pull debian@sha256:eb37f58646a901dc7727cf448cae36daaefaba79de33b5058dab79aa4c04aefb
```

-	Manifest MIME: `application/vnd.oci.image.index.v1+json`
-	Platforms: 0

## `debian:bullseye-20240926-slim`

```console
$ docker pull debian@sha256:eb37f58646a901dc7727cf448cae36daaefaba79de33b5058dab79aa4c04aefb
```

-	Manifest MIME: `application/vnd.oci.image.index.v1+json`
-	Platforms: 0

## `debian:bullseye-backports`

```console
$ docker pull debian@sha256:2b98a8a7f7a06609be2edbfd57ef389bd14bc263b26ca763fb227c3eed686011
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:bullseye-backports` - linux; amd64

```console
$ docker pull debian@sha256:587c5237c1e5dcc34ec56b86e7e99f1483e7d0961409223ca4885fb9f87c1ae6
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.1 MB (55081554 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:96222013099acc06a8cdab30238fcfd8b96e1a7cdec4e8ae0c290d0f068a7830`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:30:56 GMT
ADD file:e1bdcceaa316a43ad58ce8cf054a8e89ecf5a0dbae8125eb85e9b26fdb2fca2b in / 
# Wed, 04 Sep 2024 22:30:57 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:31:01 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:ba83bbfca9443648a883d1404b33faa0f5e096a99a2b683e3bbaee8912bca845`  
		Last Modified: Wed, 04 Sep 2024 22:34:34 GMT  
		Size: 55.1 MB (55081329 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:5242fee86d0d06ed9d60be701bfce4941547befcaa9895a766d67c2721633d68`  
		Last Modified: Wed, 04 Sep 2024 22:34:45 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:335838d0609d9add7fcd697f469b3316c1da7f8bc3bf4693eb76025af95b2b0e
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.2 MB (50240500 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9ae57fa2550ed4cdbc81baae2e63d194e4c93a614e0192bce45301223fd1d020`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:58:17 GMT
ADD file:e3043c245364d9fe593e24bbcac51334dca659e8ec7cd42b6368ba7ea83ea087 in / 
# Wed, 04 Sep 2024 21:58:19 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 21:58:23 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:3e424c6173543918aac0bf0552ca8359d23f7b7baa2d77fc8e2c9deb6756f39d`  
		Last Modified: Wed, 04 Sep 2024 22:02:04 GMT  
		Size: 50.2 MB (50240273 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:48d65d1653752b7dc0dbb4dce6af742732f48cc4edc718fabfcc94e091965b9a`  
		Last Modified: Wed, 04 Sep 2024 22:02:15 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:ebddd62f54fcc5fdb535f48f66fbe0ddcb5b31373a97e9d3c2ceac5b344485d2
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.7 MB (53731844 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:37768909c0dd4f8b4ee5a31940c12dd7949e64fe34aa706cca6a16061ccf97b5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:39:59 GMT
ADD file:aad8b86b3a958bc07504985acedcc819faa4f1ed12ca8b46d8d94c4d564cbdfa in / 
# Wed, 04 Sep 2024 21:40:00 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 21:40:03 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:d82c4492ee91810a42e0c53e955a661e9a092364bc474c9db559ea5b24b7047f`  
		Last Modified: Wed, 04 Sep 2024 21:42:52 GMT  
		Size: 53.7 MB (53731619 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:1e521936ffcc25ff5e098e2a719f2b4421eafc65ef511668c6de4e9a030a9571`  
		Last Modified: Wed, 04 Sep 2024 21:43:02 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; 386

```console
$ docker pull debian@sha256:21da4059ff47c37ef7a4a6ba48b424bb32fab747f02a23a3c4e725700dcbd41b
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.1 MB (56076394 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:75402b4a916ee50eec8b2a447a97902877a2f6e1604011acb4aae62597dc4edd`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:44:44 GMT
ADD file:1d8ae4e7067486ce0279b3d90aaecbc5973872ad64266d252bfa3fd5e4fc2e5f in / 
# Wed, 04 Sep 2024 22:44:45 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:44:49 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:c6549675a9e3cbb8e77f08089828d3b4f24a06d332dc86c4d140aa273748ba57`  
		Last Modified: Wed, 04 Sep 2024 22:48:29 GMT  
		Size: 56.1 MB (56076167 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:081f3e2acb198b98e76ffbf754df57c0cdc911109c6694d14243ade2e17920be`  
		Last Modified: Wed, 04 Sep 2024 22:48:40 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bullseye-slim`

```console
$ docker pull debian@sha256:a3fd77cb2575476ac86ce2077ced3b48d8f7cf22b5444dd1cbad7c1f3fbf22e4
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:bullseye-slim` - linux; amd64

```console
$ docker pull debian@sha256:f94d0d6faaec313eed26c957e7aef75225f0bf8b8376ef83082ed0d8792255f7
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.4 MB (31428677 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a1d53ccb2f5dc360cfb3e2ccc424d8461aab46b32bf47175ad438ba165c18df9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:31:09 GMT
ADD file:1b8ac8ec2b2cbdfc9682f076e7e579579d6df6235ed23b2e63f8eec671c52e92 in / 
# Wed, 04 Sep 2024 22:31:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6533c3eba3f3cd4c840877f9245b26929fc8c22a39f42c872aa314c32c6d654b`  
		Last Modified: Wed, 04 Sep 2024 22:34:54 GMT  
		Size: 31.4 MB (31428677 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:ac122a7511a9d58028712a4c28707d15053f3c011c431056c8858a93af0cd906
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.6 MB (26589615 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:67266cf04539b5b6c6b6f49f38aba2da8202906b210a537f264d0865d4a3be40`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:58:29 GMT
ADD file:c451ece1244c14bce110ffbe6906867ce12f8e88234988b0edba91009a9dbbf8 in / 
# Wed, 04 Sep 2024 21:58:30 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f572303598915f7fb61cc4b47f7207c9ee64d52b9db5fc03ee133ec2dd679347`  
		Last Modified: Wed, 04 Sep 2024 22:02:25 GMT  
		Size: 26.6 MB (26589615 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:fd83fda9b857c7a0dac79eb937029e8554e2553bce0c7590af73813e86f71a5a
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30074365 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:53385f8e66d26a884c129a95e8d1653dbb5197ec25c8a28073f37f74c5b0909d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:40:08 GMT
ADD file:0802ca323252c154049da951c7da8572fb921f823341726dd7664f05c009e50c in / 
# Wed, 04 Sep 2024 21:40:08 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:172514850d5c3a8b3bc66fd0f1f345729d1b0b249cc3f053febcb64a066835da`  
		Last Modified: Wed, 04 Sep 2024 21:43:10 GMT  
		Size: 30.1 MB (30074365 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; 386

```console
$ docker pull debian@sha256:0f810568366d5e9730afa52e94e0ad3855265e44f9ff13b7545fbd16c9b4e029
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32413314 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:59b7adde6af5eb650f77cd5b31bdcbbf9ee9a6424fa7a7a119795c76ce9145da`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:44:56 GMT
ADD file:9177ff00c3abd0afc64f577295a060d88f4daed1042f024f7cfcfcd8cb1da9b8 in / 
# Wed, 04 Sep 2024 22:44:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2e34c6adf259f6e5265d64b5a01b92c3cc93548f22be8c1ccc578b7e9babb30c`  
		Last Modified: Wed, 04 Sep 2024 22:48:51 GMT  
		Size: 32.4 MB (32413314 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:experimental`

```console
$ docker pull debian@sha256:e8fd9608072506d325ada69fcf6b3c17e674a6bbe3be6865f6986b86a7349519
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:experimental` - linux; amd64

```console
$ docker pull debian@sha256:922d5d77a98a9093a824ebf51363ab62c9856ebc3289c8dbfac7132298223096
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53157076 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c3bac9995e218da5b667bee7869683db920b2c922bdebb448eea475e553eae5d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:33:00 GMT
ADD file:af8767ebe9ce7ac3e3dac7f70f7b9751a2024e2cdeec3b7297efb5c06dddfc7b in / 
# Wed, 04 Sep 2024 22:33:01 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:33:12 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:d9a4dea20fdbd3cdc638548943caaeed04e788b6ce934fec7ce325374d8b880d`  
		Last Modified: Wed, 04 Sep 2024 22:37:39 GMT  
		Size: 53.2 MB (53156853 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:9e1f35e8cd9f644e6cfdb29ce5f22f004973bf4366fbb973f9cff72623ceb381`  
		Last Modified: Wed, 04 Sep 2024 22:37:58 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; arm variant v5

```console
$ docker pull debian@sha256:3219adcc5d1030d6fb5f90278ca6d63c421ce716ba3b54012b9c617e145b51a0
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.1 MB (50112637 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7422a850addc21988195516806b08b20c8a49e1b98bc3a4a8d3e3cd3b764bf6f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:50:15 GMT
ADD file:035cc76743532187e73d879925a708b94f4d30d4443fb9f56466a3e28d4b49a5 in / 
# Wed, 04 Sep 2024 21:50:16 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 21:50:25 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:53016ff6120a902ca592b9aade494e94d68b1d815e958dbcb1c159e6dd4186a4`  
		Last Modified: Wed, 04 Sep 2024 21:55:10 GMT  
		Size: 50.1 MB (50112418 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:25667b1c97c7614fb2269be1ee5c562e09c3334d42de7d949a89ca0483d591b8`  
		Last Modified: Wed, 04 Sep 2024 21:55:29 GMT  
		Size: 219.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; arm variant v7

```console
$ docker pull debian@sha256:4347b3905200fc49f63939114a4b2010365b60d78d8bd47b70698b43265c25f9
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.6 MB (47606229 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8177b72ca7367c05691c00bfc93e9503f8471ffa4c148a73f19827717f23e86a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:00:18 GMT
ADD file:188ee06c4b07174ebe1efb972f07c189698c8e6aaec4e4681653eb66ede7229f in / 
# Wed, 04 Sep 2024 22:00:19 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:00:31 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:f0fe53e8c1a634d14df73d8f5c5df0a5976514a2e5bb70738ae9a513a65bdc37`  
		Last Modified: Wed, 04 Sep 2024 22:05:21 GMT  
		Size: 47.6 MB (47606009 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:ff6d1ef272ee9604d9bc413130a443961dcf2ebef77ffc6cd8fdf995e3d5e5d8`  
		Last Modified: Wed, 04 Sep 2024 22:05:40 GMT  
		Size: 220.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:a94925b56e194b592512978045b923ffa4c47711c4f0023ace4a80fbefcd793f
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53597457 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8fc02a9d72214b31205f5a865e9ebccc629f34dab60941775bdaee28992b7461`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:41:26 GMT
ADD file:c73496213c4364e2afe7cef81c73160ee1800949da3213240afd91189005b16a in / 
# Wed, 04 Sep 2024 21:41:27 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 21:41:35 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:7a144dc5a59a2e9a4d1bc4511848122724c21361996780033589efa72cdc9052`  
		Last Modified: Wed, 04 Sep 2024 21:45:42 GMT  
		Size: 53.6 MB (53597236 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:a600f3d8136da5f8c4e5076de5514e98da99383def1983323a40f4c1e6dc3ace`  
		Last Modified: Wed, 04 Sep 2024 21:46:01 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; 386

```console
$ docker pull debian@sha256:aa08999d50c7717e2d75aa2802e7b4a96bfe35eea12491fcde78f004022cbff4
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.0 MB (54033484 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5bd0e1ca14ce5a6c562f6252f0ed02e895df31eb40c295d38449727f1a5576ac`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:46:51 GMT
ADD file:4d810a8a54eebad71b81378e954638abfda42ab042acc973165f8f1c3e708e52 in / 
# Wed, 04 Sep 2024 22:46:52 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:47:04 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:acab95fc207e4ccd7b3c64eea391b8235c092280d53daf4f59883c96eb517415`  
		Last Modified: Wed, 04 Sep 2024 22:52:06 GMT  
		Size: 54.0 MB (54033263 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:3b9fc84bbdc5d4ee16c56689b8f396c2bcb0760eb6e989c1c40ba881a483f920`  
		Last Modified: Wed, 04 Sep 2024 22:52:27 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; mips64le

```console
$ docker pull debian@sha256:d261e58407ce3bb37e70dea0e299c76889e08d8d1330175b70dd8d15b7d2e0ea
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.1 MB (52121298 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1cfb15aba87fd0596bbbc72669c3fd54f09100081461ee47e8afaa3ada03de14`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:21:27 GMT
ADD file:3af3a6c8cd92604c983d6cb309fd33ce4cadcac06889b993c99ebaaeb5bf84cf in / 
# Wed, 04 Sep 2024 22:21:32 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:22:15 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:298ea0ea36a0164973530e4d46c179547253c4db616f218cf9614b1bef05e1c5`  
		Last Modified: Wed, 04 Sep 2024 22:29:45 GMT  
		Size: 52.1 MB (52121076 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:009fa8f6adb140e03b58270a990bf1804360da7449705135f639343ba180ffef`  
		Last Modified: Wed, 04 Sep 2024 22:30:23 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; ppc64le

```console
$ docker pull debian@sha256:b4c88eed0222e76fa0c544091492777bd5740c2bfb7ac79141a31062b17cd8e9
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **57.1 MB (57091226 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e21f6e5692365e823d436a929825ba853b62343cf8481be068164976bcc5c585`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:28:39 GMT
ADD file:3eb6595bda5ffc9328dcb8f2ff158ffcfc395997a08cfb136b17db3b0ed44fa6 in / 
# Wed, 04 Sep 2024 22:28:42 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:28:57 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:7ad1f22c54c3887b009bdbcd3101fd2c7fbb3bd26e6439595f709abb116884bf`  
		Last Modified: Wed, 04 Sep 2024 22:34:07 GMT  
		Size: 57.1 MB (57091005 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:9fcabe676c2b4ac92b08cdb8c25ec491345b8029f47c3d2e7ae102c3ba3418a7`  
		Last Modified: Wed, 04 Sep 2024 22:34:31 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; riscv64

```console
$ docker pull debian@sha256:76e1e1c631a24ec6295e6419029e507309b6b58b6860df23b54ac2da8c2bef9d
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.5 MB (51474077 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bb33dc25fabbe74aee4f238d39b61f5637dac1a401edcbf0fd98a12d3056f9f9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:29:16 GMT
ADD file:c1a313790d4682be73cac33b7195e1244d63c9b47ace37fad97016b24114b491 in / 
# Wed, 04 Sep 2024 22:29:18 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:29:57 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:82ac7467e2ee77de4b3909c282320f0f19ce3fd0d3b9333bab3db2cede3f85e8`  
		Last Modified: Wed, 04 Sep 2024 22:35:05 GMT  
		Size: 51.5 MB (51473854 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:2cf0532746be37966f8c9622e7325217883710b579793e88d08695f30b264310`  
		Last Modified: Wed, 04 Sep 2024 22:35:44 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; s390x

```console
$ docker pull debian@sha256:e5b0ba6010b3422ef5136cfed97e2640f2042757753d29b5e0c1acdfdc90f010
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.8 MB (52756813 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cd41269b8e238f8ec564b4f0ec7bf3f2b180e6babf3482749c748b5ff9d97d8c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:46:08 GMT
ADD file:36ed7c04c3421390b664f58f54282b7c1b195cf0e518af9daeb01fb90a75503e in / 
# Wed, 04 Sep 2024 21:46:10 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 21:46:27 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:77c600cb326d786cab19d46a6eebfeff1cbb1218d5ee0194b5ad30e8ab4763c4`  
		Last Modified: Wed, 04 Sep 2024 21:50:18 GMT  
		Size: 52.8 MB (52756593 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:93914f73df49f80dc675440c1c2c465dd65fa937b013a2d42301127332d7c875`  
		Last Modified: Wed, 04 Sep 2024 21:50:33 GMT  
		Size: 220.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:experimental-20240926`

```console
$ docker pull debian@sha256:eb37f58646a901dc7727cf448cae36daaefaba79de33b5058dab79aa4c04aefb
```

-	Manifest MIME: `application/vnd.oci.image.index.v1+json`
-	Platforms: 0

## `debian:latest`

```console
$ docker pull debian@sha256:b8084b1a576c5504a031936e1132574f4ce1d6cc7130bbcc25a28f074539ae6b
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:latest` - linux; amd64

```console
$ docker pull debian@sha256:3521cd844df5a6f3fd71217af1fc8222db5a7138a753eef86a0550d153184cdf
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49556702 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4fd3f4b75df372cc4fd60fd5c133c666cf4738ca0973523513f564076395f8ad`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:30:36 GMT
ADD file:1129dcf71f67461f4730620f8148cc9ebc7641966fa683cdf84807219ad288b2 in / 
# Wed, 04 Sep 2024 22:30:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8cd46d290033f265db57fd808ac81c444ec5a5b3f189c3d6d85043b647336913`  
		Last Modified: Wed, 04 Sep 2024 22:33:56 GMT  
		Size: 49.6 MB (49556702 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; arm variant v5

```console
$ docker pull debian@sha256:2ce53ff4f45fc8317955a0b614e4ca7f417bd70174beaecd1640b0e7bdc55419
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.3 MB (47333195 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bfda8cf85a5f6f22975fa8d3875b3f44371cb40cf5c8d47954f088b739694890`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:48:22 GMT
ADD file:7ffef6fd95a7091f8058ff29f285ea1ff980523eddf5b8cbcd5ced08753075c5 in / 
# Wed, 04 Sep 2024 21:48:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c9500290eb8f2377c9935666a66a6f2f71959169f4c4c62904bd950e7f1cb1cf`  
		Last Modified: Wed, 04 Sep 2024 21:51:08 GMT  
		Size: 47.3 MB (47333195 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; arm variant v7

```console
$ docker pull debian@sha256:2da114e57f05dc19378e4bf2577293982f238bae9d789233368673b0f5bc96af
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.1 MB (45148448 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d88d3c81ffdd6746cb90e070661481003f8e70ff8a8aa7ec0afdf309841edbb9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:57:56 GMT
ADD file:ce9ce875a73b1b4b1e1c1d3a25f43061406d0cc45595b603c5aaf2eb033490e1 in / 
# Wed, 04 Sep 2024 21:57:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:40a80c95f31d4a590ac5cfb88f8380e036f60bcffc5a805946b43ba82dc5f6d7`  
		Last Modified: Wed, 04 Sep 2024 22:01:19 GMT  
		Size: 45.1 MB (45148448 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:21887a619d762d236e7c66666ca657622ed8749e788322400b86ab09283d8fba
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49585623 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b442aaf370465d5d37b13e37276edad101d308551c0ad5ead5f6bc83a68b1855`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:39:42 GMT
ADD file:7f28c8fde9feb67359cbf19f7d77d3f757490b5f586520257cf92d233b4bfaa4 in / 
# Wed, 04 Sep 2024 21:39:43 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:56c9b9253ff98351db158cb6789848656b8d54f411c0037347bf2358efb18f39`  
		Last Modified: Wed, 04 Sep 2024 21:42:16 GMT  
		Size: 49.6 MB (49585623 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; 386

```console
$ docker pull debian@sha256:16d461b083307155826e009dbfb0fffbd87b4ce3ae09fe03e72513a5abd6ffe7
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50574606 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:40d7f23c7210c00b2412083d922fc0c9d7b8ac3dcd6cfd4a4645383a0555e57e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:44:24 GMT
ADD file:c4052556120bbd9469f83c0cbc2abed04e22bd316491de6954bbbee12ae8b9bf in / 
# Wed, 04 Sep 2024 22:44:25 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e9d39ee40700085571f0a8e492b9b3a1fc65d55e5816aeed53fa9575b0013a98`  
		Last Modified: Wed, 04 Sep 2024 22:47:45 GMT  
		Size: 50.6 MB (50574606 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; mips64le

```console
$ docker pull debian@sha256:00d4c78b0732cac0bdbb0bca5839f5be9be45340affb46c062d5c3343f8f4b93
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49562002 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bc8ff2260a08e41ebba8716a9cbcb79e01bb33ee2e44e3c420881ad0ad45ed28`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:14:59 GMT
ADD file:7a12db48c47f9e4ffb57061ade21f94ff0eb0791886ef60b56a9820f096b39a2 in / 
# Wed, 04 Sep 2024 22:15:05 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:15bebfb1ce6517765581a89c1270a3039857497f5393a3a53eb55a6cf6c3b9e9`  
		Last Modified: Wed, 04 Sep 2024 22:23:29 GMT  
		Size: 49.6 MB (49562002 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; ppc64le

```console
$ docker pull debian@sha256:1899286cd107336e2f2c56acd3b4d0ce4f3e5d86cbf25b8f237fdc9df299115f
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53553949 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6187b3675bb104952a839816bf29d1316e31faac658923562fc7ce58d6470be5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:25:38 GMT
ADD file:c5d3aa6402ede77c4a443935fc6572b655c0144f8f41a2967e2e2b55b4c3343e in / 
# Wed, 04 Sep 2024 22:25:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d0634fe6314ffdff41b21ba805323138b719229ae2c5a32bda44147f688ed49c`  
		Last Modified: Wed, 04 Sep 2024 22:29:47 GMT  
		Size: 53.6 MB (53553949 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; s390x

```console
$ docker pull debian@sha256:909298a718fd7e56ef61d43e80b3c8348c5abdecdcc01a93db51ff4782f25d53
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47939277 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:568a2a1876461b89c4e77c8ddf65168b77968c9233a467d7b57a96816f6fec3c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:42:25 GMT
ADD file:61b2bf58d4b2b852e6903b24131c8a331d7a1afe2cc99c4dd08ddffc91545071 in / 
# Wed, 04 Sep 2024 21:42:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1f3378e3b07f08dbf0b6ce13220a2997d788ba77fb8aa9394ce275771f49ec7a`  
		Last Modified: Wed, 04 Sep 2024 21:47:26 GMT  
		Size: 47.9 MB (47939277 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldstable`

```console
$ docker pull debian@sha256:1d5a03f6636b24bebe36beb22803b48222058285da4172585edc25792c9ca381
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:oldstable` - linux; amd64

```console
$ docker pull debian@sha256:bc9827a1f8fb637535f7b5623ddda5c288dfb0582ae6ec998d55f7cc2e035cca
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.1 MB (55081332 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1b12df970ea5d18f11b9f15b4f6fb65817d17bfba978d053d2b2b9be7bfd8123`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:31:18 GMT
ADD file:fa640ee07f5a1b92a251f94b312b1eb8701db72aca7befd9f6332395216c87cb in / 
# Wed, 04 Sep 2024 22:31:18 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e6c19f19bd45c1f6fe66d27571cf1df1e4fb7a7152998ef94bc7868d64e14d5f`  
		Last Modified: Wed, 04 Sep 2024 22:35:12 GMT  
		Size: 55.1 MB (55081332 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; arm variant v7

```console
$ docker pull debian@sha256:40bfd69800d08668995f68bf745395830b0276aded15f521685cc430f600b739
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.2 MB (50240265 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:44759b089ed0e0f32f12ef2b974405f5f148afdf745c42a5d86dc2efbef76f48`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:58:38 GMT
ADD file:1491f8eddfc713dac57f3c08caaf2ad833f4185044b61395ba260517a39cb5ba in / 
# Wed, 04 Sep 2024 21:58:39 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0ad03ef6392933717d8aaa8084ca76329c1b63498bd61f17227357b08afbf819`  
		Last Modified: Wed, 04 Sep 2024 22:02:46 GMT  
		Size: 50.2 MB (50240265 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:09122bbd57542f448eb08e99cbb8eacf212fe0f829bc1d1c4bc60f9449b1ad04
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.7 MB (53731610 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:900597ff50cbd0dccc4033cb6c3f9a2794a41271d0b2c900d6653600135d8851`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:40:14 GMT
ADD file:8c9f5386a01aaa5837e1e1347350c4ddacf829295eb50a39a7e5b05ac2fd4b65 in / 
# Wed, 04 Sep 2024 21:40:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b0392e397a831b6f1f54a92f4e3593642c5fdf645b301586cfe7f9f58a627184`  
		Last Modified: Wed, 04 Sep 2024 21:43:26 GMT  
		Size: 53.7 MB (53731610 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; 386

```console
$ docker pull debian@sha256:d40946ca29ab50e991b87cc31cefb68bee502df36ac9d0262c42789ea6697b10
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.1 MB (56076147 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d6993e97000d17680e6dc3c3010ec508e4cae5eefb14732de3a57a5316b19b82`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:45:06 GMT
ADD file:80987a1bc79cda72954e1a362e5bab721a49cb043c31d7a5e0a0f6ef7724047a in / 
# Wed, 04 Sep 2024 22:45:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ddc3d843871f2ff4e892fa6a6c6e548fa150061d0f837c1e78195357bf15944b`  
		Last Modified: Wed, 04 Sep 2024 22:49:11 GMT  
		Size: 56.1 MB (56076147 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldstable-20240926`

```console
$ docker pull debian@sha256:eb37f58646a901dc7727cf448cae36daaefaba79de33b5058dab79aa4c04aefb
```

-	Manifest MIME: `application/vnd.oci.image.index.v1+json`
-	Platforms: 0

## `debian:oldstable-20240926-slim`

```console
$ docker pull debian@sha256:eb37f58646a901dc7727cf448cae36daaefaba79de33b5058dab79aa4c04aefb
```

-	Manifest MIME: `application/vnd.oci.image.index.v1+json`
-	Platforms: 0

## `debian:oldstable-backports`

```console
$ docker pull debian@sha256:b70cc471d49473f3d86e99716b5c1af193322c33374b62537522ff9ec3c9961a
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:oldstable-backports` - linux; amd64

```console
$ docker pull debian@sha256:06f14d6293f794573205190153eda56e0fbf0370c7c5481bdd114907d6856e55
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.1 MB (55081559 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d0aa371d472df6292d6aa7374990b16cdbb3f2a9ff69d9134777924f02d802a8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:31:18 GMT
ADD file:fa640ee07f5a1b92a251f94b312b1eb8701db72aca7befd9f6332395216c87cb in / 
# Wed, 04 Sep 2024 22:31:18 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:31:23 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:e6c19f19bd45c1f6fe66d27571cf1df1e4fb7a7152998ef94bc7868d64e14d5f`  
		Last Modified: Wed, 04 Sep 2024 22:35:12 GMT  
		Size: 55.1 MB (55081332 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:e377d1d68fc68df7cabcbbdc3813c29ee929e2a80d530bf338e5eae8aa332fda`  
		Last Modified: Wed, 04 Sep 2024 22:35:19 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:6e02d255907cd2aa796cb52c794e3d22cae656e4239061a674324653ff5e5f51
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.2 MB (50240491 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:55649b38f1653b909ac2e93b334b36039ed0c39ba51eb6ba2c29c721f34f95e1`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:58:38 GMT
ADD file:1491f8eddfc713dac57f3c08caaf2ad833f4185044b61395ba260517a39cb5ba in / 
# Wed, 04 Sep 2024 21:58:39 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 21:58:42 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:0ad03ef6392933717d8aaa8084ca76329c1b63498bd61f17227357b08afbf819`  
		Last Modified: Wed, 04 Sep 2024 22:02:46 GMT  
		Size: 50.2 MB (50240265 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:f3f00c05b05d67c46d071ecd7a483ff06441d48d4434fc32975f353e75d2f5d5`  
		Last Modified: Wed, 04 Sep 2024 22:02:53 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:15afb321091b757de4505365301cac3e2a7083cc828d0bd7f3a27e77feba6ebd
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.7 MB (53731837 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ba25526936242ad3ca6ff22d5660b340eae66a4bbb7f9c242a0976de6f5399c5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:40:14 GMT
ADD file:8c9f5386a01aaa5837e1e1347350c4ddacf829295eb50a39a7e5b05ac2fd4b65 in / 
# Wed, 04 Sep 2024 21:40:15 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 21:40:18 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:b0392e397a831b6f1f54a92f4e3593642c5fdf645b301586cfe7f9f58a627184`  
		Last Modified: Wed, 04 Sep 2024 21:43:26 GMT  
		Size: 53.7 MB (53731610 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:3cc77fdfebf1426538d0db5ce6f0f2bf4cfabf146788b4897b96c2b99ca157ce`  
		Last Modified: Wed, 04 Sep 2024 21:43:33 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-backports` - linux; 386

```console
$ docker pull debian@sha256:994841a528e91a7d0551e6bc0d933b8c20cb46691d94f28e349a29c8b102775c
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.1 MB (56076372 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ef12386d05d53c44368fb918eb9be3a53f69609903606856a3f1fd113189cf9b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:45:06 GMT
ADD file:80987a1bc79cda72954e1a362e5bab721a49cb043c31d7a5e0a0f6ef7724047a in / 
# Wed, 04 Sep 2024 22:45:07 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:45:11 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:ddc3d843871f2ff4e892fa6a6c6e548fa150061d0f837c1e78195357bf15944b`  
		Last Modified: Wed, 04 Sep 2024 22:49:11 GMT  
		Size: 56.1 MB (56076147 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:081f84e18f70edf646620793f4ef38903896430bfe0b1cd5b968bd1eba92b234`  
		Last Modified: Wed, 04 Sep 2024 22:49:18 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldstable-slim`

```console
$ docker pull debian@sha256:a83fc9868abdedc5ce5e043004ef393aa9d1b093f11afc09072b55c4b9c0d504
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 4
	-	linux; amd64
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:oldstable-slim` - linux; amd64

```console
$ docker pull debian@sha256:0aa24f4470f18971d091ecb679ce35936db982b822801c1d53b07529517abba6
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.4 MB (31428660 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:66b64b6ebd825c3f0be6b003b2d66f28facfc8c6ba28777308261847027f66ed`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:31:30 GMT
ADD file:a9d44b7e9a16cbc0f3ba34378a3075f0d41b7de45e5e4f2fc47b49c3d7aa1f80 in / 
# Wed, 04 Sep 2024 22:31:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b84fa374fd19d2d6bcd59e5f05b42fbaa68d78ab16115753cfb19d7671894b2c`  
		Last Modified: Wed, 04 Sep 2024 22:35:28 GMT  
		Size: 31.4 MB (31428660 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:b21413d153e78cf99199401af3908bb3d6843a80d982fedcdcdd21dd474ca386
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.6 MB (26589600 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f69c5166b90bc38af046f86c36e19afb1cb8adb73292d50fd14997b953b5f801`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:58:48 GMT
ADD file:92fd67262d0b43811936ef486ed34a8549fffa1ef3eb3ddbcdc562560bdcae15 in / 
# Wed, 04 Sep 2024 21:58:49 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:20804ac060e660a02415f6dfbdc13e8221ad98b2536e04447487abbcd47ff808`  
		Last Modified: Wed, 04 Sep 2024 22:03:03 GMT  
		Size: 26.6 MB (26589600 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:58dc783d4564bf985e83c57961bce65d84ad1be8bac95b73fa292a0d162853bf
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30074375 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9a2b543fa6bdbaee29ed7789ef668ae4f8769b13b0a46eb6c12529ff8ed20e7b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:40:22 GMT
ADD file:e1673a955283f9ae7f85a6548e7dfd0d125cf63276d844246d94878318cf27ad in / 
# Wed, 04 Sep 2024 21:40:23 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:860ee6a180a2bd8e0211885df3ad48ff7551ce25292f6f7b9208bf0694ba10bc`  
		Last Modified: Wed, 04 Sep 2024 21:43:41 GMT  
		Size: 30.1 MB (30074375 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-slim` - linux; 386

```console
$ docker pull debian@sha256:6ec4f744d634e66eda4bb2c84e0f31d36089379d1d079a3e56f13539b262268b
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32413305 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fda87c724ba03a950366a0e37eb2753db3904790f253a4be11ee21e6a2a65a56`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:45:18 GMT
ADD file:28264b5bd62a4403bfca3cf2862e77393e1191d2d7f37e1983ec80e52dae3b73 in / 
# Wed, 04 Sep 2024 22:45:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:37c223c1e1f51c3cacd9da79d8344814b8fa7c1f27136cd57ad7ec5d2c075f3f`  
		Last Modified: Wed, 04 Sep 2024 22:49:30 GMT  
		Size: 32.4 MB (32413305 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:rc-buggy`

```console
$ docker pull debian@sha256:7d3dfdf68815cb62b3759dda84a9b455a6c9e10a9f3a7906ade0609717f0f294
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:rc-buggy` - linux; amd64

```console
$ docker pull debian@sha256:2145a349f70eaed73879ef5ac12fcb59e52d942bec4490415e55b27dc815eef0
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53157076 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:74de7f344f2fce2768c5eae475f87203d681dc39590f1f17686a512a51508ced`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:31:41 GMT
ADD file:ae19349870cdfda1b68970255f5f5e8f9cd15173da02e9e3404d59044fd19f67 in / 
# Wed, 04 Sep 2024 22:31:41 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:33:15 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:b801efa715ff197e658475851b26398377386b508d479b783c9178607c76738d`  
		Last Modified: Wed, 04 Sep 2024 22:35:42 GMT  
		Size: 53.2 MB (53156851 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:cef1ac38d53081e557c7e83ceadc5231da4faa9f61c426ce4e2f98552fe66e0c`  
		Last Modified: Wed, 04 Sep 2024 22:38:05 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; arm variant v5

```console
$ docker pull debian@sha256:6b9a2b0da1bfa8d767e486d8f5179210b42f78b627c3f09215924979a7ad7f26
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.1 MB (50112643 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3294b00aee7c3edbcb4fcfcb21d06e8da0d5a4d0909c0a79a1522ad78117169a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:49:13 GMT
ADD file:8e4509f73b7c89f169188002c34bc8d03af1dafaeb6a9966b6690f196913262b in / 
# Wed, 04 Sep 2024 21:49:14 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 21:50:27 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:2257c73c251c8a677a6bc4308ec105c9b65d8a38c219fae44f204b71a1e09836`  
		Last Modified: Wed, 04 Sep 2024 21:53:07 GMT  
		Size: 50.1 MB (50112415 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:f09205e6a6a7513348431dee15055d195c20ee45b2592be57736757b8c8d3c92`  
		Last Modified: Wed, 04 Sep 2024 21:55:37 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; arm variant v7

```console
$ docker pull debian@sha256:4d9dcb7ad1b6bcaeebabd69053ba0e202a055efb6d12bf6086f2d15e7780b2b2
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.6 MB (47606233 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9ebd5b1e817eccb0822be3cfbfdc1e4023d4bd670a004fc319b8dca14e8d27d5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:58:58 GMT
ADD file:f0cfa70a19518f2db4a813f0c5ad2bd292465f48e4249c9e0d9007c839212dd0 in / 
# Wed, 04 Sep 2024 21:58:59 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:00:34 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:d301726964d3d126bb25d90398b0f8668a5991c47d9f237ff6b62cb806b4ac84`  
		Last Modified: Wed, 04 Sep 2024 22:03:18 GMT  
		Size: 47.6 MB (47606007 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:67a39215ac26fd059cd9c61860c9b90856d0493348c1861b670602bbe5f77b0b`  
		Last Modified: Wed, 04 Sep 2024 22:05:47 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:016c02b2140d813f4fd1feceeec5105939a01dfb2faacc0b6d659d26a1c4cf9b
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53597457 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b3bd4c836db1ab9647058708f3ada1b278d275376d0e918bd8a197eac6278697`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:40:29 GMT
ADD file:b8e117af2c2835c43068cfc9561a100b4a2ded0418c44e6deb66f2d0a088ee52 in / 
# Wed, 04 Sep 2024 21:40:30 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 21:41:38 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:c86b9ce17f63a085a98e1dec4b1098c61e3e6ddd530e88cc0814e54f39b2ffd3`  
		Last Modified: Wed, 04 Sep 2024 21:43:55 GMT  
		Size: 53.6 MB (53597233 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:cbb17a76187357338a6dc469cb45b107e05b2ef537f509fe8ca317f74d20b8bb`  
		Last Modified: Wed, 04 Sep 2024 21:46:08 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; 386

```console
$ docker pull debian@sha256:ac01334ffe417f457ed4f0587018b76c3ee766d45484d85bc104bba91b4cf9d9
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.0 MB (54033485 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4ed91ceb916e94fb684fe0a0f88f257f05f08b45c64e926529b3fe52dd1ff142`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:45:28 GMT
ADD file:2bc6b9eb390a3ccf12bc1146e52a121a20e72c5ac0e9e0cdde8678b8a64da9f7 in / 
# Wed, 04 Sep 2024 22:45:29 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:47:06 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:3cd4933de503c208d05c5e30920d85a6bfda122663e7dee7dc0ccda09e2913d4`  
		Last Modified: Wed, 04 Sep 2024 22:49:47 GMT  
		Size: 54.0 MB (54033260 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:f0973b60e2239b3d5ed845b461726225e79eec6d8ae136dc26d1eb5accd40462`  
		Last Modified: Wed, 04 Sep 2024 22:52:34 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; mips64le

```console
$ docker pull debian@sha256:d69f9fb87a33713abe0ad1f6ca4581dc7cc852bbbd4d2bb6f45b53d4d71f549f
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.1 MB (52121302 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:eaf37fab2a491a32e828a12f46f512c1ab9f6f313d9cb4f5ace24305b6e779ef`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:16:21 GMT
ADD file:cfc638665fbd1de945c77faa66094fdc1c2a7a3e61a02e7558b48593a3569760 in / 
# Wed, 04 Sep 2024 22:16:26 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:22:26 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:0fe676035759e9ba94505d30bcdda1c4551dcb03be9195f24905e870dce00126`  
		Last Modified: Wed, 04 Sep 2024 22:24:56 GMT  
		Size: 52.1 MB (52121073 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:2c6935ed18b2ba8ce82d7a68e7e98692ba582b3affdd40b012f0670b38d89ecc`  
		Last Modified: Wed, 04 Sep 2024 22:30:32 GMT  
		Size: 229.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; ppc64le

```console
$ docker pull debian@sha256:74609172a2137a3890874595b76f4a5dc459b5a41e0cbef5ff5c47635689eda4
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **57.1 MB (57091228 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7e4bc7bf94e96b290b03c2864f4e296154897f8e028ebcc2903ded4acb78bb8f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:26:57 GMT
ADD file:453c36dbaa10f4b5f78376389c491de28c32bc4115eeb5a4f9dc3a13f8f33c82 in / 
# Wed, 04 Sep 2024 22:27:00 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:29:01 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:70bcb153a291f652093922260a9f87ef3e1b69ec4d2c2aaa4a1c06a4f81059e1`  
		Last Modified: Wed, 04 Sep 2024 22:31:53 GMT  
		Size: 57.1 MB (57091003 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:6e5d855d32901dc1b83930c51d213fa0b6611091bd5a9fcaef70cb82bafbcd0c`  
		Last Modified: Wed, 04 Sep 2024 22:34:38 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; riscv64

```console
$ docker pull debian@sha256:9960c7f43044ff0a5ac6aea01f00ce5210b1bd3981d693b481d5ecf50d54be7e
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.5 MB (51474081 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3ed927086a9230c0e687ad0726fd59afbfcc6a988844efef3a97821b7bc3b42a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:25:42 GMT
ADD file:f7660b52d63bdf7c045c4722f75fe4e353e88b57bffc834348ad141ea0d12995 in / 
# Wed, 04 Sep 2024 22:25:44 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:30:06 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:f9fbdbff518ec38d5367f0b03978c04c3107f39b06d2bb498f646b4903fd13db`  
		Last Modified: Wed, 04 Sep 2024 22:31:12 GMT  
		Size: 51.5 MB (51473852 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:18b9acecfeb10367a53293de4aca7b22eb21ebb799aa3404405682cff688f98f`  
		Last Modified: Wed, 04 Sep 2024 22:35:54 GMT  
		Size: 229.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; s390x

```console
$ docker pull debian@sha256:366b458f9c9bf175e174b15581b07ec2f0d96dc3a08fce29641ec5e14220cb3a
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.8 MB (52756814 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f53fd26250592c3402a6f31b4104e9563f716d9dad5f7d9ddb4ff7bce8bf1dde`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:44:11 GMT
ADD file:22e5ad49d0ca958c0a6f6e6b30f8ff191ced56a0355a55b9de94b5753608a9f9 in / 
# Wed, 04 Sep 2024 21:44:13 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 21:46:34 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:9d384a3df86e2f0599dbfcd66ea74d1e111f1fdf220d3cb1876a56217b3b0016`  
		Last Modified: Wed, 04 Sep 2024 21:48:46 GMT  
		Size: 52.8 MB (52756590 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:3a96da39db2e6f1a73bde03500242156b2f7417ca03c45badad1fdca6c4b09c5`  
		Last Modified: Wed, 04 Sep 2024 21:50:37 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:rc-buggy-20240926`

```console
$ docker pull debian@sha256:eb37f58646a901dc7727cf448cae36daaefaba79de33b5058dab79aa4c04aefb
```

-	Manifest MIME: `application/vnd.oci.image.index.v1+json`
-	Platforms: 0

## `debian:sid`

```console
$ docker pull debian@sha256:08cc1a90963e48072614d24e816bb659e62ca9db485151785e12331998766627
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:sid` - linux; amd64

```console
$ docker pull debian@sha256:5f906c51a918deffe61364732362081aaead5735944189c0d103286b86ca229c
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53156851 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5009a7aa99d609103a3ef20ff23468008a4660dec01614a7f1d23897f4c26e43`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:31:41 GMT
ADD file:ae19349870cdfda1b68970255f5f5e8f9cd15173da02e9e3404d59044fd19f67 in / 
# Wed, 04 Sep 2024 22:31:41 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b801efa715ff197e658475851b26398377386b508d479b783c9178607c76738d`  
		Last Modified: Wed, 04 Sep 2024 22:35:42 GMT  
		Size: 53.2 MB (53156851 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; arm variant v5

```console
$ docker pull debian@sha256:55eb7ea4f3c8c6b4eae0b13d4487b6dca11b2cfcd320d0910ab3d0cb5e8554f8
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.1 MB (50112415 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8c0f6e06cd7db2be3fe41b4deee68b92762735fae887f41c760d4b1d6134a2f8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:49:13 GMT
ADD file:8e4509f73b7c89f169188002c34bc8d03af1dafaeb6a9966b6690f196913262b in / 
# Wed, 04 Sep 2024 21:49:14 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2257c73c251c8a677a6bc4308ec105c9b65d8a38c219fae44f204b71a1e09836`  
		Last Modified: Wed, 04 Sep 2024 21:53:07 GMT  
		Size: 50.1 MB (50112415 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; arm variant v7

```console
$ docker pull debian@sha256:e772c00e1b3040ed4c6b06169b637fcbfaa5e6052b92fefcf8736cac3abd9190
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.6 MB (47606007 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e05cc696135fea5d727f78ecb3d33db66f00bf34868f922e4ae2ca43463231e5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:58:58 GMT
ADD file:f0cfa70a19518f2db4a813f0c5ad2bd292465f48e4249c9e0d9007c839212dd0 in / 
# Wed, 04 Sep 2024 21:58:59 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d301726964d3d126bb25d90398b0f8668a5991c47d9f237ff6b62cb806b4ac84`  
		Last Modified: Wed, 04 Sep 2024 22:03:18 GMT  
		Size: 47.6 MB (47606007 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:11f1ce9a63df81d5ab331c319241f7746d8688129bec4db7066d8c547f5e6baf
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53597233 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b6e9a5d37d62c470cad459532741e65aea6baece647c6a257772a5376e3c5cc2`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:40:29 GMT
ADD file:b8e117af2c2835c43068cfc9561a100b4a2ded0418c44e6deb66f2d0a088ee52 in / 
# Wed, 04 Sep 2024 21:40:30 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c86b9ce17f63a085a98e1dec4b1098c61e3e6ddd530e88cc0814e54f39b2ffd3`  
		Last Modified: Wed, 04 Sep 2024 21:43:55 GMT  
		Size: 53.6 MB (53597233 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; 386

```console
$ docker pull debian@sha256:37374ac78f04c247f208b00014d1c9590744bb4c225219913ff57b5b72eeb836
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.0 MB (54033260 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7d695f75562170f038dac3d6c20281162780d4760d4a040131294005d6dba3f9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:45:28 GMT
ADD file:2bc6b9eb390a3ccf12bc1146e52a121a20e72c5ac0e9e0cdde8678b8a64da9f7 in / 
# Wed, 04 Sep 2024 22:45:29 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3cd4933de503c208d05c5e30920d85a6bfda122663e7dee7dc0ccda09e2913d4`  
		Last Modified: Wed, 04 Sep 2024 22:49:47 GMT  
		Size: 54.0 MB (54033260 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; mips64le

```console
$ docker pull debian@sha256:49b28fb7b97975e1703285f60fdfa6d5b04ffa4cab0ce92841c14ad552f0eced
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.1 MB (52121073 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:237683c0225d70af8f52d8091c65fa57df122564a19f7a60b60c8f2d66b26404`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:16:21 GMT
ADD file:cfc638665fbd1de945c77faa66094fdc1c2a7a3e61a02e7558b48593a3569760 in / 
# Wed, 04 Sep 2024 22:16:26 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0fe676035759e9ba94505d30bcdda1c4551dcb03be9195f24905e870dce00126`  
		Last Modified: Wed, 04 Sep 2024 22:24:56 GMT  
		Size: 52.1 MB (52121073 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; ppc64le

```console
$ docker pull debian@sha256:228450cd37d8f7e2fc4ec5ed197a3ba1eebee65e3945bc4e5db261290b8f2bae
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **57.1 MB (57091003 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e950e1ed50616cba54814dd44b08b6ca7adf0e50be5a297b2e68d8b32b0b1dd4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:26:57 GMT
ADD file:453c36dbaa10f4b5f78376389c491de28c32bc4115eeb5a4f9dc3a13f8f33c82 in / 
# Wed, 04 Sep 2024 22:27:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:70bcb153a291f652093922260a9f87ef3e1b69ec4d2c2aaa4a1c06a4f81059e1`  
		Last Modified: Wed, 04 Sep 2024 22:31:53 GMT  
		Size: 57.1 MB (57091003 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; riscv64

```console
$ docker pull debian@sha256:30cd51d4432861c414fff63abba7727994a2f106dca1c58c1e69bab723395151
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.5 MB (51473852 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:adba4d70d6ac905a70cc8aefb8cb876ec9bc2460a9ebfdd59bcf1cc9fb970e80`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:25:42 GMT
ADD file:f7660b52d63bdf7c045c4722f75fe4e353e88b57bffc834348ad141ea0d12995 in / 
# Wed, 04 Sep 2024 22:25:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f9fbdbff518ec38d5367f0b03978c04c3107f39b06d2bb498f646b4903fd13db`  
		Last Modified: Wed, 04 Sep 2024 22:31:12 GMT  
		Size: 51.5 MB (51473852 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; s390x

```console
$ docker pull debian@sha256:f68f385f8c0bcd6e2f746e512f2a0bca8eef71253ab75c1146542178a5aa86a8
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.8 MB (52756590 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d467162fd7bd8f6d3e24762aeede3f6efce04d954e2056b17044e6a956bec9dc`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:44:11 GMT
ADD file:22e5ad49d0ca958c0a6f6e6b30f8ff191ced56a0355a55b9de94b5753608a9f9 in / 
# Wed, 04 Sep 2024 21:44:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9d384a3df86e2f0599dbfcd66ea74d1e111f1fdf220d3cb1876a56217b3b0016`  
		Last Modified: Wed, 04 Sep 2024 21:48:46 GMT  
		Size: 52.8 MB (52756590 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:sid-20240926`

```console
$ docker pull debian@sha256:eb37f58646a901dc7727cf448cae36daaefaba79de33b5058dab79aa4c04aefb
```

-	Manifest MIME: `application/vnd.oci.image.index.v1+json`
-	Platforms: 0

## `debian:sid-20240926-slim`

```console
$ docker pull debian@sha256:eb37f58646a901dc7727cf448cae36daaefaba79de33b5058dab79aa4c04aefb
```

-	Manifest MIME: `application/vnd.oci.image.index.v1+json`
-	Platforms: 0

## `debian:sid-slim`

```console
$ docker pull debian@sha256:2a1b4de7f18e485af99fcd721e1c4d204d02fe5cb07e852a5a48c16e9da73226
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:sid-slim` - linux; amd64

```console
$ docker pull debian@sha256:ca4b040af6023dc79de96e18bdf7cb5bc44408149064d64c16791fe843cc8ad7
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32363391 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:08ec2547cc88203c4887d527725b51985943ad74b67ad5a2bae90904f41d7e26`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:31:48 GMT
ADD file:6f0ae87127527bf029e644d758f3fed2c672fb3d7070614795a77bc33c90c9c5 in / 
# Wed, 04 Sep 2024 22:31:49 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6f2cba0957894da7c617ce28337be50b863bcf309296f1f24bc36821ec5d16d7`  
		Last Modified: Wed, 04 Sep 2024 22:35:53 GMT  
		Size: 32.4 MB (32363391 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:f5304b1d240047654f1ec4da04c9397c174d74240752fab607b97429763a155f
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.3 MB (29314389 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2ba8429cc19003df7bf0f006a2ca4d4a7eb2b00c41de18d1b6aec2c20b2c3d43`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:49:19 GMT
ADD file:9e846b9ff1d54185bfa2bc75631cec14c55bd8630a9996b41424ad9a879eb7ad in / 
# Wed, 04 Sep 2024 21:49:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2d70863c85a4526fa2ffe3ca571623f1de813ea20222199dbbc23a044e1ea7fd`  
		Last Modified: Wed, 04 Sep 2024 21:53:19 GMT  
		Size: 29.3 MB (29314389 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:ca270876049c176bb855f87012d4fc2da93a2a47b3aa78bf65c7a548b34a96e0
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.8 MB (26808811 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4402747b709ae436ac9acdc06cf8bf4d1892211b2a94483bdfca555d49494d54`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:59:07 GMT
ADD file:2403e59d4b2caf6976388b3e987f9bd517e918931feb0a8460aa97fa87345c6a in / 
# Wed, 04 Sep 2024 21:59:08 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:cb6c156c24e2234dd6c92a084e937cf53f02b8bc7efd3a634b37e4801ad1a16e`  
		Last Modified: Wed, 04 Sep 2024 22:03:31 GMT  
		Size: 26.8 MB (26808811 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:2e3686d1b4c0d65721503ae2c8566a9aa514910afa38544d4fe4e280c6057bc5
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.8 MB (32800218 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:df4050d9c3429de13a2b94bb76e4bfccca8de76604ba50055d273263fb607d8e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:40:35 GMT
ADD file:f7836d9e2916936a249d306d67ec2fc52f67cc11e9b0458bd8d6e832ebfb36eb in / 
# Wed, 04 Sep 2024 21:40:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4d42df383d6d0750b2884d8e8a28677a6acb5a649079ed7043e5b78ab8bf32b3`  
		Last Modified: Wed, 04 Sep 2024 21:44:06 GMT  
		Size: 32.8 MB (32800218 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; 386

```console
$ docker pull debian@sha256:7cca78a6659791fd4f734dda29305264e7cbc8bb5a2ff3bde20cc635ac979903
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.2 MB (33231847 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3056423f8bd4c14a9a91d265e2df38246401799f08ea5e86fef74b517c656cd4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:45:36 GMT
ADD file:a64f465671f4db76364c010628bc0d74c144056462d5b2ccbde64846f832edb2 in / 
# Wed, 04 Sep 2024 22:45:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e22eba89fb867a40d309afb1048b113d390c5562c89afdef539fe08e3e65f1ea`  
		Last Modified: Wed, 04 Sep 2024 22:50:02 GMT  
		Size: 33.2 MB (33231847 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; mips64le

```console
$ docker pull debian@sha256:45402ab7c6fea3037cc4fa6a62291c5cb66c78dd116fea084cbeb3798067b429
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.3 MB (31321875 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:148bc2e0b66385476cc28ced5c9f0a3a5ab2a94b36d639eb942029b8859773c9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:16:53 GMT
ADD file:488ce516b5f21bfb3fd9d77064319b01c1852a3bb2b31daf137a9d63902e0791 in / 
# Wed, 04 Sep 2024 22:16:58 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e3b71eefa4b5a797716b165665f0efd952706b2d1a87cc746af97b870ab87a9c`  
		Last Modified: Wed, 04 Sep 2024 22:25:26 GMT  
		Size: 31.3 MB (31321875 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:a10cae639575e9d0e9ae06f363cb9186f67ee9866960026d5c1de6ad357a0dfe
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **36.3 MB (36290150 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:23e4fa6a1af2d877f52a62d849f9e9bdc46db477c48f3be7f0119ad85a627b6d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:27:09 GMT
ADD file:9b7e418ba5f72fc61a24f9d04d9f155789d63a70ffe902d60eca4bf0022d7420 in / 
# Wed, 04 Sep 2024 22:27:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:617e98823658a5110647a50c9307822f4f834b65aa5c42451708c8aa0f0bc603`  
		Last Modified: Wed, 04 Sep 2024 22:32:07 GMT  
		Size: 36.3 MB (36290150 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; riscv64

```console
$ docker pull debian@sha256:0f759b86a500968b337d8fc4e7a637caed7307925705f57756f8712630ff4bef
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.7 MB (30680011 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b29ec185518a604c783f0e79a97502f50a948287f33d09cb6c0edfe9bf948a98`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:26:11 GMT
ADD file:e0f02b34d0589497764d7b2479f464185612fee3f6716a859c65c8c336005642 in / 
# Wed, 04 Sep 2024 22:26:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6d2b299b33e7b2532cc6a6cbbe6166cb930015db594c673616c50cb52502f45e`  
		Last Modified: Wed, 04 Sep 2024 22:31:42 GMT  
		Size: 30.7 MB (30680011 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; s390x

```console
$ docker pull debian@sha256:5c74735915b98bf9cb97827b02dc5f1af04910ef28f533e32234f952aeaebc30
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.0 MB (31958648 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2682b266d21c4a1979d812148c02dd2a0284a68e6df6917f81a6778c16abd6a1`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:44:22 GMT
ADD file:f561fd07ee41d003f10ba05f65fbd8ac77077711570de519b45342f2f5d0d64b in / 
# Wed, 04 Sep 2024 21:44:24 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:87b7eac039797fd93a3cb6846c2dc4ebc55d653397b5c0c23cd532c462469635`  
		Last Modified: Wed, 04 Sep 2024 21:48:55 GMT  
		Size: 32.0 MB (31958648 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stable`

```console
$ docker pull debian@sha256:cd26d37c976bf0c33078c77a3e7e3456287ad80c274da5d45a9e712f4fe3cd47
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:stable` - linux; amd64

```console
$ docker pull debian@sha256:066531da9dfa9c7fe7411c30044603d3264ceb43c01a9e3cbdb4ce59b924e3d5
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49556731 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d949355c50ffe4ac5b4ea527185410a8e847596b169d10daf9518ed759ca74bf`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:31:57 GMT
ADD file:2c52b73c7ffcba2dddd245d8852699dc49eed34754c00e967efc063644b45be3 in / 
# Wed, 04 Sep 2024 22:31:58 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:666488449e265d667aeda2b2cef0bc12afed899335bffe44fd5bf7456f72e9a1`  
		Last Modified: Wed, 04 Sep 2024 22:36:07 GMT  
		Size: 49.6 MB (49556731 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; arm variant v5

```console
$ docker pull debian@sha256:9b27ebe1defd8ea44f848d704fa0f4f839bc05917636c53043a825e1a86ed23f
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.3 MB (47333176 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:51d53631ddbcb222d58f559b729fc982981736a6782b989263c56e41c1e2a46c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:49:27 GMT
ADD file:ec0bb44b5d3805a2e5db9b8066a2b84d5937d406125c67f97cb850d7524fc743 in / 
# Wed, 04 Sep 2024 21:49:28 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7d8aa53e0f7995da98c9cfdad1f9d8f965a5a84786e4b1ccecb6c758c3525301`  
		Last Modified: Wed, 04 Sep 2024 21:53:33 GMT  
		Size: 47.3 MB (47333176 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; arm variant v7

```console
$ docker pull debian@sha256:7a03019cae8287265027516e59d241a287dbff83e24c4fd49b4f3e8b940a3f2c
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.1 MB (45148451 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:34b5266ed658e19331b2384f5780f4a4503fee6f8dbfaab9778019e3ac8320fc`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:59:17 GMT
ADD file:6d69eee4be8d3dc69096ba6d0ae6083cd08d608e0f6d4bf4c63d87574dda52f6 in / 
# Wed, 04 Sep 2024 21:59:18 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1aacfd79a22fc6f8b7d0a3e28ed5a6a60a17b6a06be91a14ecf3b0de0ba02bed`  
		Last Modified: Wed, 04 Sep 2024 22:03:46 GMT  
		Size: 45.1 MB (45148451 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:f476e7f316278f435808b922624aeebfd39687de7cb58b5d8be3ad7b72fef916
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49585594 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:754344aced5d22d5f21b754c978682ead7e5467de543be09d7a0b830eeca2b5a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:40:42 GMT
ADD file:aefd88d90b7065f1ebd802cd2ce02290bd918d612ccd1e72ed134583a85b87a1 in / 
# Wed, 04 Sep 2024 21:40:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f8763adf53582aae4539130df0a79824468a12d27cb0f988a82aa6c3403901e7`  
		Last Modified: Wed, 04 Sep 2024 21:44:18 GMT  
		Size: 49.6 MB (49585594 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; 386

```console
$ docker pull debian@sha256:eaa574271f104be2f80ed2a99378e7a3c41c325e1cde9acdf274f3d143014de6
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50574598 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:93fa1896e9ca99445e446df10c34568af239d33e71bf67d228cf042bf454b246`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:45:46 GMT
ADD file:4559f9788c783aa5e9fb6e7fad945accc332ceec027dd9538d8311339c5257b1 in / 
# Wed, 04 Sep 2024 22:45:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c00c751b531b3fe84c347b00345768dd683cfdcac65da2539cdd5efb5b79be42`  
		Last Modified: Wed, 04 Sep 2024 22:50:18 GMT  
		Size: 50.6 MB (50574598 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; mips64le

```console
$ docker pull debian@sha256:ede03138dd3a3e3b3e96e0a9acfdf4b60d10fd1be94f3b2b576cb3dd729616dc
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49562004 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:072c39bfce557ab15dd490c0dbe4007f7e4e6eba93f1ba6fb261df0ba125c3a2`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:17:28 GMT
ADD file:cabec016e206ad553b5e022429ce54d85540ce329e716375f22e226573c3580e in / 
# Wed, 04 Sep 2024 22:17:33 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b141cd4b325e0a7013a84c4ab5346ecabc6277e04fac652b2a3c8839c82b17b0`  
		Last Modified: Wed, 04 Sep 2024 22:26:03 GMT  
		Size: 49.6 MB (49562004 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; ppc64le

```console
$ docker pull debian@sha256:733d6ea5962c79ee5e332ea6d1cc7fe3b28c19a5fc7bb26e5984fc776c69a5ec
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53553914 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4e680a6c9761bfbbfcc8f783d838ecbd8db88ccc9c60fd536597e24d857e3ab9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:27:20 GMT
ADD file:ae3bde839efc5a250f67ee7a4d92c9520f9afaff9ca2bbc2ccbde052ac860e59 in / 
# Wed, 04 Sep 2024 22:27:23 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5550293488d0805582bdf4862318ba749cfc0db782025b3fcb0cc3f955682006`  
		Last Modified: Wed, 04 Sep 2024 22:32:24 GMT  
		Size: 53.6 MB (53553914 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; s390x

```console
$ docker pull debian@sha256:92947ce914c13c94a3f535c0d07c7fb4c091c9619547a748f46e66c013e22128
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47939245 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b82865734185df4e2e372340bc81eca1699099634f99b96835af943f533e1462`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:44:34 GMT
ADD file:4b77787828c39ca103962aa8cadf6ce27f696a549ecc4d302e09b14c67e59712 in / 
# Wed, 04 Sep 2024 21:44:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:91a734b2cdf740a10d8113f3dfc48d10b4e0037b5ff88cc5bfc566190c351bc1`  
		Last Modified: Wed, 04 Sep 2024 21:49:06 GMT  
		Size: 47.9 MB (47939245 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stable-20240926`

```console
$ docker pull debian@sha256:eb37f58646a901dc7727cf448cae36daaefaba79de33b5058dab79aa4c04aefb
```

-	Manifest MIME: `application/vnd.oci.image.index.v1+json`
-	Platforms: 0

## `debian:stable-20240926-slim`

```console
$ docker pull debian@sha256:eb37f58646a901dc7727cf448cae36daaefaba79de33b5058dab79aa4c04aefb
```

-	Manifest MIME: `application/vnd.oci.image.index.v1+json`
-	Platforms: 0

## `debian:stable-backports`

```console
$ docker pull debian@sha256:fa073da5c290cf59b30cb37874b1bc47bfd0581e9e2a4b33480fc31571055abf
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:stable-backports` - linux; amd64

```console
$ docker pull debian@sha256:dbd5a178bc174e9f70af98e5df98b35cf49285914e843fdce2b1a3fb0d94df6f
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49556955 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c71743fae6b23d4d64db229ed8195238d766ec5775d7616fc0365113b905ecb9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:31:57 GMT
ADD file:2c52b73c7ffcba2dddd245d8852699dc49eed34754c00e967efc063644b45be3 in / 
# Wed, 04 Sep 2024 22:31:58 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:32:01 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:666488449e265d667aeda2b2cef0bc12afed899335bffe44fd5bf7456f72e9a1`  
		Last Modified: Wed, 04 Sep 2024 22:36:07 GMT  
		Size: 49.6 MB (49556731 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:48ea3b6a4efa6d98921ab7252e921cdc836c41a21fdd2e6fc64e42ffd38f48a6`  
		Last Modified: Wed, 04 Sep 2024 22:36:14 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:dd59f81dd27b7fa7e3a09001ccd9dcb37bb2517a02aff033fe3d0c2eb10116ff
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.3 MB (47333397 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4d9b58ef70d2a509d1ff1f932f59a430ef6781725c33ca1e26d1790f2edbf667`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:49:27 GMT
ADD file:ec0bb44b5d3805a2e5db9b8066a2b84d5937d406125c67f97cb850d7524fc743 in / 
# Wed, 04 Sep 2024 21:49:28 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 21:49:30 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:7d8aa53e0f7995da98c9cfdad1f9d8f965a5a84786e4b1ccecb6c758c3525301`  
		Last Modified: Wed, 04 Sep 2024 21:53:33 GMT  
		Size: 47.3 MB (47333176 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:4d18b07c3a71895a5306627f272eb5d896e76746f558841ecfaa54e11190efdb`  
		Last Modified: Wed, 04 Sep 2024 21:53:40 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:87ee647ae2881679879d7e3976544c373e1d5a08c20a98fe3cd66444e146f9fd
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.1 MB (45148672 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c4227a2edeb112d651cb4dcab8e46296a2b305375012f5b30f09431f1bdd0b15`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:59:17 GMT
ADD file:6d69eee4be8d3dc69096ba6d0ae6083cd08d608e0f6d4bf4c63d87574dda52f6 in / 
# Wed, 04 Sep 2024 21:59:18 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 21:59:21 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:1aacfd79a22fc6f8b7d0a3e28ed5a6a60a17b6a06be91a14ecf3b0de0ba02bed`  
		Last Modified: Wed, 04 Sep 2024 22:03:46 GMT  
		Size: 45.1 MB (45148451 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:ef6ee6d15dd758e70dfdd88d0913801e11eea7a8b8876367f658a8892b5b4522`  
		Last Modified: Wed, 04 Sep 2024 22:03:53 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:f9a40a2b133255bee3de7ba6c7924d216df1ee96033ccd3c2aae7341d44d7cae
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49585815 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f9f9df31f9582b0582b4e791dbf2df41f7ba3501dd1587eae3b0012588438715`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:40:42 GMT
ADD file:aefd88d90b7065f1ebd802cd2ce02290bd918d612ccd1e72ed134583a85b87a1 in / 
# Wed, 04 Sep 2024 21:40:42 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 21:40:45 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:f8763adf53582aae4539130df0a79824468a12d27cb0f988a82aa6c3403901e7`  
		Last Modified: Wed, 04 Sep 2024 21:44:18 GMT  
		Size: 49.6 MB (49585594 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:8d5a1b5652abee13ae1a7e9751fc89adeba1bc70d3542d1144691a4decf6f75d`  
		Last Modified: Wed, 04 Sep 2024 21:44:25 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; 386

```console
$ docker pull debian@sha256:50b3fde9129f1a0d81ca4fb0cf396bdc71acf0ff8d7b595bf9a72ec4340b4835
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50574819 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4f7eec41a28078af006546ccde2f36b5674a4d03547f21b040968ac733980c1e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:45:46 GMT
ADD file:4559f9788c783aa5e9fb6e7fad945accc332ceec027dd9538d8311339c5257b1 in / 
# Wed, 04 Sep 2024 22:45:47 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:45:49 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:c00c751b531b3fe84c347b00345768dd683cfdcac65da2539cdd5efb5b79be42`  
		Last Modified: Wed, 04 Sep 2024 22:50:18 GMT  
		Size: 50.6 MB (50574598 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:51cdbff0aa9ddf76e885f58e34f342e2e76073c9d1825b071bb5c0b269347f33`  
		Last Modified: Wed, 04 Sep 2024 22:50:26 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; mips64le

```console
$ docker pull debian@sha256:451433e834b5d11060eec46c049b03433e31a382f8573213f726edaa5748fc6b
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.6 MB (49562226 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:209a840b19d8630d302316bc50ddcc8f8452d7566d4407e454edaf14c9616506`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:17:28 GMT
ADD file:cabec016e206ad553b5e022429ce54d85540ce329e716375f22e226573c3580e in / 
# Wed, 04 Sep 2024 22:17:33 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:17:49 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:b141cd4b325e0a7013a84c4ab5346ecabc6277e04fac652b2a3c8839c82b17b0`  
		Last Modified: Wed, 04 Sep 2024 22:26:03 GMT  
		Size: 49.6 MB (49562004 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:7c9a61806c2aa9d676834f49ed6c1ac0b6df7dfb16bbda074f322db3794a343c`  
		Last Modified: Wed, 04 Sep 2024 22:26:13 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; ppc64le

```console
$ docker pull debian@sha256:fff657eda805c7b84c8e7f4c72a4497e83e0f943862b128fe85fa10f9857c065
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53554137 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:35e992e5bf143e32614af1467812338d896bf7a94dbb8df95ca1a96861040003`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:27:20 GMT
ADD file:ae3bde839efc5a250f67ee7a4d92c9520f9afaff9ca2bbc2ccbde052ac860e59 in / 
# Wed, 04 Sep 2024 22:27:23 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:27:28 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:5550293488d0805582bdf4862318ba749cfc0db782025b3fcb0cc3f955682006`  
		Last Modified: Wed, 04 Sep 2024 22:32:24 GMT  
		Size: 53.6 MB (53553914 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:530ef257ea4a66219a63a38855443159b912cf0be2897a5b74ecbe952155c825`  
		Last Modified: Wed, 04 Sep 2024 22:32:32 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; s390x

```console
$ docker pull debian@sha256:351197cf69d4d424994e4d123eb85cbf284c8d3e47e1ecef77384e36c00bee5e
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.9 MB (47939467 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:786c14735873ecd1e0b094bafe019187509440f3c940c6af7fd3cae1fac965af`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:44:34 GMT
ADD file:4b77787828c39ca103962aa8cadf6ce27f696a549ecc4d302e09b14c67e59712 in / 
# Wed, 04 Sep 2024 21:44:36 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 21:44:43 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:91a734b2cdf740a10d8113f3dfc48d10b4e0037b5ff88cc5bfc566190c351bc1`  
		Last Modified: Wed, 04 Sep 2024 21:49:06 GMT  
		Size: 47.9 MB (47939245 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:38edae4cac2814b3400f71ab7bc96f6d7a1ad3492fa15adff9ece1d03abbb975`  
		Last Modified: Wed, 04 Sep 2024 21:49:11 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stable-slim`

```console
$ docker pull debian@sha256:64bc71feaa7ec2ac758a6a3a37c0f0d6ebccf0a45e3f5af1f1d3b5d4cb316b29
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:stable-slim` - linux; amd64

```console
$ docker pull debian@sha256:682aa8bc390b6ac3ea0c5ba7db5247a6701b0f7243127a74ee87ccf42787a06f
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29126486 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0767adb87afe098a9bbbeefb08689a8d51bcbcc0237dbaf2f4a27c19070554cd`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:32:08 GMT
ADD file:3f9a5a9060ce494e6f2848e59e36d4fa2238062540fbd0172876019641103e0c in / 
# Wed, 04 Sep 2024 22:32:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:864792f950f9d1c9f0947f4cfd33342bb2cf2e827fdea071a47e5f3f30a20bd1`  
		Last Modified: Wed, 04 Sep 2024 22:36:23 GMT  
		Size: 29.1 MB (29126486 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:0c6cfecc39523edabdf1ff8c1ce2f3e64e795bcaf1673c7b30034a20cb073e5e
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.9 MB (26887425 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e8aebc937b188e4ac72a6faa3b24d2917176fe5487e14e8cd961375395fe361a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:49:36 GMT
ADD file:41a1119fac73210417f3f47ff5378e3d567f001b890195375d00b29a165a664f in / 
# Wed, 04 Sep 2024 21:49:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:12f27c9a9f1f3871729a46a7ede99f3078d3b26de8c4a98939735ead64af8e0c`  
		Last Modified: Wed, 04 Sep 2024 21:53:49 GMT  
		Size: 26.9 MB (26887425 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:79643aa07b8c759fa6713ba08465adff6bd534f7ca1ee194f318ee38f09436da
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.7 MB (24718257 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ccccf0e07d43bc09bfee7fa15bc08c8fefc5bfc7dbe9acfebbe0eee7e19b46f8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:59:27 GMT
ADD file:40e82447f62cec8aff3a6b7d3b2bb1f5d5543cb78e8e158a1edb137b35f541f1 in / 
# Wed, 04 Sep 2024 21:59:28 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4857956e1922c0d65c5bb1aeab99fb89ffa97ec9d34d37f35c518f30e5568b90`  
		Last Modified: Wed, 04 Sep 2024 22:04:02 GMT  
		Size: 24.7 MB (24718257 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:f3af70165241aa465faad97affe1f8355913c7613cfaf7a7ab9566d13a112ee0
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.2 MB (29156516 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a641429485b84e8b1dfa79f3e514598b7be0ed6de24aec75b3f54485251a1931`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:40:50 GMT
ADD file:b54868cad6196df78619393604b51966c0f36f35d6e9d3293dc64eb9978a10c5 in / 
# Wed, 04 Sep 2024 21:40:50 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6dc19f158cfdb059505cc784fbdf347bd13e7878dedba9c23d29d64f6c712d5b`  
		Last Modified: Wed, 04 Sep 2024 21:44:33 GMT  
		Size: 29.2 MB (29156516 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; 386

```console
$ docker pull debian@sha256:976d786cc603ee29f1e2b76d13a297c83bc05ea77469471fd1330f27cc9d6089
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30144313 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c36b10169c0a2c4119c03e06d379cfe97d2b3e9aef2d7de07b97460ad6ae5668`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:45:56 GMT
ADD file:601ac47868188fbcca363da770fe60b84e1a1b5b52afb470ed5060f8cb02c9de in / 
# Wed, 04 Sep 2024 22:45:57 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:400e6c7f56256f113352160fbc2ff722363c2659b28d5e0f4d5265d43e056554`  
		Last Modified: Wed, 04 Sep 2024 22:50:36 GMT  
		Size: 30.1 MB (30144313 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; mips64le

```console
$ docker pull debian@sha256:e15b48e2fe57f4cffad731e2ba3e24063835fa2e52c293ddb2acfc314199fe52
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.1 MB (29125031 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7136fe058bcbab121dd45c41caf6fc91665c1c0a5461d1fd4139566896d7990c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:18:12 GMT
ADD file:2256509aa2d15af526b512de6a313c7e89c6aa48dd2dbea1fd687dd53a532a5c in / 
# Wed, 04 Sep 2024 22:18:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b97975616433365376cefa952de35e4f50b646278d65dab6241efedb488345ca`  
		Last Modified: Wed, 04 Sep 2024 22:26:37 GMT  
		Size: 29.1 MB (29125031 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:d9c96eba3ee87b7fa78d053a7cc5d94a73d4011ba2268e25c38b0c2d7063b334
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.1 MB (33122345 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5176dd554a240b8afe040fd021c1bdb785a9e00cd79f23ba99d7f870406706ad`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:27:35 GMT
ADD file:8c22b74f69aac92daa073bcd897b94947a83f1882f604b0a0e4b563e1a9e22aa in / 
# Wed, 04 Sep 2024 22:27:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:49e0be21019c0546c53e25133538450f9ff36d77f66a60eaab71d28dcabf89ab`  
		Last Modified: Wed, 04 Sep 2024 22:32:42 GMT  
		Size: 33.1 MB (33122345 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; s390x

```console
$ docker pull debian@sha256:cc0ed0af62c1b9e705d2158adfe14e3502f307746ba1b7d933403c2a78e67d6b
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.5 MB (27490320 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f2c6cfe02091e8e28da7adf6d6ddca7547b964cfeb12b49737a17d7c7f0fcb84`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:44:53 GMT
ADD file:db85a89c676219de6abf01250417362b615a2ce7cb9605bae53435c0e937f717 in / 
# Wed, 04 Sep 2024 21:44:54 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:51dc959f5fc2bc1b1bbd330deb32952f8f210bc29d72f66a3069975165b82ae4`  
		Last Modified: Wed, 04 Sep 2024 21:49:18 GMT  
		Size: 27.5 MB (27490320 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:testing`

```console
$ docker pull debian@sha256:f8104e7584b5e5370d17b84fe487989ae8e3e3faacec012a04a852a7fbf616ae
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:testing` - linux; amd64

```console
$ docker pull debian@sha256:7965b37cee33449413970b5417267ac3f56b44edba348be3596e9079497236ac
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53152940 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8f4b48ee77f4d6c5136369b577774d075995e12fec2dd132c8e3dfa2ac7e8a7e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:32:17 GMT
ADD file:1fc45771728eda64de7376c84e22e736076b87ea1b407f029597e49a03bfe372 in / 
# Wed, 04 Sep 2024 22:32:18 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a0989ecf4472e2d3327e122cab700e70a9aea1e22dbc31b893c142eebe6cc665`  
		Last Modified: Wed, 04 Sep 2024 22:36:36 GMT  
		Size: 53.2 MB (53152940 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; arm variant v5

```console
$ docker pull debian@sha256:f99cf47e61ac5f5600307734968423f3220ee6b9676417b364e9125e0170f2cf
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.1 MB (50107073 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0e213656522d8a0afaacd698f2869cf260c9a7eae2c825aea58e16a120dad027`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:49:43 GMT
ADD file:1aff6dddbfddcca844322bda5bfd098a855c07d34fe5704dca7171f5c1f56ece in / 
# Wed, 04 Sep 2024 21:49:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a50fe0a1bbe705d3679428469219f780f9136db589d5f4f8595d79515abf6bcf`  
		Last Modified: Wed, 04 Sep 2024 21:54:03 GMT  
		Size: 50.1 MB (50107073 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; arm variant v7

```console
$ docker pull debian@sha256:935f21802c120f21c54f1803abaf199932cef4ed9cb3a79753ba521799c41462
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.6 MB (47600364 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b71dae6eb3d3531495fa26dec550d19a999a3112738780f4801fb744c291f67d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:59:37 GMT
ADD file:58e7480ad070c187e966d1f49cf6bfd6820edf36f8bfb4adf8cf4a7539a92063 in / 
# Wed, 04 Sep 2024 21:59:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:95514cfe4ae89b3874639f2bbaad4efb19ac5fa1f1880dc900f00a4707da1483`  
		Last Modified: Wed, 04 Sep 2024 22:04:20 GMT  
		Size: 47.6 MB (47600364 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:3aaed8fd8c555c1f7d9992a6a37ccb6d18aeae479b7c7b16b1513a9c19d86d63
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53594358 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:af780185339edd30dfcb1b411ae32018d1977f9e3e498d9f27cfc9fcf65d3cc3`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:40:57 GMT
ADD file:394779f4486ebaf8490ab95548a898d31798f528981c6abde9a64ae9541ad916 in / 
# Wed, 04 Sep 2024 21:40:57 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1d833a2b98580c8c6fbc93eec4d1f948a740704ada36d9bfe4881cc92113a70f`  
		Last Modified: Wed, 04 Sep 2024 21:44:46 GMT  
		Size: 53.6 MB (53594358 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; 386

```console
$ docker pull debian@sha256:55b232d12d39f13207a5f4907050297ca8d5735a0b4a6730d70e705e3e929f9c
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.0 MB (54024268 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8306950b32daad6cfaeb13722b355c3538b1bca0519cadf4bbf1dbb3b3995b85`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:46:07 GMT
ADD file:0fc6bb2c2d834994abff10259e23033c8986cd73484cc8f57bb0010f634d57cd in / 
# Wed, 04 Sep 2024 22:46:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:716214a7650a904a011161c59556cdbcf47abba341c6cfd5163da0ffc9fd7687`  
		Last Modified: Wed, 04 Sep 2024 22:50:53 GMT  
		Size: 54.0 MB (54024268 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; mips64le

```console
$ docker pull debian@sha256:afdca5226e6b1a0c9d8557406770e1258638f33dc8356623d3c82eaad16e1934
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.2 MB (52218422 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:35ceac4201ef159377e17078fac3f243f1429eb395eaee6824827bf227431129`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:18:47 GMT
ADD file:c36c80ea02568189aa3e85f8072a712f381a7231dd4efdeb5737ea44bf812876 in / 
# Wed, 04 Sep 2024 22:18:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:79d5c9efe01709f7e899d8d1b6c857c73fa6bcf862f4440fcb296993dcf36fad`  
		Last Modified: Wed, 04 Sep 2024 22:27:16 GMT  
		Size: 52.2 MB (52218422 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; ppc64le

```console
$ docker pull debian@sha256:8b2dc28af3b561899e5bdbd14c007a92c8e4e73abe26cff1081f7fb37beb70ba
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **57.1 MB (57077769 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c469aa3c0d9aeea45bdb2a0dd77aeee8213ba78c40721fd2e0cd73b39e8cd020`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:27:46 GMT
ADD file:e76ca2322a73a59209e43148c51a8c79f7c3e572eff94a2519628b16edb02b11 in / 
# Wed, 04 Sep 2024 22:27:49 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3b2659f6b1aa74a9122222e29d09e5d1b06a1b6eadfed4e19fd744c16754d022`  
		Last Modified: Wed, 04 Sep 2024 22:32:58 GMT  
		Size: 57.1 MB (57077769 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; riscv64

```console
$ docker pull debian@sha256:0ef650636e9d5b4faf1a308681169f93399e7a388d697cccc49c8757724b12f1
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.5 MB (51475533 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:98a73ca1927ffc1285c02034303d5c88ff537781c3b69105300babe602d04060`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:26:48 GMT
ADD file:8af277225e65f8175288ecc8586a6222e16bd50188e97473bad54f94e4c177a4 in / 
# Wed, 04 Sep 2024 22:26:50 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:934dbf213425c83bf8ffd8f4b0231c081388d35c0108cb1f47338fdd8c4fe172`  
		Last Modified: Wed, 04 Sep 2024 22:32:26 GMT  
		Size: 51.5 MB (51475533 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; s390x

```console
$ docker pull debian@sha256:0139a6963b5baf59367d87f1fbf9d072c6c2351923a1c423498e6ae84f5567ae
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.7 MB (52746481 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:dca23533ea6595bf13442a539e48b51cc5633c646d2c284e8262dededcaed3b1`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:45:05 GMT
ADD file:59e982fbd6b5364ff9e3c3a4656bf6cb6795d637a02c788e4db61959964e2b65 in / 
# Wed, 04 Sep 2024 21:45:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ff4f98e3072ebdb612b0732715fb700fbfdf55f51351132742a0b343fd7fbfc8`  
		Last Modified: Wed, 04 Sep 2024 21:49:29 GMT  
		Size: 52.7 MB (52746481 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:testing-20240926`

```console
$ docker pull debian@sha256:eb37f58646a901dc7727cf448cae36daaefaba79de33b5058dab79aa4c04aefb
```

-	Manifest MIME: `application/vnd.oci.image.index.v1+json`
-	Platforms: 0

## `debian:testing-20240926-slim`

```console
$ docker pull debian@sha256:eb37f58646a901dc7727cf448cae36daaefaba79de33b5058dab79aa4c04aefb
```

-	Manifest MIME: `application/vnd.oci.image.index.v1+json`
-	Platforms: 0

## `debian:testing-backports`

```console
$ docker pull debian@sha256:744b0996502dc54448302e63150d143c2a9281a31efe4116b8d481f403142a66
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:testing-backports` - linux; amd64

```console
$ docker pull debian@sha256:234fd9e64a3c19844eaf741b84f891d962c941f1f11caff1816c4b9b1ce487a7
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53153161 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5e0bcaf2616d16c18df83ed5fb86b3c2a1d750d3301d617eb7927136c421aec8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:32:17 GMT
ADD file:1fc45771728eda64de7376c84e22e736076b87ea1b407f029597e49a03bfe372 in / 
# Wed, 04 Sep 2024 22:32:18 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:32:23 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:a0989ecf4472e2d3327e122cab700e70a9aea1e22dbc31b893c142eebe6cc665`  
		Last Modified: Wed, 04 Sep 2024 22:36:36 GMT  
		Size: 53.2 MB (53152940 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:5560c124ea0daaa63f3661bc68331e7eeff9760baf7e91293cbed1e018efbd6a`  
		Last Modified: Wed, 04 Sep 2024 22:36:44 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:7266d469c5684e64fdc4771279e8c92c1cef612c293a592fc4efc5b55aa07ebb
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.1 MB (50107295 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4776fad490aa9f15fbe31003e87c225c3eaaa88f581896fea262c5070dd80c98`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:49:43 GMT
ADD file:1aff6dddbfddcca844322bda5bfd098a855c07d34fe5704dca7171f5c1f56ece in / 
# Wed, 04 Sep 2024 21:49:44 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 21:49:46 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:a50fe0a1bbe705d3679428469219f780f9136db589d5f4f8595d79515abf6bcf`  
		Last Modified: Wed, 04 Sep 2024 21:54:03 GMT  
		Size: 50.1 MB (50107073 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:a295897e198650ad0c8c7425f41f8695b5e1c319d03971703ae5b389a537b158`  
		Last Modified: Wed, 04 Sep 2024 21:54:12 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:b5c8fb0e94a8b6efe85e57869994cc665e8bdd36a30acaa0543d434a2b5e94d2
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.6 MB (47600589 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6dc410bba7a5b4723193879edf3f122d24b281113797957b2f11320ba9ad6834`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:59:37 GMT
ADD file:58e7480ad070c187e966d1f49cf6bfd6820edf36f8bfb4adf8cf4a7539a92063 in / 
# Wed, 04 Sep 2024 21:59:38 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 21:59:41 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:95514cfe4ae89b3874639f2bbaad4efb19ac5fa1f1880dc900f00a4707da1483`  
		Last Modified: Wed, 04 Sep 2024 22:04:20 GMT  
		Size: 47.6 MB (47600364 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:5ffc6b76bc413b9321f0cebb3a500eb19b2fe5b40096e23161d4d3bf7e45afc0`  
		Last Modified: Wed, 04 Sep 2024 22:04:29 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:259de614e5adbca203a3c5656e9cb3bcfc533c70f3969fa4f572462e9e963b0c
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53594580 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:12a31460be948ad0030a6ba35a8ebb75086127f10daf07a542ee72d3a706c1d4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:40:57 GMT
ADD file:394779f4486ebaf8490ab95548a898d31798f528981c6abde9a64ae9541ad916 in / 
# Wed, 04 Sep 2024 21:40:57 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 21:41:00 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:1d833a2b98580c8c6fbc93eec4d1f948a740704ada36d9bfe4881cc92113a70f`  
		Last Modified: Wed, 04 Sep 2024 21:44:46 GMT  
		Size: 53.6 MB (53594358 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:7651c52de501fd52bde87e9099070ac23c277073f556c906021ba9535f210ade`  
		Last Modified: Wed, 04 Sep 2024 21:44:54 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; 386

```console
$ docker pull debian@sha256:ea9961279bfa6150470c8ca04140d14a7c71dd55a9704c792a402a06221b063b
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.0 MB (54024489 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a749a171ce50cd0977e5a378487d360881216f38a590340f12a79eae0d2ab018`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:46:07 GMT
ADD file:0fc6bb2c2d834994abff10259e23033c8986cd73484cc8f57bb0010f634d57cd in / 
# Wed, 04 Sep 2024 22:46:07 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:46:12 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:716214a7650a904a011161c59556cdbcf47abba341c6cfd5163da0ffc9fd7687`  
		Last Modified: Wed, 04 Sep 2024 22:50:53 GMT  
		Size: 54.0 MB (54024268 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:6064123e551e4278274878883f4a99230985ae1e05d71ef8eef4fb2932540df8`  
		Last Modified: Wed, 04 Sep 2024 22:51:01 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; mips64le

```console
$ docker pull debian@sha256:2f8bd58bc7bc062b14471ab6c66a6bfd16ccaaf5f0210563c5867576d1a5cb90
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.2 MB (52218646 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:29ca0e88af8a6d7f96a3466e6403cf8e5dd5c9ef445601ba19c1ce3ae8002514`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:18:47 GMT
ADD file:c36c80ea02568189aa3e85f8072a712f381a7231dd4efdeb5737ea44bf812876 in / 
# Wed, 04 Sep 2024 22:18:53 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:19:08 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:79d5c9efe01709f7e899d8d1b6c857c73fa6bcf862f4440fcb296993dcf36fad`  
		Last Modified: Wed, 04 Sep 2024 22:27:16 GMT  
		Size: 52.2 MB (52218422 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:8cb91120f20646fa84de82d445e8b17435cdbd6567a1d4552085f1009ec1b02d`  
		Last Modified: Wed, 04 Sep 2024 22:27:26 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; ppc64le

```console
$ docker pull debian@sha256:1443748e880daef042c8161c997e943e8e1bf8a667b1cab20f86f49a0b9218aa
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **57.1 MB (57077990 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:550a6e8986776dba50500118ac82e8603c90d3dc92f12ca000de8167fd3299fd`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:27:46 GMT
ADD file:e76ca2322a73a59209e43148c51a8c79f7c3e572eff94a2519628b16edb02b11 in / 
# Wed, 04 Sep 2024 22:27:49 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:27:54 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:3b2659f6b1aa74a9122222e29d09e5d1b06a1b6eadfed4e19fd744c16754d022`  
		Last Modified: Wed, 04 Sep 2024 22:32:58 GMT  
		Size: 57.1 MB (57077769 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:39199d7e99ca92e2f03e36b4067784f021163201b568056dd5ee2bfde7adccf5`  
		Last Modified: Wed, 04 Sep 2024 22:33:06 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; riscv64

```console
$ docker pull debian@sha256:c5c85d87f9b98231911887f5b6bd8971bbcefe87f2d8279a1356368fcc8aa0c6
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.5 MB (51475755 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:21e1d2d4c5270955ab9b547bd45e4fd06f87d87b96786b22f64c0ce658c7c070`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:26:48 GMT
ADD file:8af277225e65f8175288ecc8586a6222e16bd50188e97473bad54f94e4c177a4 in / 
# Wed, 04 Sep 2024 22:26:50 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:27:03 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:934dbf213425c83bf8ffd8f4b0231c081388d35c0108cb1f47338fdd8c4fe172`  
		Last Modified: Wed, 04 Sep 2024 22:32:26 GMT  
		Size: 51.5 MB (51475533 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:17d2ea2f28c6bbc836f272437123387dd318b6d45efbe5df5d41d99417743361`  
		Last Modified: Wed, 04 Sep 2024 22:32:36 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; s390x

```console
$ docker pull debian@sha256:b5b11fc830864310f26ff3b8d82a2216defd977b66fcc89fb7ca669df575b13a
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.7 MB (52746701 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3d825c97cb5f6fc232bddd7fe36a9d331248943bd4b876c5f13562dba53b5499`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:45:05 GMT
ADD file:59e982fbd6b5364ff9e3c3a4656bf6cb6795d637a02c788e4db61959964e2b65 in / 
# Wed, 04 Sep 2024 21:45:07 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 21:45:15 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:ff4f98e3072ebdb612b0732715fb700fbfdf55f51351132742a0b343fd7fbfc8`  
		Last Modified: Wed, 04 Sep 2024 21:49:29 GMT  
		Size: 52.7 MB (52746481 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:fd9d15d8ccef68b03dfd0744aed04e68b335a892560e79b98cf9cfee1a5a2cac`  
		Last Modified: Wed, 04 Sep 2024 21:49:35 GMT  
		Size: 220.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:testing-slim`

```console
$ docker pull debian@sha256:044a9c8415df59e451323420d35e383f25da38de0f5398792c2f276908662829
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:testing-slim` - linux; amd64

```console
$ docker pull debian@sha256:34a5d8986f2addddf13758c4906c9dc89cab554d82b004ae05a6a7d41e9836a0
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32355887 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f3808436bcd528d9e7c841426e2b5b9d860056b3dde08e38b7a4bb43ac4ff871`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:32:30 GMT
ADD file:d13e32b1fa22b2e0cb6f70403eb0ce6987a15d1aa9f7615c018064370296383e in / 
# Wed, 04 Sep 2024 22:32:30 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:66c5da685a9431319f1cb41ece73fe68048a0d9fd92fc87acb646d9abb8e745b`  
		Last Modified: Wed, 04 Sep 2024 22:36:53 GMT  
		Size: 32.4 MB (32355887 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:01fdaa78d7fd897cdde1a6f2c5fc186401ef9a67ca4913a9f08082f5d4829da4
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.3 MB (29312379 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e046d1f628f38c0d3092ed33a2e9d14fa09fabc9dc2181e8e1297ff18018706a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:49:52 GMT
ADD file:782acada6b32bc95830d6ef4279316c165ea6288235f6f4385df58363a0ba213 in / 
# Wed, 04 Sep 2024 21:49:52 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f0b8576705e61d7bfce587e80c225beb2325f6e8d4d6e09c80ce88191642ab73`  
		Last Modified: Wed, 04 Sep 2024 21:54:22 GMT  
		Size: 29.3 MB (29312379 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:6230a074abfb5e1838912d953a13723f39f951799ca474a463c4b1c1fa6bedc5
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.8 MB (26804505 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:52812d32bd4129a04cd6747d28e34e5fa0d1eb499535cfbfd9efac06502c1161`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:59:48 GMT
ADD file:8532a47de61697b9f91f5d7fe83586a520ecab1bf4a67fb32471b8307a3db5d9 in / 
# Wed, 04 Sep 2024 21:59:49 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1414ee399414a5fdf8ea7dc3c32d615f9b01dc4e8e4e58fa15e5ec149c0c0b85`  
		Last Modified: Wed, 04 Sep 2024 22:04:39 GMT  
		Size: 26.8 MB (26804505 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:eb97a526942ff849ff6e77554779a4c67b8f56984bd44bb568ce983327f37105
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.8 MB (32795344 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a90d2656f899ff15817ff18dc52cea1dac7f00aa9c9aba2de704922ae7c555b2`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:41:05 GMT
ADD file:2b040df41ee203fb3ba336ec9e9c594962441c783a56cadbf4bf6478d4178bcf in / 
# Wed, 04 Sep 2024 21:41:05 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:af8aa7625045002ee1baabecc92ce499a606e8cd6b51a6ac8f3d3ef10b49a7c4`  
		Last Modified: Wed, 04 Sep 2024 21:45:02 GMT  
		Size: 32.8 MB (32795344 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; 386

```console
$ docker pull debian@sha256:166571cca0dd21a05b1dc011a5bdd740c55861480f0433101e07398a2e642d2e
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.2 MB (33227882 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2ece0fe892e19a154180887cf3784b3bc5a80f6b604d9989b90cef24473402cf`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:46:18 GMT
ADD file:e8c9a65f342a4e923f16a5e40935442cdc58512a688d5ba0646bf9018933f1f7 in / 
# Wed, 04 Sep 2024 22:46:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:03457b9fb9b9eab194d93af1ee9290e8ae2b88c71f630502f8cd4ef3c012aeb0`  
		Last Modified: Wed, 04 Sep 2024 22:51:12 GMT  
		Size: 33.2 MB (33227882 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; mips64le

```console
$ docker pull debian@sha256:621fba401a44c477867e57359cd6b5ed8ec9a15d3fb7ca64eb066cc2465208ac
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.4 MB (31419855 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0d5083c7e6f583ce32adcaa976d5d79d2ae356ff839103858e75fbd085f3c067`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:19:31 GMT
ADD file:d7c2ce3ed8a4b906d29e119916927cfa2c53944cdd1140dfb57c3a6effc90855 in / 
# Wed, 04 Sep 2024 22:19:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:dc54e2b19b1be6021bc0efc17178b9438223c08aaa3d46d023799476fecdc179`  
		Last Modified: Wed, 04 Sep 2024 22:27:52 GMT  
		Size: 31.4 MB (31419855 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:75b5676374ad6e6487f74cb58962b547327a010a8213e1760e520a04195c4b73
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **36.3 MB (36278041 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fe6452bf4f8dc68aad2ba363459a8270c88bc8de60f8db7efe729337e61a4d03`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:28:01 GMT
ADD file:3b7bc3974fe5a27b73bb49780c824120451a678181d28324c19e3a533d02cf3a in / 
# Wed, 04 Sep 2024 22:28:03 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:98c979af729b14d6232161112ceb7544edc64fd72dc9553179a9a51b8cfdfd4c`  
		Last Modified: Wed, 04 Sep 2024 22:33:17 GMT  
		Size: 36.3 MB (36278041 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; riscv64

```console
$ docker pull debian@sha256:116d5cb4aa4625f82a409d822af914207e95f802efca940fb844c03c886b36f1
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.7 MB (30680296 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ba6ad8f8d7638b98c44ff3604327a385abdcc96dadc68231b6b9d31f017ca6dc`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:27:25 GMT
ADD file:98b43d04e9420af7cca5c6aa48b6d25531c4548770c63645bb9b4776a8b46b4f in / 
# Wed, 04 Sep 2024 22:27:27 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5b8728573930ea2bf1e3ca095565ac351ef3fb979415f9e9490f16381ab396fb`  
		Last Modified: Wed, 04 Sep 2024 22:33:03 GMT  
		Size: 30.7 MB (30680296 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; s390x

```console
$ docker pull debian@sha256:dd3c1c8346764b96b0c2fb9bd83f9f1d19b6623327ee95582250588917c92d86
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.0 MB (31951075 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:12d6219e5f98a6bf166ba3ce60fb554ebf01c50d70d6b0d6045bec2706b5c9c0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:45:25 GMT
ADD file:5a59592675ae6b09eef5c77114cd5e01eb191dcd54a3eaf1c038f1157d8725aa in / 
# Wed, 04 Sep 2024 21:45:26 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d617f17979dc8bbdcd1643ca7234b835370be99dc8a5038da57ccf81dd17a469`  
		Last Modified: Wed, 04 Sep 2024 21:49:42 GMT  
		Size: 32.0 MB (31951075 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:trixie`

```console
$ docker pull debian@sha256:9f7b513ffd1a78901e6802e47ac03c3e8b5d5bafb06054ed8c757f9a043c2e60
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:trixie` - linux; amd64

```console
$ docker pull debian@sha256:c1acf23aae28db6d6347024734d617f6c895f887bcb4c3e0a02140b14403defa
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53152948 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cdbb325bf57e7870ac6027ad137b62984ee479e8ec6f0c0e891b4a42d459aa13`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:32:39 GMT
ADD file:ed4581bde732d71193f12e8501c7543059ca0c0c5f15f40c1028474d77fb7400 in / 
# Wed, 04 Sep 2024 22:32:39 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2896aaf66dc1af0c9c081d65bb8e63535523af07f049f294d89f549ce0b8febd`  
		Last Modified: Wed, 04 Sep 2024 22:37:07 GMT  
		Size: 53.2 MB (53152948 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie` - linux; arm variant v5

```console
$ docker pull debian@sha256:7ecd5e9deb5c9392fba9ce0a0ce8e7b5c7a93026ecdd65fe06ebe65df2641ecf
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.1 MB (50107080 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d61893120477618891597e1d3b8570670e90fff7140d908f85368a689a76e309`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:49:59 GMT
ADD file:9818fa184cd3832d7a2e958d07cbdf6f16e481580a7abf2ca8cab41151573065 in / 
# Wed, 04 Sep 2024 21:50:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:59aa270c3f603e92ab2ded83e0aa06f94cd27fb99cc5c9166b81e03b32d905a6`  
		Last Modified: Wed, 04 Sep 2024 21:54:36 GMT  
		Size: 50.1 MB (50107080 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie` - linux; arm variant v7

```console
$ docker pull debian@sha256:9aa9a4c7533e557ce15856b8f37f86079ca34c7ecca4fa44ab9fea74412288a4
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.6 MB (47600373 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4f744fd5f3c2c7df1f9ab33f4d2f1b9cee86eff739dc18d79facae23c9e43e64`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:59:58 GMT
ADD file:09cac9d3912a7a93696c655d21effc386d2cb7ff21832c28ce416cd006e647f3 in / 
# Wed, 04 Sep 2024 21:59:59 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b4fae8e82872ebbe80a6660f7c8967b490f12720a699b611d1fc451d460e9c10`  
		Last Modified: Wed, 04 Sep 2024 22:04:53 GMT  
		Size: 47.6 MB (47600373 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:a1ff2b99873304afe6d6d748ac9207dc98a0a748aa5f68abe6c844392eb91b4b
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53594338 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:36b8995861b16d97ea7c8367db3a9c160907b8d446e3557c58e6f7c2c2fc2cc9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:41:11 GMT
ADD file:0d3579de5c93cf19bff9f7634a0a159ccc6f879b5b3b127688adfdb71440fc3a in / 
# Wed, 04 Sep 2024 21:41:12 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6df7e1b9cec91c022805e35821c4d6cf9ec8f98fd36df834cd2b60410faffd11`  
		Last Modified: Wed, 04 Sep 2024 21:45:14 GMT  
		Size: 53.6 MB (53594338 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie` - linux; 386

```console
$ docker pull debian@sha256:b962092cc2a016ab6dc510e3717b9c737b8c48fe070bd71514632c4403c9c4e4
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.0 MB (54024286 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:632a5b039980cccd9c31be1e117baae7683039e8bd3e604ff6ed07ed8a97c957`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:46:29 GMT
ADD file:6ca0a177e1bacc5298df02655f64b86d9c9b9ef5ac4afddf6bf3b8ffb6845a5d in / 
# Wed, 04 Sep 2024 22:46:29 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3fc63360233033ade654647f98461e34e405a88696c8a8470032f9ca8e3d1a43`  
		Last Modified: Wed, 04 Sep 2024 22:51:30 GMT  
		Size: 54.0 MB (54024286 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie` - linux; mips64le

```console
$ docker pull debian@sha256:c5178762ab0274c2b184ae8f11b48a65d6dc738ca5fe63742018039de20f3bf7
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.2 MB (52218452 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:78bc946e5047693521330566f3fdc2c1597400349a58c65d1ac57330273b8960`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:20:06 GMT
ADD file:1d08fc8d7e30f98aa4f42de7aad81e751ab03c9887521ea6bc5e7f7ccdac33a1 in / 
# Wed, 04 Sep 2024 22:20:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b4eb2d8078dccea930e749ede5720f2f057f44ed6d24c4a5fefb751441787ce4`  
		Last Modified: Wed, 04 Sep 2024 22:28:31 GMT  
		Size: 52.2 MB (52218452 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie` - linux; ppc64le

```console
$ docker pull debian@sha256:29fcf50fd538cfcd90d757ff87c8951fe4cc70c1a75aff6e22853a7120cebd9f
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **57.1 MB (57077783 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:30794a10ca47419c38b7e5f8c2d06f225748df7f4ac05fac596a6365c0aaf64b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:28:12 GMT
ADD file:72b28cd12b51875b02b32c08c19d9763a8b995f28917285fc8c454420a98ee23 in / 
# Wed, 04 Sep 2024 22:28:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8d710a2aeb72c4bf2e1980a336408edaec79dc17907bf54201076e8a3da2f3f1`  
		Last Modified: Wed, 04 Sep 2024 22:33:33 GMT  
		Size: 57.1 MB (57077783 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie` - linux; riscv64

```console
$ docker pull debian@sha256:65012f9911213e3c59f5578883932dcc4b895f1d0019c497756c866c799f3058
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.5 MB (51475526 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d590afc55277c0cb8c7e2f16481b4a153deadd73fa1c5b3d9c48b24f5de9d1a9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:28:03 GMT
ADD file:3536049154c2250abe969642d7b35b085e3d25447b8953cc1e072b690a306aa1 in / 
# Wed, 04 Sep 2024 22:28:05 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:38a3e7af214e5e0705f0ceebc8bef13cf31414ef0eb3f3ad8a05574549c77869`  
		Last Modified: Wed, 04 Sep 2024 22:33:47 GMT  
		Size: 51.5 MB (51475526 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie` - linux; s390x

```console
$ docker pull debian@sha256:175166c5b361d29cfff9afec22fedb7ffb31ebb60774a1b7d2572b07c5f1f953
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.7 MB (52746453 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fa54469a406a399a6c6c54c6783fdf1ea56a2c982cbb0e2bc1c53e73ebcedb96`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:45:38 GMT
ADD file:d8c6bc2767ccd7e1cc72d27d8bcb6ae32ab902174ec47c087c8a760be26c991c in / 
# Wed, 04 Sep 2024 21:45:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:35e23b80cf3051c9b7be598388de687426f0b24d259df069449caf8431f56244`  
		Last Modified: Wed, 04 Sep 2024 21:49:53 GMT  
		Size: 52.7 MB (52746453 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:trixie-20240926`

```console
$ docker pull debian@sha256:eb37f58646a901dc7727cf448cae36daaefaba79de33b5058dab79aa4c04aefb
```

-	Manifest MIME: `application/vnd.oci.image.index.v1+json`
-	Platforms: 0

## `debian:trixie-20240926-slim`

```console
$ docker pull debian@sha256:eb37f58646a901dc7727cf448cae36daaefaba79de33b5058dab79aa4c04aefb
```

-	Manifest MIME: `application/vnd.oci.image.index.v1+json`
-	Platforms: 0

## `debian:trixie-backports`

```console
$ docker pull debian@sha256:0c840f6f38ee0a0ab8c00e73cccd2f9bd0295800df07c6d6fa8eefc8046b7c85
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:trixie-backports` - linux; amd64

```console
$ docker pull debian@sha256:3674af50cda5c838675e5e9e97bc3e7a3870f8824553c4720e5cdff2542bc5c4
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53153173 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:11b252f52c2117c126ae6bb9991b38571f41697444b058c0cc37fd00d01301c0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:32:39 GMT
ADD file:ed4581bde732d71193f12e8501c7543059ca0c0c5f15f40c1028474d77fb7400 in / 
# Wed, 04 Sep 2024 22:32:39 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:32:44 GMT
RUN echo 'deb http://deb.debian.org/debian trixie-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:2896aaf66dc1af0c9c081d65bb8e63535523af07f049f294d89f549ce0b8febd`  
		Last Modified: Wed, 04 Sep 2024 22:37:07 GMT  
		Size: 53.2 MB (53152948 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:57a928115156124e1003c72c6da74c08bc78500c0df71319b6e33b224280a9e4`  
		Last Modified: Wed, 04 Sep 2024 22:37:15 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:d8c1bf06082b31f1380fbc60ee5dd0d858b73140bf310900d9b52e398e2aa0c1
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.1 MB (50107303 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cd64f0d206f2f20fdf33d70c7555be4a10cc458957b6e3e96f2c3156f725ec92`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:49:59 GMT
ADD file:9818fa184cd3832d7a2e958d07cbdf6f16e481580a7abf2ca8cab41151573065 in / 
# Wed, 04 Sep 2024 21:50:00 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 21:50:02 GMT
RUN echo 'deb http://deb.debian.org/debian trixie-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:59aa270c3f603e92ab2ded83e0aa06f94cd27fb99cc5c9166b81e03b32d905a6`  
		Last Modified: Wed, 04 Sep 2024 21:54:36 GMT  
		Size: 50.1 MB (50107080 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:b1c0158545cfe41fa0d7399535951c162d3bdcec0ee0963cfbbc546dbcbe1514`  
		Last Modified: Wed, 04 Sep 2024 21:54:44 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:93d2b780b11329e21982f5527da272c7ee8495757fc7d0d1d487901429f0e110
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.6 MB (47600599 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2630fb569bf5d0c71b8bade2b0105a1cd14f3a67c84a7444a7c417b77b326295`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:59:58 GMT
ADD file:09cac9d3912a7a93696c655d21effc386d2cb7ff21832c28ce416cd006e647f3 in / 
# Wed, 04 Sep 2024 21:59:59 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:00:02 GMT
RUN echo 'deb http://deb.debian.org/debian trixie-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:b4fae8e82872ebbe80a6660f7c8967b490f12720a699b611d1fc451d460e9c10`  
		Last Modified: Wed, 04 Sep 2024 22:04:53 GMT  
		Size: 47.6 MB (47600373 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:cdca1c41ebee8045460f54596a0070ddaf60cd6319d5d4962425025e2a5919d0`  
		Last Modified: Wed, 04 Sep 2024 22:05:00 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:56b5e7b1a26cf95e2625d302baffff068f3901d1f589d33f95a3e24102866ab3
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53594563 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:237620039e3ee4aae0fec74a430db3b8e2e627d1e9a2bede1b5cab770403f303`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:41:11 GMT
ADD file:0d3579de5c93cf19bff9f7634a0a159ccc6f879b5b3b127688adfdb71440fc3a in / 
# Wed, 04 Sep 2024 21:41:12 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 21:41:15 GMT
RUN echo 'deb http://deb.debian.org/debian trixie-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:6df7e1b9cec91c022805e35821c4d6cf9ec8f98fd36df834cd2b60410faffd11`  
		Last Modified: Wed, 04 Sep 2024 21:45:14 GMT  
		Size: 53.6 MB (53594338 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:94f3cb60707e31637c943150854ad9be94c696f2fa1ba4b310f10879bb6a1ca0`  
		Last Modified: Wed, 04 Sep 2024 21:45:22 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-backports` - linux; 386

```console
$ docker pull debian@sha256:f54b3ce576d4d6c2dd2ca8d444a2fef80dd2055c85eccdbcaae8b4f326c52b5e
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.0 MB (54024509 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:db97281d74ed5e302da8e027461bec0ce00f8f17e99fc4d3fbd107301540fe72`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:46:29 GMT
ADD file:6ca0a177e1bacc5298df02655f64b86d9c9b9ef5ac4afddf6bf3b8ffb6845a5d in / 
# Wed, 04 Sep 2024 22:46:29 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:46:33 GMT
RUN echo 'deb http://deb.debian.org/debian trixie-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:3fc63360233033ade654647f98461e34e405a88696c8a8470032f9ca8e3d1a43`  
		Last Modified: Wed, 04 Sep 2024 22:51:30 GMT  
		Size: 54.0 MB (54024286 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:e1a27d699688e09241c00952262bf780922ca881e0504f610e69d2dd50269fbc`  
		Last Modified: Wed, 04 Sep 2024 22:51:38 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-backports` - linux; mips64le

```console
$ docker pull debian@sha256:44f8fa1be6b6562bece6039227b11b9c2bfeaec8fc9481622f4f8a7f7bb8dd88
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.2 MB (52218677 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1fdb32bc18942b9c10999f0cdb688bba28af6355370f4029dfb28cdb60fa978f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:20:06 GMT
ADD file:1d08fc8d7e30f98aa4f42de7aad81e751ab03c9887521ea6bc5e7f7ccdac33a1 in / 
# Wed, 04 Sep 2024 22:20:11 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:20:27 GMT
RUN echo 'deb http://deb.debian.org/debian trixie-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:b4eb2d8078dccea930e749ede5720f2f057f44ed6d24c4a5fefb751441787ce4`  
		Last Modified: Wed, 04 Sep 2024 22:28:31 GMT  
		Size: 52.2 MB (52218452 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:3fb967ff4a80a637116cfa40f28ad8e83c790a1256c72fec14328034d3d4f45b`  
		Last Modified: Wed, 04 Sep 2024 22:28:40 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-backports` - linux; ppc64le

```console
$ docker pull debian@sha256:20a68c381cd7a857767ebe6e68f46465285ef632e0ca567bdb43d5f211a00c4f
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **57.1 MB (57078008 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5af949dd86ac5754efb3c58d959bfaab398860778c9d49faebb090759b2809f8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:28:12 GMT
ADD file:72b28cd12b51875b02b32c08c19d9763a8b995f28917285fc8c454420a98ee23 in / 
# Wed, 04 Sep 2024 22:28:15 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:28:19 GMT
RUN echo 'deb http://deb.debian.org/debian trixie-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:8d710a2aeb72c4bf2e1980a336408edaec79dc17907bf54201076e8a3da2f3f1`  
		Last Modified: Wed, 04 Sep 2024 22:33:33 GMT  
		Size: 57.1 MB (57077783 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:e356975183d4c46a0239b8113369d5e06569d957f994cb6f25418eb1342090f2`  
		Last Modified: Wed, 04 Sep 2024 22:33:41 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-backports` - linux; riscv64

```console
$ docker pull debian@sha256:96aad8116036d4d010e4f2bc278ad248e89c7656c58e10752ed50c12bd55ea8b
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.5 MB (51475751 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1a42d8ce856fd234709ff99aaa64a19e9aae2f6e872f552ddc579f496fd9237e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:28:03 GMT
ADD file:3536049154c2250abe969642d7b35b085e3d25447b8953cc1e072b690a306aa1 in / 
# Wed, 04 Sep 2024 22:28:05 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 22:28:17 GMT
RUN echo 'deb http://deb.debian.org/debian trixie-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:38a3e7af214e5e0705f0ceebc8bef13cf31414ef0eb3f3ad8a05574549c77869`  
		Last Modified: Wed, 04 Sep 2024 22:33:47 GMT  
		Size: 51.5 MB (51475526 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:abe853d9c0fbb58c46d314446eeb6458612c8bdf15d74756dcf70dfbbc608cd3`  
		Last Modified: Wed, 04 Sep 2024 22:33:56 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-backports` - linux; s390x

```console
$ docker pull debian@sha256:972f65a1be5b24b6fee02d1f8fde22686f5ab954daf14fc8733d95acf62a4b5f
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.7 MB (52746678 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fc400e03cd0bb6be74d0cd994fd96c66e468b3ad1a7c289165048d96ef1a72a5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:45:38 GMT
ADD file:d8c6bc2767ccd7e1cc72d27d8bcb6ae32ab902174ec47c087c8a760be26c991c in / 
# Wed, 04 Sep 2024 21:45:40 GMT
CMD ["bash"]
# Wed, 04 Sep 2024 21:45:47 GMT
RUN echo 'deb http://deb.debian.org/debian trixie-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:35e23b80cf3051c9b7be598388de687426f0b24d259df069449caf8431f56244`  
		Last Modified: Wed, 04 Sep 2024 21:49:53 GMT  
		Size: 52.7 MB (52746453 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:4bcec886a2ce18dd72e992d37233999cfbbb785926023daecb929517c3399a9f`  
		Last Modified: Wed, 04 Sep 2024 21:49:59 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:trixie-slim`

```console
$ docker pull debian@sha256:43788d0fe1fee841d09707fbb9355c938c6eb5b0c6bde2de31a07443f1bc6750
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:trixie-slim` - linux; amd64

```console
$ docker pull debian@sha256:b8818aa9e75b892246f7522ce28d57bf28ee0ac29c3aa25e47560efac021397c
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32355860 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:05b38a2c2a1888910112234c2e7673440f5e5d6b088bae92d9e4ece3ab1cd3db`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:32:51 GMT
ADD file:d4e3f8f1f0c766dab8b55aeb88b4b7c9291179d8a5306b4b012ea5a41c50f545 in / 
# Wed, 04 Sep 2024 22:32:52 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e4ad3369125dc4897d9974445833d7f089562fa4257d5afc4fdc53585c4521b4`  
		Last Modified: Wed, 04 Sep 2024 22:37:24 GMT  
		Size: 32.4 MB (32355860 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:274c67af8b2d0093bda966545ca0e0d3a77ac7d2f6d40639d59a126d84f301e6
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.3 MB (29312403 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cca33619a5af7db8f79a0969714dfbb285a2ae0b70f3a4be4261ac3fdd41a364`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:50:08 GMT
ADD file:dfd972b2081ec5ef6ea1f5aa9dee95d7dc3dea9de4cb862dcb3e2a5075452735 in / 
# Wed, 04 Sep 2024 21:50:08 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:35a1413a403ce6f0db09f02f4288e3f5e8606acde0c94a8b032f65af0f533100`  
		Last Modified: Wed, 04 Sep 2024 21:54:54 GMT  
		Size: 29.3 MB (29312403 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:d08ac29e24d384df7d0d5ab9be4755f8c73e7ee3808c090626984359038e58a4
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.8 MB (26804520 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:501e9e4b3771e24814d681b5e55a8dbc8d80c8e6eb7177d218b214e5165792d2`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:00:08 GMT
ADD file:f2bc49beadf45a32d53de6c482f6885197ffd7f2d4e0c2552050aa3bc7d2d945 in / 
# Wed, 04 Sep 2024 22:00:10 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:04361eb3fadc393f1a0f20b27aa65f4b234cae9809c7a45625e97041c375440b`  
		Last Modified: Wed, 04 Sep 2024 22:05:08 GMT  
		Size: 26.8 MB (26804520 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:689d844a5e539f8b59c3b507f0a0f14033c184490c13a9f5e6287ba8289d9e7e
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.8 MB (32795330 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:db16521ffc26349617799331b09744b54d2eaf7d54f8b8402acdea50ff15aabc`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:41:20 GMT
ADD file:c5529b66baee58b3ad056a24dabaab4abca8f2ed258e95130f541458824bc121 in / 
# Wed, 04 Sep 2024 21:41:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8c78c23a0b828751ef7844a0bb255fe18367690c7df6ec58c6688c1eba3ba1ac`  
		Last Modified: Wed, 04 Sep 2024 21:45:30 GMT  
		Size: 32.8 MB (32795330 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-slim` - linux; 386

```console
$ docker pull debian@sha256:d80214935426b02c162971a3213e1a14312f01a632c68f0ef09f27dcd6f33267
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.2 MB (33227885 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bea54b0b4b7831eeff302e51f23f46da6006adaf52aa49f89ca91e23caef104a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:46:41 GMT
ADD file:171bcad7f48984aaab01a304f37531fe38c0506d567f06a5600121c58a48f359 in / 
# Wed, 04 Sep 2024 22:46:41 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6227a3209108da373bac1023e72ce216b0efc37322530ac844cc361fdc229c74`  
		Last Modified: Wed, 04 Sep 2024 22:51:49 GMT  
		Size: 33.2 MB (33227885 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-slim` - linux; mips64le

```console
$ docker pull debian@sha256:5286efd15a69d485fd6f999bb234ad16fe58dd93ae5723b999c84019523ac269
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.4 MB (31419853 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:66cb79d902758057cfd8fcf0ece581ee13003ec237ba52b0530e3d2ffca4493d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:20:49 GMT
ADD file:b29383693790705c2c85bb59d3cf7bf3d80732d901856b2b9173e303aaff742c in / 
# Wed, 04 Sep 2024 22:20:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5009a284951ec613d2749aaa7e9ee44595048a0d3311cc4b2a0d148f317a870b`  
		Last Modified: Wed, 04 Sep 2024 22:29:06 GMT  
		Size: 31.4 MB (31419853 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:05e713a079077c742332a76bb6a5036ce64fbf8920f95347b9a4780d19887695
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **36.3 MB (36278032 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:aaf0c66c7be93ff2cb108307a50fa010ace27b6c4aac765205a17360c58893ec`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:28:27 GMT
ADD file:de2f89298e12731ebb9fafceefa2aacbc807bfbde14a4fe4d5e456c29455e08e in / 
# Wed, 04 Sep 2024 22:28:29 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9fcf7f5af1c91490840059374979d1c2b1d81bb0b03cc964eb4f33be624e1fa3`  
		Last Modified: Wed, 04 Sep 2024 22:33:52 GMT  
		Size: 36.3 MB (36278032 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-slim` - linux; riscv64

```console
$ docker pull debian@sha256:36e90a235f947b5e822ef3c1a380038e84e0caee5a8124c09e7b790e8de8e52b
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.7 MB (30680316 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:337bc332c5d5efdba050b982b7e01c3f369a40d2e4c08efb6b44823cfadf4738`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:28:40 GMT
ADD file:b605481b2b8d402e0c98c98fdaabe0b9a9d551ddcd8ed8560e85477383e69510 in / 
# Wed, 04 Sep 2024 22:28:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7fd02d0433eac185e9c533d7bc05ac6b87ee2af9fa61fa771a9639e144efad2b`  
		Last Modified: Wed, 04 Sep 2024 22:34:22 GMT  
		Size: 30.7 MB (30680316 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:trixie-slim` - linux; s390x

```console
$ docker pull debian@sha256:f28b284ac8b5eeab472af6f811be7d5e12e86868298de31a6a28ce526acb974b
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.0 MB (31951059 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:da3c6388df0dba6d71cf0c7b155c5f7261a701df6f76ac9e8fa1a1d0b9c7c14e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:45:56 GMT
ADD file:a521d51791c746158da3d65cc9aa866064db8ba4ade93920260df3a07a69e868 in / 
# Wed, 04 Sep 2024 21:45:58 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:423a9f870272983880dfdb04d6c1e021bee6818bbd75e9cce24d9b46ae872d8c`  
		Last Modified: Wed, 04 Sep 2024 21:50:06 GMT  
		Size: 32.0 MB (31951059 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:unstable`

```console
$ docker pull debian@sha256:4f9b105ea78ff6e98385c617db07c27010f12267a50a90efb7651e9e29621ae4
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:unstable` - linux; amd64

```console
$ docker pull debian@sha256:6793bacb8422992f9afdfece11244abf7d4cd343fff791ca65200d9abd67cea9
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53156853 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9065cd12104b046e42c8c836584dc872bf7640a992814762cee8ed0094a38a37`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:33:00 GMT
ADD file:af8767ebe9ce7ac3e3dac7f70f7b9751a2024e2cdeec3b7297efb5c06dddfc7b in / 
# Wed, 04 Sep 2024 22:33:01 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d9a4dea20fdbd3cdc638548943caaeed04e788b6ce934fec7ce325374d8b880d`  
		Last Modified: Wed, 04 Sep 2024 22:37:39 GMT  
		Size: 53.2 MB (53156853 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; arm variant v5

```console
$ docker pull debian@sha256:b8be3142beb1ede3dd10b7aa23eb7195fdf2db6335cd64e0cbac1e3f840cf6ac
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.1 MB (50112418 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4e90667b5d4f8a2cd3a9f8b8a0bf5fdcd68ce202192166f8a3307e6d3ddb4488`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:50:15 GMT
ADD file:035cc76743532187e73d879925a708b94f4d30d4443fb9f56466a3e28d4b49a5 in / 
# Wed, 04 Sep 2024 21:50:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:53016ff6120a902ca592b9aade494e94d68b1d815e958dbcb1c159e6dd4186a4`  
		Last Modified: Wed, 04 Sep 2024 21:55:10 GMT  
		Size: 50.1 MB (50112418 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; arm variant v7

```console
$ docker pull debian@sha256:3c25fb97f08fa3c028642add48bcd9c0b7e2386a2de282a3c58e0b2a82aaf782
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **47.6 MB (47606009 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:623ab58ba8a0f4dda46dd4d7bbaf273c8f2e062399699464f070608cd6514ce6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:00:18 GMT
ADD file:188ee06c4b07174ebe1efb972f07c189698c8e6aaec4e4681653eb66ede7229f in / 
# Wed, 04 Sep 2024 22:00:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f0fe53e8c1a634d14df73d8f5c5df0a5976514a2e5bb70738ae9a513a65bdc37`  
		Last Modified: Wed, 04 Sep 2024 22:05:21 GMT  
		Size: 47.6 MB (47606009 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:5b03718be2f3b54c7c104e80feba3ddf78aa1d4c5ad1c9fb5bd6ca3fcfa64726
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53597236 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:99d001f6d0a2e69b2a98e1f15a0bf8bf1921ea3c4a945062bdf77bddf049e031`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:41:26 GMT
ADD file:c73496213c4364e2afe7cef81c73160ee1800949da3213240afd91189005b16a in / 
# Wed, 04 Sep 2024 21:41:27 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7a144dc5a59a2e9a4d1bc4511848122724c21361996780033589efa72cdc9052`  
		Last Modified: Wed, 04 Sep 2024 21:45:42 GMT  
		Size: 53.6 MB (53597236 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; 386

```console
$ docker pull debian@sha256:7edf637fa85c74eb97224a55d8eb789aef8ca8ad13c1e92a161b04ca22ff3f74
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.0 MB (54033263 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7ebb810c00ff9bdab18824f22e6590a1aaf8f0389e89f51f0fe2c44acfd72b32`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:46:51 GMT
ADD file:4d810a8a54eebad71b81378e954638abfda42ab042acc973165f8f1c3e708e52 in / 
# Wed, 04 Sep 2024 22:46:52 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:acab95fc207e4ccd7b3c64eea391b8235c092280d53daf4f59883c96eb517415`  
		Last Modified: Wed, 04 Sep 2024 22:52:06 GMT  
		Size: 54.0 MB (54033263 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; mips64le

```console
$ docker pull debian@sha256:6c5bfa8e9f3d87e339f0fbefdd1d66b1128c1e4bf61df4bcb8fa355c8017dde6
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.1 MB (52121076 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:03ef0c9a6caa1cbe6160181b3cc9faf12cc846a83543825ae6ebd367d2f63079`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:21:27 GMT
ADD file:3af3a6c8cd92604c983d6cb309fd33ce4cadcac06889b993c99ebaaeb5bf84cf in / 
# Wed, 04 Sep 2024 22:21:32 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:298ea0ea36a0164973530e4d46c179547253c4db616f218cf9614b1bef05e1c5`  
		Last Modified: Wed, 04 Sep 2024 22:29:45 GMT  
		Size: 52.1 MB (52121076 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; ppc64le

```console
$ docker pull debian@sha256:4d2b0ac5319a760d712ea37cc3fc92885990ba940161b18cc4230db2b527818b
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **57.1 MB (57091005 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4d3b1894e705676fa6dc9dd7c93e2931bef048590a82d15bb7bf47bdbf2250d4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:28:39 GMT
ADD file:3eb6595bda5ffc9328dcb8f2ff158ffcfc395997a08cfb136b17db3b0ed44fa6 in / 
# Wed, 04 Sep 2024 22:28:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7ad1f22c54c3887b009bdbcd3101fd2c7fbb3bd26e6439595f709abb116884bf`  
		Last Modified: Wed, 04 Sep 2024 22:34:07 GMT  
		Size: 57.1 MB (57091005 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; riscv64

```console
$ docker pull debian@sha256:e1b77b615f622473f0ec760edb8ec0fccb2e6a91677f4e855e5760946b8e353c
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.5 MB (51473854 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:282dad6ced6faf7f69ba107c1155d652be6fe827f724cc2a8fc5b8c3bfa9164a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:29:16 GMT
ADD file:c1a313790d4682be73cac33b7195e1244d63c9b47ace37fad97016b24114b491 in / 
# Wed, 04 Sep 2024 22:29:18 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:82ac7467e2ee77de4b3909c282320f0f19ce3fd0d3b9333bab3db2cede3f85e8`  
		Last Modified: Wed, 04 Sep 2024 22:35:05 GMT  
		Size: 51.5 MB (51473854 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; s390x

```console
$ docker pull debian@sha256:1ef9fff97278cb16877a82d32869d9c23edc9aa91067e2a2e1aa2d0f73fdc057
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.8 MB (52756593 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:df3d1a7a7c63072add029f2e8ea578b863d02f1e4454c557a5d1cce26b96690b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:46:08 GMT
ADD file:36ed7c04c3421390b664f58f54282b7c1b195cf0e518af9daeb01fb90a75503e in / 
# Wed, 04 Sep 2024 21:46:10 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:77c600cb326d786cab19d46a6eebfeff1cbb1218d5ee0194b5ad30e8ab4763c4`  
		Last Modified: Wed, 04 Sep 2024 21:50:18 GMT  
		Size: 52.8 MB (52756593 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:unstable-20240926`

```console
$ docker pull debian@sha256:eb37f58646a901dc7727cf448cae36daaefaba79de33b5058dab79aa4c04aefb
```

-	Manifest MIME: `application/vnd.oci.image.index.v1+json`
-	Platforms: 0

## `debian:unstable-20240926-slim`

```console
$ docker pull debian@sha256:eb37f58646a901dc7727cf448cae36daaefaba79de33b5058dab79aa4c04aefb
```

-	Manifest MIME: `application/vnd.oci.image.index.v1+json`
-	Platforms: 0

## `debian:unstable-slim`

```console
$ docker pull debian@sha256:8b29383aadfaf164cd6f27d3c213e710a06457f28ae8597e18efd5eacc6ad6b3
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:unstable-slim` - linux; amd64

```console
$ docker pull debian@sha256:c8c885f76483a339233ee6fac26f53c1ddbca3a82da3ee352506e51247e6bec4
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32363393 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:882bc518251c5835682b676be7bcac0ef0a45f16700301064b4a9e73aad785ad`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:33:08 GMT
ADD file:656ebaee03f32f55c923aeead9db83ccb52337ef0577594a5f749d40d9b80d96 in / 
# Wed, 04 Sep 2024 22:33:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:bb1ae7f1aa8b80aab4feb0d357102ad792a80968d1bdcb204d8bda19559b5f16`  
		Last Modified: Wed, 04 Sep 2024 22:37:50 GMT  
		Size: 32.4 MB (32363393 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:33d1ebd88b01e0d1dfb8dc5679a8750d876b383a21e5db6ddfa94843efab302f
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.3 MB (29314392 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3f12b6bd732879daff07e762719d0437408fce2a37ff1e88238328426abeeb3e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:50:22 GMT
ADD file:c4d3113890a610db827a8d5f6582bd7efb6a644880e6510773f1769e4ce01250 in / 
# Wed, 04 Sep 2024 21:50:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b3f34332c60e479905e9b91390e4dfb005f8b189314dd8be387448e87cc3d13a`  
		Last Modified: Wed, 04 Sep 2024 21:55:21 GMT  
		Size: 29.3 MB (29314392 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:73cd5159c1a808e05b0f6341440ed80ce9bc20bb41577670d376d35e70cc8c4e
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.8 MB (26808814 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f34832f2d9ab2cfc097790a8c7a0f43926a416aa1c783ec9ed08311e1e8ac933`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:00:26 GMT
ADD file:328240dccc5c59c7181d1415c1c72602f654da3176644d23e9359739edb6aad7 in / 
# Wed, 04 Sep 2024 22:00:28 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:10467f9e615e6f88c76c8128d2e20e36fb79e7118ad6cb0f7052bb4fc9560f61`  
		Last Modified: Wed, 04 Sep 2024 22:05:33 GMT  
		Size: 26.8 MB (26808814 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:bc6cddf5dc7e7e01171bd5dcaf0a0c7323abb0edccd29799f76dfe7577eb785f
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.8 MB (32800221 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9a0e22843a52446e7af89ea37c2ff43965197f61f48e2ccdea5474801bd6a941`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:41:32 GMT
ADD file:4bec688b08d56cd2061b6fdd19e9ff3ab89f152fdc2e5e2f94a8c5436b0e49c8 in / 
# Wed, 04 Sep 2024 21:41:33 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:036bfa80212c306a67393143a9b89d94dd86f2f9a6f75d8b6a5cf0fdc2824076`  
		Last Modified: Wed, 04 Sep 2024 21:45:53 GMT  
		Size: 32.8 MB (32800221 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; 386

```console
$ docker pull debian@sha256:6a9f03df5ef4a446539e19cfe6fc48ff1ecc58db7fa3da5c83a2b0d38faaec48
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **33.2 MB (33231850 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:77adbb74f7a37e7b63bfbea1951da7890b3a86e2971fd334731c9b15bcbb8a2d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:47:01 GMT
ADD file:cebb1bb6381e16eb15e97fc8f99de77b30696f8f276aa4cfdf2168fdcf55b471 in / 
# Wed, 04 Sep 2024 22:47:01 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2ed359ca0b847a4e18518b8e7f4033e05cc7467027183ce8384ee63620b30812`  
		Last Modified: Wed, 04 Sep 2024 22:52:20 GMT  
		Size: 33.2 MB (33231850 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; mips64le

```console
$ docker pull debian@sha256:7a0d0d2bac83f456c757144f4bb8f1f76085b064e22a1af8ed9bb3ad797800bb
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.3 MB (31321878 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:43a9d0bf71f771882ea703ad062a6a239cae332c5a4248866418c185ca2487e0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:21:58 GMT
ADD file:e7d47da3da9cbc2a3b81f06545d8738775c2bf357e465e966937c8d885b1d391 in / 
# Wed, 04 Sep 2024 22:22:01 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4bed7cdbc82405f220571cace94a832c5d20cc091cea6a5abeb30af2121abf6a`  
		Last Modified: Wed, 04 Sep 2024 22:30:14 GMT  
		Size: 31.3 MB (31321878 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:d34d153f688d2208a5168c31c4fec4bbf15823ec6a3a4946e17bec8abe5a9469
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **36.3 MB (36290152 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ef66b1c48345372fd6e5289d3766ff251daf1a0d670a13fb208d30ef55bb29da`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:28:50 GMT
ADD file:63ad481320a558e633a43586cd4701f21904c5b45e6aa0d478427dc474d70218 in / 
# Wed, 04 Sep 2024 22:28:52 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f89ad42a1a3f998422946cb96bd9f5ad84d7e189be691f470b0409d3d2c4f617`  
		Last Modified: Wed, 04 Sep 2024 22:34:21 GMT  
		Size: 36.3 MB (36290152 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; riscv64

```console
$ docker pull debian@sha256:02ecede3c6b8e19fe8d66f257dbe793cb1c4f4743d5c04dcb06a3cb410fb4ee0
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.7 MB (30680014 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8cad678d41b33ef84023dadede9a07d6c0b94382c0cf2ec93c7f0a8266955f5f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 22:29:46 GMT
ADD file:0a0b5f06b61421e927194892bf03b07399b3c069ffe46dd35dee218301909737 in / 
# Wed, 04 Sep 2024 22:29:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:46e004c87513ace2185e4c881596f5039e2003928ddf0e2b7f69cf77f1ecf35a`  
		Last Modified: Wed, 04 Sep 2024 22:35:35 GMT  
		Size: 30.7 MB (30680014 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; s390x

```console
$ docker pull debian@sha256:4bfbbbe038e59b413799a4f8eaff58954ab7b20ef413470c6e0374dc467171c3
```

-	Docker Version: 23.0.11
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.0 MB (31958650 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:42f2aa813d8fbb438faf1af5b1e4d8eea3f670a2f541ff1300a3126d132ced5c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 04 Sep 2024 21:46:19 GMT
ADD file:6fbca1d022e62f054e3e27b7aed20465e2f77bed693f28d18900db55bb02a4b7 in / 
# Wed, 04 Sep 2024 21:46:21 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:331d6eb20f21e1d2b8b8beee692c51b5278dd9e88342472a36d82ca7ca82d34f`  
		Last Modified: Wed, 04 Sep 2024 21:50:27 GMT  
		Size: 32.0 MB (31958650 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
