<!-- THIS FILE IS GENERATED VIA './update-remote.sh' -->

# Tags of `debian`

-	[`debian:10`](#debian10)
-	[`debian:10-slim`](#debian10-slim)
-	[`debian:10.11`](#debian1011)
-	[`debian:10.11-slim`](#debian1011-slim)
-	[`debian:11`](#debian11)
-	[`debian:11-slim`](#debian11-slim)
-	[`debian:11.1`](#debian111)
-	[`debian:11.1-slim`](#debian111-slim)
-	[`debian:9`](#debian9)
-	[`debian:9-slim`](#debian9-slim)
-	[`debian:9.13`](#debian913)
-	[`debian:9.13-slim`](#debian913-slim)
-	[`debian:bookworm`](#debianbookworm)
-	[`debian:bookworm-20211115`](#debianbookworm-20211115)
-	[`debian:bookworm-20211115-slim`](#debianbookworm-20211115-slim)
-	[`debian:bookworm-backports`](#debianbookworm-backports)
-	[`debian:bookworm-slim`](#debianbookworm-slim)
-	[`debian:bullseye`](#debianbullseye)
-	[`debian:bullseye-20211115`](#debianbullseye-20211115)
-	[`debian:bullseye-20211115-slim`](#debianbullseye-20211115-slim)
-	[`debian:bullseye-backports`](#debianbullseye-backports)
-	[`debian:bullseye-slim`](#debianbullseye-slim)
-	[`debian:buster`](#debianbuster)
-	[`debian:buster-20211115`](#debianbuster-20211115)
-	[`debian:buster-20211115-slim`](#debianbuster-20211115-slim)
-	[`debian:buster-backports`](#debianbuster-backports)
-	[`debian:buster-slim`](#debianbuster-slim)
-	[`debian:experimental`](#debianexperimental)
-	[`debian:experimental-20211115`](#debianexperimental-20211115)
-	[`debian:latest`](#debianlatest)
-	[`debian:oldoldstable`](#debianoldoldstable)
-	[`debian:oldoldstable-20211115`](#debianoldoldstable-20211115)
-	[`debian:oldoldstable-20211115-slim`](#debianoldoldstable-20211115-slim)
-	[`debian:oldoldstable-backports`](#debianoldoldstable-backports)
-	[`debian:oldoldstable-slim`](#debianoldoldstable-slim)
-	[`debian:oldstable`](#debianoldstable)
-	[`debian:oldstable-20211115`](#debianoldstable-20211115)
-	[`debian:oldstable-20211115-slim`](#debianoldstable-20211115-slim)
-	[`debian:oldstable-backports`](#debianoldstable-backports)
-	[`debian:oldstable-slim`](#debianoldstable-slim)
-	[`debian:rc-buggy`](#debianrc-buggy)
-	[`debian:rc-buggy-20211115`](#debianrc-buggy-20211115)
-	[`debian:sid`](#debiansid)
-	[`debian:sid-20211115`](#debiansid-20211115)
-	[`debian:sid-20211115-slim`](#debiansid-20211115-slim)
-	[`debian:sid-slim`](#debiansid-slim)
-	[`debian:stable`](#debianstable)
-	[`debian:stable-20211115`](#debianstable-20211115)
-	[`debian:stable-20211115-slim`](#debianstable-20211115-slim)
-	[`debian:stable-backports`](#debianstable-backports)
-	[`debian:stable-slim`](#debianstable-slim)
-	[`debian:stretch`](#debianstretch)
-	[`debian:stretch-20211115`](#debianstretch-20211115)
-	[`debian:stretch-20211115-slim`](#debianstretch-20211115-slim)
-	[`debian:stretch-backports`](#debianstretch-backports)
-	[`debian:stretch-slim`](#debianstretch-slim)
-	[`debian:testing`](#debiantesting)
-	[`debian:testing-20211115`](#debiantesting-20211115)
-	[`debian:testing-20211115-slim`](#debiantesting-20211115-slim)
-	[`debian:testing-backports`](#debiantesting-backports)
-	[`debian:testing-slim`](#debiantesting-slim)
-	[`debian:unstable`](#debianunstable)
-	[`debian:unstable-20211115`](#debianunstable-20211115)
-	[`debian:unstable-20211115-slim`](#debianunstable-20211115-slim)
-	[`debian:unstable-slim`](#debianunstable-slim)

## `debian:10`

```console
$ docker pull debian@sha256:e5333f8697a86fa1be53b2a7e994247083f61885166df0cdda9f812acb514d7c
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:10` - linux; amd64

```console
$ docker pull debian@sha256:89c36409980d71271f8b46e327cda450ede47542d182671145e92773b752e153
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50437098 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4906f6b1e4402ac10252e2231ea0cb36dd72b494e8d1505bfa23ad79cabb950d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:20:51 GMT
ADD file:cb5ed7070880d4c0177fbe6dd278adb7926e38cd73e6abd582fd8d67e4bbf06c in / 
# Wed, 17 Nov 2021 02:20:51 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1230f17f526cb07307d41462355b10488b994e8bdafe0d8f275a405fa3b33831`  
		Last Modified: Wed, 17 Nov 2021 02:26:06 GMT  
		Size: 50.4 MB (50437098 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10` - linux; arm variant v5

```console
$ docker pull debian@sha256:600c5fc194e46731b51dec790839ccc9af45cd37a1982eb289924e66339dce85
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **48.2 MB (48154304 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bbd8f738f959a12ae79abc575d02024dd757f535f87330a6bdd422cfa32b3a61`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:51:05 GMT
ADD file:b19cf756330e0e1577062a56604b50597d35a6f4be67ac86210ddf4b4203a32d in / 
# Wed, 17 Nov 2021 02:51:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:141c2db3ba99259b105dd1f16b4f6365959cb21d4978470a6bd73ef86ef15d08`  
		Last Modified: Wed, 17 Nov 2021 03:06:59 GMT  
		Size: 48.2 MB (48154304 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10` - linux; arm variant v7

```console
$ docker pull debian@sha256:8b0544ec886aacd2cc8ebd70695d01bb7b292e899da3e3d5ce4a5f174a29e4d2
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45918099 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6f800c08efc4810532acfdaade1f1edb9a0a724ea504e9e3972c7a770f218d74`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:00:12 GMT
ADD file:d71807613acdf86685f6d640a90e27b7b63cfe0f13d88668ee943aca089d8876 in / 
# Wed, 17 Nov 2021 02:00:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:10acd81022d17f452439d0385615fa066b8f0ddcf3c1a803872eec5bdc6acf64`  
		Last Modified: Wed, 17 Nov 2021 02:16:00 GMT  
		Size: 45.9 MB (45918099 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:a90e90c601b9358c34f18a864c02c4f9e8527cc4f79c2f5d3779e8dd11fd5d53
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49222990 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d5af59919bab386fa40f9282adc3769cbfb687d3cf86570a8bd827d8c9ceafce`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:27 GMT
ADD file:c7e9e884b1494f96e33e9d9a892b51367af8943edb282c4101cc99ea78a7753f in / 
# Wed, 17 Nov 2021 02:40:28 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:626979e62c56e192812e818d52b82f063d9f9514b1b7bae14e534e4a7c98117a`  
		Last Modified: Wed, 17 Nov 2021 02:47:40 GMT  
		Size: 49.2 MB (49222990 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10` - linux; 386

```console
$ docker pull debian@sha256:b840656acb560ba0d10da515368d47f4dc1bb8ffa8df678653d6959063d24c97
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51207728 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3e751f1d934e26435fca29c8eb20e64100daf4f0ccf0b77f4482411ceabbb5b6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:06 GMT
ADD file:87367b4870b0dc997a37b7648ff6efa8e42bed68b0573be2ade992c206fbdd2e in / 
# Wed, 17 Nov 2021 02:40:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1f740e90292e8fcb22dbbbdbe335252fd87d406cd6e139738853fc65c8bb54f4`  
		Last Modified: Wed, 17 Nov 2021 02:48:17 GMT  
		Size: 51.2 MB (51207728 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10` - linux; mips64le

```console
$ docker pull debian@sha256:56bc08b7eb6db843bc54606a37cfa7a302e8059430b0cf0b62005893283a7b84
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.1 MB (49079535 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:598bee8f0f0e68db53acfa1bd428cb335f6c8d94cf370bcd6a7d74bc465ac010`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:10:39 GMT
ADD file:4769a173ec92e69a1b5cd79af0ed018bf150d1ee278de07f4bdab45f55f3c818 in / 
# Wed, 17 Nov 2021 02:10:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8c028c43f55209ca75650433b923c8952e16d106529242daad16c3be3efd66a3`  
		Last Modified: Wed, 17 Nov 2021 02:19:53 GMT  
		Size: 49.1 MB (49079535 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10` - linux; ppc64le

```console
$ docker pull debian@sha256:180ef8c7873776990cfe1d853cbeced32438e7e87dbe079be541ba8bd2fd6ca5
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.2 MB (54183702 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:27d2b2aa100535ac6ebeae5978be19d3f1732010b93251d59a6a4a7cb159f6c4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:29:10 GMT
ADD file:fc0d43ba74311deef20b2131cca3a8e86b083624d63560251191adca21417f2f in / 
# Wed, 17 Nov 2021 03:29:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:aad6e1fe6f7c56b0e1b5850f40e4fec91ffaed4b6235e730f5c7ff43db397d3d`  
		Last Modified: Wed, 17 Nov 2021 03:56:39 GMT  
		Size: 54.2 MB (54183702 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10` - linux; s390x

```console
$ docker pull debian@sha256:f4bb73391d3779dfe30a3ab72aa444bff7164c2326ab80f55302e8b584463ac5
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.0 MB (49005439 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:20b18aa76fa9a01d5ef11e20329da3e4097e7771eb7310578351dfdbc8bae229`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:40 GMT
ADD file:dc4ee96f81059accdc88a686f80f1714bc9d2f383af71c0d130c08560cea115e in / 
# Wed, 17 Nov 2021 02:42:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c12eae84d2674466d924a139a276e361afa9f63de21cf77eae19bd94eaac2629`  
		Last Modified: Wed, 17 Nov 2021 02:48:42 GMT  
		Size: 49.0 MB (49005439 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:10-slim`

```console
$ docker pull debian@sha256:cfadaa70a452076a478dcfcf2da3a3dc3b741450afed5ab67a7f3df161387c48
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:10-slim` - linux; amd64

```console
$ docker pull debian@sha256:69b11eb7768d4783408354087ff1719d875a9f218e4ac76b39e9c35792048968
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.2 MB (27153675 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cad9ce16f840672d1615867950084a0de9b7022ec6fff89e05bde0d32632e281`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:21:02 GMT
ADD file:3c54ad257f2e04f7294ce879b884820cf4726c8e93ec548172825963e40c79ad in / 
# Wed, 17 Nov 2021 02:21:02 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a10c77af261312e7c92bcc184f2d1726175ff7f142e44b01c5779cd79348b9fd`  
		Last Modified: Wed, 17 Nov 2021 02:26:31 GMT  
		Size: 27.2 MB (27153675 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:9ad9871d1ab8f29c2d32b7c1d807f8106ac173edf4c682c8cdc071db124694e7
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.9 MB (24886310 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d4cbba96ee571035afdcd4a54b8250797669f76c63804c5eb78647e94cddc985`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:51:39 GMT
ADD file:e580d4d2066301375327ea51dd8db3af956e5a465495bf09d69d6deec9b0bfae in / 
# Wed, 17 Nov 2021 02:51:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:dba255a4b166bb02562ca152d0da236ba8f211de5bf9d79e35ee023b1fce203e`  
		Last Modified: Wed, 17 Nov 2021 03:07:41 GMT  
		Size: 24.9 MB (24886310 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:d83f190bbdc742a155dff8a752dc4e389ec05363062e69be33454db1d612e436
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.8 MB (22754359 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6b3c02d80c3d132723646efd4136d3e6605d1a8fb1b6c14cbfa40b122b22d709`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:00:46 GMT
ADD file:4ac0de0d740f0c221cd4f912861a67009942fa5bdad24ac8b62c690dfa15024a in / 
# Wed, 17 Nov 2021 02:00:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7bd9e64406c6f3044bd15f89c367f07ec4eaafb1035a5c3ee2f7b138be68017f`  
		Last Modified: Wed, 17 Nov 2021 02:16:39 GMT  
		Size: 22.8 MB (22754359 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:fc8026d13e90bef9a4948392328cf18b4b32481cdb64407e98231956c69155af
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.9 MB (25923117 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:84512f753d36bb945369edc0a0f47a63b00207d5a208af4560367cc8b1bc02f4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:44 GMT
ADD file:b7921dd77c7620d46a18a4b5952557620210bf7ad9de20fe8e695a371188122a in / 
# Wed, 17 Nov 2021 02:40:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8ccb9871ed7bebcea8889b08e06fe5bd0116711ca7b110429b987475bf4d40e8`  
		Last Modified: Wed, 17 Nov 2021 02:48:07 GMT  
		Size: 25.9 MB (25923117 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10-slim` - linux; 386

```console
$ docker pull debian@sha256:359cbc872ec80684e26c21b07f099397f1ad3726c0f3133aaa842df78bb75fc1
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.8 MB (27804550 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:33a21f76255c18fc2a4df80a31457b07da2bfa9ee0653811a0f1543bdd62b70a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:22 GMT
ADD file:2adea3474b7ebda2bf77361304ee3e6966a9118aafd8220b81c4027be1f4d583 in / 
# Wed, 17 Nov 2021 02:40:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7af5c9e671306e395c50a515c7bbe33d6f418ea08535b01c884457f78114eb08`  
		Last Modified: Wed, 17 Nov 2021 02:48:49 GMT  
		Size: 27.8 MB (27804550 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10-slim` - linux; mips64le

```console
$ docker pull debian@sha256:8430b6dd5211b03cfecd7e948b66a06104216212514aae3b514533d1c26093c8
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.8 MB (25820129 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2998851c9df1a3aa7d369ccbcedf64a341c3f7a0024903670cbab29e2cb145b9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:11:05 GMT
ADD file:442e2528f8dc5583975e61c403642ead88c26af80e5769940d11d2420eb6e56d in / 
# Wed, 17 Nov 2021 02:11:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f95d3989b23de40aa9c78765869cf0e26ca2ed018dd048613fbc1f9df33d1bc1`  
		Last Modified: Wed, 17 Nov 2021 02:20:33 GMT  
		Size: 25.8 MB (25820129 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:56bcd470fc295d3603cea9d4a9d0de2453a6aaa07350a972ee9828cb20096eed
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.6 MB (30562278 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1268eb8161a3e00c3c709fae3d5b9373630dd956aca82fe343f2d60eb009faad`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:30:22 GMT
ADD file:c0f8b5d4d419bef7a494df5330ecf7bbea3cbb6462308ca0b2f26771f125dea0 in / 
# Wed, 17 Nov 2021 03:30:29 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:459de0a111d5c931674a5150ef4ae6bb1ffb1685380f4aafdba04a37d556c5de`  
		Last Modified: Wed, 17 Nov 2021 03:58:14 GMT  
		Size: 30.6 MB (30562278 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10-slim` - linux; s390x

```console
$ docker pull debian@sha256:a546bb460638bdf3a08744b537a3a3bfbaaabe14738f84692c4fe87622f5fb3f
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.8 MB (25769063 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9ade07e7330ffcfe24ae9e162a0b36b4f63979d4ca3a0e2407118c473562fcf8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:59 GMT
ADD file:3ff5fbc9c53cf76aad0fe0e1a525cb0bc0d70e8f2a3d5741dfb8b3b8a9448ffd in / 
# Wed, 17 Nov 2021 02:43:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b8b917e61b3be882db10e68210f73caf5cc788bfb07aec0a659e30f4796b476b`  
		Last Modified: Wed, 17 Nov 2021 02:49:01 GMT  
		Size: 25.8 MB (25769063 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:10.11`

```console
$ docker pull debian@sha256:e5333f8697a86fa1be53b2a7e994247083f61885166df0cdda9f812acb514d7c
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:10.11` - linux; amd64

```console
$ docker pull debian@sha256:89c36409980d71271f8b46e327cda450ede47542d182671145e92773b752e153
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50437098 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4906f6b1e4402ac10252e2231ea0cb36dd72b494e8d1505bfa23ad79cabb950d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:20:51 GMT
ADD file:cb5ed7070880d4c0177fbe6dd278adb7926e38cd73e6abd582fd8d67e4bbf06c in / 
# Wed, 17 Nov 2021 02:20:51 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1230f17f526cb07307d41462355b10488b994e8bdafe0d8f275a405fa3b33831`  
		Last Modified: Wed, 17 Nov 2021 02:26:06 GMT  
		Size: 50.4 MB (50437098 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.11` - linux; arm variant v5

```console
$ docker pull debian@sha256:600c5fc194e46731b51dec790839ccc9af45cd37a1982eb289924e66339dce85
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **48.2 MB (48154304 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bbd8f738f959a12ae79abc575d02024dd757f535f87330a6bdd422cfa32b3a61`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:51:05 GMT
ADD file:b19cf756330e0e1577062a56604b50597d35a6f4be67ac86210ddf4b4203a32d in / 
# Wed, 17 Nov 2021 02:51:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:141c2db3ba99259b105dd1f16b4f6365959cb21d4978470a6bd73ef86ef15d08`  
		Last Modified: Wed, 17 Nov 2021 03:06:59 GMT  
		Size: 48.2 MB (48154304 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.11` - linux; arm variant v7

```console
$ docker pull debian@sha256:8b0544ec886aacd2cc8ebd70695d01bb7b292e899da3e3d5ce4a5f174a29e4d2
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45918099 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6f800c08efc4810532acfdaade1f1edb9a0a724ea504e9e3972c7a770f218d74`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:00:12 GMT
ADD file:d71807613acdf86685f6d640a90e27b7b63cfe0f13d88668ee943aca089d8876 in / 
# Wed, 17 Nov 2021 02:00:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:10acd81022d17f452439d0385615fa066b8f0ddcf3c1a803872eec5bdc6acf64`  
		Last Modified: Wed, 17 Nov 2021 02:16:00 GMT  
		Size: 45.9 MB (45918099 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.11` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:a90e90c601b9358c34f18a864c02c4f9e8527cc4f79c2f5d3779e8dd11fd5d53
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49222990 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d5af59919bab386fa40f9282adc3769cbfb687d3cf86570a8bd827d8c9ceafce`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:27 GMT
ADD file:c7e9e884b1494f96e33e9d9a892b51367af8943edb282c4101cc99ea78a7753f in / 
# Wed, 17 Nov 2021 02:40:28 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:626979e62c56e192812e818d52b82f063d9f9514b1b7bae14e534e4a7c98117a`  
		Last Modified: Wed, 17 Nov 2021 02:47:40 GMT  
		Size: 49.2 MB (49222990 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.11` - linux; 386

```console
$ docker pull debian@sha256:b840656acb560ba0d10da515368d47f4dc1bb8ffa8df678653d6959063d24c97
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51207728 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3e751f1d934e26435fca29c8eb20e64100daf4f0ccf0b77f4482411ceabbb5b6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:06 GMT
ADD file:87367b4870b0dc997a37b7648ff6efa8e42bed68b0573be2ade992c206fbdd2e in / 
# Wed, 17 Nov 2021 02:40:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1f740e90292e8fcb22dbbbdbe335252fd87d406cd6e139738853fc65c8bb54f4`  
		Last Modified: Wed, 17 Nov 2021 02:48:17 GMT  
		Size: 51.2 MB (51207728 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.11` - linux; mips64le

```console
$ docker pull debian@sha256:56bc08b7eb6db843bc54606a37cfa7a302e8059430b0cf0b62005893283a7b84
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.1 MB (49079535 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:598bee8f0f0e68db53acfa1bd428cb335f6c8d94cf370bcd6a7d74bc465ac010`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:10:39 GMT
ADD file:4769a173ec92e69a1b5cd79af0ed018bf150d1ee278de07f4bdab45f55f3c818 in / 
# Wed, 17 Nov 2021 02:10:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8c028c43f55209ca75650433b923c8952e16d106529242daad16c3be3efd66a3`  
		Last Modified: Wed, 17 Nov 2021 02:19:53 GMT  
		Size: 49.1 MB (49079535 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.11` - linux; ppc64le

```console
$ docker pull debian@sha256:180ef8c7873776990cfe1d853cbeced32438e7e87dbe079be541ba8bd2fd6ca5
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.2 MB (54183702 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:27d2b2aa100535ac6ebeae5978be19d3f1732010b93251d59a6a4a7cb159f6c4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:29:10 GMT
ADD file:fc0d43ba74311deef20b2131cca3a8e86b083624d63560251191adca21417f2f in / 
# Wed, 17 Nov 2021 03:29:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:aad6e1fe6f7c56b0e1b5850f40e4fec91ffaed4b6235e730f5c7ff43db397d3d`  
		Last Modified: Wed, 17 Nov 2021 03:56:39 GMT  
		Size: 54.2 MB (54183702 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.11` - linux; s390x

```console
$ docker pull debian@sha256:f4bb73391d3779dfe30a3ab72aa444bff7164c2326ab80f55302e8b584463ac5
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.0 MB (49005439 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:20b18aa76fa9a01d5ef11e20329da3e4097e7771eb7310578351dfdbc8bae229`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:40 GMT
ADD file:dc4ee96f81059accdc88a686f80f1714bc9d2f383af71c0d130c08560cea115e in / 
# Wed, 17 Nov 2021 02:42:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c12eae84d2674466d924a139a276e361afa9f63de21cf77eae19bd94eaac2629`  
		Last Modified: Wed, 17 Nov 2021 02:48:42 GMT  
		Size: 49.0 MB (49005439 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:10.11-slim`

```console
$ docker pull debian@sha256:cfadaa70a452076a478dcfcf2da3a3dc3b741450afed5ab67a7f3df161387c48
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:10.11-slim` - linux; amd64

```console
$ docker pull debian@sha256:69b11eb7768d4783408354087ff1719d875a9f218e4ac76b39e9c35792048968
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.2 MB (27153675 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cad9ce16f840672d1615867950084a0de9b7022ec6fff89e05bde0d32632e281`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:21:02 GMT
ADD file:3c54ad257f2e04f7294ce879b884820cf4726c8e93ec548172825963e40c79ad in / 
# Wed, 17 Nov 2021 02:21:02 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a10c77af261312e7c92bcc184f2d1726175ff7f142e44b01c5779cd79348b9fd`  
		Last Modified: Wed, 17 Nov 2021 02:26:31 GMT  
		Size: 27.2 MB (27153675 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.11-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:9ad9871d1ab8f29c2d32b7c1d807f8106ac173edf4c682c8cdc071db124694e7
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.9 MB (24886310 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d4cbba96ee571035afdcd4a54b8250797669f76c63804c5eb78647e94cddc985`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:51:39 GMT
ADD file:e580d4d2066301375327ea51dd8db3af956e5a465495bf09d69d6deec9b0bfae in / 
# Wed, 17 Nov 2021 02:51:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:dba255a4b166bb02562ca152d0da236ba8f211de5bf9d79e35ee023b1fce203e`  
		Last Modified: Wed, 17 Nov 2021 03:07:41 GMT  
		Size: 24.9 MB (24886310 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.11-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:d83f190bbdc742a155dff8a752dc4e389ec05363062e69be33454db1d612e436
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.8 MB (22754359 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6b3c02d80c3d132723646efd4136d3e6605d1a8fb1b6c14cbfa40b122b22d709`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:00:46 GMT
ADD file:4ac0de0d740f0c221cd4f912861a67009942fa5bdad24ac8b62c690dfa15024a in / 
# Wed, 17 Nov 2021 02:00:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7bd9e64406c6f3044bd15f89c367f07ec4eaafb1035a5c3ee2f7b138be68017f`  
		Last Modified: Wed, 17 Nov 2021 02:16:39 GMT  
		Size: 22.8 MB (22754359 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.11-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:fc8026d13e90bef9a4948392328cf18b4b32481cdb64407e98231956c69155af
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.9 MB (25923117 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:84512f753d36bb945369edc0a0f47a63b00207d5a208af4560367cc8b1bc02f4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:44 GMT
ADD file:b7921dd77c7620d46a18a4b5952557620210bf7ad9de20fe8e695a371188122a in / 
# Wed, 17 Nov 2021 02:40:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8ccb9871ed7bebcea8889b08e06fe5bd0116711ca7b110429b987475bf4d40e8`  
		Last Modified: Wed, 17 Nov 2021 02:48:07 GMT  
		Size: 25.9 MB (25923117 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.11-slim` - linux; 386

```console
$ docker pull debian@sha256:359cbc872ec80684e26c21b07f099397f1ad3726c0f3133aaa842df78bb75fc1
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.8 MB (27804550 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:33a21f76255c18fc2a4df80a31457b07da2bfa9ee0653811a0f1543bdd62b70a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:22 GMT
ADD file:2adea3474b7ebda2bf77361304ee3e6966a9118aafd8220b81c4027be1f4d583 in / 
# Wed, 17 Nov 2021 02:40:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7af5c9e671306e395c50a515c7bbe33d6f418ea08535b01c884457f78114eb08`  
		Last Modified: Wed, 17 Nov 2021 02:48:49 GMT  
		Size: 27.8 MB (27804550 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.11-slim` - linux; mips64le

```console
$ docker pull debian@sha256:8430b6dd5211b03cfecd7e948b66a06104216212514aae3b514533d1c26093c8
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.8 MB (25820129 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2998851c9df1a3aa7d369ccbcedf64a341c3f7a0024903670cbab29e2cb145b9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:11:05 GMT
ADD file:442e2528f8dc5583975e61c403642ead88c26af80e5769940d11d2420eb6e56d in / 
# Wed, 17 Nov 2021 02:11:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f95d3989b23de40aa9c78765869cf0e26ca2ed018dd048613fbc1f9df33d1bc1`  
		Last Modified: Wed, 17 Nov 2021 02:20:33 GMT  
		Size: 25.8 MB (25820129 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.11-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:56bcd470fc295d3603cea9d4a9d0de2453a6aaa07350a972ee9828cb20096eed
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.6 MB (30562278 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1268eb8161a3e00c3c709fae3d5b9373630dd956aca82fe343f2d60eb009faad`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:30:22 GMT
ADD file:c0f8b5d4d419bef7a494df5330ecf7bbea3cbb6462308ca0b2f26771f125dea0 in / 
# Wed, 17 Nov 2021 03:30:29 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:459de0a111d5c931674a5150ef4ae6bb1ffb1685380f4aafdba04a37d556c5de`  
		Last Modified: Wed, 17 Nov 2021 03:58:14 GMT  
		Size: 30.6 MB (30562278 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:10.11-slim` - linux; s390x

```console
$ docker pull debian@sha256:a546bb460638bdf3a08744b537a3a3bfbaaabe14738f84692c4fe87622f5fb3f
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.8 MB (25769063 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9ade07e7330ffcfe24ae9e162a0b36b4f63979d4ca3a0e2407118c473562fcf8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:59 GMT
ADD file:3ff5fbc9c53cf76aad0fe0e1a525cb0bc0d70e8f2a3d5741dfb8b3b8a9448ffd in / 
# Wed, 17 Nov 2021 02:43:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b8b917e61b3be882db10e68210f73caf5cc788bfb07aec0a659e30f4796b476b`  
		Last Modified: Wed, 17 Nov 2021 02:49:01 GMT  
		Size: 25.8 MB (25769063 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:11`

```console
$ docker pull debian@sha256:e8c184b56a94db0947a9d51ec68f42ef5584442f20547fa3bd8cbd00203b2e7a
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:11` - linux; amd64

```console
$ docker pull debian@sha256:6480d244c39d52f2de8ee0417cc385aa437cabfbfd8e3c127882c4d03f8ab86d
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.9 MB (54932774 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:827e5611389abf13dad1057e92f163b771febc0bcdb19fa2d634a7eb0641e0cc`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:20:30 GMT
ADD file:5259fc086e8295ddbf02e48abef38e9bf93a183079d3631aa7a59306b7f2f9df in / 
# Wed, 17 Nov 2021 02:20:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:647acf3d48c2780e00cd27bb0984367415f270d78477ef9d5b238e6ebd5290da`  
		Last Modified: Wed, 17 Nov 2021 02:25:17 GMT  
		Size: 54.9 MB (54932774 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; arm variant v5

```console
$ docker pull debian@sha256:f76b8cf174e44cdc775877460bc3e19c3e30e66b271cb5d39f726cfa7094044f
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52467904 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:49f6c0d2973ae831b31a88d01129b59c15cdf95d69ea05d1eb401f8a920846d8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:50:02 GMT
ADD file:48c0196bfa5dfd1137285bf9104248d62f15a734133d8df549f35b6f7989ca19 in / 
# Wed, 17 Nov 2021 02:50:03 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ce39e946c04dab3f8b463d502d71849e8e6d7291ece7e323f0ec9dac1061af26`  
		Last Modified: Wed, 17 Nov 2021 03:05:19 GMT  
		Size: 52.5 MB (52467904 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; arm variant v7

```console
$ docker pull debian@sha256:8926b33de48e3a6a7f84356ce88179481f44523f12d55ad8c3ba287dc191ffdc
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.1 MB (50134152 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e9cf93b473f42691f8231524112260deb7aa0f6f9a371fa9b8f552f24285519d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 01:59:07 GMT
ADD file:43b45f65ff4ba720f6f9a107b5f80818d5ea6ebc4835840b11e9a506351da32f in / 
# Wed, 17 Nov 2021 01:59:08 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:dbbf55137aab3c7acd1e012a70f1ea1adeaef8d39b474829965d3480a02bf572`  
		Last Modified: Wed, 17 Nov 2021 02:14:27 GMT  
		Size: 50.1 MB (50134152 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:c9178c59ba331e2a0745ddf7109c44fe17f19dca2eb7ecb8c14a4c566124a025
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53619024 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:eec76c447f59d03e1d07a8bc96368012b2f05e4a52b8812a5671faef329f1eaf`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:39:59 GMT
ADD file:eb667e18226da8a1ca5a41beacddc65cd1efec01c3d01e50845c5e89e91ea17c in / 
# Wed, 17 Nov 2021 02:40:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ce18cd0ba11c2dfa8c1a2d2b325e1e019b14d4ae25ffdeffb0f9686c5496bf5c`  
		Last Modified: Wed, 17 Nov 2021 02:46:45 GMT  
		Size: 53.6 MB (53619024 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; 386

```console
$ docker pull debian@sha256:d1b5cbf0a8cbcd44147c1d46b9b8f778f436ed5174a55b1b84cab79163215905
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.9 MB (55937586 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:59b34e04dc278408b2e7fc09cacf1411ecd25f3105a711a746f1d22f1d39a87a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:39:36 GMT
ADD file:b8f9021573a53ec69fa566a396d9a8c68392bd6d659a665c0aea8fbd00164f12 in / 
# Wed, 17 Nov 2021 02:39:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8d649f3a0e8410b121648ccc552acdac0960691a4da33ff4db9825f9b51ff3f4`  
		Last Modified: Wed, 17 Nov 2021 02:47:12 GMT  
		Size: 55.9 MB (55937586 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; mips64le

```console
$ docker pull debian@sha256:d89c75015f88e1ca939d94c69ac13e4f8134ae432acfa6900076c1fbb30cad76
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53183806 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5035dbb39f67711b5184433a96cafd0db4bc9b6a653c914110ecb01166a99a7b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:09:46 GMT
ADD file:2971df49f24648b8d5166d87e392e2547d5b03469356f20d112ce6feb3c329fc in / 
# Wed, 17 Nov 2021 02:09:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b095a0b63d99a4c4cd9c1c353f381a69ef041e6f80798d807ae51a34b04920a0`  
		Last Modified: Wed, 17 Nov 2021 02:18:22 GMT  
		Size: 53.2 MB (53183806 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; ppc64le

```console
$ docker pull debian@sha256:952516c1afd1ea16fa745b83502d222062d24df80fdcd73cece4640609732c60
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.8 MB (58819505 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c78f4b353abfd74cf67ea010720e7bbee93d8e5383b5429a6ab9f35ad982f5ec`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:27:04 GMT
ADD file:85375acb19da58f9eabaa67f8cb425cd1ff2dadb2888bbd9dbfa6223d16fef23 in / 
# Wed, 17 Nov 2021 03:27:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ede9cd7c8f1d380c971a130fb3d8a3914bf3a9106702164c23b2cce536fa618c`  
		Last Modified: Wed, 17 Nov 2021 03:52:39 GMT  
		Size: 58.8 MB (58819505 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11` - linux; s390x

```console
$ docker pull debian@sha256:f9147074e1223cfc833be4e33b5b80a80017cd3a4c6f00e3af27bde69dd10c80
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53207179 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:40a4f800f6aa1a5b14da6ed5b5e9f4ecfc77ff178aa0685d69f7cfc918a93695`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:07 GMT
ADD file:b7524e775a2033d246d1096765a0eb6eaa5ba6ed6c8b395522579d2c4e7a2e38 in / 
# Wed, 17 Nov 2021 02:42:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ffa47acd07831fbc96680a705da20704bac0016c20dc3c454ecd45cda44b368d`  
		Last Modified: Wed, 17 Nov 2021 02:48:05 GMT  
		Size: 53.2 MB (53207179 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:11-slim`

```console
$ docker pull debian@sha256:a23887a2e830b815955e010f30d4c2430cd5ef82e93c130471024bc9f808d5d3
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:11-slim` - linux; amd64

```console
$ docker pull debian@sha256:832234a41bd12f5bcbed92f5ada9af6e2e2b3a394b2353f072b4ad413a3b3320
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.4 MB (31370267 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:66b2aecdb9f0f055837a85eca27f1c790e78d935b21bd000559f57e04f75e06b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:20:41 GMT
ADD file:a2405ebb9892d98be2eb585f6121864d12b3fd983ebf15e5f0b7486e106a79c6 in / 
# Wed, 17 Nov 2021 02:20:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:eff15d958d664f0874d16aee393cc44387031ee0a68ef8542d0056c747f378e8`  
		Last Modified: Wed, 17 Nov 2021 02:25:45 GMT  
		Size: 31.4 MB (31370267 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:101787af86135c7b1346049404e63165bf65a9a2f1334f54571374d80f0ecac9
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **28.9 MB (28911006 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:968cb14e039277aaef3a19da601adf10cc0125e61520ebbbbffb29ad6fca6fef`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:50:37 GMT
ADD file:738a04a17bdb9077594ad9a847333abe28216a7f04d3058718a5e21c236c24bb in / 
# Wed, 17 Nov 2021 02:50:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a960a56baa1baffbe2aa1e0c1fb02f9ee4816337d02fec259b312c409d77fafc`  
		Last Modified: Wed, 17 Nov 2021 03:06:09 GMT  
		Size: 28.9 MB (28911006 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:e2917d74741b83dac5e2e1cac59b8c79115aa1334b908bf020e9605f46a2fb77
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.6 MB (26573160 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:eaa955c4e58c7dc8e698c7e54814ed361ee80aad303a67c39606d6c4fa849f37`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 01:59:43 GMT
ADD file:cd2ac52107a2ea6657f23850a4b29366309eb39fa177321e0a9fd6d58562ae80 in / 
# Wed, 17 Nov 2021 01:59:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b6e5ca4da96841e58eb27c88695a059e5105fad5a066de803f4b94ae4002ba66`  
		Last Modified: Wed, 17 Nov 2021 02:15:13 GMT  
		Size: 26.6 MB (26573160 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:3da6322809c63296f41e7ec624178ad0aafa5d505514b33a892291716f694cd7
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30056521 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bbe83eb8d1d726e8431b21e7dc2e6abcb80a06ceb49f8be7f66f8a9e71e2131d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:15 GMT
ADD file:4203242b2b09a65239092c4780b59181da7b861b3c0be40810b3588aa200f72c in / 
# Wed, 17 Nov 2021 02:40:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:eb9a2845ed124d072b117aba4f0508e00c1ecd0d147dc324d14b00d24092046c`  
		Last Modified: Wed, 17 Nov 2021 02:47:17 GMT  
		Size: 30.1 MB (30056521 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; 386

```console
$ docker pull debian@sha256:3e3052c4c57f1a775f79b1ae8ae52fbfa4181b152727fa246f761918ae5e19cd
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32380683 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:da9019e0671ab5347f4945a286cd7000bf3e5265aaadbce47f8aa701a38f2ecb`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:39:53 GMT
ADD file:ff018048717c0f5aa1730480d0bfea1c2bf94d6e2dae2d7cef46d05ffb0d93e1 in / 
# Wed, 17 Nov 2021 02:39:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0bae0e1d4cb1d56ffeb9d05de7a097d3157267eef07b6d8131344d99bd97c431`  
		Last Modified: Wed, 17 Nov 2021 02:47:49 GMT  
		Size: 32.4 MB (32380683 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; mips64le

```console
$ docker pull debian@sha256:dcb16e0376a09dc11c225b5dd0fa774fe6eeff81394d5c978b2989631fab2e94
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.6 MB (29630388 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:db59a85a8dc8d727406d0dd376749ab01e399052396a6fd26b85275ac171959b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:10:13 GMT
ADD file:10bc783d286d0a800f59d5009e87ecb4651659c82cae1325132756c7c8b1dec0 in / 
# Wed, 17 Nov 2021 02:10:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:08bca22fbffb4e58224e9e4f6a56f1ca5099c002b1aeb6181df63af308a3da58`  
		Last Modified: Wed, 17 Nov 2021 02:19:09 GMT  
		Size: 29.6 MB (29630388 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:54c3b01fb94774a2593072f7278f315f7473e0c7b6ccf441a215e6e0aa2a8f8b
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **35.3 MB (35271382 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b66d7a689a3b6f026b857d81a3a68ebf63940fa33788c8597f8d755ede3de49b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:28:22 GMT
ADD file:5ed330f0fe1328f694fcaefb961cf4da4d8a4ff03100b21af718b69316168706 in / 
# Wed, 17 Nov 2021 03:28:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:258ff2a13858db8f51b65662e02137c0abcfd2528ca73e92b7a40061d938fb1e`  
		Last Modified: Wed, 17 Nov 2021 03:54:34 GMT  
		Size: 35.3 MB (35271382 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11-slim` - linux; s390x

```console
$ docker pull debian@sha256:17b59bff96c1014cf9a9a101c0fc39f905c2bde2e79c218051bdfa970b2019da
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.7 MB (29650960 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c0c1b8308ceb984c947f45ba0ce176b263cfff066eb3b2419b86fc30c5e0d204`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:27 GMT
ADD file:42f73dac307a6a5232e3100f3d3b36e8125f73a4ca504b106e60e2b66d242b2f in / 
# Wed, 17 Nov 2021 02:42:28 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8d71e885edf612b507bbdcc48a970548d436b8709eeedfc2c98f6843a214b5e2`  
		Last Modified: Wed, 17 Nov 2021 02:48:25 GMT  
		Size: 29.7 MB (29650960 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:11.1`

```console
$ docker pull debian@sha256:e8c184b56a94db0947a9d51ec68f42ef5584442f20547fa3bd8cbd00203b2e7a
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:11.1` - linux; amd64

```console
$ docker pull debian@sha256:6480d244c39d52f2de8ee0417cc385aa437cabfbfd8e3c127882c4d03f8ab86d
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.9 MB (54932774 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:827e5611389abf13dad1057e92f163b771febc0bcdb19fa2d634a7eb0641e0cc`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:20:30 GMT
ADD file:5259fc086e8295ddbf02e48abef38e9bf93a183079d3631aa7a59306b7f2f9df in / 
# Wed, 17 Nov 2021 02:20:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:647acf3d48c2780e00cd27bb0984367415f270d78477ef9d5b238e6ebd5290da`  
		Last Modified: Wed, 17 Nov 2021 02:25:17 GMT  
		Size: 54.9 MB (54932774 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.1` - linux; arm variant v5

```console
$ docker pull debian@sha256:f76b8cf174e44cdc775877460bc3e19c3e30e66b271cb5d39f726cfa7094044f
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52467904 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:49f6c0d2973ae831b31a88d01129b59c15cdf95d69ea05d1eb401f8a920846d8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:50:02 GMT
ADD file:48c0196bfa5dfd1137285bf9104248d62f15a734133d8df549f35b6f7989ca19 in / 
# Wed, 17 Nov 2021 02:50:03 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ce39e946c04dab3f8b463d502d71849e8e6d7291ece7e323f0ec9dac1061af26`  
		Last Modified: Wed, 17 Nov 2021 03:05:19 GMT  
		Size: 52.5 MB (52467904 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.1` - linux; arm variant v7

```console
$ docker pull debian@sha256:8926b33de48e3a6a7f84356ce88179481f44523f12d55ad8c3ba287dc191ffdc
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.1 MB (50134152 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e9cf93b473f42691f8231524112260deb7aa0f6f9a371fa9b8f552f24285519d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 01:59:07 GMT
ADD file:43b45f65ff4ba720f6f9a107b5f80818d5ea6ebc4835840b11e9a506351da32f in / 
# Wed, 17 Nov 2021 01:59:08 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:dbbf55137aab3c7acd1e012a70f1ea1adeaef8d39b474829965d3480a02bf572`  
		Last Modified: Wed, 17 Nov 2021 02:14:27 GMT  
		Size: 50.1 MB (50134152 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.1` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:c9178c59ba331e2a0745ddf7109c44fe17f19dca2eb7ecb8c14a4c566124a025
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53619024 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:eec76c447f59d03e1d07a8bc96368012b2f05e4a52b8812a5671faef329f1eaf`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:39:59 GMT
ADD file:eb667e18226da8a1ca5a41beacddc65cd1efec01c3d01e50845c5e89e91ea17c in / 
# Wed, 17 Nov 2021 02:40:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ce18cd0ba11c2dfa8c1a2d2b325e1e019b14d4ae25ffdeffb0f9686c5496bf5c`  
		Last Modified: Wed, 17 Nov 2021 02:46:45 GMT  
		Size: 53.6 MB (53619024 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.1` - linux; 386

```console
$ docker pull debian@sha256:d1b5cbf0a8cbcd44147c1d46b9b8f778f436ed5174a55b1b84cab79163215905
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.9 MB (55937586 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:59b34e04dc278408b2e7fc09cacf1411ecd25f3105a711a746f1d22f1d39a87a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:39:36 GMT
ADD file:b8f9021573a53ec69fa566a396d9a8c68392bd6d659a665c0aea8fbd00164f12 in / 
# Wed, 17 Nov 2021 02:39:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8d649f3a0e8410b121648ccc552acdac0960691a4da33ff4db9825f9b51ff3f4`  
		Last Modified: Wed, 17 Nov 2021 02:47:12 GMT  
		Size: 55.9 MB (55937586 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.1` - linux; mips64le

```console
$ docker pull debian@sha256:d89c75015f88e1ca939d94c69ac13e4f8134ae432acfa6900076c1fbb30cad76
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53183806 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5035dbb39f67711b5184433a96cafd0db4bc9b6a653c914110ecb01166a99a7b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:09:46 GMT
ADD file:2971df49f24648b8d5166d87e392e2547d5b03469356f20d112ce6feb3c329fc in / 
# Wed, 17 Nov 2021 02:09:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b095a0b63d99a4c4cd9c1c353f381a69ef041e6f80798d807ae51a34b04920a0`  
		Last Modified: Wed, 17 Nov 2021 02:18:22 GMT  
		Size: 53.2 MB (53183806 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.1` - linux; ppc64le

```console
$ docker pull debian@sha256:952516c1afd1ea16fa745b83502d222062d24df80fdcd73cece4640609732c60
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.8 MB (58819505 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c78f4b353abfd74cf67ea010720e7bbee93d8e5383b5429a6ab9f35ad982f5ec`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:27:04 GMT
ADD file:85375acb19da58f9eabaa67f8cb425cd1ff2dadb2888bbd9dbfa6223d16fef23 in / 
# Wed, 17 Nov 2021 03:27:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ede9cd7c8f1d380c971a130fb3d8a3914bf3a9106702164c23b2cce536fa618c`  
		Last Modified: Wed, 17 Nov 2021 03:52:39 GMT  
		Size: 58.8 MB (58819505 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.1` - linux; s390x

```console
$ docker pull debian@sha256:f9147074e1223cfc833be4e33b5b80a80017cd3a4c6f00e3af27bde69dd10c80
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53207179 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:40a4f800f6aa1a5b14da6ed5b5e9f4ecfc77ff178aa0685d69f7cfc918a93695`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:07 GMT
ADD file:b7524e775a2033d246d1096765a0eb6eaa5ba6ed6c8b395522579d2c4e7a2e38 in / 
# Wed, 17 Nov 2021 02:42:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ffa47acd07831fbc96680a705da20704bac0016c20dc3c454ecd45cda44b368d`  
		Last Modified: Wed, 17 Nov 2021 02:48:05 GMT  
		Size: 53.2 MB (53207179 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:11.1-slim`

```console
$ docker pull debian@sha256:a23887a2e830b815955e010f30d4c2430cd5ef82e93c130471024bc9f808d5d3
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:11.1-slim` - linux; amd64

```console
$ docker pull debian@sha256:832234a41bd12f5bcbed92f5ada9af6e2e2b3a394b2353f072b4ad413a3b3320
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.4 MB (31370267 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:66b2aecdb9f0f055837a85eca27f1c790e78d935b21bd000559f57e04f75e06b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:20:41 GMT
ADD file:a2405ebb9892d98be2eb585f6121864d12b3fd983ebf15e5f0b7486e106a79c6 in / 
# Wed, 17 Nov 2021 02:20:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:eff15d958d664f0874d16aee393cc44387031ee0a68ef8542d0056c747f378e8`  
		Last Modified: Wed, 17 Nov 2021 02:25:45 GMT  
		Size: 31.4 MB (31370267 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.1-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:101787af86135c7b1346049404e63165bf65a9a2f1334f54571374d80f0ecac9
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **28.9 MB (28911006 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:968cb14e039277aaef3a19da601adf10cc0125e61520ebbbbffb29ad6fca6fef`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:50:37 GMT
ADD file:738a04a17bdb9077594ad9a847333abe28216a7f04d3058718a5e21c236c24bb in / 
# Wed, 17 Nov 2021 02:50:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a960a56baa1baffbe2aa1e0c1fb02f9ee4816337d02fec259b312c409d77fafc`  
		Last Modified: Wed, 17 Nov 2021 03:06:09 GMT  
		Size: 28.9 MB (28911006 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.1-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:e2917d74741b83dac5e2e1cac59b8c79115aa1334b908bf020e9605f46a2fb77
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.6 MB (26573160 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:eaa955c4e58c7dc8e698c7e54814ed361ee80aad303a67c39606d6c4fa849f37`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 01:59:43 GMT
ADD file:cd2ac52107a2ea6657f23850a4b29366309eb39fa177321e0a9fd6d58562ae80 in / 
# Wed, 17 Nov 2021 01:59:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b6e5ca4da96841e58eb27c88695a059e5105fad5a066de803f4b94ae4002ba66`  
		Last Modified: Wed, 17 Nov 2021 02:15:13 GMT  
		Size: 26.6 MB (26573160 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.1-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:3da6322809c63296f41e7ec624178ad0aafa5d505514b33a892291716f694cd7
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30056521 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bbe83eb8d1d726e8431b21e7dc2e6abcb80a06ceb49f8be7f66f8a9e71e2131d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:15 GMT
ADD file:4203242b2b09a65239092c4780b59181da7b861b3c0be40810b3588aa200f72c in / 
# Wed, 17 Nov 2021 02:40:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:eb9a2845ed124d072b117aba4f0508e00c1ecd0d147dc324d14b00d24092046c`  
		Last Modified: Wed, 17 Nov 2021 02:47:17 GMT  
		Size: 30.1 MB (30056521 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.1-slim` - linux; 386

```console
$ docker pull debian@sha256:3e3052c4c57f1a775f79b1ae8ae52fbfa4181b152727fa246f761918ae5e19cd
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32380683 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:da9019e0671ab5347f4945a286cd7000bf3e5265aaadbce47f8aa701a38f2ecb`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:39:53 GMT
ADD file:ff018048717c0f5aa1730480d0bfea1c2bf94d6e2dae2d7cef46d05ffb0d93e1 in / 
# Wed, 17 Nov 2021 02:39:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0bae0e1d4cb1d56ffeb9d05de7a097d3157267eef07b6d8131344d99bd97c431`  
		Last Modified: Wed, 17 Nov 2021 02:47:49 GMT  
		Size: 32.4 MB (32380683 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.1-slim` - linux; mips64le

```console
$ docker pull debian@sha256:dcb16e0376a09dc11c225b5dd0fa774fe6eeff81394d5c978b2989631fab2e94
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.6 MB (29630388 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:db59a85a8dc8d727406d0dd376749ab01e399052396a6fd26b85275ac171959b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:10:13 GMT
ADD file:10bc783d286d0a800f59d5009e87ecb4651659c82cae1325132756c7c8b1dec0 in / 
# Wed, 17 Nov 2021 02:10:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:08bca22fbffb4e58224e9e4f6a56f1ca5099c002b1aeb6181df63af308a3da58`  
		Last Modified: Wed, 17 Nov 2021 02:19:09 GMT  
		Size: 29.6 MB (29630388 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.1-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:54c3b01fb94774a2593072f7278f315f7473e0c7b6ccf441a215e6e0aa2a8f8b
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **35.3 MB (35271382 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b66d7a689a3b6f026b857d81a3a68ebf63940fa33788c8597f8d755ede3de49b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:28:22 GMT
ADD file:5ed330f0fe1328f694fcaefb961cf4da4d8a4ff03100b21af718b69316168706 in / 
# Wed, 17 Nov 2021 03:28:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:258ff2a13858db8f51b65662e02137c0abcfd2528ca73e92b7a40061d938fb1e`  
		Last Modified: Wed, 17 Nov 2021 03:54:34 GMT  
		Size: 35.3 MB (35271382 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:11.1-slim` - linux; s390x

```console
$ docker pull debian@sha256:17b59bff96c1014cf9a9a101c0fc39f905c2bde2e79c218051bdfa970b2019da
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.7 MB (29650960 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c0c1b8308ceb984c947f45ba0ce176b263cfff066eb3b2419b86fc30c5e0d204`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:27 GMT
ADD file:42f73dac307a6a5232e3100f3d3b36e8125f73a4ca504b106e60e2b66d242b2f in / 
# Wed, 17 Nov 2021 02:42:28 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8d71e885edf612b507bbdcc48a970548d436b8709eeedfc2c98f6843a214b5e2`  
		Last Modified: Wed, 17 Nov 2021 02:48:25 GMT  
		Size: 29.7 MB (29650960 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:9`

```console
$ docker pull debian@sha256:d8ee86bf0afeb901de293c692a540307a670306cbdb7a06e2c840f17b0c35374
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 5
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:9` - linux; amd64

```console
$ docker pull debian@sha256:a8eab13d0f587368d77fc953a8cb8a665a976c046c419d19be7af613554f990d
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.4 MB (45380444 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:066e7b4151cfe95cbd9c2244792a815c5b32bb380fe4ec851917569c6f750664`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:22:28 GMT
ADD file:d7365dd9cf34b10ca189f85c95c21a0c33e44092f85ffb5884d5e737fb0b9be1 in / 
# Wed, 17 Nov 2021 02:22:28 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:200cae5c9d88bb9cf1dd3fcfb831d671403f078d2310416fa3b304337d8279c0`  
		Last Modified: Wed, 17 Nov 2021 02:29:09 GMT  
		Size: 45.4 MB (45380444 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9` - linux; arm variant v5

```console
$ docker pull debian@sha256:37693dbcd033d85fbb9b15e32d711a418ae8f30f756210d8b1abb489e252225b
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **44.1 MB (44091379 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e84cd38de4fc6ca9807b62c600666c5599de965df3c10aae21c8e20049e176bd`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:55:43 GMT
ADD file:a8b0f40ff55722f4bb2e3ac65c3fefe9acaa513f89b73ed81ec04585058e58ba in / 
# Wed, 17 Nov 2021 02:55:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ec68ab8028a430064eed52d9b833f1a135dcce543b02666ee5acd73962b79070`  
		Last Modified: Wed, 17 Nov 2021 03:13:32 GMT  
		Size: 44.1 MB (44091379 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9` - linux; arm variant v7

```console
$ docker pull debian@sha256:9029da5bf6cbb6199e5f6d0eed4aaeaad998668abe02d29ea294ad2a3351e98e
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **42.1 MB (42116868 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6e2da5cf76886cdc5bdc612c501e562f2fce8be0d0aa51ec37bb1f39c4e32e5e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:04:57 GMT
ADD file:61da3fcef3aea99b4927189cfcb823a65b0e911a3f4436e44ea84a57a7919ff3 in / 
# Wed, 17 Nov 2021 02:04:58 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e4d54b576c2fcdd259e57bac14c4e1a2623f6e5dc9a0b1facaed7fe59d891e86`  
		Last Modified: Wed, 17 Nov 2021 02:22:03 GMT  
		Size: 42.1 MB (42116868 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:70f4f03a8fd72cf60c3f8023f010721fece3fe599c9e7bc6eae10024eaa6a128
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **43.2 MB (43174478 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fad7665a3afc5981cd910850721e9f91cbf791c0962e4e250e298cee3059a4e0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:33 GMT
ADD file:3225671be4edc3599e29ebd06516b95573246e68bf29649c402ed87c03c109b5 in / 
# Wed, 17 Nov 2021 02:42:34 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c64a4bafe4ad6fc05fcc6d76d2a06f168f331ffbc87e518fb4ca4ea22e75527e`  
		Last Modified: Wed, 17 Nov 2021 02:51:11 GMT  
		Size: 43.2 MB (43174478 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9` - linux; 386

```console
$ docker pull debian@sha256:baa27aa91f579ac7cb42c594b2202ff5184fa4a64d7e88f0e1486725f61b0de2
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **46.1 MB (46097202 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7b1834ace8ddd99da39204c4a6faec3b1501af684a54e103348a966587edfe1a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:21 GMT
ADD file:8e57eaeffee1cd4c93865af9d6370c68bddae095016766191b1338ae675f131c in / 
# Wed, 17 Nov 2021 02:42:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:24fc2b60c45394a399438249c8e7f92f6fa415f9c477524d5af3ab4782d51519`  
		Last Modified: Wed, 17 Nov 2021 02:52:15 GMT  
		Size: 46.1 MB (46097202 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:9-slim`

```console
$ docker pull debian@sha256:d8566545bd3750e270b8aaf703d0a048fda080cd287a249823a5a11b7c95adbf
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 5
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:9-slim` - linux; amd64

```console
$ docker pull debian@sha256:bd4c41c9e68cc553f0c5266f18bef07491c52a08752f81a015892c2dc5e15114
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.5 MB (22527686 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a692eb442a03726bc7790f146f97afe590a942a526ec282f4c4d1bb724a9443a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:22:38 GMT
ADD file:2868c3af63afe6b8aadac07b8776e5468a3ed135fd84ad22df15e48f0610c7ba in / 
# Wed, 17 Nov 2021 02:22:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:89166b5eeae47d8c878cb88f20cc67eda2a550a697f4be317c7b7abea566b76f`  
		Last Modified: Wed, 17 Nov 2021 02:29:32 GMT  
		Size: 22.5 MB (22527686 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:2aa2a15f320cd740783a33c547f4cd885d0b6e5b1ba3860024f8c8badc368c25
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **21.2 MB (21204234 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9e45f7a72e70f7c7a8193bfb2fdd770e1fa690422cb7f05e2a1332f8f8f63d60`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:56:16 GMT
ADD file:b857fb98b0880bbb3add8dd317cd1eb0f339332d4836baaedc8f2f424b5f2c98 in / 
# Wed, 17 Nov 2021 02:56:17 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ca67a5e1c16d08a9b8520dae81a39d168ef2a816d8905715eb6a328c89a2ce37`  
		Last Modified: Wed, 17 Nov 2021 03:14:12 GMT  
		Size: 21.2 MB (21204234 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:9a52fdfda53f199f56b7d9a7d4dcc78a655da675926c88c1f4c4af044f9a5b79
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **19.3 MB (19316397 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:eecd35162e59af35e26178b7706ea9d74dc473fef1aed9fd0545734ab75f1304`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:05:31 GMT
ADD file:f28546ff6a3759d3a957c25d4310c3065e1367dff4ae304caae1cf1dc64d061d in / 
# Wed, 17 Nov 2021 02:05:32 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:614e23553da11f1294dc969bcab416c9f981794bdbaee5641335a222730c63b2`  
		Last Modified: Wed, 17 Nov 2021 02:22:41 GMT  
		Size: 19.3 MB (19316397 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:b1902145e37b8fdd8c48092c245efcd8ac341091ed7ff3dbf91d7495bd293d88
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **20.4 MB (20389442 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:db5e084a09bc69d57eb2f658d7c5fa96910e232ee934ba9d2119a0d05fb7c5ed`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:48 GMT
ADD file:2adad5eee701d55a9f58c07f0706eb574d0ad6b74b0cc52a9e622f50639961c9 in / 
# Wed, 17 Nov 2021 02:42:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:34b92b588f0f390db0d125ab0ee4c85510c52367371c4bcf5af80a6ab18f0a9b`  
		Last Modified: Wed, 17 Nov 2021 02:51:38 GMT  
		Size: 20.4 MB (20389442 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9-slim` - linux; 386

```console
$ docker pull debian@sha256:043ab2d585630cc218d3b5b0e09038405e13bff9585bee28cc02f5c93f9e9f86
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **23.2 MB (23156564 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:143712197327374e7944c356bf4336da6737d3df4709128da5224bfd486aa8dc`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:36 GMT
ADD file:3c9c1d0f88db0341cbe43b594cf6d2d865e39d9518b8e2b124cb9736bebe18f3 in / 
# Wed, 17 Nov 2021 02:42:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d213b39bd355350279fd232f3eedd77a11f7fb2a3d5a0017357dd45fa88342d1`  
		Last Modified: Wed, 17 Nov 2021 02:52:49 GMT  
		Size: 23.2 MB (23156564 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:9.13`

```console
$ docker pull debian@sha256:d8ee86bf0afeb901de293c692a540307a670306cbdb7a06e2c840f17b0c35374
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 5
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:9.13` - linux; amd64

```console
$ docker pull debian@sha256:a8eab13d0f587368d77fc953a8cb8a665a976c046c419d19be7af613554f990d
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.4 MB (45380444 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:066e7b4151cfe95cbd9c2244792a815c5b32bb380fe4ec851917569c6f750664`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:22:28 GMT
ADD file:d7365dd9cf34b10ca189f85c95c21a0c33e44092f85ffb5884d5e737fb0b9be1 in / 
# Wed, 17 Nov 2021 02:22:28 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:200cae5c9d88bb9cf1dd3fcfb831d671403f078d2310416fa3b304337d8279c0`  
		Last Modified: Wed, 17 Nov 2021 02:29:09 GMT  
		Size: 45.4 MB (45380444 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9.13` - linux; arm variant v5

```console
$ docker pull debian@sha256:37693dbcd033d85fbb9b15e32d711a418ae8f30f756210d8b1abb489e252225b
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **44.1 MB (44091379 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e84cd38de4fc6ca9807b62c600666c5599de965df3c10aae21c8e20049e176bd`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:55:43 GMT
ADD file:a8b0f40ff55722f4bb2e3ac65c3fefe9acaa513f89b73ed81ec04585058e58ba in / 
# Wed, 17 Nov 2021 02:55:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ec68ab8028a430064eed52d9b833f1a135dcce543b02666ee5acd73962b79070`  
		Last Modified: Wed, 17 Nov 2021 03:13:32 GMT  
		Size: 44.1 MB (44091379 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9.13` - linux; arm variant v7

```console
$ docker pull debian@sha256:9029da5bf6cbb6199e5f6d0eed4aaeaad998668abe02d29ea294ad2a3351e98e
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **42.1 MB (42116868 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6e2da5cf76886cdc5bdc612c501e562f2fce8be0d0aa51ec37bb1f39c4e32e5e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:04:57 GMT
ADD file:61da3fcef3aea99b4927189cfcb823a65b0e911a3f4436e44ea84a57a7919ff3 in / 
# Wed, 17 Nov 2021 02:04:58 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e4d54b576c2fcdd259e57bac14c4e1a2623f6e5dc9a0b1facaed7fe59d891e86`  
		Last Modified: Wed, 17 Nov 2021 02:22:03 GMT  
		Size: 42.1 MB (42116868 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9.13` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:70f4f03a8fd72cf60c3f8023f010721fece3fe599c9e7bc6eae10024eaa6a128
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **43.2 MB (43174478 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fad7665a3afc5981cd910850721e9f91cbf791c0962e4e250e298cee3059a4e0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:33 GMT
ADD file:3225671be4edc3599e29ebd06516b95573246e68bf29649c402ed87c03c109b5 in / 
# Wed, 17 Nov 2021 02:42:34 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c64a4bafe4ad6fc05fcc6d76d2a06f168f331ffbc87e518fb4ca4ea22e75527e`  
		Last Modified: Wed, 17 Nov 2021 02:51:11 GMT  
		Size: 43.2 MB (43174478 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9.13` - linux; 386

```console
$ docker pull debian@sha256:baa27aa91f579ac7cb42c594b2202ff5184fa4a64d7e88f0e1486725f61b0de2
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **46.1 MB (46097202 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7b1834ace8ddd99da39204c4a6faec3b1501af684a54e103348a966587edfe1a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:21 GMT
ADD file:8e57eaeffee1cd4c93865af9d6370c68bddae095016766191b1338ae675f131c in / 
# Wed, 17 Nov 2021 02:42:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:24fc2b60c45394a399438249c8e7f92f6fa415f9c477524d5af3ab4782d51519`  
		Last Modified: Wed, 17 Nov 2021 02:52:15 GMT  
		Size: 46.1 MB (46097202 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:9.13-slim`

```console
$ docker pull debian@sha256:d8566545bd3750e270b8aaf703d0a048fda080cd287a249823a5a11b7c95adbf
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 5
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:9.13-slim` - linux; amd64

```console
$ docker pull debian@sha256:bd4c41c9e68cc553f0c5266f18bef07491c52a08752f81a015892c2dc5e15114
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.5 MB (22527686 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a692eb442a03726bc7790f146f97afe590a942a526ec282f4c4d1bb724a9443a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:22:38 GMT
ADD file:2868c3af63afe6b8aadac07b8776e5468a3ed135fd84ad22df15e48f0610c7ba in / 
# Wed, 17 Nov 2021 02:22:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:89166b5eeae47d8c878cb88f20cc67eda2a550a697f4be317c7b7abea566b76f`  
		Last Modified: Wed, 17 Nov 2021 02:29:32 GMT  
		Size: 22.5 MB (22527686 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9.13-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:2aa2a15f320cd740783a33c547f4cd885d0b6e5b1ba3860024f8c8badc368c25
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **21.2 MB (21204234 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9e45f7a72e70f7c7a8193bfb2fdd770e1fa690422cb7f05e2a1332f8f8f63d60`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:56:16 GMT
ADD file:b857fb98b0880bbb3add8dd317cd1eb0f339332d4836baaedc8f2f424b5f2c98 in / 
# Wed, 17 Nov 2021 02:56:17 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ca67a5e1c16d08a9b8520dae81a39d168ef2a816d8905715eb6a328c89a2ce37`  
		Last Modified: Wed, 17 Nov 2021 03:14:12 GMT  
		Size: 21.2 MB (21204234 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9.13-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:9a52fdfda53f199f56b7d9a7d4dcc78a655da675926c88c1f4c4af044f9a5b79
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **19.3 MB (19316397 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:eecd35162e59af35e26178b7706ea9d74dc473fef1aed9fd0545734ab75f1304`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:05:31 GMT
ADD file:f28546ff6a3759d3a957c25d4310c3065e1367dff4ae304caae1cf1dc64d061d in / 
# Wed, 17 Nov 2021 02:05:32 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:614e23553da11f1294dc969bcab416c9f981794bdbaee5641335a222730c63b2`  
		Last Modified: Wed, 17 Nov 2021 02:22:41 GMT  
		Size: 19.3 MB (19316397 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9.13-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:b1902145e37b8fdd8c48092c245efcd8ac341091ed7ff3dbf91d7495bd293d88
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **20.4 MB (20389442 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:db5e084a09bc69d57eb2f658d7c5fa96910e232ee934ba9d2119a0d05fb7c5ed`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:48 GMT
ADD file:2adad5eee701d55a9f58c07f0706eb574d0ad6b74b0cc52a9e622f50639961c9 in / 
# Wed, 17 Nov 2021 02:42:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:34b92b588f0f390db0d125ab0ee4c85510c52367371c4bcf5af80a6ab18f0a9b`  
		Last Modified: Wed, 17 Nov 2021 02:51:38 GMT  
		Size: 20.4 MB (20389442 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:9.13-slim` - linux; 386

```console
$ docker pull debian@sha256:043ab2d585630cc218d3b5b0e09038405e13bff9585bee28cc02f5c93f9e9f86
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **23.2 MB (23156564 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:143712197327374e7944c356bf4336da6737d3df4709128da5224bfd486aa8dc`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:36 GMT
ADD file:3c9c1d0f88db0341cbe43b594cf6d2d865e39d9518b8e2b124cb9736bebe18f3 in / 
# Wed, 17 Nov 2021 02:42:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d213b39bd355350279fd232f3eedd77a11f7fb2a3d5a0017357dd45fa88342d1`  
		Last Modified: Wed, 17 Nov 2021 02:52:49 GMT  
		Size: 23.2 MB (23156564 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bookworm`

```console
$ docker pull debian@sha256:0e40baf6d51456330505acc7f59825ff826fe26cc16d836bf0cd2a753350a9c1
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bookworm` - linux; amd64

```console
$ docker pull debian@sha256:784eb3d1a1d1a155a9ab2eeaaa262444703a38cdcfeeec3bc191fc707d4793ac
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.5 MB (55457132 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4e1c267f633c7835ed3c91fcf4cfbec1fb6dade6f175cc4d50a7105d925ff672`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:20:09 GMT
ADD file:601f6b8dd671536125813546a4e1c4a913eac9c0e1e5f1da23424c151afd0a86 in / 
# Wed, 17 Nov 2021 02:20:10 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2b2cd17dc00b5ed987bf109cf2a80e6f71bbc49808221e9ce4f632b0c3bf2cbf`  
		Last Modified: Wed, 17 Nov 2021 02:24:39 GMT  
		Size: 55.5 MB (55457132 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; arm variant v5

```console
$ docker pull debian@sha256:1b436c6c9dcb101abee2bb1969a242252307b4b962ed532901e2167215360ffd
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.0 MB (52954065 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:dfc501b4f14312acf3dbf64d7dfd50f8dd3dde95436417255ce3061bfd00a75a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:49:05 GMT
ADD file:60965734792efb0a8934b5931e629d56125bcc5c63c0c04e80182bf8a1bb8f76 in / 
# Wed, 17 Nov 2021 02:49:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4ac9e63a36c99e7be2a07d5f4e6656cfe302edce5271e79a0a75af86159241bc`  
		Last Modified: Wed, 17 Nov 2021 03:03:58 GMT  
		Size: 53.0 MB (52954065 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; arm variant v7

```console
$ docker pull debian@sha256:83d3e15582bd65ef6a7a351867f877ab514a14669077f9641fb0793af9015a45
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50589015 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:44c48417a7d8e7a19b03a016d2baaec2a6cc19d8be90a9de9f6b93d265f99a22`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 01:58:07 GMT
ADD file:6def097172b517cba3f58db99b3dfb25785929bad969e154025a1db5fce45c12 in / 
# Wed, 17 Nov 2021 01:58:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6316dbdb4f8715ca0987361e8e193aa4f97570b0b307991e95a5bd5475b35ccb`  
		Last Modified: Wed, 17 Nov 2021 02:13:14 GMT  
		Size: 50.6 MB (50589015 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:c73efb084c4d5b270acedb93d6d111efb691f4572096b6fec2ba9ce7b5409a12
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.5 MB (54464387 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bb30e09ce47a0a0f2e5369bb3557315e482ec74f1ee6163d6d939502daea071c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:39:34 GMT
ADD file:cd982be7a5accd4c715a3de7a8682f869612c5401ec485ff4622a9590caf415e in / 
# Wed, 17 Nov 2021 02:39:34 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c646ef4eeacb8eade7583e3231bf320159c2c92b39835abd5631d8bcffea30b6`  
		Last Modified: Wed, 17 Nov 2021 02:45:59 GMT  
		Size: 54.5 MB (54464387 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; 386

```console
$ docker pull debian@sha256:af0d3c8823212818185c50a9a950be7c43e041bc9274d06db4647794e523a404
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.5 MB (56490626 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:304b61983620ab29cebeb41af58de9afc21994e39a166214f28d9e5b157be2e8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:39:06 GMT
ADD file:952f365b9933a7679a00c40feb459116dec4821f36ebbe7b93b86fe3db39e9a0 in / 
# Wed, 17 Nov 2021 02:39:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e4f07c4a13c97a4884800f77eed83407145089c45564213ae68f34cf4e49061d`  
		Last Modified: Wed, 17 Nov 2021 02:46:22 GMT  
		Size: 56.5 MB (56490626 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; mips64le

```console
$ docker pull debian@sha256:1fa841151c6ab28ea822275a8f70ec17106be3fff68deac4bc9c83b619790cc2
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.1 MB (54085680 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b41949fe18c7e304460afd992f6e1fd142932e8e25c680056813fcb2c4b807f6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:08:52 GMT
ADD file:30ef36b7192de64a1479731fca2ac7203d29511ecbb38732e0bcf5b572e8b5aa in / 
# Wed, 17 Nov 2021 02:08:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:76a3c406fc66bcc272351684f5b38ae64b35042b0075a004959ca5b74cacb50f`  
		Last Modified: Wed, 17 Nov 2021 02:17:02 GMT  
		Size: 54.1 MB (54085680 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; ppc64le

```console
$ docker pull debian@sha256:0f1dd3189777fecf9799133c2fa70f31a1b07a68a9f0bb1af4dfaae9b3376a82
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **59.7 MB (59706184 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:75910b3beefa3c3fcf3ab3ea68af1653a5f0fcbd46352529c57783efeac7b9d6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:25:04 GMT
ADD file:fad90d698c5f566b126df2df0ac9c0abcdc50b5d25b2a1e33ae2e8aa18b9567b in / 
# Wed, 17 Nov 2021 03:25:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2de091795abecdff7dace8e57ee5679fae062a958230bcb89652ea0b3f44f240`  
		Last Modified: Wed, 17 Nov 2021 03:47:07 GMT  
		Size: 59.7 MB (59706184 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm` - linux; s390x

```console
$ docker pull debian@sha256:f32a1999d92e99e71096b62cefe507fef56494d545a462bea289be106a47ef1d
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.7 MB (53669238 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:891110eda4b55eb9bdfcbb503da5cc889774360b86ef535cc9d141059f00d11d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:36 GMT
ADD file:7913ed6a2385b3bfbd659a88c7bae1ef30f61c8147e8e89b5fc5c4be13a393ea in / 
# Wed, 17 Nov 2021 02:41:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4457396a5c3a0ebad221540b501d8baee17387a6ab52192d3b8f63e151492792`  
		Last Modified: Wed, 17 Nov 2021 02:47:34 GMT  
		Size: 53.7 MB (53669238 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bookworm-20211115`

```console
$ docker pull debian@sha256:0e40baf6d51456330505acc7f59825ff826fe26cc16d836bf0cd2a753350a9c1
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bookworm-20211115` - linux; amd64

```console
$ docker pull debian@sha256:784eb3d1a1d1a155a9ab2eeaaa262444703a38cdcfeeec3bc191fc707d4793ac
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.5 MB (55457132 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4e1c267f633c7835ed3c91fcf4cfbec1fb6dade6f175cc4d50a7105d925ff672`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:20:09 GMT
ADD file:601f6b8dd671536125813546a4e1c4a913eac9c0e1e5f1da23424c151afd0a86 in / 
# Wed, 17 Nov 2021 02:20:10 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2b2cd17dc00b5ed987bf109cf2a80e6f71bbc49808221e9ce4f632b0c3bf2cbf`  
		Last Modified: Wed, 17 Nov 2021 02:24:39 GMT  
		Size: 55.5 MB (55457132 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20211115` - linux; arm variant v5

```console
$ docker pull debian@sha256:1b436c6c9dcb101abee2bb1969a242252307b4b962ed532901e2167215360ffd
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.0 MB (52954065 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:dfc501b4f14312acf3dbf64d7dfd50f8dd3dde95436417255ce3061bfd00a75a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:49:05 GMT
ADD file:60965734792efb0a8934b5931e629d56125bcc5c63c0c04e80182bf8a1bb8f76 in / 
# Wed, 17 Nov 2021 02:49:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4ac9e63a36c99e7be2a07d5f4e6656cfe302edce5271e79a0a75af86159241bc`  
		Last Modified: Wed, 17 Nov 2021 03:03:58 GMT  
		Size: 53.0 MB (52954065 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20211115` - linux; arm variant v7

```console
$ docker pull debian@sha256:83d3e15582bd65ef6a7a351867f877ab514a14669077f9641fb0793af9015a45
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50589015 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:44c48417a7d8e7a19b03a016d2baaec2a6cc19d8be90a9de9f6b93d265f99a22`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 01:58:07 GMT
ADD file:6def097172b517cba3f58db99b3dfb25785929bad969e154025a1db5fce45c12 in / 
# Wed, 17 Nov 2021 01:58:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:6316dbdb4f8715ca0987361e8e193aa4f97570b0b307991e95a5bd5475b35ccb`  
		Last Modified: Wed, 17 Nov 2021 02:13:14 GMT  
		Size: 50.6 MB (50589015 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20211115` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:c73efb084c4d5b270acedb93d6d111efb691f4572096b6fec2ba9ce7b5409a12
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.5 MB (54464387 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bb30e09ce47a0a0f2e5369bb3557315e482ec74f1ee6163d6d939502daea071c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:39:34 GMT
ADD file:cd982be7a5accd4c715a3de7a8682f869612c5401ec485ff4622a9590caf415e in / 
# Wed, 17 Nov 2021 02:39:34 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c646ef4eeacb8eade7583e3231bf320159c2c92b39835abd5631d8bcffea30b6`  
		Last Modified: Wed, 17 Nov 2021 02:45:59 GMT  
		Size: 54.5 MB (54464387 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20211115` - linux; 386

```console
$ docker pull debian@sha256:af0d3c8823212818185c50a9a950be7c43e041bc9274d06db4647794e523a404
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.5 MB (56490626 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:304b61983620ab29cebeb41af58de9afc21994e39a166214f28d9e5b157be2e8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:39:06 GMT
ADD file:952f365b9933a7679a00c40feb459116dec4821f36ebbe7b93b86fe3db39e9a0 in / 
# Wed, 17 Nov 2021 02:39:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e4f07c4a13c97a4884800f77eed83407145089c45564213ae68f34cf4e49061d`  
		Last Modified: Wed, 17 Nov 2021 02:46:22 GMT  
		Size: 56.5 MB (56490626 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20211115` - linux; mips64le

```console
$ docker pull debian@sha256:1fa841151c6ab28ea822275a8f70ec17106be3fff68deac4bc9c83b619790cc2
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.1 MB (54085680 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b41949fe18c7e304460afd992f6e1fd142932e8e25c680056813fcb2c4b807f6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:08:52 GMT
ADD file:30ef36b7192de64a1479731fca2ac7203d29511ecbb38732e0bcf5b572e8b5aa in / 
# Wed, 17 Nov 2021 02:08:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:76a3c406fc66bcc272351684f5b38ae64b35042b0075a004959ca5b74cacb50f`  
		Last Modified: Wed, 17 Nov 2021 02:17:02 GMT  
		Size: 54.1 MB (54085680 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20211115` - linux; ppc64le

```console
$ docker pull debian@sha256:0f1dd3189777fecf9799133c2fa70f31a1b07a68a9f0bb1af4dfaae9b3376a82
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **59.7 MB (59706184 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:75910b3beefa3c3fcf3ab3ea68af1653a5f0fcbd46352529c57783efeac7b9d6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:25:04 GMT
ADD file:fad90d698c5f566b126df2df0ac9c0abcdc50b5d25b2a1e33ae2e8aa18b9567b in / 
# Wed, 17 Nov 2021 03:25:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2de091795abecdff7dace8e57ee5679fae062a958230bcb89652ea0b3f44f240`  
		Last Modified: Wed, 17 Nov 2021 03:47:07 GMT  
		Size: 59.7 MB (59706184 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20211115` - linux; s390x

```console
$ docker pull debian@sha256:f32a1999d92e99e71096b62cefe507fef56494d545a462bea289be106a47ef1d
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.7 MB (53669238 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:891110eda4b55eb9bdfcbb503da5cc889774360b86ef535cc9d141059f00d11d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:36 GMT
ADD file:7913ed6a2385b3bfbd659a88c7bae1ef30f61c8147e8e89b5fc5c4be13a393ea in / 
# Wed, 17 Nov 2021 02:41:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4457396a5c3a0ebad221540b501d8baee17387a6ab52192d3b8f63e151492792`  
		Last Modified: Wed, 17 Nov 2021 02:47:34 GMT  
		Size: 53.7 MB (53669238 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bookworm-20211115-slim`

```console
$ docker pull debian@sha256:a3db73ce8bc3465421efb0fe3dcaf254e8adc3e3be6e5745b9a9eb92e882cbd1
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bookworm-20211115-slim` - linux; amd64

```console
$ docker pull debian@sha256:ea21bbca87cb11cf811ca9c8630bd6c00b83e9c3b3e8446001c379d31e9658ea
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.8 MB (31765019 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8dba587709c149f428cc9d9b5fce1dbff2e78432e1e9b1ac7604da16180755c0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:20:21 GMT
ADD file:f6a42d605d4b9bb1c7dbc11bbe40e52c29ce84a105ca59bc749a8b62e0ab8140 in / 
# Wed, 17 Nov 2021 02:20:21 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:56030c746a094051547bf543cf29b7bac8c96943f7e203c5ac6280c857b5f3c0`  
		Last Modified: Wed, 17 Nov 2021 02:25:00 GMT  
		Size: 31.8 MB (31765019 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20211115-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:9fe780ae9fef126f47d0134ef99cdbecc57dadfe69c959db7c125ae69728e049
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.3 MB (29263942 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a588ba71be66c2d35615fb35df0455d960a6df817287a0e66613d197ca18e0f3`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:49:37 GMT
ADD file:2ed526fdaa7f32c640608d4ab775a5956b4e4a9e429820f9d9c0636961f3dc3f in / 
# Wed, 17 Nov 2021 02:49:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c206fb8d661cc778397445d3a37dac107d68155be82dcff57ce5e007cbaba441`  
		Last Modified: Wed, 17 Nov 2021 03:04:36 GMT  
		Size: 29.3 MB (29263942 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20211115-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:6cf4aaa1b34ad64537c29caeb88d9a8dd48d60ab12c7ed753294f1e5a46ac3ea
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.9 MB (26899989 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:322d0bd767928c6aeedb2c11b74daaa91ad624527b0d9159feddd54926e6630d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 01:58:40 GMT
ADD file:4c06ca77978294fa3eb18f0df3ef98ea4491aef5a477fa53c6d5942cb6fe365d in / 
# Wed, 17 Nov 2021 01:58:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:cfa3d8a825a9bfb5c23ae006b4caf40c63f379aa86d45d10eb8a2fe8572ba4c8`  
		Last Modified: Wed, 17 Nov 2021 02:13:48 GMT  
		Size: 26.9 MB (26899989 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20211115-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:e3293e2425fd08f4f91ad601d3b1d9a374d9842a1aca8288a16f3b32477913c9
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.8 MB (30772538 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:513c51e6f110287de50c0882a95dc208df56e84b190346d1c6eb847337468e38`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:39:49 GMT
ADD file:3311da0214df29eaa7c561d69f5f7e757cdc4b19bd579aa4b32f7dc4c6c9e28e in / 
# Wed, 17 Nov 2021 02:39:49 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b46d1da7411c6d59039410f54fcb809e5fc45196919c3812fe522f7969506fa2`  
		Last Modified: Wed, 17 Nov 2021 02:46:27 GMT  
		Size: 30.8 MB (30772538 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20211115-slim` - linux; 386

```console
$ docker pull debian@sha256:cd47b971e0618501f8ed67661674e960634da538de0c0d95217728891e577b83
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.8 MB (32801554 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:544ea71b29b7881c56f7cf0c9c42f386827f5dafb840cdfaac6c66ed6051339c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:39:23 GMT
ADD file:2f8e62268074da72b3d9b49fb2bd4b883674688a9fe1363b6e40d903046e4399 in / 
# Wed, 17 Nov 2021 02:39:23 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9e307e9f12112c05ab802885128be0b2349b81bc5f632b228bfe7824f111582c`  
		Last Modified: Wed, 17 Nov 2021 02:46:48 GMT  
		Size: 32.8 MB (32801554 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20211115-slim` - linux; mips64le

```console
$ docker pull debian@sha256:d776359f8422fb6b6b1c3dab3b158f5b3cf6ad30890c9368dfb4625cdfb0d791
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.4 MB (30394357 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c079bf2a035a27ca4a456f6ff1f708df958fb1137a1115a312a5fe2ecaea3413`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:09:18 GMT
ADD file:0cfe7d3ac7c14e1f82948992cc72b4bac40a843084dab11b9afb58b2fe0b5494 in / 
# Wed, 17 Nov 2021 02:09:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:91969958f90204260cdc194899359e77f92564724d1df7797a25d47022f13369`  
		Last Modified: Wed, 17 Nov 2021 02:17:40 GMT  
		Size: 30.4 MB (30394357 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20211115-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:7e67704d41f705d7c1c4b8e58aa1f17b6775fc949178c2d545f176b0ee6d4ba4
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **36.0 MB (36014976 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:54f3e021f31b16bda7bae14b93e9edb489d2e618a22d74acc91985c64cf06e2c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:26:34 GMT
ADD file:362555703d58a643f3413b5e1ba57f072ccf9d4616a70af821535aa49d33ff64 in / 
# Wed, 17 Nov 2021 03:26:43 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:02f2c90951c6ac202d9cad544ed879bfec4607a0cdc8c3a609af6dfc52039cd4`  
		Last Modified: Wed, 17 Nov 2021 03:49:02 GMT  
		Size: 36.0 MB (36014976 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-20211115-slim` - linux; s390x

```console
$ docker pull debian@sha256:052cc677312d68bfafcd480725d99c181822d51616b1a6eeafd0009f6c6912e4
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.0 MB (29977969 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:dc9a1da6a3744955a8da2879d6079669d2a51fbe66d8939461f4a1886fb1f7d6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:54 GMT
ADD file:b2e63719d40a8ecf8bd2a30f2979ee12ba66cb0598e7b11e49c87090cac4300a in / 
# Wed, 17 Nov 2021 02:41:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7d552b601ba4f3b4cdb64a6b647cdf2514134bc4db43c322f6ae07a527818a4f`  
		Last Modified: Wed, 17 Nov 2021 02:47:52 GMT  
		Size: 30.0 MB (29977969 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bookworm-backports`

```console
$ docker pull debian@sha256:b39bff6ed0ddb0d1dbd7c3ee39bb4ac8e3a06278e15c6399078cb5ca1a4c400a
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bookworm-backports` - linux; amd64

```console
$ docker pull debian@sha256:c6cd809af904051227ab90f5fe831da3193a748002ad78a98bab4d5b91f27be5
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.5 MB (55457359 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:16f48ca9acc0c85c4f5112ac60ed131debabb8c54fc110fb8af92ec83b6f58f5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:20:09 GMT
ADD file:601f6b8dd671536125813546a4e1c4a913eac9c0e1e5f1da23424c151afd0a86 in / 
# Wed, 17 Nov 2021 02:20:10 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:20:13 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:2b2cd17dc00b5ed987bf109cf2a80e6f71bbc49808221e9ce4f632b0c3bf2cbf`  
		Last Modified: Wed, 17 Nov 2021 02:24:39 GMT  
		Size: 55.5 MB (55457132 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:8aebc0444b2b5be1f26201f35a806a6698e6bcbb496872c02ef2a446451a745d`  
		Last Modified: Wed, 17 Nov 2021 02:24:48 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:f4b2523ae776542963c1784bc628c0ccce8448c0aae765bb0fdc4375100d9928
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.0 MB (52954291 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:04f3456ef5911d20a17d4a9f0e098e43d8bd6536b21431f366c5736e8830ed48`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:49:05 GMT
ADD file:60965734792efb0a8934b5931e629d56125bcc5c63c0c04e80182bf8a1bb8f76 in / 
# Wed, 17 Nov 2021 02:49:06 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:49:19 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:4ac9e63a36c99e7be2a07d5f4e6656cfe302edce5271e79a0a75af86159241bc`  
		Last Modified: Wed, 17 Nov 2021 03:03:58 GMT  
		Size: 53.0 MB (52954065 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:2d2bd641730534e6c1b857b7a8129274789d21a60bc0ea0715ce97d57f9f8476`  
		Last Modified: Wed, 17 Nov 2021 03:04:10 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:a12111d3107afe6e5cba6bd7284ad5fac71667d6ebe9a86f756011c9253065ee
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50589242 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:58fda4853768dab32f5ea58f84d28ab2f888e02017edd6c19a217ad3c23d1fe1`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 01:58:07 GMT
ADD file:6def097172b517cba3f58db99b3dfb25785929bad969e154025a1db5fce45c12 in / 
# Wed, 17 Nov 2021 01:58:09 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 01:58:21 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:6316dbdb4f8715ca0987361e8e193aa4f97570b0b307991e95a5bd5475b35ccb`  
		Last Modified: Wed, 17 Nov 2021 02:13:14 GMT  
		Size: 50.6 MB (50589015 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:8a593cabba4a80ac3c0d10f1ebf5685b08a5ed4cd93a935e5663efe65cad1874`  
		Last Modified: Wed, 17 Nov 2021 02:13:26 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:553a3ea7b9ef880c02121ded7674510c5d8849df4f3cf72101e5b6612779b0e6
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.5 MB (54464614 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c7cc4f49d0f64481e618b845ce734645c1f9bb323be3496b196eddeda9d9a31f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:39:34 GMT
ADD file:cd982be7a5accd4c715a3de7a8682f869612c5401ec485ff4622a9590caf415e in / 
# Wed, 17 Nov 2021 02:39:34 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:39:40 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:c646ef4eeacb8eade7583e3231bf320159c2c92b39835abd5631d8bcffea30b6`  
		Last Modified: Wed, 17 Nov 2021 02:45:59 GMT  
		Size: 54.5 MB (54464387 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:d8bcba5a68f6a2b8bcbf116fe8ea48697b964cd39cd77fe24a89c53d76f2d989`  
		Last Modified: Wed, 17 Nov 2021 02:46:10 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; 386

```console
$ docker pull debian@sha256:e20eaae130866b755850bcd10c2286dea0808feb92243ac57e5cd82d7eb11766
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.5 MB (56490851 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fe6dc1b7a2d435195ae6cadb7f718ac0caba4be936af78099afa87e1defad3e8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:39:06 GMT
ADD file:952f365b9933a7679a00c40feb459116dec4821f36ebbe7b93b86fe3db39e9a0 in / 
# Wed, 17 Nov 2021 02:39:06 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:39:14 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:e4f07c4a13c97a4884800f77eed83407145089c45564213ae68f34cf4e49061d`  
		Last Modified: Wed, 17 Nov 2021 02:46:22 GMT  
		Size: 56.5 MB (56490626 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:8e73ea4efdf59fe96473b744ae0d9bee91e614a9d4618bb3ef4a5fe8b54a114e`  
		Last Modified: Wed, 17 Nov 2021 02:46:34 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; mips64le

```console
$ docker pull debian@sha256:becaa89cc62c033a5a51640af12896cc543d80532e487a3d5471a9f6b161eac5
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.1 MB (54085908 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2687a6a6698aa28b538f21cc026bcf40b3a709bd6b1945b3eea28c299dc647bf`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:08:52 GMT
ADD file:30ef36b7192de64a1479731fca2ac7203d29511ecbb38732e0bcf5b572e8b5aa in / 
# Wed, 17 Nov 2021 02:08:53 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:09:00 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:76a3c406fc66bcc272351684f5b38ae64b35042b0075a004959ca5b74cacb50f`  
		Last Modified: Wed, 17 Nov 2021 02:17:02 GMT  
		Size: 54.1 MB (54085680 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:17c3722bf0b9420a39cfe5a2435c3a4463d29169fec26b7b5a355eb9a2809e77`  
		Last Modified: Wed, 17 Nov 2021 02:17:12 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; ppc64le

```console
$ docker pull debian@sha256:9071efe09ce6d8e6a9b6b7226d8a205f5b266177c41c78598f833b14d090ffb4
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **59.7 MB (59706412 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bb59e9d3c59f71159bfcdac5dcb149b440289b4c1f08b42b845879d826978274`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:25:04 GMT
ADD file:fad90d698c5f566b126df2df0ac9c0abcdc50b5d25b2a1e33ae2e8aa18b9567b in / 
# Wed, 17 Nov 2021 03:25:19 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 03:26:09 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:2de091795abecdff7dace8e57ee5679fae062a958230bcb89652ea0b3f44f240`  
		Last Modified: Wed, 17 Nov 2021 03:47:07 GMT  
		Size: 59.7 MB (59706184 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:81c229d90083d4647f97c42e0f7feb12f8079d91fa6ff6dfbc0952f19e11cec9`  
		Last Modified: Wed, 17 Nov 2021 03:47:22 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-backports` - linux; s390x

```console
$ docker pull debian@sha256:6b921b3ac95fa35d54bce79c71c521bb79945ce081dbb843ea590bcfaaa01352
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.7 MB (53669463 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0b9a9322bbd38a47801edcbecc6975762fa99b8a17b3df5f0f0d24072a05ed03`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:36 GMT
ADD file:7913ed6a2385b3bfbd659a88c7bae1ef30f61c8147e8e89b5fc5c4be13a393ea in / 
# Wed, 17 Nov 2021 02:41:38 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:41:45 GMT
RUN echo 'deb http://deb.debian.org/debian bookworm-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:4457396a5c3a0ebad221540b501d8baee17387a6ab52192d3b8f63e151492792`  
		Last Modified: Wed, 17 Nov 2021 02:47:34 GMT  
		Size: 53.7 MB (53669238 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:ad19338e0216949f11a92dd8945b6e22f31f21cf795fc761001c8a74e348415b`  
		Last Modified: Wed, 17 Nov 2021 02:47:42 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bookworm-slim`

```console
$ docker pull debian@sha256:a3db73ce8bc3465421efb0fe3dcaf254e8adc3e3be6e5745b9a9eb92e882cbd1
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bookworm-slim` - linux; amd64

```console
$ docker pull debian@sha256:ea21bbca87cb11cf811ca9c8630bd6c00b83e9c3b3e8446001c379d31e9658ea
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.8 MB (31765019 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8dba587709c149f428cc9d9b5fce1dbff2e78432e1e9b1ac7604da16180755c0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:20:21 GMT
ADD file:f6a42d605d4b9bb1c7dbc11bbe40e52c29ce84a105ca59bc749a8b62e0ab8140 in / 
# Wed, 17 Nov 2021 02:20:21 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:56030c746a094051547bf543cf29b7bac8c96943f7e203c5ac6280c857b5f3c0`  
		Last Modified: Wed, 17 Nov 2021 02:25:00 GMT  
		Size: 31.8 MB (31765019 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:9fe780ae9fef126f47d0134ef99cdbecc57dadfe69c959db7c125ae69728e049
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.3 MB (29263942 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a588ba71be66c2d35615fb35df0455d960a6df817287a0e66613d197ca18e0f3`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:49:37 GMT
ADD file:2ed526fdaa7f32c640608d4ab775a5956b4e4a9e429820f9d9c0636961f3dc3f in / 
# Wed, 17 Nov 2021 02:49:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c206fb8d661cc778397445d3a37dac107d68155be82dcff57ce5e007cbaba441`  
		Last Modified: Wed, 17 Nov 2021 03:04:36 GMT  
		Size: 29.3 MB (29263942 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:6cf4aaa1b34ad64537c29caeb88d9a8dd48d60ab12c7ed753294f1e5a46ac3ea
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.9 MB (26899989 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:322d0bd767928c6aeedb2c11b74daaa91ad624527b0d9159feddd54926e6630d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 01:58:40 GMT
ADD file:4c06ca77978294fa3eb18f0df3ef98ea4491aef5a477fa53c6d5942cb6fe365d in / 
# Wed, 17 Nov 2021 01:58:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:cfa3d8a825a9bfb5c23ae006b4caf40c63f379aa86d45d10eb8a2fe8572ba4c8`  
		Last Modified: Wed, 17 Nov 2021 02:13:48 GMT  
		Size: 26.9 MB (26899989 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:e3293e2425fd08f4f91ad601d3b1d9a374d9842a1aca8288a16f3b32477913c9
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.8 MB (30772538 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:513c51e6f110287de50c0882a95dc208df56e84b190346d1c6eb847337468e38`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:39:49 GMT
ADD file:3311da0214df29eaa7c561d69f5f7e757cdc4b19bd579aa4b32f7dc4c6c9e28e in / 
# Wed, 17 Nov 2021 02:39:49 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b46d1da7411c6d59039410f54fcb809e5fc45196919c3812fe522f7969506fa2`  
		Last Modified: Wed, 17 Nov 2021 02:46:27 GMT  
		Size: 30.8 MB (30772538 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; 386

```console
$ docker pull debian@sha256:cd47b971e0618501f8ed67661674e960634da538de0c0d95217728891e577b83
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.8 MB (32801554 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:544ea71b29b7881c56f7cf0c9c42f386827f5dafb840cdfaac6c66ed6051339c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:39:23 GMT
ADD file:2f8e62268074da72b3d9b49fb2bd4b883674688a9fe1363b6e40d903046e4399 in / 
# Wed, 17 Nov 2021 02:39:23 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9e307e9f12112c05ab802885128be0b2349b81bc5f632b228bfe7824f111582c`  
		Last Modified: Wed, 17 Nov 2021 02:46:48 GMT  
		Size: 32.8 MB (32801554 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; mips64le

```console
$ docker pull debian@sha256:d776359f8422fb6b6b1c3dab3b158f5b3cf6ad30890c9368dfb4625cdfb0d791
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.4 MB (30394357 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c079bf2a035a27ca4a456f6ff1f708df958fb1137a1115a312a5fe2ecaea3413`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:09:18 GMT
ADD file:0cfe7d3ac7c14e1f82948992cc72b4bac40a843084dab11b9afb58b2fe0b5494 in / 
# Wed, 17 Nov 2021 02:09:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:91969958f90204260cdc194899359e77f92564724d1df7797a25d47022f13369`  
		Last Modified: Wed, 17 Nov 2021 02:17:40 GMT  
		Size: 30.4 MB (30394357 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:7e67704d41f705d7c1c4b8e58aa1f17b6775fc949178c2d545f176b0ee6d4ba4
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **36.0 MB (36014976 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:54f3e021f31b16bda7bae14b93e9edb489d2e618a22d74acc91985c64cf06e2c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:26:34 GMT
ADD file:362555703d58a643f3413b5e1ba57f072ccf9d4616a70af821535aa49d33ff64 in / 
# Wed, 17 Nov 2021 03:26:43 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:02f2c90951c6ac202d9cad544ed879bfec4607a0cdc8c3a609af6dfc52039cd4`  
		Last Modified: Wed, 17 Nov 2021 03:49:02 GMT  
		Size: 36.0 MB (36014976 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bookworm-slim` - linux; s390x

```console
$ docker pull debian@sha256:052cc677312d68bfafcd480725d99c181822d51616b1a6eeafd0009f6c6912e4
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.0 MB (29977969 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:dc9a1da6a3744955a8da2879d6079669d2a51fbe66d8939461f4a1886fb1f7d6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:54 GMT
ADD file:b2e63719d40a8ecf8bd2a30f2979ee12ba66cb0598e7b11e49c87090cac4300a in / 
# Wed, 17 Nov 2021 02:41:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7d552b601ba4f3b4cdb64a6b647cdf2514134bc4db43c322f6ae07a527818a4f`  
		Last Modified: Wed, 17 Nov 2021 02:47:52 GMT  
		Size: 30.0 MB (29977969 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bullseye`

```console
$ docker pull debian@sha256:e8c184b56a94db0947a9d51ec68f42ef5584442f20547fa3bd8cbd00203b2e7a
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bullseye` - linux; amd64

```console
$ docker pull debian@sha256:6480d244c39d52f2de8ee0417cc385aa437cabfbfd8e3c127882c4d03f8ab86d
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.9 MB (54932774 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:827e5611389abf13dad1057e92f163b771febc0bcdb19fa2d634a7eb0641e0cc`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:20:30 GMT
ADD file:5259fc086e8295ddbf02e48abef38e9bf93a183079d3631aa7a59306b7f2f9df in / 
# Wed, 17 Nov 2021 02:20:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:647acf3d48c2780e00cd27bb0984367415f270d78477ef9d5b238e6ebd5290da`  
		Last Modified: Wed, 17 Nov 2021 02:25:17 GMT  
		Size: 54.9 MB (54932774 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; arm variant v5

```console
$ docker pull debian@sha256:f76b8cf174e44cdc775877460bc3e19c3e30e66b271cb5d39f726cfa7094044f
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52467904 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:49f6c0d2973ae831b31a88d01129b59c15cdf95d69ea05d1eb401f8a920846d8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:50:02 GMT
ADD file:48c0196bfa5dfd1137285bf9104248d62f15a734133d8df549f35b6f7989ca19 in / 
# Wed, 17 Nov 2021 02:50:03 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ce39e946c04dab3f8b463d502d71849e8e6d7291ece7e323f0ec9dac1061af26`  
		Last Modified: Wed, 17 Nov 2021 03:05:19 GMT  
		Size: 52.5 MB (52467904 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; arm variant v7

```console
$ docker pull debian@sha256:8926b33de48e3a6a7f84356ce88179481f44523f12d55ad8c3ba287dc191ffdc
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.1 MB (50134152 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e9cf93b473f42691f8231524112260deb7aa0f6f9a371fa9b8f552f24285519d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 01:59:07 GMT
ADD file:43b45f65ff4ba720f6f9a107b5f80818d5ea6ebc4835840b11e9a506351da32f in / 
# Wed, 17 Nov 2021 01:59:08 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:dbbf55137aab3c7acd1e012a70f1ea1adeaef8d39b474829965d3480a02bf572`  
		Last Modified: Wed, 17 Nov 2021 02:14:27 GMT  
		Size: 50.1 MB (50134152 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:c9178c59ba331e2a0745ddf7109c44fe17f19dca2eb7ecb8c14a4c566124a025
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53619024 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:eec76c447f59d03e1d07a8bc96368012b2f05e4a52b8812a5671faef329f1eaf`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:39:59 GMT
ADD file:eb667e18226da8a1ca5a41beacddc65cd1efec01c3d01e50845c5e89e91ea17c in / 
# Wed, 17 Nov 2021 02:40:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ce18cd0ba11c2dfa8c1a2d2b325e1e019b14d4ae25ffdeffb0f9686c5496bf5c`  
		Last Modified: Wed, 17 Nov 2021 02:46:45 GMT  
		Size: 53.6 MB (53619024 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; 386

```console
$ docker pull debian@sha256:d1b5cbf0a8cbcd44147c1d46b9b8f778f436ed5174a55b1b84cab79163215905
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.9 MB (55937586 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:59b34e04dc278408b2e7fc09cacf1411ecd25f3105a711a746f1d22f1d39a87a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:39:36 GMT
ADD file:b8f9021573a53ec69fa566a396d9a8c68392bd6d659a665c0aea8fbd00164f12 in / 
# Wed, 17 Nov 2021 02:39:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8d649f3a0e8410b121648ccc552acdac0960691a4da33ff4db9825f9b51ff3f4`  
		Last Modified: Wed, 17 Nov 2021 02:47:12 GMT  
		Size: 55.9 MB (55937586 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; mips64le

```console
$ docker pull debian@sha256:d89c75015f88e1ca939d94c69ac13e4f8134ae432acfa6900076c1fbb30cad76
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53183806 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5035dbb39f67711b5184433a96cafd0db4bc9b6a653c914110ecb01166a99a7b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:09:46 GMT
ADD file:2971df49f24648b8d5166d87e392e2547d5b03469356f20d112ce6feb3c329fc in / 
# Wed, 17 Nov 2021 02:09:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b095a0b63d99a4c4cd9c1c353f381a69ef041e6f80798d807ae51a34b04920a0`  
		Last Modified: Wed, 17 Nov 2021 02:18:22 GMT  
		Size: 53.2 MB (53183806 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; ppc64le

```console
$ docker pull debian@sha256:952516c1afd1ea16fa745b83502d222062d24df80fdcd73cece4640609732c60
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.8 MB (58819505 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c78f4b353abfd74cf67ea010720e7bbee93d8e5383b5429a6ab9f35ad982f5ec`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:27:04 GMT
ADD file:85375acb19da58f9eabaa67f8cb425cd1ff2dadb2888bbd9dbfa6223d16fef23 in / 
# Wed, 17 Nov 2021 03:27:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ede9cd7c8f1d380c971a130fb3d8a3914bf3a9106702164c23b2cce536fa618c`  
		Last Modified: Wed, 17 Nov 2021 03:52:39 GMT  
		Size: 58.8 MB (58819505 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye` - linux; s390x

```console
$ docker pull debian@sha256:f9147074e1223cfc833be4e33b5b80a80017cd3a4c6f00e3af27bde69dd10c80
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53207179 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:40a4f800f6aa1a5b14da6ed5b5e9f4ecfc77ff178aa0685d69f7cfc918a93695`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:07 GMT
ADD file:b7524e775a2033d246d1096765a0eb6eaa5ba6ed6c8b395522579d2c4e7a2e38 in / 
# Wed, 17 Nov 2021 02:42:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ffa47acd07831fbc96680a705da20704bac0016c20dc3c454ecd45cda44b368d`  
		Last Modified: Wed, 17 Nov 2021 02:48:05 GMT  
		Size: 53.2 MB (53207179 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bullseye-20211115`

```console
$ docker pull debian@sha256:e8c184b56a94db0947a9d51ec68f42ef5584442f20547fa3bd8cbd00203b2e7a
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bullseye-20211115` - linux; amd64

```console
$ docker pull debian@sha256:6480d244c39d52f2de8ee0417cc385aa437cabfbfd8e3c127882c4d03f8ab86d
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.9 MB (54932774 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:827e5611389abf13dad1057e92f163b771febc0bcdb19fa2d634a7eb0641e0cc`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:20:30 GMT
ADD file:5259fc086e8295ddbf02e48abef38e9bf93a183079d3631aa7a59306b7f2f9df in / 
# Wed, 17 Nov 2021 02:20:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:647acf3d48c2780e00cd27bb0984367415f270d78477ef9d5b238e6ebd5290da`  
		Last Modified: Wed, 17 Nov 2021 02:25:17 GMT  
		Size: 54.9 MB (54932774 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20211115` - linux; arm variant v5

```console
$ docker pull debian@sha256:f76b8cf174e44cdc775877460bc3e19c3e30e66b271cb5d39f726cfa7094044f
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52467904 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:49f6c0d2973ae831b31a88d01129b59c15cdf95d69ea05d1eb401f8a920846d8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:50:02 GMT
ADD file:48c0196bfa5dfd1137285bf9104248d62f15a734133d8df549f35b6f7989ca19 in / 
# Wed, 17 Nov 2021 02:50:03 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ce39e946c04dab3f8b463d502d71849e8e6d7291ece7e323f0ec9dac1061af26`  
		Last Modified: Wed, 17 Nov 2021 03:05:19 GMT  
		Size: 52.5 MB (52467904 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20211115` - linux; arm variant v7

```console
$ docker pull debian@sha256:8926b33de48e3a6a7f84356ce88179481f44523f12d55ad8c3ba287dc191ffdc
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.1 MB (50134152 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e9cf93b473f42691f8231524112260deb7aa0f6f9a371fa9b8f552f24285519d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 01:59:07 GMT
ADD file:43b45f65ff4ba720f6f9a107b5f80818d5ea6ebc4835840b11e9a506351da32f in / 
# Wed, 17 Nov 2021 01:59:08 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:dbbf55137aab3c7acd1e012a70f1ea1adeaef8d39b474829965d3480a02bf572`  
		Last Modified: Wed, 17 Nov 2021 02:14:27 GMT  
		Size: 50.1 MB (50134152 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20211115` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:c9178c59ba331e2a0745ddf7109c44fe17f19dca2eb7ecb8c14a4c566124a025
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53619024 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:eec76c447f59d03e1d07a8bc96368012b2f05e4a52b8812a5671faef329f1eaf`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:39:59 GMT
ADD file:eb667e18226da8a1ca5a41beacddc65cd1efec01c3d01e50845c5e89e91ea17c in / 
# Wed, 17 Nov 2021 02:40:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ce18cd0ba11c2dfa8c1a2d2b325e1e019b14d4ae25ffdeffb0f9686c5496bf5c`  
		Last Modified: Wed, 17 Nov 2021 02:46:45 GMT  
		Size: 53.6 MB (53619024 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20211115` - linux; 386

```console
$ docker pull debian@sha256:d1b5cbf0a8cbcd44147c1d46b9b8f778f436ed5174a55b1b84cab79163215905
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.9 MB (55937586 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:59b34e04dc278408b2e7fc09cacf1411ecd25f3105a711a746f1d22f1d39a87a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:39:36 GMT
ADD file:b8f9021573a53ec69fa566a396d9a8c68392bd6d659a665c0aea8fbd00164f12 in / 
# Wed, 17 Nov 2021 02:39:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8d649f3a0e8410b121648ccc552acdac0960691a4da33ff4db9825f9b51ff3f4`  
		Last Modified: Wed, 17 Nov 2021 02:47:12 GMT  
		Size: 55.9 MB (55937586 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20211115` - linux; mips64le

```console
$ docker pull debian@sha256:d89c75015f88e1ca939d94c69ac13e4f8134ae432acfa6900076c1fbb30cad76
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53183806 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5035dbb39f67711b5184433a96cafd0db4bc9b6a653c914110ecb01166a99a7b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:09:46 GMT
ADD file:2971df49f24648b8d5166d87e392e2547d5b03469356f20d112ce6feb3c329fc in / 
# Wed, 17 Nov 2021 02:09:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b095a0b63d99a4c4cd9c1c353f381a69ef041e6f80798d807ae51a34b04920a0`  
		Last Modified: Wed, 17 Nov 2021 02:18:22 GMT  
		Size: 53.2 MB (53183806 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20211115` - linux; ppc64le

```console
$ docker pull debian@sha256:952516c1afd1ea16fa745b83502d222062d24df80fdcd73cece4640609732c60
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.8 MB (58819505 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c78f4b353abfd74cf67ea010720e7bbee93d8e5383b5429a6ab9f35ad982f5ec`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:27:04 GMT
ADD file:85375acb19da58f9eabaa67f8cb425cd1ff2dadb2888bbd9dbfa6223d16fef23 in / 
# Wed, 17 Nov 2021 03:27:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ede9cd7c8f1d380c971a130fb3d8a3914bf3a9106702164c23b2cce536fa618c`  
		Last Modified: Wed, 17 Nov 2021 03:52:39 GMT  
		Size: 58.8 MB (58819505 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20211115` - linux; s390x

```console
$ docker pull debian@sha256:f9147074e1223cfc833be4e33b5b80a80017cd3a4c6f00e3af27bde69dd10c80
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53207179 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:40a4f800f6aa1a5b14da6ed5b5e9f4ecfc77ff178aa0685d69f7cfc918a93695`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:07 GMT
ADD file:b7524e775a2033d246d1096765a0eb6eaa5ba6ed6c8b395522579d2c4e7a2e38 in / 
# Wed, 17 Nov 2021 02:42:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ffa47acd07831fbc96680a705da20704bac0016c20dc3c454ecd45cda44b368d`  
		Last Modified: Wed, 17 Nov 2021 02:48:05 GMT  
		Size: 53.2 MB (53207179 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bullseye-20211115-slim`

```console
$ docker pull debian@sha256:a23887a2e830b815955e010f30d4c2430cd5ef82e93c130471024bc9f808d5d3
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bullseye-20211115-slim` - linux; amd64

```console
$ docker pull debian@sha256:832234a41bd12f5bcbed92f5ada9af6e2e2b3a394b2353f072b4ad413a3b3320
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.4 MB (31370267 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:66b2aecdb9f0f055837a85eca27f1c790e78d935b21bd000559f57e04f75e06b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:20:41 GMT
ADD file:a2405ebb9892d98be2eb585f6121864d12b3fd983ebf15e5f0b7486e106a79c6 in / 
# Wed, 17 Nov 2021 02:20:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:eff15d958d664f0874d16aee393cc44387031ee0a68ef8542d0056c747f378e8`  
		Last Modified: Wed, 17 Nov 2021 02:25:45 GMT  
		Size: 31.4 MB (31370267 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20211115-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:101787af86135c7b1346049404e63165bf65a9a2f1334f54571374d80f0ecac9
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **28.9 MB (28911006 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:968cb14e039277aaef3a19da601adf10cc0125e61520ebbbbffb29ad6fca6fef`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:50:37 GMT
ADD file:738a04a17bdb9077594ad9a847333abe28216a7f04d3058718a5e21c236c24bb in / 
# Wed, 17 Nov 2021 02:50:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a960a56baa1baffbe2aa1e0c1fb02f9ee4816337d02fec259b312c409d77fafc`  
		Last Modified: Wed, 17 Nov 2021 03:06:09 GMT  
		Size: 28.9 MB (28911006 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20211115-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:e2917d74741b83dac5e2e1cac59b8c79115aa1334b908bf020e9605f46a2fb77
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.6 MB (26573160 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:eaa955c4e58c7dc8e698c7e54814ed361ee80aad303a67c39606d6c4fa849f37`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 01:59:43 GMT
ADD file:cd2ac52107a2ea6657f23850a4b29366309eb39fa177321e0a9fd6d58562ae80 in / 
# Wed, 17 Nov 2021 01:59:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b6e5ca4da96841e58eb27c88695a059e5105fad5a066de803f4b94ae4002ba66`  
		Last Modified: Wed, 17 Nov 2021 02:15:13 GMT  
		Size: 26.6 MB (26573160 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20211115-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:3da6322809c63296f41e7ec624178ad0aafa5d505514b33a892291716f694cd7
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30056521 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bbe83eb8d1d726e8431b21e7dc2e6abcb80a06ceb49f8be7f66f8a9e71e2131d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:15 GMT
ADD file:4203242b2b09a65239092c4780b59181da7b861b3c0be40810b3588aa200f72c in / 
# Wed, 17 Nov 2021 02:40:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:eb9a2845ed124d072b117aba4f0508e00c1ecd0d147dc324d14b00d24092046c`  
		Last Modified: Wed, 17 Nov 2021 02:47:17 GMT  
		Size: 30.1 MB (30056521 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20211115-slim` - linux; 386

```console
$ docker pull debian@sha256:3e3052c4c57f1a775f79b1ae8ae52fbfa4181b152727fa246f761918ae5e19cd
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32380683 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:da9019e0671ab5347f4945a286cd7000bf3e5265aaadbce47f8aa701a38f2ecb`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:39:53 GMT
ADD file:ff018048717c0f5aa1730480d0bfea1c2bf94d6e2dae2d7cef46d05ffb0d93e1 in / 
# Wed, 17 Nov 2021 02:39:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0bae0e1d4cb1d56ffeb9d05de7a097d3157267eef07b6d8131344d99bd97c431`  
		Last Modified: Wed, 17 Nov 2021 02:47:49 GMT  
		Size: 32.4 MB (32380683 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20211115-slim` - linux; mips64le

```console
$ docker pull debian@sha256:dcb16e0376a09dc11c225b5dd0fa774fe6eeff81394d5c978b2989631fab2e94
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.6 MB (29630388 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:db59a85a8dc8d727406d0dd376749ab01e399052396a6fd26b85275ac171959b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:10:13 GMT
ADD file:10bc783d286d0a800f59d5009e87ecb4651659c82cae1325132756c7c8b1dec0 in / 
# Wed, 17 Nov 2021 02:10:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:08bca22fbffb4e58224e9e4f6a56f1ca5099c002b1aeb6181df63af308a3da58`  
		Last Modified: Wed, 17 Nov 2021 02:19:09 GMT  
		Size: 29.6 MB (29630388 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20211115-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:54c3b01fb94774a2593072f7278f315f7473e0c7b6ccf441a215e6e0aa2a8f8b
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **35.3 MB (35271382 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b66d7a689a3b6f026b857d81a3a68ebf63940fa33788c8597f8d755ede3de49b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:28:22 GMT
ADD file:5ed330f0fe1328f694fcaefb961cf4da4d8a4ff03100b21af718b69316168706 in / 
# Wed, 17 Nov 2021 03:28:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:258ff2a13858db8f51b65662e02137c0abcfd2528ca73e92b7a40061d938fb1e`  
		Last Modified: Wed, 17 Nov 2021 03:54:34 GMT  
		Size: 35.3 MB (35271382 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-20211115-slim` - linux; s390x

```console
$ docker pull debian@sha256:17b59bff96c1014cf9a9a101c0fc39f905c2bde2e79c218051bdfa970b2019da
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.7 MB (29650960 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c0c1b8308ceb984c947f45ba0ce176b263cfff066eb3b2419b86fc30c5e0d204`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:27 GMT
ADD file:42f73dac307a6a5232e3100f3d3b36e8125f73a4ca504b106e60e2b66d242b2f in / 
# Wed, 17 Nov 2021 02:42:28 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8d71e885edf612b507bbdcc48a970548d436b8709eeedfc2c98f6843a214b5e2`  
		Last Modified: Wed, 17 Nov 2021 02:48:25 GMT  
		Size: 29.7 MB (29650960 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bullseye-backports`

```console
$ docker pull debian@sha256:8d312115abc63f36333804abf457bb38c27f63f1aaefae3666ea6185b8a891ef
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bullseye-backports` - linux; amd64

```console
$ docker pull debian@sha256:8350438c4551fb45d6d07af8dc528d941e4a73b8bedcce45e0b3f27727c7e48a
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.9 MB (54932999 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2589353678e1cbcf321112690526d5402cddd994873a0f91e1f9008573e76eb8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:20:30 GMT
ADD file:5259fc086e8295ddbf02e48abef38e9bf93a183079d3631aa7a59306b7f2f9df in / 
# Wed, 17 Nov 2021 02:20:31 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:20:34 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:647acf3d48c2780e00cd27bb0984367415f270d78477ef9d5b238e6ebd5290da`  
		Last Modified: Wed, 17 Nov 2021 02:25:17 GMT  
		Size: 54.9 MB (54932774 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:80eca9b7964a1743355cf46926471d01ad57d02a0dff9e9b6880b83044bb299e`  
		Last Modified: Wed, 17 Nov 2021 02:25:34 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:ee099c1e52cb7482f2b689d327b06841f3da705a6e7a4992debbe9745a79b343
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52468133 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cecd6529211c2cb8fed5939084081632ecc2f8450e46f5baeaa38ac38737e595`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:50:02 GMT
ADD file:48c0196bfa5dfd1137285bf9104248d62f15a734133d8df549f35b6f7989ca19 in / 
# Wed, 17 Nov 2021 02:50:03 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:50:19 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:ce39e946c04dab3f8b463d502d71849e8e6d7291ece7e323f0ec9dac1061af26`  
		Last Modified: Wed, 17 Nov 2021 03:05:19 GMT  
		Size: 52.5 MB (52467904 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:887b594cee331bf5136b3fcee568a446ad2d334db748314aa88c0d5115478761`  
		Last Modified: Wed, 17 Nov 2021 03:05:43 GMT  
		Size: 229.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:e957db4d555fdc3eb86f8e8b749b895a1b1e2643e198bd4731ea63535b14b90b
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.1 MB (50134378 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:18e13e5a51bfa3cb0ea7f1f5da62f8dff94648e23e5ffcbe32e8f76771d7c544`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 01:59:07 GMT
ADD file:43b45f65ff4ba720f6f9a107b5f80818d5ea6ebc4835840b11e9a506351da32f in / 
# Wed, 17 Nov 2021 01:59:08 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 01:59:24 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:dbbf55137aab3c7acd1e012a70f1ea1adeaef8d39b474829965d3480a02bf572`  
		Last Modified: Wed, 17 Nov 2021 02:14:27 GMT  
		Size: 50.1 MB (50134152 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:7d9e428fdd0c08de2640f62c0f97e2c0e6b0ca158776b1827dd6d8027dbed114`  
		Last Modified: Wed, 17 Nov 2021 02:14:51 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:3a8d9af8333e83826eb18c646dcfa9d4f1fec3dab122b099fcb7cd972b683976
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53619250 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:be8b0682757566bfa1dfe014d98b27f436ef8569fde461f45066822ecb5c86b3`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:39:59 GMT
ADD file:eb667e18226da8a1ca5a41beacddc65cd1efec01c3d01e50845c5e89e91ea17c in / 
# Wed, 17 Nov 2021 02:40:00 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:40:07 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:ce18cd0ba11c2dfa8c1a2d2b325e1e019b14d4ae25ffdeffb0f9686c5496bf5c`  
		Last Modified: Wed, 17 Nov 2021 02:46:45 GMT  
		Size: 53.6 MB (53619024 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:2eb6423c474eee9c8b503cb8e03df9c3ff5413232ce19f719c10ea2f7ef4efcb`  
		Last Modified: Wed, 17 Nov 2021 02:47:05 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; 386

```console
$ docker pull debian@sha256:23b79124d800145cbe0e6b36770ea9ecc9603fd8054a221062b1b75c56d29f59
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.9 MB (55937812 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f5d7829ff0e20953100e39fe1b235e5d646b32eb0067bb2e7957ae08fd5d7167`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:39:36 GMT
ADD file:b8f9021573a53ec69fa566a396d9a8c68392bd6d659a665c0aea8fbd00164f12 in / 
# Wed, 17 Nov 2021 02:39:37 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:39:44 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:8d649f3a0e8410b121648ccc552acdac0960691a4da33ff4db9825f9b51ff3f4`  
		Last Modified: Wed, 17 Nov 2021 02:47:12 GMT  
		Size: 55.9 MB (55937586 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:c5e456f6e9727774f8218282b1edae9d69130820f39dfb0a375fb52a59260fed`  
		Last Modified: Wed, 17 Nov 2021 02:47:34 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; mips64le

```console
$ docker pull debian@sha256:ce23e59d80507bf0cf6cbc033a00e89d6b0ff962c29681a897a58a527fc757a9
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53184034 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:47991a7daa6d994337c8b9ad33f8963ae32917653605014a2aef3fda1e531aca`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:09:46 GMT
ADD file:2971df49f24648b8d5166d87e392e2547d5b03469356f20d112ce6feb3c329fc in / 
# Wed, 17 Nov 2021 02:09:47 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:09:54 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:b095a0b63d99a4c4cd9c1c353f381a69ef041e6f80798d807ae51a34b04920a0`  
		Last Modified: Wed, 17 Nov 2021 02:18:22 GMT  
		Size: 53.2 MB (53183806 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:92b9277a0cbe5e8322155dd006220ee30deec9b1b7209a682bf52db18282e887`  
		Last Modified: Wed, 17 Nov 2021 02:18:40 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; ppc64le

```console
$ docker pull debian@sha256:dc5bd3960c610318d5ed435bd1652bce56065f6e66fa6598f9db710cf4fd16b9
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.8 MB (58819733 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:425eb4520a7d1ec9dc7e77c095c26c51396bc7ec6d20542135bd8b385e9bffa4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:27:04 GMT
ADD file:85375acb19da58f9eabaa67f8cb425cd1ff2dadb2888bbd9dbfa6223d16fef23 in / 
# Wed, 17 Nov 2021 03:27:20 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 03:27:56 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:ede9cd7c8f1d380c971a130fb3d8a3914bf3a9106702164c23b2cce536fa618c`  
		Last Modified: Wed, 17 Nov 2021 03:52:39 GMT  
		Size: 58.8 MB (58819505 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:db37e9eb2f72ebe52d30bc2bd731d6081d570c5a021ad20f2bc558d789078a60`  
		Last Modified: Wed, 17 Nov 2021 03:53:04 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-backports` - linux; s390x

```console
$ docker pull debian@sha256:e93a27f13cc196aaf71d6c9214ab1b44fdf00598bcf436267b61bba3ae525871
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53207405 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0ccfcf32dcf1e4d5c1c23a51b0179888a1674ddbf1d123c9f43f6477fa17d37c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:07 GMT
ADD file:b7524e775a2033d246d1096765a0eb6eaa5ba6ed6c8b395522579d2c4e7a2e38 in / 
# Wed, 17 Nov 2021 02:42:09 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:42:18 GMT
RUN echo 'deb http://deb.debian.org/debian bullseye-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:ffa47acd07831fbc96680a705da20704bac0016c20dc3c454ecd45cda44b368d`  
		Last Modified: Wed, 17 Nov 2021 02:48:05 GMT  
		Size: 53.2 MB (53207179 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:9a435f0d5379ec0cf1aa220b8bd1d996c89718af0408de28ddc2e7ac478b3c7d`  
		Last Modified: Wed, 17 Nov 2021 02:48:17 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:bullseye-slim`

```console
$ docker pull debian@sha256:a23887a2e830b815955e010f30d4c2430cd5ef82e93c130471024bc9f808d5d3
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:bullseye-slim` - linux; amd64

```console
$ docker pull debian@sha256:832234a41bd12f5bcbed92f5ada9af6e2e2b3a394b2353f072b4ad413a3b3320
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.4 MB (31370267 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:66b2aecdb9f0f055837a85eca27f1c790e78d935b21bd000559f57e04f75e06b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:20:41 GMT
ADD file:a2405ebb9892d98be2eb585f6121864d12b3fd983ebf15e5f0b7486e106a79c6 in / 
# Wed, 17 Nov 2021 02:20:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:eff15d958d664f0874d16aee393cc44387031ee0a68ef8542d0056c747f378e8`  
		Last Modified: Wed, 17 Nov 2021 02:25:45 GMT  
		Size: 31.4 MB (31370267 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:101787af86135c7b1346049404e63165bf65a9a2f1334f54571374d80f0ecac9
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **28.9 MB (28911006 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:968cb14e039277aaef3a19da601adf10cc0125e61520ebbbbffb29ad6fca6fef`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:50:37 GMT
ADD file:738a04a17bdb9077594ad9a847333abe28216a7f04d3058718a5e21c236c24bb in / 
# Wed, 17 Nov 2021 02:50:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a960a56baa1baffbe2aa1e0c1fb02f9ee4816337d02fec259b312c409d77fafc`  
		Last Modified: Wed, 17 Nov 2021 03:06:09 GMT  
		Size: 28.9 MB (28911006 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:e2917d74741b83dac5e2e1cac59b8c79115aa1334b908bf020e9605f46a2fb77
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.6 MB (26573160 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:eaa955c4e58c7dc8e698c7e54814ed361ee80aad303a67c39606d6c4fa849f37`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 01:59:43 GMT
ADD file:cd2ac52107a2ea6657f23850a4b29366309eb39fa177321e0a9fd6d58562ae80 in / 
# Wed, 17 Nov 2021 01:59:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b6e5ca4da96841e58eb27c88695a059e5105fad5a066de803f4b94ae4002ba66`  
		Last Modified: Wed, 17 Nov 2021 02:15:13 GMT  
		Size: 26.6 MB (26573160 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:3da6322809c63296f41e7ec624178ad0aafa5d505514b33a892291716f694cd7
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30056521 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bbe83eb8d1d726e8431b21e7dc2e6abcb80a06ceb49f8be7f66f8a9e71e2131d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:15 GMT
ADD file:4203242b2b09a65239092c4780b59181da7b861b3c0be40810b3588aa200f72c in / 
# Wed, 17 Nov 2021 02:40:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:eb9a2845ed124d072b117aba4f0508e00c1ecd0d147dc324d14b00d24092046c`  
		Last Modified: Wed, 17 Nov 2021 02:47:17 GMT  
		Size: 30.1 MB (30056521 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; 386

```console
$ docker pull debian@sha256:3e3052c4c57f1a775f79b1ae8ae52fbfa4181b152727fa246f761918ae5e19cd
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32380683 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:da9019e0671ab5347f4945a286cd7000bf3e5265aaadbce47f8aa701a38f2ecb`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:39:53 GMT
ADD file:ff018048717c0f5aa1730480d0bfea1c2bf94d6e2dae2d7cef46d05ffb0d93e1 in / 
# Wed, 17 Nov 2021 02:39:53 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0bae0e1d4cb1d56ffeb9d05de7a097d3157267eef07b6d8131344d99bd97c431`  
		Last Modified: Wed, 17 Nov 2021 02:47:49 GMT  
		Size: 32.4 MB (32380683 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; mips64le

```console
$ docker pull debian@sha256:dcb16e0376a09dc11c225b5dd0fa774fe6eeff81394d5c978b2989631fab2e94
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.6 MB (29630388 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:db59a85a8dc8d727406d0dd376749ab01e399052396a6fd26b85275ac171959b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:10:13 GMT
ADD file:10bc783d286d0a800f59d5009e87ecb4651659c82cae1325132756c7c8b1dec0 in / 
# Wed, 17 Nov 2021 02:10:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:08bca22fbffb4e58224e9e4f6a56f1ca5099c002b1aeb6181df63af308a3da58`  
		Last Modified: Wed, 17 Nov 2021 02:19:09 GMT  
		Size: 29.6 MB (29630388 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:54c3b01fb94774a2593072f7278f315f7473e0c7b6ccf441a215e6e0aa2a8f8b
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **35.3 MB (35271382 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b66d7a689a3b6f026b857d81a3a68ebf63940fa33788c8597f8d755ede3de49b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:28:22 GMT
ADD file:5ed330f0fe1328f694fcaefb961cf4da4d8a4ff03100b21af718b69316168706 in / 
# Wed, 17 Nov 2021 03:28:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:258ff2a13858db8f51b65662e02137c0abcfd2528ca73e92b7a40061d938fb1e`  
		Last Modified: Wed, 17 Nov 2021 03:54:34 GMT  
		Size: 35.3 MB (35271382 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:bullseye-slim` - linux; s390x

```console
$ docker pull debian@sha256:17b59bff96c1014cf9a9a101c0fc39f905c2bde2e79c218051bdfa970b2019da
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.7 MB (29650960 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c0c1b8308ceb984c947f45ba0ce176b263cfff066eb3b2419b86fc30c5e0d204`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:27 GMT
ADD file:42f73dac307a6a5232e3100f3d3b36e8125f73a4ca504b106e60e2b66d242b2f in / 
# Wed, 17 Nov 2021 02:42:28 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8d71e885edf612b507bbdcc48a970548d436b8709eeedfc2c98f6843a214b5e2`  
		Last Modified: Wed, 17 Nov 2021 02:48:25 GMT  
		Size: 29.7 MB (29650960 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:buster`

```console
$ docker pull debian@sha256:e5333f8697a86fa1be53b2a7e994247083f61885166df0cdda9f812acb514d7c
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:buster` - linux; amd64

```console
$ docker pull debian@sha256:89c36409980d71271f8b46e327cda450ede47542d182671145e92773b752e153
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50437098 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4906f6b1e4402ac10252e2231ea0cb36dd72b494e8d1505bfa23ad79cabb950d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:20:51 GMT
ADD file:cb5ed7070880d4c0177fbe6dd278adb7926e38cd73e6abd582fd8d67e4bbf06c in / 
# Wed, 17 Nov 2021 02:20:51 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1230f17f526cb07307d41462355b10488b994e8bdafe0d8f275a405fa3b33831`  
		Last Modified: Wed, 17 Nov 2021 02:26:06 GMT  
		Size: 50.4 MB (50437098 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster` - linux; arm variant v5

```console
$ docker pull debian@sha256:600c5fc194e46731b51dec790839ccc9af45cd37a1982eb289924e66339dce85
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **48.2 MB (48154304 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bbd8f738f959a12ae79abc575d02024dd757f535f87330a6bdd422cfa32b3a61`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:51:05 GMT
ADD file:b19cf756330e0e1577062a56604b50597d35a6f4be67ac86210ddf4b4203a32d in / 
# Wed, 17 Nov 2021 02:51:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:141c2db3ba99259b105dd1f16b4f6365959cb21d4978470a6bd73ef86ef15d08`  
		Last Modified: Wed, 17 Nov 2021 03:06:59 GMT  
		Size: 48.2 MB (48154304 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster` - linux; arm variant v7

```console
$ docker pull debian@sha256:8b0544ec886aacd2cc8ebd70695d01bb7b292e899da3e3d5ce4a5f174a29e4d2
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45918099 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6f800c08efc4810532acfdaade1f1edb9a0a724ea504e9e3972c7a770f218d74`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:00:12 GMT
ADD file:d71807613acdf86685f6d640a90e27b7b63cfe0f13d88668ee943aca089d8876 in / 
# Wed, 17 Nov 2021 02:00:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:10acd81022d17f452439d0385615fa066b8f0ddcf3c1a803872eec5bdc6acf64`  
		Last Modified: Wed, 17 Nov 2021 02:16:00 GMT  
		Size: 45.9 MB (45918099 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:a90e90c601b9358c34f18a864c02c4f9e8527cc4f79c2f5d3779e8dd11fd5d53
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49222990 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d5af59919bab386fa40f9282adc3769cbfb687d3cf86570a8bd827d8c9ceafce`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:27 GMT
ADD file:c7e9e884b1494f96e33e9d9a892b51367af8943edb282c4101cc99ea78a7753f in / 
# Wed, 17 Nov 2021 02:40:28 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:626979e62c56e192812e818d52b82f063d9f9514b1b7bae14e534e4a7c98117a`  
		Last Modified: Wed, 17 Nov 2021 02:47:40 GMT  
		Size: 49.2 MB (49222990 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster` - linux; 386

```console
$ docker pull debian@sha256:b840656acb560ba0d10da515368d47f4dc1bb8ffa8df678653d6959063d24c97
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51207728 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3e751f1d934e26435fca29c8eb20e64100daf4f0ccf0b77f4482411ceabbb5b6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:06 GMT
ADD file:87367b4870b0dc997a37b7648ff6efa8e42bed68b0573be2ade992c206fbdd2e in / 
# Wed, 17 Nov 2021 02:40:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1f740e90292e8fcb22dbbbdbe335252fd87d406cd6e139738853fc65c8bb54f4`  
		Last Modified: Wed, 17 Nov 2021 02:48:17 GMT  
		Size: 51.2 MB (51207728 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster` - linux; mips64le

```console
$ docker pull debian@sha256:56bc08b7eb6db843bc54606a37cfa7a302e8059430b0cf0b62005893283a7b84
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.1 MB (49079535 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:598bee8f0f0e68db53acfa1bd428cb335f6c8d94cf370bcd6a7d74bc465ac010`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:10:39 GMT
ADD file:4769a173ec92e69a1b5cd79af0ed018bf150d1ee278de07f4bdab45f55f3c818 in / 
# Wed, 17 Nov 2021 02:10:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8c028c43f55209ca75650433b923c8952e16d106529242daad16c3be3efd66a3`  
		Last Modified: Wed, 17 Nov 2021 02:19:53 GMT  
		Size: 49.1 MB (49079535 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster` - linux; ppc64le

```console
$ docker pull debian@sha256:180ef8c7873776990cfe1d853cbeced32438e7e87dbe079be541ba8bd2fd6ca5
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.2 MB (54183702 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:27d2b2aa100535ac6ebeae5978be19d3f1732010b93251d59a6a4a7cb159f6c4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:29:10 GMT
ADD file:fc0d43ba74311deef20b2131cca3a8e86b083624d63560251191adca21417f2f in / 
# Wed, 17 Nov 2021 03:29:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:aad6e1fe6f7c56b0e1b5850f40e4fec91ffaed4b6235e730f5c7ff43db397d3d`  
		Last Modified: Wed, 17 Nov 2021 03:56:39 GMT  
		Size: 54.2 MB (54183702 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster` - linux; s390x

```console
$ docker pull debian@sha256:f4bb73391d3779dfe30a3ab72aa444bff7164c2326ab80f55302e8b584463ac5
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.0 MB (49005439 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:20b18aa76fa9a01d5ef11e20329da3e4097e7771eb7310578351dfdbc8bae229`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:40 GMT
ADD file:dc4ee96f81059accdc88a686f80f1714bc9d2f383af71c0d130c08560cea115e in / 
# Wed, 17 Nov 2021 02:42:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c12eae84d2674466d924a139a276e361afa9f63de21cf77eae19bd94eaac2629`  
		Last Modified: Wed, 17 Nov 2021 02:48:42 GMT  
		Size: 49.0 MB (49005439 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:buster-20211115`

```console
$ docker pull debian@sha256:e5333f8697a86fa1be53b2a7e994247083f61885166df0cdda9f812acb514d7c
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:buster-20211115` - linux; amd64

```console
$ docker pull debian@sha256:89c36409980d71271f8b46e327cda450ede47542d182671145e92773b752e153
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50437098 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4906f6b1e4402ac10252e2231ea0cb36dd72b494e8d1505bfa23ad79cabb950d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:20:51 GMT
ADD file:cb5ed7070880d4c0177fbe6dd278adb7926e38cd73e6abd582fd8d67e4bbf06c in / 
# Wed, 17 Nov 2021 02:20:51 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1230f17f526cb07307d41462355b10488b994e8bdafe0d8f275a405fa3b33831`  
		Last Modified: Wed, 17 Nov 2021 02:26:06 GMT  
		Size: 50.4 MB (50437098 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20211115` - linux; arm variant v5

```console
$ docker pull debian@sha256:600c5fc194e46731b51dec790839ccc9af45cd37a1982eb289924e66339dce85
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **48.2 MB (48154304 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bbd8f738f959a12ae79abc575d02024dd757f535f87330a6bdd422cfa32b3a61`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:51:05 GMT
ADD file:b19cf756330e0e1577062a56604b50597d35a6f4be67ac86210ddf4b4203a32d in / 
# Wed, 17 Nov 2021 02:51:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:141c2db3ba99259b105dd1f16b4f6365959cb21d4978470a6bd73ef86ef15d08`  
		Last Modified: Wed, 17 Nov 2021 03:06:59 GMT  
		Size: 48.2 MB (48154304 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20211115` - linux; arm variant v7

```console
$ docker pull debian@sha256:8b0544ec886aacd2cc8ebd70695d01bb7b292e899da3e3d5ce4a5f174a29e4d2
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45918099 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6f800c08efc4810532acfdaade1f1edb9a0a724ea504e9e3972c7a770f218d74`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:00:12 GMT
ADD file:d71807613acdf86685f6d640a90e27b7b63cfe0f13d88668ee943aca089d8876 in / 
# Wed, 17 Nov 2021 02:00:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:10acd81022d17f452439d0385615fa066b8f0ddcf3c1a803872eec5bdc6acf64`  
		Last Modified: Wed, 17 Nov 2021 02:16:00 GMT  
		Size: 45.9 MB (45918099 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20211115` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:a90e90c601b9358c34f18a864c02c4f9e8527cc4f79c2f5d3779e8dd11fd5d53
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49222990 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d5af59919bab386fa40f9282adc3769cbfb687d3cf86570a8bd827d8c9ceafce`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:27 GMT
ADD file:c7e9e884b1494f96e33e9d9a892b51367af8943edb282c4101cc99ea78a7753f in / 
# Wed, 17 Nov 2021 02:40:28 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:626979e62c56e192812e818d52b82f063d9f9514b1b7bae14e534e4a7c98117a`  
		Last Modified: Wed, 17 Nov 2021 02:47:40 GMT  
		Size: 49.2 MB (49222990 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20211115` - linux; 386

```console
$ docker pull debian@sha256:b840656acb560ba0d10da515368d47f4dc1bb8ffa8df678653d6959063d24c97
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51207728 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3e751f1d934e26435fca29c8eb20e64100daf4f0ccf0b77f4482411ceabbb5b6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:06 GMT
ADD file:87367b4870b0dc997a37b7648ff6efa8e42bed68b0573be2ade992c206fbdd2e in / 
# Wed, 17 Nov 2021 02:40:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1f740e90292e8fcb22dbbbdbe335252fd87d406cd6e139738853fc65c8bb54f4`  
		Last Modified: Wed, 17 Nov 2021 02:48:17 GMT  
		Size: 51.2 MB (51207728 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20211115` - linux; mips64le

```console
$ docker pull debian@sha256:56bc08b7eb6db843bc54606a37cfa7a302e8059430b0cf0b62005893283a7b84
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.1 MB (49079535 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:598bee8f0f0e68db53acfa1bd428cb335f6c8d94cf370bcd6a7d74bc465ac010`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:10:39 GMT
ADD file:4769a173ec92e69a1b5cd79af0ed018bf150d1ee278de07f4bdab45f55f3c818 in / 
# Wed, 17 Nov 2021 02:10:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8c028c43f55209ca75650433b923c8952e16d106529242daad16c3be3efd66a3`  
		Last Modified: Wed, 17 Nov 2021 02:19:53 GMT  
		Size: 49.1 MB (49079535 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20211115` - linux; ppc64le

```console
$ docker pull debian@sha256:180ef8c7873776990cfe1d853cbeced32438e7e87dbe079be541ba8bd2fd6ca5
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.2 MB (54183702 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:27d2b2aa100535ac6ebeae5978be19d3f1732010b93251d59a6a4a7cb159f6c4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:29:10 GMT
ADD file:fc0d43ba74311deef20b2131cca3a8e86b083624d63560251191adca21417f2f in / 
# Wed, 17 Nov 2021 03:29:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:aad6e1fe6f7c56b0e1b5850f40e4fec91ffaed4b6235e730f5c7ff43db397d3d`  
		Last Modified: Wed, 17 Nov 2021 03:56:39 GMT  
		Size: 54.2 MB (54183702 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20211115` - linux; s390x

```console
$ docker pull debian@sha256:f4bb73391d3779dfe30a3ab72aa444bff7164c2326ab80f55302e8b584463ac5
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.0 MB (49005439 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:20b18aa76fa9a01d5ef11e20329da3e4097e7771eb7310578351dfdbc8bae229`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:40 GMT
ADD file:dc4ee96f81059accdc88a686f80f1714bc9d2f383af71c0d130c08560cea115e in / 
# Wed, 17 Nov 2021 02:42:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c12eae84d2674466d924a139a276e361afa9f63de21cf77eae19bd94eaac2629`  
		Last Modified: Wed, 17 Nov 2021 02:48:42 GMT  
		Size: 49.0 MB (49005439 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:buster-20211115-slim`

```console
$ docker pull debian@sha256:cfadaa70a452076a478dcfcf2da3a3dc3b741450afed5ab67a7f3df161387c48
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:buster-20211115-slim` - linux; amd64

```console
$ docker pull debian@sha256:69b11eb7768d4783408354087ff1719d875a9f218e4ac76b39e9c35792048968
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.2 MB (27153675 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cad9ce16f840672d1615867950084a0de9b7022ec6fff89e05bde0d32632e281`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:21:02 GMT
ADD file:3c54ad257f2e04f7294ce879b884820cf4726c8e93ec548172825963e40c79ad in / 
# Wed, 17 Nov 2021 02:21:02 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a10c77af261312e7c92bcc184f2d1726175ff7f142e44b01c5779cd79348b9fd`  
		Last Modified: Wed, 17 Nov 2021 02:26:31 GMT  
		Size: 27.2 MB (27153675 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20211115-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:9ad9871d1ab8f29c2d32b7c1d807f8106ac173edf4c682c8cdc071db124694e7
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.9 MB (24886310 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d4cbba96ee571035afdcd4a54b8250797669f76c63804c5eb78647e94cddc985`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:51:39 GMT
ADD file:e580d4d2066301375327ea51dd8db3af956e5a465495bf09d69d6deec9b0bfae in / 
# Wed, 17 Nov 2021 02:51:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:dba255a4b166bb02562ca152d0da236ba8f211de5bf9d79e35ee023b1fce203e`  
		Last Modified: Wed, 17 Nov 2021 03:07:41 GMT  
		Size: 24.9 MB (24886310 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20211115-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:d83f190bbdc742a155dff8a752dc4e389ec05363062e69be33454db1d612e436
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.8 MB (22754359 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6b3c02d80c3d132723646efd4136d3e6605d1a8fb1b6c14cbfa40b122b22d709`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:00:46 GMT
ADD file:4ac0de0d740f0c221cd4f912861a67009942fa5bdad24ac8b62c690dfa15024a in / 
# Wed, 17 Nov 2021 02:00:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7bd9e64406c6f3044bd15f89c367f07ec4eaafb1035a5c3ee2f7b138be68017f`  
		Last Modified: Wed, 17 Nov 2021 02:16:39 GMT  
		Size: 22.8 MB (22754359 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20211115-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:fc8026d13e90bef9a4948392328cf18b4b32481cdb64407e98231956c69155af
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.9 MB (25923117 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:84512f753d36bb945369edc0a0f47a63b00207d5a208af4560367cc8b1bc02f4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:44 GMT
ADD file:b7921dd77c7620d46a18a4b5952557620210bf7ad9de20fe8e695a371188122a in / 
# Wed, 17 Nov 2021 02:40:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8ccb9871ed7bebcea8889b08e06fe5bd0116711ca7b110429b987475bf4d40e8`  
		Last Modified: Wed, 17 Nov 2021 02:48:07 GMT  
		Size: 25.9 MB (25923117 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20211115-slim` - linux; 386

```console
$ docker pull debian@sha256:359cbc872ec80684e26c21b07f099397f1ad3726c0f3133aaa842df78bb75fc1
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.8 MB (27804550 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:33a21f76255c18fc2a4df80a31457b07da2bfa9ee0653811a0f1543bdd62b70a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:22 GMT
ADD file:2adea3474b7ebda2bf77361304ee3e6966a9118aafd8220b81c4027be1f4d583 in / 
# Wed, 17 Nov 2021 02:40:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7af5c9e671306e395c50a515c7bbe33d6f418ea08535b01c884457f78114eb08`  
		Last Modified: Wed, 17 Nov 2021 02:48:49 GMT  
		Size: 27.8 MB (27804550 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20211115-slim` - linux; mips64le

```console
$ docker pull debian@sha256:8430b6dd5211b03cfecd7e948b66a06104216212514aae3b514533d1c26093c8
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.8 MB (25820129 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2998851c9df1a3aa7d369ccbcedf64a341c3f7a0024903670cbab29e2cb145b9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:11:05 GMT
ADD file:442e2528f8dc5583975e61c403642ead88c26af80e5769940d11d2420eb6e56d in / 
# Wed, 17 Nov 2021 02:11:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f95d3989b23de40aa9c78765869cf0e26ca2ed018dd048613fbc1f9df33d1bc1`  
		Last Modified: Wed, 17 Nov 2021 02:20:33 GMT  
		Size: 25.8 MB (25820129 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20211115-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:56bcd470fc295d3603cea9d4a9d0de2453a6aaa07350a972ee9828cb20096eed
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.6 MB (30562278 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1268eb8161a3e00c3c709fae3d5b9373630dd956aca82fe343f2d60eb009faad`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:30:22 GMT
ADD file:c0f8b5d4d419bef7a494df5330ecf7bbea3cbb6462308ca0b2f26771f125dea0 in / 
# Wed, 17 Nov 2021 03:30:29 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:459de0a111d5c931674a5150ef4ae6bb1ffb1685380f4aafdba04a37d556c5de`  
		Last Modified: Wed, 17 Nov 2021 03:58:14 GMT  
		Size: 30.6 MB (30562278 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-20211115-slim` - linux; s390x

```console
$ docker pull debian@sha256:a546bb460638bdf3a08744b537a3a3bfbaaabe14738f84692c4fe87622f5fb3f
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.8 MB (25769063 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9ade07e7330ffcfe24ae9e162a0b36b4f63979d4ca3a0e2407118c473562fcf8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:59 GMT
ADD file:3ff5fbc9c53cf76aad0fe0e1a525cb0bc0d70e8f2a3d5741dfb8b3b8a9448ffd in / 
# Wed, 17 Nov 2021 02:43:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b8b917e61b3be882db10e68210f73caf5cc788bfb07aec0a659e30f4796b476b`  
		Last Modified: Wed, 17 Nov 2021 02:49:01 GMT  
		Size: 25.8 MB (25769063 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:buster-backports`

```console
$ docker pull debian@sha256:f04c4cfcc192382273af1c1423ebba9d5b436face7e47b5ff5b1497529e56e1e
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:buster-backports` - linux; amd64

```console
$ docker pull debian@sha256:fa16640823e33a5c94dab6bcb16bcc8feb37f7e8a071d4dbfd4f3a02c1458ca4
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50437320 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1f389fbfe39589cb28362455dc98388cb5f7e6bb95664a51ced670e3c3ae6e4a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:20:51 GMT
ADD file:cb5ed7070880d4c0177fbe6dd278adb7926e38cd73e6abd582fd8d67e4bbf06c in / 
# Wed, 17 Nov 2021 02:20:51 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:20:55 GMT
RUN echo 'deb http://deb.debian.org/debian buster-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:1230f17f526cb07307d41462355b10488b994e8bdafe0d8f275a405fa3b33831`  
		Last Modified: Wed, 17 Nov 2021 02:26:06 GMT  
		Size: 50.4 MB (50437098 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:ea1d365fccf5329e62283deb47a8dcd67f598107ef33e7137895a33336c9aa65`  
		Last Modified: Wed, 17 Nov 2021 02:26:21 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:d03268d487ecb693d45bee3650c7ae38183a7379ca52b9f49c1a57a1e0563e6f
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **48.2 MB (48154528 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:77ca30cb718fc7b52f867676df720ccb9c63aebc61ee616a2a75c80973b41606`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:51:05 GMT
ADD file:b19cf756330e0e1577062a56604b50597d35a6f4be67ac86210ddf4b4203a32d in / 
# Wed, 17 Nov 2021 02:51:07 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:51:22 GMT
RUN echo 'deb http://deb.debian.org/debian buster-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:141c2db3ba99259b105dd1f16b4f6365959cb21d4978470a6bd73ef86ef15d08`  
		Last Modified: Wed, 17 Nov 2021 03:06:59 GMT  
		Size: 48.2 MB (48154304 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:e48905284f0154fbb5a1f034b007920c484418c303eb8c461716059483f5e845`  
		Last Modified: Wed, 17 Nov 2021 03:07:18 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:857ec3112731ba234add6acc7c99f654b6205a977254c7c28a72811a11f1ae53
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45918323 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a7991117c6a2ec2a3d736bf37515e8f51d193068bf33c3127a13c3d33b14935f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:00:12 GMT
ADD file:d71807613acdf86685f6d640a90e27b7b63cfe0f13d88668ee943aca089d8876 in / 
# Wed, 17 Nov 2021 02:00:13 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:00:28 GMT
RUN echo 'deb http://deb.debian.org/debian buster-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:10acd81022d17f452439d0385615fa066b8f0ddcf3c1a803872eec5bdc6acf64`  
		Last Modified: Wed, 17 Nov 2021 02:16:00 GMT  
		Size: 45.9 MB (45918099 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:8e1f5cd20cbfac21a12a2c5f85395f19795e1327297e1268d381edbdfeb0b85b`  
		Last Modified: Wed, 17 Nov 2021 02:16:20 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:cf9b0e4cd6b2913a0109c7ed39a745b98beb1fff912fdd7aac097757457a3532
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49223214 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:095bcebb1970bf5d42d757ee6fe38d02a5533095370387311f0973edfac474eb`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:27 GMT
ADD file:c7e9e884b1494f96e33e9d9a892b51367af8943edb282c4101cc99ea78a7753f in / 
# Wed, 17 Nov 2021 02:40:28 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:40:35 GMT
RUN echo 'deb http://deb.debian.org/debian buster-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:626979e62c56e192812e818d52b82f063d9f9514b1b7bae14e534e4a7c98117a`  
		Last Modified: Wed, 17 Nov 2021 02:47:40 GMT  
		Size: 49.2 MB (49222990 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:76a142dd68d6b234935186e085ee6ca4d3990d58e29f3bc89f301eb21a3fcf7f`  
		Last Modified: Wed, 17 Nov 2021 02:47:56 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-backports` - linux; 386

```console
$ docker pull debian@sha256:ddc3ba1a187102ae2de6fe8631ed145202973d548100aee1b3505b862a8e4096
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51207952 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:41b940a276ee4669549fd6fc613c98e582e47fc5230b71b9e59028da40afd13e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:06 GMT
ADD file:87367b4870b0dc997a37b7648ff6efa8e42bed68b0573be2ade992c206fbdd2e in / 
# Wed, 17 Nov 2021 02:40:07 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:40:13 GMT
RUN echo 'deb http://deb.debian.org/debian buster-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:1f740e90292e8fcb22dbbbdbe335252fd87d406cd6e139738853fc65c8bb54f4`  
		Last Modified: Wed, 17 Nov 2021 02:48:17 GMT  
		Size: 51.2 MB (51207728 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:0805511207a5413b43b826b427a7a9376941b9e9bd25d957367e1c80a15a5c8c`  
		Last Modified: Wed, 17 Nov 2021 02:48:35 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-backports` - linux; mips64le

```console
$ docker pull debian@sha256:90ebd3461ed035c3af383d20f63edda9a8d0f86224eead89f58aa534d9d6d851
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.1 MB (49079759 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2b76fb87ab66c0e20a099accdbfbad84aa8548511fd5bce253b6b33baef6b538`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:10:39 GMT
ADD file:4769a173ec92e69a1b5cd79af0ed018bf150d1ee278de07f4bdab45f55f3c818 in / 
# Wed, 17 Nov 2021 02:10:40 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:10:48 GMT
RUN echo 'deb http://deb.debian.org/debian buster-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:8c028c43f55209ca75650433b923c8952e16d106529242daad16c3be3efd66a3`  
		Last Modified: Wed, 17 Nov 2021 02:19:53 GMT  
		Size: 49.1 MB (49079535 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:b3ca2ad13dd9b55f9a63dd96976f8aca10242a05f16caea721f697193081df3c`  
		Last Modified: Wed, 17 Nov 2021 02:20:08 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-backports` - linux; ppc64le

```console
$ docker pull debian@sha256:057ded4168f1f20a3ff9f8f58ad3feb04e228fd1898f274419413438afa30820
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.2 MB (54183926 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:16b7aa843f820aa3220b23a0cd2fe8856a0d80670169ffa62b18a0f59277a27f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:29:10 GMT
ADD file:fc0d43ba74311deef20b2131cca3a8e86b083624d63560251191adca21417f2f in / 
# Wed, 17 Nov 2021 03:29:20 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 03:29:57 GMT
RUN echo 'deb http://deb.debian.org/debian buster-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:aad6e1fe6f7c56b0e1b5850f40e4fec91ffaed4b6235e730f5c7ff43db397d3d`  
		Last Modified: Wed, 17 Nov 2021 03:56:39 GMT  
		Size: 54.2 MB (54183702 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:59c43e04afb43b19bebd093168c4e86b8f5fccc39ae6edbff703fd549cec5f51`  
		Last Modified: Wed, 17 Nov 2021 03:56:57 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-backports` - linux; s390x

```console
$ docker pull debian@sha256:e46c35ef6233ba45fe3a010267d324dfe9506e77af4d1fda04b8ed26402980ea
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.0 MB (49005661 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3e5e8c4151155c6da97939d4334448b0033d6949e8801994a6269bef5b15b0c2`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:40 GMT
ADD file:dc4ee96f81059accdc88a686f80f1714bc9d2f383af71c0d130c08560cea115e in / 
# Wed, 17 Nov 2021 02:42:42 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:42:50 GMT
RUN echo 'deb http://deb.debian.org/debian buster-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:c12eae84d2674466d924a139a276e361afa9f63de21cf77eae19bd94eaac2629`  
		Last Modified: Wed, 17 Nov 2021 02:48:42 GMT  
		Size: 49.0 MB (49005439 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:b00ebbe55196ab0850ea52c63f7f243f11a7857810569ea40aab682f32e81c2d`  
		Last Modified: Wed, 17 Nov 2021 02:48:54 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:buster-slim`

```console
$ docker pull debian@sha256:cfadaa70a452076a478dcfcf2da3a3dc3b741450afed5ab67a7f3df161387c48
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:buster-slim` - linux; amd64

```console
$ docker pull debian@sha256:69b11eb7768d4783408354087ff1719d875a9f218e4ac76b39e9c35792048968
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.2 MB (27153675 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cad9ce16f840672d1615867950084a0de9b7022ec6fff89e05bde0d32632e281`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:21:02 GMT
ADD file:3c54ad257f2e04f7294ce879b884820cf4726c8e93ec548172825963e40c79ad in / 
# Wed, 17 Nov 2021 02:21:02 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a10c77af261312e7c92bcc184f2d1726175ff7f142e44b01c5779cd79348b9fd`  
		Last Modified: Wed, 17 Nov 2021 02:26:31 GMT  
		Size: 27.2 MB (27153675 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:9ad9871d1ab8f29c2d32b7c1d807f8106ac173edf4c682c8cdc071db124694e7
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.9 MB (24886310 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d4cbba96ee571035afdcd4a54b8250797669f76c63804c5eb78647e94cddc985`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:51:39 GMT
ADD file:e580d4d2066301375327ea51dd8db3af956e5a465495bf09d69d6deec9b0bfae in / 
# Wed, 17 Nov 2021 02:51:40 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:dba255a4b166bb02562ca152d0da236ba8f211de5bf9d79e35ee023b1fce203e`  
		Last Modified: Wed, 17 Nov 2021 03:07:41 GMT  
		Size: 24.9 MB (24886310 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:d83f190bbdc742a155dff8a752dc4e389ec05363062e69be33454db1d612e436
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.8 MB (22754359 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6b3c02d80c3d132723646efd4136d3e6605d1a8fb1b6c14cbfa40b122b22d709`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:00:46 GMT
ADD file:4ac0de0d740f0c221cd4f912861a67009942fa5bdad24ac8b62c690dfa15024a in / 
# Wed, 17 Nov 2021 02:00:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7bd9e64406c6f3044bd15f89c367f07ec4eaafb1035a5c3ee2f7b138be68017f`  
		Last Modified: Wed, 17 Nov 2021 02:16:39 GMT  
		Size: 22.8 MB (22754359 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:fc8026d13e90bef9a4948392328cf18b4b32481cdb64407e98231956c69155af
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.9 MB (25923117 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:84512f753d36bb945369edc0a0f47a63b00207d5a208af4560367cc8b1bc02f4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:44 GMT
ADD file:b7921dd77c7620d46a18a4b5952557620210bf7ad9de20fe8e695a371188122a in / 
# Wed, 17 Nov 2021 02:40:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8ccb9871ed7bebcea8889b08e06fe5bd0116711ca7b110429b987475bf4d40e8`  
		Last Modified: Wed, 17 Nov 2021 02:48:07 GMT  
		Size: 25.9 MB (25923117 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-slim` - linux; 386

```console
$ docker pull debian@sha256:359cbc872ec80684e26c21b07f099397f1ad3726c0f3133aaa842df78bb75fc1
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.8 MB (27804550 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:33a21f76255c18fc2a4df80a31457b07da2bfa9ee0653811a0f1543bdd62b70a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:22 GMT
ADD file:2adea3474b7ebda2bf77361304ee3e6966a9118aafd8220b81c4027be1f4d583 in / 
# Wed, 17 Nov 2021 02:40:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7af5c9e671306e395c50a515c7bbe33d6f418ea08535b01c884457f78114eb08`  
		Last Modified: Wed, 17 Nov 2021 02:48:49 GMT  
		Size: 27.8 MB (27804550 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-slim` - linux; mips64le

```console
$ docker pull debian@sha256:8430b6dd5211b03cfecd7e948b66a06104216212514aae3b514533d1c26093c8
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.8 MB (25820129 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2998851c9df1a3aa7d369ccbcedf64a341c3f7a0024903670cbab29e2cb145b9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:11:05 GMT
ADD file:442e2528f8dc5583975e61c403642ead88c26af80e5769940d11d2420eb6e56d in / 
# Wed, 17 Nov 2021 02:11:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f95d3989b23de40aa9c78765869cf0e26ca2ed018dd048613fbc1f9df33d1bc1`  
		Last Modified: Wed, 17 Nov 2021 02:20:33 GMT  
		Size: 25.8 MB (25820129 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:56bcd470fc295d3603cea9d4a9d0de2453a6aaa07350a972ee9828cb20096eed
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.6 MB (30562278 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1268eb8161a3e00c3c709fae3d5b9373630dd956aca82fe343f2d60eb009faad`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:30:22 GMT
ADD file:c0f8b5d4d419bef7a494df5330ecf7bbea3cbb6462308ca0b2f26771f125dea0 in / 
# Wed, 17 Nov 2021 03:30:29 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:459de0a111d5c931674a5150ef4ae6bb1ffb1685380f4aafdba04a37d556c5de`  
		Last Modified: Wed, 17 Nov 2021 03:58:14 GMT  
		Size: 30.6 MB (30562278 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:buster-slim` - linux; s390x

```console
$ docker pull debian@sha256:a546bb460638bdf3a08744b537a3a3bfbaaabe14738f84692c4fe87622f5fb3f
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.8 MB (25769063 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9ade07e7330ffcfe24ae9e162a0b36b4f63979d4ca3a0e2407118c473562fcf8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:59 GMT
ADD file:3ff5fbc9c53cf76aad0fe0e1a525cb0bc0d70e8f2a3d5741dfb8b3b8a9448ffd in / 
# Wed, 17 Nov 2021 02:43:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b8b917e61b3be882db10e68210f73caf5cc788bfb07aec0a659e30f4796b476b`  
		Last Modified: Wed, 17 Nov 2021 02:49:01 GMT  
		Size: 25.8 MB (25769063 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:experimental`

```console
$ docker pull debian@sha256:5fbaf548cc847fc74caa74b6a9d0c0cc3ff4e6eac863f980080bb1ae3a8c3d1b
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:experimental` - linux; amd64

```console
$ docker pull debian@sha256:c34e1569aca56b9192091930a3ed888130d68a835bcf44eae46b64098be018a2
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.8 MB (55758326 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1155b79982f5cb3cb4ca852f8db318fe9657f4f255df38ab0a63558d2f4e4d51`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:23:08 GMT
ADD file:db09d197fcb0e66f610eb98b792583f6ec40f708cb0d0177e510c2a158f566fa in / 
# Wed, 17 Nov 2021 02:23:09 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:23:19 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:a6543f35db5e93a9d2a1ff8c5abaf275a6bfe30ceb4f008f8047cacfd2d0f324`  
		Last Modified: Wed, 17 Nov 2021 02:30:33 GMT  
		Size: 55.8 MB (55758105 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:510e6744cd6ef82181b5126f01e80d20252c4fd4245ffa53d41ec779ee543d1c`  
		Last Modified: Wed, 17 Nov 2021 02:30:57 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; arm variant v5

```console
$ docker pull debian@sha256:f285fb4cba39f82f3e9edf346f1ebd8f747ab13a01a16053b90799ce9e2f7ddc
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53238960 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8e8987a7b943d00c071d485dd3af35769aa9ddd5c18b898cf56aaefd589346db`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:57:44 GMT
ADD file:4f7ce5598f98e9f01ec7174a57364d9d28802a554530db59c008960d0a40c239 in / 
# Wed, 17 Nov 2021 02:57:45 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:58:18 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:5c59a35f8148e14582e723208c2af72918650b046899af2d952e25ad86cc3dd2`  
		Last Modified: Wed, 17 Nov 2021 03:16:24 GMT  
		Size: 53.2 MB (53238737 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:10c4cb694ca349e0fcee01dce7cc639e0e468f453e3bc939a6128afdb7ab791b`  
		Last Modified: Wed, 17 Nov 2021 03:17:06 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; arm variant v7

```console
$ docker pull debian@sha256:045e674644b35bc4a2bf075445145fe9a410f7274f66d801bea71fb370dbc5c8
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.9 MB (50860530 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:aec3c000773d608dd166eee6d899e220139f9c6f4ebd0fbaedf7ed5e65944cbd`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:07:01 GMT
ADD file:930f169a86ba880df2aaea12f3e8598f194b73f18795a8cd70efcd27b241bda3 in / 
# Wed, 17 Nov 2021 02:07:02 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:07:35 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:61a45c58489b0c8e11c36c2b4e975b6bd6dd5cd0b16c91ccb452966e84c75021`  
		Last Modified: Wed, 17 Nov 2021 02:24:50 GMT  
		Size: 50.9 MB (50860307 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:8647d476eb9176d77a596dd5cef2be97e3386236a4e2f12e22d27fc8742d717a`  
		Last Modified: Wed, 17 Nov 2021 02:25:31 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:6460b222d57825e8dbba8071810b435418e8459e2fc5a4a9096ebb47a83c1874
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.8 MB (54767296 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6157a41c40dfece8c9e8f4aeccfec249a4c19835344d6ce672af84d61aeae663`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:43:27 GMT
ADD file:44b7578caffebde5ddfd201775779df99cb1d525113ba58609f8cdf1f47092ce in / 
# Wed, 17 Nov 2021 02:43:27 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:43:43 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:af0aea1ea198f6a1207b9812f667f234e3ccf829e7b41959a314d3616636bc52`  
		Last Modified: Wed, 17 Nov 2021 02:52:44 GMT  
		Size: 54.8 MB (54767075 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:419ceac417071c25631219677cdd697c6d7d12c3b7071efe3130a7a7bd6910b8`  
		Last Modified: Wed, 17 Nov 2021 02:53:10 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; 386

```console
$ docker pull debian@sha256:67e28b400930f0be61f9ac08fa63a6175f1e0b860fe342583c1300cc1ce6864d
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.8 MB (56817061 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:05c0c534ca7cb93c07b3173e666f97d46b4c01e0e0a41c3d12bf4d7db871f3ff`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:43:20 GMT
ADD file:b1c54ba84e5f6c88c1f3a56a44deef95ed0ae707568246e0aad79bfb2ad13ff7 in / 
# Wed, 17 Nov 2021 02:43:20 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:43:36 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:5ae2b303578d13cb8c2bec7380b5933eb31c2dc36e90d6745fabbb1efa42d287`  
		Last Modified: Wed, 17 Nov 2021 02:54:04 GMT  
		Size: 56.8 MB (56816838 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:156ce42d4e4e409da10973009e6fa0698ea418b75cc8adf3f94111a79431590d`  
		Last Modified: Wed, 17 Nov 2021 02:54:32 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; mips64le

```console
$ docker pull debian@sha256:3cb33f89962a7fbe0dd037c6db997b05501c3bd2864ef5522f34e47746355cbb
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.4 MB (54365685 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6baa4eb242aa136297ce4887ab58269c4f82464cfc3eb6d011e953052c1fc169`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:15:00 GMT
ADD file:65990c5fc1629d9c60c5eae96c8b5b16dc64c2e1e4d049da2d9d45ca8126a2b6 in / 
# Wed, 17 Nov 2021 02:15:01 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:15:28 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:a0ac2e403b44e2769c44c8edeefcb754cc01a8aed8345df0bc46cedfb659667e`  
		Last Modified: Wed, 17 Nov 2021 02:26:29 GMT  
		Size: 54.4 MB (54365461 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:6bdc3e2e4c4165a83be05dc4ec812788c45f4bd9e9aa9ba661a73512081685e3`  
		Last Modified: Wed, 17 Nov 2021 02:27:11 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; ppc64le

```console
$ docker pull debian@sha256:682b290c786b2bea60bb40637a31e82c464b357185c85c39b88365e51f94a2d0
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **60.0 MB (60045252 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fedf50d85bd176395364a6f442d183c2fbfffbaf05c51965593223f886efa87c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:38:38 GMT
ADD file:f98cd5c0b6d1b0eca9c66a96528f16cee04c4c39c0099aeb696452eba40f7ce5 in / 
# Wed, 17 Nov 2021 03:38:55 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 03:40:33 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:20a855c65aabb17aca96cb4aba443be5ede8aeccac91e0e21c41cf07a4ae22c2`  
		Last Modified: Wed, 17 Nov 2021 04:17:17 GMT  
		Size: 60.0 MB (60045027 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:8329727e1f1f8d329e49169175a09e367d1cadc563448ad984132019f26b854c`  
		Last Modified: Wed, 17 Nov 2021 04:19:16 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; riscv64

```console
$ docker pull debian@sha256:9a4fd411b6cad4efff57b06e5d7c5247dc56614442f0a923f50fd0bf50c3d0cf
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.5 MB (51522622 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c6a90333fb1c00ce7d932c4281569862fd236fba77d8ef4ebe04691fc75841d1`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:18:02 GMT
ADD file:0ec1f46a764439430fb9f09f4839b10976be8fc64fc12afa4c027f1c62f25260 in / 
# Wed, 17 Nov 2021 02:18:04 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:21:13 GMT
RUN echo 'deb http://deb.debian.org/debian-ports experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:2bc932115d57a62ccf3e02aa3e687abcf6d81b71a70bd56a6c3eef683ff58d23`  
		Last Modified: Wed, 17 Nov 2021 02:34:09 GMT  
		Size: 51.5 MB (51522393 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:5e5161a894d80361fe39f50145322762014675b9abb46699d942082d90b4dfc4`  
		Last Modified: Wed, 17 Nov 2021 02:36:47 GMT  
		Size: 229.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental` - linux; s390x

```console
$ docker pull debian@sha256:d73fa202edf98345edf9e6534f97065f21b1cec3046305536c1060c47d487150
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.0 MB (53965842 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fc8704ff4f7e75c9e0aa046a651029ee9276445f9942de9fcfc702e9355478b0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:45:12 GMT
ADD file:3e240d4862da4173e8dc2b043c11c59b5b8d8ea71c087729ecc0bcedf15bc5c2 in / 
# Wed, 17 Nov 2021 02:45:15 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:45:32 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:e50a73e9a583b8d41392e2bc8c7f048886a3cea5c4319e2b919dfbc1866d9e9f`  
		Last Modified: Wed, 17 Nov 2021 02:51:10 GMT  
		Size: 54.0 MB (53965621 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:1c88b35c8c27cbb17920e80d41e0eb4913c843ef5a4f7985f320cfe0c2cd0230`  
		Last Modified: Wed, 17 Nov 2021 02:51:28 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:experimental-20211115`

```console
$ docker pull debian@sha256:5fbaf548cc847fc74caa74b6a9d0c0cc3ff4e6eac863f980080bb1ae3a8c3d1b
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:experimental-20211115` - linux; amd64

```console
$ docker pull debian@sha256:c34e1569aca56b9192091930a3ed888130d68a835bcf44eae46b64098be018a2
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.8 MB (55758326 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1155b79982f5cb3cb4ca852f8db318fe9657f4f255df38ab0a63558d2f4e4d51`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:23:08 GMT
ADD file:db09d197fcb0e66f610eb98b792583f6ec40f708cb0d0177e510c2a158f566fa in / 
# Wed, 17 Nov 2021 02:23:09 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:23:19 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:a6543f35db5e93a9d2a1ff8c5abaf275a6bfe30ceb4f008f8047cacfd2d0f324`  
		Last Modified: Wed, 17 Nov 2021 02:30:33 GMT  
		Size: 55.8 MB (55758105 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:510e6744cd6ef82181b5126f01e80d20252c4fd4245ffa53d41ec779ee543d1c`  
		Last Modified: Wed, 17 Nov 2021 02:30:57 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental-20211115` - linux; arm variant v5

```console
$ docker pull debian@sha256:f285fb4cba39f82f3e9edf346f1ebd8f747ab13a01a16053b90799ce9e2f7ddc
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53238960 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8e8987a7b943d00c071d485dd3af35769aa9ddd5c18b898cf56aaefd589346db`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:57:44 GMT
ADD file:4f7ce5598f98e9f01ec7174a57364d9d28802a554530db59c008960d0a40c239 in / 
# Wed, 17 Nov 2021 02:57:45 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:58:18 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:5c59a35f8148e14582e723208c2af72918650b046899af2d952e25ad86cc3dd2`  
		Last Modified: Wed, 17 Nov 2021 03:16:24 GMT  
		Size: 53.2 MB (53238737 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:10c4cb694ca349e0fcee01dce7cc639e0e468f453e3bc939a6128afdb7ab791b`  
		Last Modified: Wed, 17 Nov 2021 03:17:06 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental-20211115` - linux; arm variant v7

```console
$ docker pull debian@sha256:045e674644b35bc4a2bf075445145fe9a410f7274f66d801bea71fb370dbc5c8
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.9 MB (50860530 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:aec3c000773d608dd166eee6d899e220139f9c6f4ebd0fbaedf7ed5e65944cbd`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:07:01 GMT
ADD file:930f169a86ba880df2aaea12f3e8598f194b73f18795a8cd70efcd27b241bda3 in / 
# Wed, 17 Nov 2021 02:07:02 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:07:35 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:61a45c58489b0c8e11c36c2b4e975b6bd6dd5cd0b16c91ccb452966e84c75021`  
		Last Modified: Wed, 17 Nov 2021 02:24:50 GMT  
		Size: 50.9 MB (50860307 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:8647d476eb9176d77a596dd5cef2be97e3386236a4e2f12e22d27fc8742d717a`  
		Last Modified: Wed, 17 Nov 2021 02:25:31 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental-20211115` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:6460b222d57825e8dbba8071810b435418e8459e2fc5a4a9096ebb47a83c1874
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.8 MB (54767296 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6157a41c40dfece8c9e8f4aeccfec249a4c19835344d6ce672af84d61aeae663`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:43:27 GMT
ADD file:44b7578caffebde5ddfd201775779df99cb1d525113ba58609f8cdf1f47092ce in / 
# Wed, 17 Nov 2021 02:43:27 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:43:43 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:af0aea1ea198f6a1207b9812f667f234e3ccf829e7b41959a314d3616636bc52`  
		Last Modified: Wed, 17 Nov 2021 02:52:44 GMT  
		Size: 54.8 MB (54767075 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:419ceac417071c25631219677cdd697c6d7d12c3b7071efe3130a7a7bd6910b8`  
		Last Modified: Wed, 17 Nov 2021 02:53:10 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental-20211115` - linux; 386

```console
$ docker pull debian@sha256:67e28b400930f0be61f9ac08fa63a6175f1e0b860fe342583c1300cc1ce6864d
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.8 MB (56817061 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:05c0c534ca7cb93c07b3173e666f97d46b4c01e0e0a41c3d12bf4d7db871f3ff`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:43:20 GMT
ADD file:b1c54ba84e5f6c88c1f3a56a44deef95ed0ae707568246e0aad79bfb2ad13ff7 in / 
# Wed, 17 Nov 2021 02:43:20 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:43:36 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:5ae2b303578d13cb8c2bec7380b5933eb31c2dc36e90d6745fabbb1efa42d287`  
		Last Modified: Wed, 17 Nov 2021 02:54:04 GMT  
		Size: 56.8 MB (56816838 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:156ce42d4e4e409da10973009e6fa0698ea418b75cc8adf3f94111a79431590d`  
		Last Modified: Wed, 17 Nov 2021 02:54:32 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental-20211115` - linux; mips64le

```console
$ docker pull debian@sha256:3cb33f89962a7fbe0dd037c6db997b05501c3bd2864ef5522f34e47746355cbb
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.4 MB (54365685 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6baa4eb242aa136297ce4887ab58269c4f82464cfc3eb6d011e953052c1fc169`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:15:00 GMT
ADD file:65990c5fc1629d9c60c5eae96c8b5b16dc64c2e1e4d049da2d9d45ca8126a2b6 in / 
# Wed, 17 Nov 2021 02:15:01 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:15:28 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:a0ac2e403b44e2769c44c8edeefcb754cc01a8aed8345df0bc46cedfb659667e`  
		Last Modified: Wed, 17 Nov 2021 02:26:29 GMT  
		Size: 54.4 MB (54365461 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:6bdc3e2e4c4165a83be05dc4ec812788c45f4bd9e9aa9ba661a73512081685e3`  
		Last Modified: Wed, 17 Nov 2021 02:27:11 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental-20211115` - linux; ppc64le

```console
$ docker pull debian@sha256:682b290c786b2bea60bb40637a31e82c464b357185c85c39b88365e51f94a2d0
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **60.0 MB (60045252 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fedf50d85bd176395364a6f442d183c2fbfffbaf05c51965593223f886efa87c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:38:38 GMT
ADD file:f98cd5c0b6d1b0eca9c66a96528f16cee04c4c39c0099aeb696452eba40f7ce5 in / 
# Wed, 17 Nov 2021 03:38:55 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 03:40:33 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:20a855c65aabb17aca96cb4aba443be5ede8aeccac91e0e21c41cf07a4ae22c2`  
		Last Modified: Wed, 17 Nov 2021 04:17:17 GMT  
		Size: 60.0 MB (60045027 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:8329727e1f1f8d329e49169175a09e367d1cadc563448ad984132019f26b854c`  
		Last Modified: Wed, 17 Nov 2021 04:19:16 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental-20211115` - linux; riscv64

```console
$ docker pull debian@sha256:9a4fd411b6cad4efff57b06e5d7c5247dc56614442f0a923f50fd0bf50c3d0cf
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.5 MB (51522622 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c6a90333fb1c00ce7d932c4281569862fd236fba77d8ef4ebe04691fc75841d1`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:18:02 GMT
ADD file:0ec1f46a764439430fb9f09f4839b10976be8fc64fc12afa4c027f1c62f25260 in / 
# Wed, 17 Nov 2021 02:18:04 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:21:13 GMT
RUN echo 'deb http://deb.debian.org/debian-ports experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:2bc932115d57a62ccf3e02aa3e687abcf6d81b71a70bd56a6c3eef683ff58d23`  
		Last Modified: Wed, 17 Nov 2021 02:34:09 GMT  
		Size: 51.5 MB (51522393 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:5e5161a894d80361fe39f50145322762014675b9abb46699d942082d90b4dfc4`  
		Last Modified: Wed, 17 Nov 2021 02:36:47 GMT  
		Size: 229.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:experimental-20211115` - linux; s390x

```console
$ docker pull debian@sha256:d73fa202edf98345edf9e6534f97065f21b1cec3046305536c1060c47d487150
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.0 MB (53965842 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fc8704ff4f7e75c9e0aa046a651029ee9276445f9942de9fcfc702e9355478b0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:45:12 GMT
ADD file:3e240d4862da4173e8dc2b043c11c59b5b8d8ea71c087729ecc0bcedf15bc5c2 in / 
# Wed, 17 Nov 2021 02:45:15 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:45:32 GMT
RUN echo 'deb http://deb.debian.org/debian experimental main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:e50a73e9a583b8d41392e2bc8c7f048886a3cea5c4319e2b919dfbc1866d9e9f`  
		Last Modified: Wed, 17 Nov 2021 02:51:10 GMT  
		Size: 54.0 MB (53965621 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:1c88b35c8c27cbb17920e80d41e0eb4913c843ef5a4f7985f320cfe0c2cd0230`  
		Last Modified: Wed, 17 Nov 2021 02:51:28 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:latest`

```console
$ docker pull debian@sha256:e8c184b56a94db0947a9d51ec68f42ef5584442f20547fa3bd8cbd00203b2e7a
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:latest` - linux; amd64

```console
$ docker pull debian@sha256:6480d244c39d52f2de8ee0417cc385aa437cabfbfd8e3c127882c4d03f8ab86d
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.9 MB (54932774 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:827e5611389abf13dad1057e92f163b771febc0bcdb19fa2d634a7eb0641e0cc`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:20:30 GMT
ADD file:5259fc086e8295ddbf02e48abef38e9bf93a183079d3631aa7a59306b7f2f9df in / 
# Wed, 17 Nov 2021 02:20:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:647acf3d48c2780e00cd27bb0984367415f270d78477ef9d5b238e6ebd5290da`  
		Last Modified: Wed, 17 Nov 2021 02:25:17 GMT  
		Size: 54.9 MB (54932774 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; arm variant v5

```console
$ docker pull debian@sha256:f76b8cf174e44cdc775877460bc3e19c3e30e66b271cb5d39f726cfa7094044f
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52467904 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:49f6c0d2973ae831b31a88d01129b59c15cdf95d69ea05d1eb401f8a920846d8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:50:02 GMT
ADD file:48c0196bfa5dfd1137285bf9104248d62f15a734133d8df549f35b6f7989ca19 in / 
# Wed, 17 Nov 2021 02:50:03 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ce39e946c04dab3f8b463d502d71849e8e6d7291ece7e323f0ec9dac1061af26`  
		Last Modified: Wed, 17 Nov 2021 03:05:19 GMT  
		Size: 52.5 MB (52467904 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; arm variant v7

```console
$ docker pull debian@sha256:8926b33de48e3a6a7f84356ce88179481f44523f12d55ad8c3ba287dc191ffdc
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.1 MB (50134152 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e9cf93b473f42691f8231524112260deb7aa0f6f9a371fa9b8f552f24285519d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 01:59:07 GMT
ADD file:43b45f65ff4ba720f6f9a107b5f80818d5ea6ebc4835840b11e9a506351da32f in / 
# Wed, 17 Nov 2021 01:59:08 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:dbbf55137aab3c7acd1e012a70f1ea1adeaef8d39b474829965d3480a02bf572`  
		Last Modified: Wed, 17 Nov 2021 02:14:27 GMT  
		Size: 50.1 MB (50134152 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:c9178c59ba331e2a0745ddf7109c44fe17f19dca2eb7ecb8c14a4c566124a025
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53619024 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:eec76c447f59d03e1d07a8bc96368012b2f05e4a52b8812a5671faef329f1eaf`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:39:59 GMT
ADD file:eb667e18226da8a1ca5a41beacddc65cd1efec01c3d01e50845c5e89e91ea17c in / 
# Wed, 17 Nov 2021 02:40:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ce18cd0ba11c2dfa8c1a2d2b325e1e019b14d4ae25ffdeffb0f9686c5496bf5c`  
		Last Modified: Wed, 17 Nov 2021 02:46:45 GMT  
		Size: 53.6 MB (53619024 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; 386

```console
$ docker pull debian@sha256:d1b5cbf0a8cbcd44147c1d46b9b8f778f436ed5174a55b1b84cab79163215905
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.9 MB (55937586 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:59b34e04dc278408b2e7fc09cacf1411ecd25f3105a711a746f1d22f1d39a87a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:39:36 GMT
ADD file:b8f9021573a53ec69fa566a396d9a8c68392bd6d659a665c0aea8fbd00164f12 in / 
# Wed, 17 Nov 2021 02:39:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8d649f3a0e8410b121648ccc552acdac0960691a4da33ff4db9825f9b51ff3f4`  
		Last Modified: Wed, 17 Nov 2021 02:47:12 GMT  
		Size: 55.9 MB (55937586 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; mips64le

```console
$ docker pull debian@sha256:d89c75015f88e1ca939d94c69ac13e4f8134ae432acfa6900076c1fbb30cad76
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53183806 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5035dbb39f67711b5184433a96cafd0db4bc9b6a653c914110ecb01166a99a7b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:09:46 GMT
ADD file:2971df49f24648b8d5166d87e392e2547d5b03469356f20d112ce6feb3c329fc in / 
# Wed, 17 Nov 2021 02:09:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b095a0b63d99a4c4cd9c1c353f381a69ef041e6f80798d807ae51a34b04920a0`  
		Last Modified: Wed, 17 Nov 2021 02:18:22 GMT  
		Size: 53.2 MB (53183806 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; ppc64le

```console
$ docker pull debian@sha256:952516c1afd1ea16fa745b83502d222062d24df80fdcd73cece4640609732c60
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.8 MB (58819505 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c78f4b353abfd74cf67ea010720e7bbee93d8e5383b5429a6ab9f35ad982f5ec`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:27:04 GMT
ADD file:85375acb19da58f9eabaa67f8cb425cd1ff2dadb2888bbd9dbfa6223d16fef23 in / 
# Wed, 17 Nov 2021 03:27:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ede9cd7c8f1d380c971a130fb3d8a3914bf3a9106702164c23b2cce536fa618c`  
		Last Modified: Wed, 17 Nov 2021 03:52:39 GMT  
		Size: 58.8 MB (58819505 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:latest` - linux; s390x

```console
$ docker pull debian@sha256:f9147074e1223cfc833be4e33b5b80a80017cd3a4c6f00e3af27bde69dd10c80
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53207179 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:40a4f800f6aa1a5b14da6ed5b5e9f4ecfc77ff178aa0685d69f7cfc918a93695`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:07 GMT
ADD file:b7524e775a2033d246d1096765a0eb6eaa5ba6ed6c8b395522579d2c4e7a2e38 in / 
# Wed, 17 Nov 2021 02:42:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ffa47acd07831fbc96680a705da20704bac0016c20dc3c454ecd45cda44b368d`  
		Last Modified: Wed, 17 Nov 2021 02:48:05 GMT  
		Size: 53.2 MB (53207179 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldoldstable`

```console
$ docker pull debian@sha256:f7ada9876f74d855eb28d67fba69a8f1bef1ff11c247687fae8464f7c9a54bac
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 5
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:oldoldstable` - linux; amd64

```console
$ docker pull debian@sha256:4b24d14cf491aff4554b82a4243f1fbcbc0abedd3624b5039183a8916a09aa85
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.4 MB (45380426 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:afcb734f3e6a67f365f7583b8fdacc3f8c7a239f6de8eae2b851becfc7e3a7fc`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:21:11 GMT
ADD file:41411e498f99721f107b9f5f8ab721441c8b3b171ae2a11cac626a562e2fe147 in / 
# Wed, 17 Nov 2021 02:21:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:95dc2f784b302e53b28c5011779964a13473fa55794a88c73489c34978f95069`  
		Last Modified: Wed, 17 Nov 2021 02:26:52 GMT  
		Size: 45.4 MB (45380426 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable` - linux; arm variant v5

```console
$ docker pull debian@sha256:272f6e69541340f3d84c2e2ab6ec90ffbe7bd5a25f12925f45592ca5e6b91b86
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **44.1 MB (44091400 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a3a2b5a87d80cdb8b71446dafa879cdb2307008ba79f8878dfb29cbe7e30b580`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:52:06 GMT
ADD file:68aa157a5d8e0ac1947f151a597ad6fd7e29853cbe29a9be5cd21482e4afa35f in / 
# Wed, 17 Nov 2021 02:52:08 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:25da075bdc56c2191aa7ef6e7631f3c24a42006f6f84920454aabedb691ac1e7`  
		Last Modified: Wed, 17 Nov 2021 03:08:26 GMT  
		Size: 44.1 MB (44091400 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable` - linux; arm variant v7

```console
$ docker pull debian@sha256:c98d8c945dfe642fdf0ccf3b4e534e1d4e9b75c049cc9b077f22263aef8f2a02
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **42.1 MB (42116885 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:947f8dded7fee186db9c97342fd48e365e9e712c4121059f6e332d8e50510ddc`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:01:14 GMT
ADD file:9fe034588d3d2f104d5aeabbd50c5962d6eaca10ab66ca53a733c09c2198092d in / 
# Wed, 17 Nov 2021 02:01:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b09753a0fa5832cd6796122a4aba45a3ed13f0371593065aef0ffef747c6030c`  
		Last Modified: Wed, 17 Nov 2021 02:17:25 GMT  
		Size: 42.1 MB (42116885 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:6466a424bedae060eab9e3bc988e34ee63f56bd493c75a6d92a51e2a34e7ce65
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **43.2 MB (43174476 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d220c5fa4dc224548655faa78c6df602d8a663e29044a304784610e4ed6f4ad0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:55 GMT
ADD file:7a50bbd1118cbbcc95d6c4e1ae912bab0503d10209cf27e88bd7241a76390f9b in / 
# Wed, 17 Nov 2021 02:40:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:08e624181b9f21c13fbf44c78f84881aba565b51554e1bae8e119c729bd3d356`  
		Last Modified: Wed, 17 Nov 2021 02:48:29 GMT  
		Size: 43.2 MB (43174476 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable` - linux; 386

```console
$ docker pull debian@sha256:3f6d3e94efdc9fe72d548509e7b38fbf6055cf7ea600d1ccd1343c4adfee8c2b
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **46.1 MB (46097226 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fb4a17ae430874793583bdd06f8fa1c8ef37fff442f1197bcaba7fa4a5fd7be4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:34 GMT
ADD file:d3dfab18ec2c72e27a89da2e6f06868d67ed554aa27eebb2febe59ba6a8769a3 in / 
# Wed, 17 Nov 2021 02:40:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:06ce23efa1728c9324b4972ad13ac7716bb38b5bb8a38c549671e53368ea18dc`  
		Last Modified: Wed, 17 Nov 2021 02:49:16 GMT  
		Size: 46.1 MB (46097226 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldoldstable-20211115`

```console
$ docker pull debian@sha256:f7ada9876f74d855eb28d67fba69a8f1bef1ff11c247687fae8464f7c9a54bac
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 5
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:oldoldstable-20211115` - linux; amd64

```console
$ docker pull debian@sha256:4b24d14cf491aff4554b82a4243f1fbcbc0abedd3624b5039183a8916a09aa85
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.4 MB (45380426 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:afcb734f3e6a67f365f7583b8fdacc3f8c7a239f6de8eae2b851becfc7e3a7fc`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:21:11 GMT
ADD file:41411e498f99721f107b9f5f8ab721441c8b3b171ae2a11cac626a562e2fe147 in / 
# Wed, 17 Nov 2021 02:21:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:95dc2f784b302e53b28c5011779964a13473fa55794a88c73489c34978f95069`  
		Last Modified: Wed, 17 Nov 2021 02:26:52 GMT  
		Size: 45.4 MB (45380426 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-20211115` - linux; arm variant v5

```console
$ docker pull debian@sha256:272f6e69541340f3d84c2e2ab6ec90ffbe7bd5a25f12925f45592ca5e6b91b86
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **44.1 MB (44091400 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a3a2b5a87d80cdb8b71446dafa879cdb2307008ba79f8878dfb29cbe7e30b580`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:52:06 GMT
ADD file:68aa157a5d8e0ac1947f151a597ad6fd7e29853cbe29a9be5cd21482e4afa35f in / 
# Wed, 17 Nov 2021 02:52:08 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:25da075bdc56c2191aa7ef6e7631f3c24a42006f6f84920454aabedb691ac1e7`  
		Last Modified: Wed, 17 Nov 2021 03:08:26 GMT  
		Size: 44.1 MB (44091400 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-20211115` - linux; arm variant v7

```console
$ docker pull debian@sha256:c98d8c945dfe642fdf0ccf3b4e534e1d4e9b75c049cc9b077f22263aef8f2a02
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **42.1 MB (42116885 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:947f8dded7fee186db9c97342fd48e365e9e712c4121059f6e332d8e50510ddc`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:01:14 GMT
ADD file:9fe034588d3d2f104d5aeabbd50c5962d6eaca10ab66ca53a733c09c2198092d in / 
# Wed, 17 Nov 2021 02:01:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b09753a0fa5832cd6796122a4aba45a3ed13f0371593065aef0ffef747c6030c`  
		Last Modified: Wed, 17 Nov 2021 02:17:25 GMT  
		Size: 42.1 MB (42116885 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-20211115` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:6466a424bedae060eab9e3bc988e34ee63f56bd493c75a6d92a51e2a34e7ce65
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **43.2 MB (43174476 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d220c5fa4dc224548655faa78c6df602d8a663e29044a304784610e4ed6f4ad0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:55 GMT
ADD file:7a50bbd1118cbbcc95d6c4e1ae912bab0503d10209cf27e88bd7241a76390f9b in / 
# Wed, 17 Nov 2021 02:40:56 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:08e624181b9f21c13fbf44c78f84881aba565b51554e1bae8e119c729bd3d356`  
		Last Modified: Wed, 17 Nov 2021 02:48:29 GMT  
		Size: 43.2 MB (43174476 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-20211115` - linux; 386

```console
$ docker pull debian@sha256:3f6d3e94efdc9fe72d548509e7b38fbf6055cf7ea600d1ccd1343c4adfee8c2b
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **46.1 MB (46097226 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fb4a17ae430874793583bdd06f8fa1c8ef37fff442f1197bcaba7fa4a5fd7be4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:34 GMT
ADD file:d3dfab18ec2c72e27a89da2e6f06868d67ed554aa27eebb2febe59ba6a8769a3 in / 
# Wed, 17 Nov 2021 02:40:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:06ce23efa1728c9324b4972ad13ac7716bb38b5bb8a38c549671e53368ea18dc`  
		Last Modified: Wed, 17 Nov 2021 02:49:16 GMT  
		Size: 46.1 MB (46097226 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldoldstable-20211115-slim`

```console
$ docker pull debian@sha256:2c20fa6af4b74101715fa405d6566d9c1f8825306feade2583a767c13f570902
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 5
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:oldoldstable-20211115-slim` - linux; amd64

```console
$ docker pull debian@sha256:7c27a262e4dd2a7ff2048e81b4e7b1532f437a9b3f9c04828335c3716e5a994f
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.5 MB (22527623 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a8e18c54ff972faed4631f43e912a03c7fddffd38c61aa02eab9dff7253e30ad`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:21:21 GMT
ADD file:8654bb32e37c5eee135acdb3a23489fc848e9839df82806607218c2b647d0481 in / 
# Wed, 17 Nov 2021 02:21:21 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:bec764d8c89e71193ba6bcc7840e5edc81cb04e9b76ba5cd4257a256d99f52e7`  
		Last Modified: Wed, 17 Nov 2021 02:27:12 GMT  
		Size: 22.5 MB (22527623 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-20211115-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:90792fb457a483c655c6a8dd7cdf04a163c0ad670be904ea0ef59897e9f961e0
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **21.2 MB (21204265 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bd2238adf922008e04e39f8451248da899524225204ee68f00bf8a8dd3894fb5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:52:37 GMT
ADD file:48a8cefd9b99bd7569c9fdb73c5e6b6b166d87c9dc292bf621fa23da836313ad in / 
# Wed, 17 Nov 2021 02:52:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f6d0b8fd4f18dd7f07a47cf7c614e400abc60bc04ddda20315f8c52b33407f79`  
		Last Modified: Wed, 17 Nov 2021 03:09:00 GMT  
		Size: 21.2 MB (21204265 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-20211115-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:85d54e97f74eab2bd3f52a41471288e554b92be880c4d08047d390876d0fa089
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **19.3 MB (19316329 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8c790c36d6b503cdea7538c401cfe61781787ead70f18af82caaf3023191e6ef`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:01:45 GMT
ADD file:94d742b0db1358a4882b0759fc2a6132406accbabe51f718a37e7b487f4f9f31 in / 
# Wed, 17 Nov 2021 02:01:46 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:adb1cf6e55c5670e7407fa37124ba05523b20734ec5ef8f7816baaf2d98da34d`  
		Last Modified: Wed, 17 Nov 2021 02:17:55 GMT  
		Size: 19.3 MB (19316329 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-20211115-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:e4333dc1197273e4bb4b743e30e1218d77274d9f593d7cb92ddbac05b1149178
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **20.4 MB (20389460 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:096884d8efc9007907b2a8f7b7238322b3b25e74c1f65fd02c35e15ef648c6f1`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:09 GMT
ADD file:107298ded2cf34927c4905756c91ade153e9bb4d0a368633af4f1ecadfe32882 in / 
# Wed, 17 Nov 2021 02:41:10 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1668e2fa367189e899358f042f5144b1ced110e0641ee7b1b86e9d2a81955c22`  
		Last Modified: Wed, 17 Nov 2021 02:48:55 GMT  
		Size: 20.4 MB (20389460 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-20211115-slim` - linux; 386

```console
$ docker pull debian@sha256:4122d3f156f6c0cd0ff15564ceef224c63abc712a6ce2c011d8978c3ea6c8fbd
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **23.2 MB (23156521 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:158cd6b27e25c9b927271af570051fece5af9ea2932156677d9cae7b760ebaf7`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:48 GMT
ADD file:f44fe58637b5100fbfd109f3f4482552422d1c7d0cc6f24ef215ece40cfbbfde in / 
# Wed, 17 Nov 2021 02:40:49 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f6e113afbbf666033a42e9dea4ca50d891aca1c67d21e429a699a1959ba7fc65`  
		Last Modified: Wed, 17 Nov 2021 02:49:40 GMT  
		Size: 23.2 MB (23156521 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldoldstable-backports`

```console
$ docker pull debian@sha256:48ff0c45e352deb3c6887eb26688791dee1906a1bcd5444c452a4adc5f9b19b3
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 5
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:oldoldstable-backports` - linux; amd64

```console
$ docker pull debian@sha256:ccb588b16198037b8d238c294b6b15964b44379c35d28f7bfb0d591650f00c4e
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.4 MB (45380652 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:358bae219553235e04a37d1285aae66c127db65b647ad5725b908023e7089682`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:21:11 GMT
ADD file:41411e498f99721f107b9f5f8ab721441c8b3b171ae2a11cac626a562e2fe147 in / 
# Wed, 17 Nov 2021 02:21:11 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:21:15 GMT
RUN echo 'deb http://deb.debian.org/debian oldoldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:95dc2f784b302e53b28c5011779964a13473fa55794a88c73489c34978f95069`  
		Last Modified: Wed, 17 Nov 2021 02:26:52 GMT  
		Size: 45.4 MB (45380426 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:17e854a23605b11c1d81bd6e18e20935704fc60bf076f2ac7d30a3a7197d1ba0`  
		Last Modified: Wed, 17 Nov 2021 02:27:02 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:1df20084f8e7bb0f596ec23b9ad9c3e76ccfa8dc2e2193fd31ee59b4aba4e9be
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **44.1 MB (44091629 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bf2ad52cc6663b6708dae8b731b45c11d38aa3bd7bec9be6d872a8cbcea509ff`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:52:06 GMT
ADD file:68aa157a5d8e0ac1947f151a597ad6fd7e29853cbe29a9be5cd21482e4afa35f in / 
# Wed, 17 Nov 2021 02:52:08 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:52:21 GMT
RUN echo 'deb http://deb.debian.org/debian oldoldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:25da075bdc56c2191aa7ef6e7631f3c24a42006f6f84920454aabedb691ac1e7`  
		Last Modified: Wed, 17 Nov 2021 03:08:26 GMT  
		Size: 44.1 MB (44091400 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:57b841ca46d16f569d615d69d1914ab8023ee8db35e6a9353e0d2b113cbe4209`  
		Last Modified: Wed, 17 Nov 2021 03:08:38 GMT  
		Size: 229.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:cb37b86bbe2ba86b56a5c7203c87f1e6f01dd0e00c569b40ccc51009e6291b2e
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **42.1 MB (42117111 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a7e2ce471b56e3825aaa732f5a4be369e4b8ac48246d4182caf19126e10cc45e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:01:14 GMT
ADD file:9fe034588d3d2f104d5aeabbd50c5962d6eaca10ab66ca53a733c09c2198092d in / 
# Wed, 17 Nov 2021 02:01:15 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:01:28 GMT
RUN echo 'deb http://deb.debian.org/debian oldoldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:b09753a0fa5832cd6796122a4aba45a3ed13f0371593065aef0ffef747c6030c`  
		Last Modified: Wed, 17 Nov 2021 02:17:25 GMT  
		Size: 42.1 MB (42116885 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:d08254bee10751f58926233ae8599c4b0b561e99ac1e45446d67688ecb984055`  
		Last Modified: Wed, 17 Nov 2021 02:17:37 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:8fdbfa45bd7a8726650d2d9fac77131deb41faad7d7688b6968eaaab7377e5be
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **43.2 MB (43174701 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1b06b908f38d2799d6c6f591f9b1a1980354bb617845ac1039dc41714d571d25`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:55 GMT
ADD file:7a50bbd1118cbbcc95d6c4e1ae912bab0503d10209cf27e88bd7241a76390f9b in / 
# Wed, 17 Nov 2021 02:40:56 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:41:01 GMT
RUN echo 'deb http://deb.debian.org/debian oldoldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:08e624181b9f21c13fbf44c78f84881aba565b51554e1bae8e119c729bd3d356`  
		Last Modified: Wed, 17 Nov 2021 02:48:29 GMT  
		Size: 43.2 MB (43174476 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:02842259569577bd1aed1ebb82de30ef678f78e4477945e85d1411807e4aa19e`  
		Last Modified: Wed, 17 Nov 2021 02:48:40 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-backports` - linux; 386

```console
$ docker pull debian@sha256:b5122ba3a72e11bba5931b952b9f2a4c87665d80135ddc3704f41fe5ebf573a7
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **46.1 MB (46097451 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:051e765d98da904e947021dcbce64e92f4390b4ed609f516d692ac2bb065670e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:34 GMT
ADD file:d3dfab18ec2c72e27a89da2e6f06868d67ed554aa27eebb2febe59ba6a8769a3 in / 
# Wed, 17 Nov 2021 02:40:35 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:40:41 GMT
RUN echo 'deb http://deb.debian.org/debian oldoldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:06ce23efa1728c9324b4972ad13ac7716bb38b5bb8a38c549671e53368ea18dc`  
		Last Modified: Wed, 17 Nov 2021 02:49:16 GMT  
		Size: 46.1 MB (46097226 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:d2c8c9ca27609632844301d834992b0141f71bc4815ce285b3af7313f0caf41b`  
		Last Modified: Wed, 17 Nov 2021 02:49:27 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldoldstable-slim`

```console
$ docker pull debian@sha256:2c20fa6af4b74101715fa405d6566d9c1f8825306feade2583a767c13f570902
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 5
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:oldoldstable-slim` - linux; amd64

```console
$ docker pull debian@sha256:7c27a262e4dd2a7ff2048e81b4e7b1532f437a9b3f9c04828335c3716e5a994f
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.5 MB (22527623 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a8e18c54ff972faed4631f43e912a03c7fddffd38c61aa02eab9dff7253e30ad`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:21:21 GMT
ADD file:8654bb32e37c5eee135acdb3a23489fc848e9839df82806607218c2b647d0481 in / 
# Wed, 17 Nov 2021 02:21:21 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:bec764d8c89e71193ba6bcc7840e5edc81cb04e9b76ba5cd4257a256d99f52e7`  
		Last Modified: Wed, 17 Nov 2021 02:27:12 GMT  
		Size: 22.5 MB (22527623 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:90792fb457a483c655c6a8dd7cdf04a163c0ad670be904ea0ef59897e9f961e0
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **21.2 MB (21204265 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bd2238adf922008e04e39f8451248da899524225204ee68f00bf8a8dd3894fb5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:52:37 GMT
ADD file:48a8cefd9b99bd7569c9fdb73c5e6b6b166d87c9dc292bf621fa23da836313ad in / 
# Wed, 17 Nov 2021 02:52:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f6d0b8fd4f18dd7f07a47cf7c614e400abc60bc04ddda20315f8c52b33407f79`  
		Last Modified: Wed, 17 Nov 2021 03:09:00 GMT  
		Size: 21.2 MB (21204265 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:85d54e97f74eab2bd3f52a41471288e554b92be880c4d08047d390876d0fa089
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **19.3 MB (19316329 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8c790c36d6b503cdea7538c401cfe61781787ead70f18af82caaf3023191e6ef`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:01:45 GMT
ADD file:94d742b0db1358a4882b0759fc2a6132406accbabe51f718a37e7b487f4f9f31 in / 
# Wed, 17 Nov 2021 02:01:46 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:adb1cf6e55c5670e7407fa37124ba05523b20734ec5ef8f7816baaf2d98da34d`  
		Last Modified: Wed, 17 Nov 2021 02:17:55 GMT  
		Size: 19.3 MB (19316329 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:e4333dc1197273e4bb4b743e30e1218d77274d9f593d7cb92ddbac05b1149178
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **20.4 MB (20389460 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:096884d8efc9007907b2a8f7b7238322b3b25e74c1f65fd02c35e15ef648c6f1`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:09 GMT
ADD file:107298ded2cf34927c4905756c91ade153e9bb4d0a368633af4f1ecadfe32882 in / 
# Wed, 17 Nov 2021 02:41:10 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1668e2fa367189e899358f042f5144b1ced110e0641ee7b1b86e9d2a81955c22`  
		Last Modified: Wed, 17 Nov 2021 02:48:55 GMT  
		Size: 20.4 MB (20389460 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldoldstable-slim` - linux; 386

```console
$ docker pull debian@sha256:4122d3f156f6c0cd0ff15564ceef224c63abc712a6ce2c011d8978c3ea6c8fbd
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **23.2 MB (23156521 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:158cd6b27e25c9b927271af570051fece5af9ea2932156677d9cae7b760ebaf7`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:40:48 GMT
ADD file:f44fe58637b5100fbfd109f3f4482552422d1c7d0cc6f24ef215ece40cfbbfde in / 
# Wed, 17 Nov 2021 02:40:49 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f6e113afbbf666033a42e9dea4ca50d891aca1c67d21e429a699a1959ba7fc65`  
		Last Modified: Wed, 17 Nov 2021 02:49:40 GMT  
		Size: 23.2 MB (23156521 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldstable`

```console
$ docker pull debian@sha256:9bc4e72cf0ed7b6d948480323baeacad8ec41a7e0dd448c52daef5230d190fea
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:oldstable` - linux; amd64

```console
$ docker pull debian@sha256:d55ed754c7bb4e0a85ff9564f43a3e04d5d35ab29c3af9d28c62a0dc555e4058
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50437114 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:45737e26352f121230db136cd7f402ef80ca920c447845a8ae35dd3d75b8000c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:21:30 GMT
ADD file:aced752439c3c72517c140929de0ae8f976ab707d1478009d82941c990f76350 in / 
# Wed, 17 Nov 2021 02:21:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e48a3e484caf9b71146dfed9e8b197f3cb75d6a16e3574444620ce01a70aff44`  
		Last Modified: Wed, 17 Nov 2021 02:27:27 GMT  
		Size: 50.4 MB (50437114 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; arm variant v5

```console
$ docker pull debian@sha256:a4d143db4c77ce2941a0412110e269bbd12da6bcbae1538384a6ac6f301f9e24
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **48.2 MB (48154253 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:66217d7c2dce7d0d95de8ade50eabad43eb33785b7f92467d8a2ddad792c1c73`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:53:02 GMT
ADD file:5be4d0bc23a41d7eda9d1dad80f038f9b00d3586628cceca67b22a7ad1cef857 in / 
# Wed, 17 Nov 2021 02:53:05 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:468846ae46a4adb2e0d8890885e5b310f36a4aea4883dd3803adbfd8287b27e7`  
		Last Modified: Wed, 17 Nov 2021 03:09:40 GMT  
		Size: 48.2 MB (48154253 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; arm variant v7

```console
$ docker pull debian@sha256:ca1bcd102cbdc5009c638d0b403969d0d58a2e2a380740d627cbf1fbe795a084
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45918108 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:81954d6cecf7a2eb83239f36d6a1c00c33e18f57785b4e73a233a3fdb2412d04`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:02:12 GMT
ADD file:23851ef44d699801f834722aa6530215ac3a6dcd500c76d1bdcded99688a68d8 in / 
# Wed, 17 Nov 2021 02:02:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b959e8636c3fbc4fbcb87bab1fd1c0342f5c73abfb922f435e0607f1c1937f03`  
		Last Modified: Wed, 17 Nov 2021 02:18:34 GMT  
		Size: 45.9 MB (45918108 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:ca1641fed45a92a3c7505fdedc529b207f0464b2a1938f7e9314c4971a780d58
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49223004 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1c9fd5d8746377e7a86ae8e4052ff0d685910cd7099946f656980df94b396362`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:20 GMT
ADD file:0bf72871c9dc02db2e698d59b049f2c2e2006efd248ac786ac9aea42bcd22fc9 in / 
# Wed, 17 Nov 2021 02:41:21 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2926301144acff341799afd8cde6828df5c64ca6be41a692ce22a8eeababdfaa`  
		Last Modified: Wed, 17 Nov 2021 02:49:11 GMT  
		Size: 49.2 MB (49223004 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; 386

```console
$ docker pull debian@sha256:9c635fa1fa918888a457250bd556935c3e0780917d462a43fee96216fab20651
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51207735 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6aac6c8153b1df42021b7f9491536282b574115a848537cb1fdda2aefde26f78`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:01 GMT
ADD file:4d619a3e402494b6b2b7078d2900116a696c6229528ac7e0f43d4718d9282fee in / 
# Wed, 17 Nov 2021 02:41:02 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:262b40fd2e74c9a9af1338cd432d8a2f67c4181745bf6a64e1720132db04dc44`  
		Last Modified: Wed, 17 Nov 2021 02:50:01 GMT  
		Size: 51.2 MB (51207735 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; mips64le

```console
$ docker pull debian@sha256:5ebcab828ba60fa21f9822d0d0ebb3128341265095d50813b839f8843f54d4a3
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.1 MB (49079508 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:14a4f9031b3da89f155cf7349b56d95245672137701f9b0e72e4aa94555d8ba7`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:11:31 GMT
ADD file:3f62fbdf824f2fcd14e636dbc1bd299b45af61be75ce88c29ce65ab16c3fbe9c in / 
# Wed, 17 Nov 2021 02:11:32 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0811a00b03aec1e2055c56152922ac2f7f4fdc9ded11878e6fb79cb05c66310a`  
		Last Modified: Wed, 17 Nov 2021 02:21:17 GMT  
		Size: 49.1 MB (49079508 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; ppc64le

```console
$ docker pull debian@sha256:6539374d0e9e5a750684b8092d315aa52d54c329a15842b446970daed895480f
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.2 MB (54183775 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:507eb042a3691b903a244e928e561ee118d02aa5d7eb99c343a4887015059c1f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:30:53 GMT
ADD file:fb62616778cd378dd7e27257d6690a397d27b22aa7afa78026f5f0edb0c26087 in / 
# Wed, 17 Nov 2021 03:31:14 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4c0a5998b10e435c4caf5a1b3d6f33eb49ac12985b9e77464ccad3513447823c`  
		Last Modified: Wed, 17 Nov 2021 04:00:40 GMT  
		Size: 54.2 MB (54183775 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable` - linux; s390x

```console
$ docker pull debian@sha256:284ff854e47275bd5636295ff0517398a27a143e75b297d81a5ce5cd2b07d9af
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.0 MB (49005421 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c6359b59e8a9de4da2239b4dd5dd3950773f5aecf074870095aa0ffc74f83903`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:43:11 GMT
ADD file:8093c4a2da0918a23f3506c7248de0407ee728c18fa9f43d6e6bcc4c1bd13691 in / 
# Wed, 17 Nov 2021 02:43:14 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0fec2256c1fde808b8caa285d6c8df9138e9318b933d893a2ed591acdb766c02`  
		Last Modified: Wed, 17 Nov 2021 02:49:20 GMT  
		Size: 49.0 MB (49005421 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldstable-20211115`

```console
$ docker pull debian@sha256:9bc4e72cf0ed7b6d948480323baeacad8ec41a7e0dd448c52daef5230d190fea
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:oldstable-20211115` - linux; amd64

```console
$ docker pull debian@sha256:d55ed754c7bb4e0a85ff9564f43a3e04d5d35ab29c3af9d28c62a0dc555e4058
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50437114 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:45737e26352f121230db136cd7f402ef80ca920c447845a8ae35dd3d75b8000c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:21:30 GMT
ADD file:aced752439c3c72517c140929de0ae8f976ab707d1478009d82941c990f76350 in / 
# Wed, 17 Nov 2021 02:21:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e48a3e484caf9b71146dfed9e8b197f3cb75d6a16e3574444620ce01a70aff44`  
		Last Modified: Wed, 17 Nov 2021 02:27:27 GMT  
		Size: 50.4 MB (50437114 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20211115` - linux; arm variant v5

```console
$ docker pull debian@sha256:a4d143db4c77ce2941a0412110e269bbd12da6bcbae1538384a6ac6f301f9e24
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **48.2 MB (48154253 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:66217d7c2dce7d0d95de8ade50eabad43eb33785b7f92467d8a2ddad792c1c73`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:53:02 GMT
ADD file:5be4d0bc23a41d7eda9d1dad80f038f9b00d3586628cceca67b22a7ad1cef857 in / 
# Wed, 17 Nov 2021 02:53:05 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:468846ae46a4adb2e0d8890885e5b310f36a4aea4883dd3803adbfd8287b27e7`  
		Last Modified: Wed, 17 Nov 2021 03:09:40 GMT  
		Size: 48.2 MB (48154253 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20211115` - linux; arm variant v7

```console
$ docker pull debian@sha256:ca1bcd102cbdc5009c638d0b403969d0d58a2e2a380740d627cbf1fbe795a084
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45918108 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:81954d6cecf7a2eb83239f36d6a1c00c33e18f57785b4e73a233a3fdb2412d04`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:02:12 GMT
ADD file:23851ef44d699801f834722aa6530215ac3a6dcd500c76d1bdcded99688a68d8 in / 
# Wed, 17 Nov 2021 02:02:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b959e8636c3fbc4fbcb87bab1fd1c0342f5c73abfb922f435e0607f1c1937f03`  
		Last Modified: Wed, 17 Nov 2021 02:18:34 GMT  
		Size: 45.9 MB (45918108 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20211115` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:ca1641fed45a92a3c7505fdedc529b207f0464b2a1938f7e9314c4971a780d58
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49223004 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1c9fd5d8746377e7a86ae8e4052ff0d685910cd7099946f656980df94b396362`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:20 GMT
ADD file:0bf72871c9dc02db2e698d59b049f2c2e2006efd248ac786ac9aea42bcd22fc9 in / 
# Wed, 17 Nov 2021 02:41:21 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2926301144acff341799afd8cde6828df5c64ca6be41a692ce22a8eeababdfaa`  
		Last Modified: Wed, 17 Nov 2021 02:49:11 GMT  
		Size: 49.2 MB (49223004 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20211115` - linux; 386

```console
$ docker pull debian@sha256:9c635fa1fa918888a457250bd556935c3e0780917d462a43fee96216fab20651
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51207735 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6aac6c8153b1df42021b7f9491536282b574115a848537cb1fdda2aefde26f78`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:01 GMT
ADD file:4d619a3e402494b6b2b7078d2900116a696c6229528ac7e0f43d4718d9282fee in / 
# Wed, 17 Nov 2021 02:41:02 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:262b40fd2e74c9a9af1338cd432d8a2f67c4181745bf6a64e1720132db04dc44`  
		Last Modified: Wed, 17 Nov 2021 02:50:01 GMT  
		Size: 51.2 MB (51207735 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20211115` - linux; mips64le

```console
$ docker pull debian@sha256:5ebcab828ba60fa21f9822d0d0ebb3128341265095d50813b839f8843f54d4a3
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.1 MB (49079508 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:14a4f9031b3da89f155cf7349b56d95245672137701f9b0e72e4aa94555d8ba7`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:11:31 GMT
ADD file:3f62fbdf824f2fcd14e636dbc1bd299b45af61be75ce88c29ce65ab16c3fbe9c in / 
# Wed, 17 Nov 2021 02:11:32 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0811a00b03aec1e2055c56152922ac2f7f4fdc9ded11878e6fb79cb05c66310a`  
		Last Modified: Wed, 17 Nov 2021 02:21:17 GMT  
		Size: 49.1 MB (49079508 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20211115` - linux; ppc64le

```console
$ docker pull debian@sha256:6539374d0e9e5a750684b8092d315aa52d54c329a15842b446970daed895480f
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.2 MB (54183775 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:507eb042a3691b903a244e928e561ee118d02aa5d7eb99c343a4887015059c1f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:30:53 GMT
ADD file:fb62616778cd378dd7e27257d6690a397d27b22aa7afa78026f5f0edb0c26087 in / 
# Wed, 17 Nov 2021 03:31:14 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:4c0a5998b10e435c4caf5a1b3d6f33eb49ac12985b9e77464ccad3513447823c`  
		Last Modified: Wed, 17 Nov 2021 04:00:40 GMT  
		Size: 54.2 MB (54183775 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20211115` - linux; s390x

```console
$ docker pull debian@sha256:284ff854e47275bd5636295ff0517398a27a143e75b297d81a5ce5cd2b07d9af
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.0 MB (49005421 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c6359b59e8a9de4da2239b4dd5dd3950773f5aecf074870095aa0ffc74f83903`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:43:11 GMT
ADD file:8093c4a2da0918a23f3506c7248de0407ee728c18fa9f43d6e6bcc4c1bd13691 in / 
# Wed, 17 Nov 2021 02:43:14 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0fec2256c1fde808b8caa285d6c8df9138e9318b933d893a2ed591acdb766c02`  
		Last Modified: Wed, 17 Nov 2021 02:49:20 GMT  
		Size: 49.0 MB (49005421 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldstable-20211115-slim`

```console
$ docker pull debian@sha256:e05f7b2d232e12eb4e66c1968d7005fac0552c3624e6cdd9df89d7f193304a2d
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:oldstable-20211115-slim` - linux; amd64

```console
$ docker pull debian@sha256:c97243b9d028caae09042c1f07af1e776860a992986df7668abf3726270cfebb
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.2 MB (27153714 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b193521538d2e6679bdcf37a9ffb99048650f7381281c9c05ea35544e892e828`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:21:41 GMT
ADD file:d13d732a3048948d5d33d1ba095ba2b6baea6bba4107841f61f1fe906ce35ae9 in / 
# Wed, 17 Nov 2021 02:21:41 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d80f374d806797f256a2b71ef8ddbde6c42e1d1933d071546e968abdae6526ef`  
		Last Modified: Wed, 17 Nov 2021 02:27:47 GMT  
		Size: 27.2 MB (27153714 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20211115-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:da4d572b3fb54c89a7e9c17d8830c150e69d2a920f793c58f852a74cb4cf6310
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.9 MB (24886274 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7b4401d716721a7c2229748bddbddead7ec42cf37e600967fd1ed396b98f99d4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:53:34 GMT
ADD file:c3f1af5238d3e98857995b038a331f557de7575e6d711ec903feb7d7337da140 in / 
# Wed, 17 Nov 2021 02:53:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ee955d5aab6d6ca0721f54971e5a9598687d0748d695a52edc1530c01ecf9ff9`  
		Last Modified: Wed, 17 Nov 2021 03:10:15 GMT  
		Size: 24.9 MB (24886274 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20211115-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:4f812925bf0ec283bb34171566848b0afa0d20482fad384aaf27d460b61519d9
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.8 MB (22754353 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:634d2a7ea2c4e307acb27d3ceb0d6886030edc17cec3b94671a350f376c00249`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:02:43 GMT
ADD file:373ab08864151c9e2d5130a5eef82824f788c40096bebce592b14e98048c5a98 in / 
# Wed, 17 Nov 2021 02:02:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:266964c1aceca63a60186e4cb01ef1dd720f5707bf56c2975f9b72537d86702d`  
		Last Modified: Wed, 17 Nov 2021 02:19:06 GMT  
		Size: 22.8 MB (22754353 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20211115-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:5abacc4a5578aefcf368f1bff0b46b5c543701a6795767049c1e798a3bbcc095
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.9 MB (25923141 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:59157fdc9aeab54e084d5ffc8c73442c42d3e892a7cb6e6cfd2f01f03da01094`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:35 GMT
ADD file:0b612a81dcaf314a36e281b77d728d0c6e1edcb1f611bf2cf052308f0765a0bb in / 
# Wed, 17 Nov 2021 02:41:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9df104a53267adf6fe0b2121af62df826dfc0e326082fed5cfd6b6c80ceb845b`  
		Last Modified: Wed, 17 Nov 2021 02:49:33 GMT  
		Size: 25.9 MB (25923141 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20211115-slim` - linux; 386

```console
$ docker pull debian@sha256:9216cdfd17566e56a5a4242020e3509e2a9694753c14b5161558f3d74f6c6427
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.8 MB (27804578 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9e6d847d339e8316d046f6159eb9374bcaac5dda4c2b99d6910b8734cacad7cc`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:16 GMT
ADD file:46773fb3b8ba22c0c0732c2039cd819eea9805a8ac2d69aa7ee6886feca95758 in / 
# Wed, 17 Nov 2021 02:41:17 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:153cd923809f51b62e7e1085f3be08eb5a3d1334fed6e1ade9c228250d597646`  
		Last Modified: Wed, 17 Nov 2021 02:50:27 GMT  
		Size: 27.8 MB (27804578 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20211115-slim` - linux; mips64le

```console
$ docker pull debian@sha256:340408696e29d201ac9cc0652433a18bc5d574176f9fb435281020bccfee142f
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.8 MB (25820133 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1186c664b6f2b64b411d776dd64b7885c0429d1334e7660411a9b1b4a8a2b9f6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:11:57 GMT
ADD file:af26266fca7c393b70e76bb7bc52e0c8f5f8e0cf772ab08435f8b9f43a5e784e in / 
# Wed, 17 Nov 2021 02:11:58 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8d02699ccfa65acdbb92f29abea6c0f1685ef0da1d7777e3e677eceb8feb42d9`  
		Last Modified: Wed, 17 Nov 2021 02:21:53 GMT  
		Size: 25.8 MB (25820133 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20211115-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:63177be65dc81f42e2f47e2f87023a6e7d6c47abc604b822cafef9eeca6bd649
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.6 MB (30562284 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:569bdfcaa052c544a97d6d6d18e59787ece66ec41ecff3409606708f84999989`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:32:11 GMT
ADD file:f511337e14850ffe8858fdac5a9401fcecbcbd5e813ddfb20972aecf28e1f8b1 in / 
# Wed, 17 Nov 2021 03:32:28 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c62c227604cc4c4db062ecc07d257fbabc22e3709278f72c9a697f77aa49e72e`  
		Last Modified: Wed, 17 Nov 2021 04:02:01 GMT  
		Size: 30.6 MB (30562284 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-20211115-slim` - linux; s390x

```console
$ docker pull debian@sha256:fd8c319dc2dc34f3a38caa41c625a170dbe33e2aad3e8ac76a2be98404826745
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.8 MB (25769086 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b91833fb164c2127ff72e0be627bf115d9fdc4abdff0271c63d5b6eb19a7687a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:43:30 GMT
ADD file:b9e1b0a6cdb39c2451d47371d16c18394e4b17859b76cd0731caececa728fd3c in / 
# Wed, 17 Nov 2021 02:43:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a123b44811539f19da7dd59a8612581af6f05f56c220b3ca5ac98cef6e6ebd02`  
		Last Modified: Wed, 17 Nov 2021 02:49:35 GMT  
		Size: 25.8 MB (25769086 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldstable-backports`

```console
$ docker pull debian@sha256:3319d7159a1f4aa79f049d1741f469c9a4e1ddb6140883d2cda6bcd718f2f672
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:oldstable-backports` - linux; amd64

```console
$ docker pull debian@sha256:239d9e4eb78c851a2a66710231b14b6be0ad7c8c01dfaeba15780066998a1438
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.4 MB (50437340 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0d1a14faab04137586c9950be2c3f38ba3b6881bb6527989351a112e8107d763`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:21:30 GMT
ADD file:aced752439c3c72517c140929de0ae8f976ab707d1478009d82941c990f76350 in / 
# Wed, 17 Nov 2021 02:21:31 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:21:34 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:e48a3e484caf9b71146dfed9e8b197f3cb75d6a16e3574444620ce01a70aff44`  
		Last Modified: Wed, 17 Nov 2021 02:27:27 GMT  
		Size: 50.4 MB (50437114 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:deebcf83c54247ca085f33e534d83db7267d76a1898270c338c0d42d95d57fbf`  
		Last Modified: Wed, 17 Nov 2021 02:27:37 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:daa680dfd2e73899f7e7b4ea9f11e01784e1d514446e7c54be6711ae93232eaf
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **48.2 MB (48154478 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:5d5779c161b2592b12af076c1f38a41355041b35489be23353ce4b0ea09ad30d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:53:02 GMT
ADD file:5be4d0bc23a41d7eda9d1dad80f038f9b00d3586628cceca67b22a7ad1cef857 in / 
# Wed, 17 Nov 2021 02:53:05 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:53:17 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:468846ae46a4adb2e0d8890885e5b310f36a4aea4883dd3803adbfd8287b27e7`  
		Last Modified: Wed, 17 Nov 2021 03:09:40 GMT  
		Size: 48.2 MB (48154253 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:570e89d8a6851c1151cfb70c18f66cd2356c846d8b9a025690a0df8414b7fb10`  
		Last Modified: Wed, 17 Nov 2021 03:09:52 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:d8eab25cdec6ee0208542e2cd49d649065538a83ba8f49f840a83d1f8042b4e6
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.9 MB (45918333 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:727832e9bf3b5d7eafbef22872f4e50bebfd9b313e444d675f9c682eba7ae018`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:02:12 GMT
ADD file:23851ef44d699801f834722aa6530215ac3a6dcd500c76d1bdcded99688a68d8 in / 
# Wed, 17 Nov 2021 02:02:13 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:02:25 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:b959e8636c3fbc4fbcb87bab1fd1c0342f5c73abfb922f435e0607f1c1937f03`  
		Last Modified: Wed, 17 Nov 2021 02:18:34 GMT  
		Size: 45.9 MB (45918108 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:b32fad563492aadc2b788336a7156f6a35255f2134e6511e46b18aa2e72f3edd`  
		Last Modified: Wed, 17 Nov 2021 02:18:46 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:61f740b7821dd406e23e93d0a8feb12f2ece0589ba5b792fe26a25950ecaaac9
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.2 MB (49223228 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:784da462fd513a639ea87ed9f4f94362ab272c1d2a6c5a53641dda3672380cc3`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:20 GMT
ADD file:0bf72871c9dc02db2e698d59b049f2c2e2006efd248ac786ac9aea42bcd22fc9 in / 
# Wed, 17 Nov 2021 02:41:21 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:41:27 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:2926301144acff341799afd8cde6828df5c64ca6be41a692ce22a8eeababdfaa`  
		Last Modified: Wed, 17 Nov 2021 02:49:11 GMT  
		Size: 49.2 MB (49223004 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:24cf6ea5a67f105ab3701247bd2d44a1b8cbb48f1108d960c64e7d2e838ba386`  
		Last Modified: Wed, 17 Nov 2021 02:49:22 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-backports` - linux; 386

```console
$ docker pull debian@sha256:4aff0a338465244a0f98eb776a2037e55ae4e333e35c5bb2b3d618afac6e5ec4
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.2 MB (51207961 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7aa9433d10245875e043ec67c0dd73f2d2f62b8dd30d97ec314d40735c6b2423`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:01 GMT
ADD file:4d619a3e402494b6b2b7078d2900116a696c6229528ac7e0f43d4718d9282fee in / 
# Wed, 17 Nov 2021 02:41:02 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:41:08 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:262b40fd2e74c9a9af1338cd432d8a2f67c4181745bf6a64e1720132db04dc44`  
		Last Modified: Wed, 17 Nov 2021 02:50:01 GMT  
		Size: 51.2 MB (51207735 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:fbb174e8d6e29cbd39def53eaf7e397dd0b9dfa8518d41e0c5a1cd5c17a090e6`  
		Last Modified: Wed, 17 Nov 2021 02:50:13 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-backports` - linux; mips64le

```console
$ docker pull debian@sha256:cfd66a4e0de2ea48998174579c0c2d604abea656b038706f6f3533e0ca318e52
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.1 MB (49079735 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:32c0ef77ef491b866f95985460bd355f69e20527ae3a0c33f2f1bf6be9abedfa`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:11:31 GMT
ADD file:3f62fbdf824f2fcd14e636dbc1bd299b45af61be75ce88c29ce65ab16c3fbe9c in / 
# Wed, 17 Nov 2021 02:11:32 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:11:40 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:0811a00b03aec1e2055c56152922ac2f7f4fdc9ded11878e6fb79cb05c66310a`  
		Last Modified: Wed, 17 Nov 2021 02:21:17 GMT  
		Size: 49.1 MB (49079508 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:9164b9cd9ca845163f611db4dea3f0b203ead603f5f04ecb35887d98590fbed4`  
		Last Modified: Wed, 17 Nov 2021 02:21:27 GMT  
		Size: 227.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-backports` - linux; ppc64le

```console
$ docker pull debian@sha256:ded4dfd1da929e17b696a0ce766e6dbff701c93e9cde5ec20ceef8492562f4d3
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.2 MB (54184003 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:abf55232a6aba6d7750abbb9a677598ee413f4ab0eef90ced53e44c88355acb0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:30:53 GMT
ADD file:fb62616778cd378dd7e27257d6690a397d27b22aa7afa78026f5f0edb0c26087 in / 
# Wed, 17 Nov 2021 03:31:14 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 03:31:47 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:4c0a5998b10e435c4caf5a1b3d6f33eb49ac12985b9e77464ccad3513447823c`  
		Last Modified: Wed, 17 Nov 2021 04:00:40 GMT  
		Size: 54.2 MB (54183775 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:70cc44c649c5811ae3d8fbf91f8ceb4e7225829d136a0ea9efd368b0630a8ae8`  
		Last Modified: Wed, 17 Nov 2021 04:00:54 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-backports` - linux; s390x

```console
$ docker pull debian@sha256:22e6acb44277f5fc96883b016d438c149412434c4c8d4de2a961a7b5d138574d
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **49.0 MB (49005646 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8090595779a06c91b5c5ed0c0302404e3d40f696fa65d1f14c03051480e4cfc0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:43:11 GMT
ADD file:8093c4a2da0918a23f3506c7248de0407ee728c18fa9f43d6e6bcc4c1bd13691 in / 
# Wed, 17 Nov 2021 02:43:14 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:43:21 GMT
RUN echo 'deb http://deb.debian.org/debian oldstable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:0fec2256c1fde808b8caa285d6c8df9138e9318b933d893a2ed591acdb766c02`  
		Last Modified: Wed, 17 Nov 2021 02:49:20 GMT  
		Size: 49.0 MB (49005421 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:c1b6eb6b9bed02316d5347909404146eab77a4b65294d5f790fac5b85392c653`  
		Last Modified: Wed, 17 Nov 2021 02:49:27 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:oldstable-slim`

```console
$ docker pull debian@sha256:e05f7b2d232e12eb4e66c1968d7005fac0552c3624e6cdd9df89d7f193304a2d
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:oldstable-slim` - linux; amd64

```console
$ docker pull debian@sha256:c97243b9d028caae09042c1f07af1e776860a992986df7668abf3726270cfebb
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.2 MB (27153714 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b193521538d2e6679bdcf37a9ffb99048650f7381281c9c05ea35544e892e828`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:21:41 GMT
ADD file:d13d732a3048948d5d33d1ba095ba2b6baea6bba4107841f61f1fe906ce35ae9 in / 
# Wed, 17 Nov 2021 02:21:41 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d80f374d806797f256a2b71ef8ddbde6c42e1d1933d071546e968abdae6526ef`  
		Last Modified: Wed, 17 Nov 2021 02:27:47 GMT  
		Size: 27.2 MB (27153714 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:da4d572b3fb54c89a7e9c17d8830c150e69d2a920f793c58f852a74cb4cf6310
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **24.9 MB (24886274 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7b4401d716721a7c2229748bddbddead7ec42cf37e600967fd1ed396b98f99d4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:53:34 GMT
ADD file:c3f1af5238d3e98857995b038a331f557de7575e6d711ec903feb7d7337da140 in / 
# Wed, 17 Nov 2021 02:53:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ee955d5aab6d6ca0721f54971e5a9598687d0748d695a52edc1530c01ecf9ff9`  
		Last Modified: Wed, 17 Nov 2021 03:10:15 GMT  
		Size: 24.9 MB (24886274 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:4f812925bf0ec283bb34171566848b0afa0d20482fad384aaf27d460b61519d9
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.8 MB (22754353 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:634d2a7ea2c4e307acb27d3ceb0d6886030edc17cec3b94671a350f376c00249`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:02:43 GMT
ADD file:373ab08864151c9e2d5130a5eef82824f788c40096bebce592b14e98048c5a98 in / 
# Wed, 17 Nov 2021 02:02:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:266964c1aceca63a60186e4cb01ef1dd720f5707bf56c2975f9b72537d86702d`  
		Last Modified: Wed, 17 Nov 2021 02:19:06 GMT  
		Size: 22.8 MB (22754353 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:5abacc4a5578aefcf368f1bff0b46b5c543701a6795767049c1e798a3bbcc095
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.9 MB (25923141 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:59157fdc9aeab54e084d5ffc8c73442c42d3e892a7cb6e6cfd2f01f03da01094`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:35 GMT
ADD file:0b612a81dcaf314a36e281b77d728d0c6e1edcb1f611bf2cf052308f0765a0bb in / 
# Wed, 17 Nov 2021 02:41:36 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9df104a53267adf6fe0b2121af62df826dfc0e326082fed5cfd6b6c80ceb845b`  
		Last Modified: Wed, 17 Nov 2021 02:49:33 GMT  
		Size: 25.9 MB (25923141 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-slim` - linux; 386

```console
$ docker pull debian@sha256:9216cdfd17566e56a5a4242020e3509e2a9694753c14b5161558f3d74f6c6427
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.8 MB (27804578 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9e6d847d339e8316d046f6159eb9374bcaac5dda4c2b99d6910b8734cacad7cc`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:16 GMT
ADD file:46773fb3b8ba22c0c0732c2039cd819eea9805a8ac2d69aa7ee6886feca95758 in / 
# Wed, 17 Nov 2021 02:41:17 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:153cd923809f51b62e7e1085f3be08eb5a3d1334fed6e1ade9c228250d597646`  
		Last Modified: Wed, 17 Nov 2021 02:50:27 GMT  
		Size: 27.8 MB (27804578 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-slim` - linux; mips64le

```console
$ docker pull debian@sha256:340408696e29d201ac9cc0652433a18bc5d574176f9fb435281020bccfee142f
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.8 MB (25820133 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1186c664b6f2b64b411d776dd64b7885c0429d1334e7660411a9b1b4a8a2b9f6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:11:57 GMT
ADD file:af26266fca7c393b70e76bb7bc52e0c8f5f8e0cf772ab08435f8b9f43a5e784e in / 
# Wed, 17 Nov 2021 02:11:58 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8d02699ccfa65acdbb92f29abea6c0f1685ef0da1d7777e3e677eceb8feb42d9`  
		Last Modified: Wed, 17 Nov 2021 02:21:53 GMT  
		Size: 25.8 MB (25820133 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:63177be65dc81f42e2f47e2f87023a6e7d6c47abc604b822cafef9eeca6bd649
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.6 MB (30562284 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:569bdfcaa052c544a97d6d6d18e59787ece66ec41ecff3409606708f84999989`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:32:11 GMT
ADD file:f511337e14850ffe8858fdac5a9401fcecbcbd5e813ddfb20972aecf28e1f8b1 in / 
# Wed, 17 Nov 2021 03:32:28 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c62c227604cc4c4db062ecc07d257fbabc22e3709278f72c9a697f77aa49e72e`  
		Last Modified: Wed, 17 Nov 2021 04:02:01 GMT  
		Size: 30.6 MB (30562284 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:oldstable-slim` - linux; s390x

```console
$ docker pull debian@sha256:fd8c319dc2dc34f3a38caa41c625a170dbe33e2aad3e8ac76a2be98404826745
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **25.8 MB (25769086 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b91833fb164c2127ff72e0be627bf115d9fdc4abdff0271c63d5b6eb19a7687a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:43:30 GMT
ADD file:b9e1b0a6cdb39c2451d47371d16c18394e4b17859b76cd0731caececa728fd3c in / 
# Wed, 17 Nov 2021 02:43:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a123b44811539f19da7dd59a8612581af6f05f56c220b3ca5ac98cef6e6ebd02`  
		Last Modified: Wed, 17 Nov 2021 02:49:35 GMT  
		Size: 25.8 MB (25769086 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:rc-buggy`

```console
$ docker pull debian@sha256:5bd1780862892bed142b6885eb6d84c1360277b222f7dcee521bddf40db68013
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:rc-buggy` - linux; amd64

```console
$ docker pull debian@sha256:297cc93165d2ca676dc67f86c1190c4ddc3a53bd71c622e1a6b27b9efd2c2362
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.8 MB (55758317 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:dfd5266ad852a3b4e61c0d107a89fe0e283b14dab1ee3d2823a422ccf3ac302e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:21:50 GMT
ADD file:1f62518cec36eb3320e38344c0d36779557214cfce8bc32eda000183a34a0ffa in / 
# Wed, 17 Nov 2021 02:21:51 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:23:22 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:0fef5443775fe054990b02e8f72c4c5bc7792c6a5bf6ef8df110873a81676a87`  
		Last Modified: Wed, 17 Nov 2021 02:28:03 GMT  
		Size: 55.8 MB (55758091 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:a959b47f81cc0d14fbb210c9fc123b9c76b0123bc4bf18c4c1f0695198ea50ea`  
		Last Modified: Wed, 17 Nov 2021 02:31:06 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; arm variant v5

```console
$ docker pull debian@sha256:9b96e706cd57943194c8cfc210f4545105920d55c1b37c51f64a0e66dc376e81
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53238986 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:35d64e59a76a3dca4b08b82787aabae0f8acfe764d0619c791553c0895e11dac`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:54:00 GMT
ADD file:38a4488625c2978d1b1b906a50ef44be1d10f998ff616015c53a2da2cfb92f35 in / 
# Wed, 17 Nov 2021 02:54:01 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:58:30 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:586f1470620e32dd1fa4fe0da1865c1bfc9ac3f49aa5cc629609b8f3a6139cb3`  
		Last Modified: Wed, 17 Nov 2021 03:11:02 GMT  
		Size: 53.2 MB (53238757 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:41e71108b8bfb22a4ec1f63ec0e45c6a513b78a061de981691ed838547a8f90c`  
		Last Modified: Wed, 17 Nov 2021 03:17:17 GMT  
		Size: 229.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; arm variant v7

```console
$ docker pull debian@sha256:b3c4fff805c799adaeb6f303688b2a506eebaeb4fa44b5acd62894106a1d5a64
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.9 MB (50860534 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:dd03e682005cb71cf59753fd8cbe2ef48938f59f40dc27f2f581c04b0e63359d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:03:10 GMT
ADD file:df29fec66c741f67158d8ed2094810758d4a7cfb2c7cba3dbe60e5bc11ed824a in / 
# Wed, 17 Nov 2021 02:03:11 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:07:48 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:339de498e0e3324e5657ae3bd0868b26e4518664b7405a1fb321434233c56211`  
		Last Modified: Wed, 17 Nov 2021 02:19:45 GMT  
		Size: 50.9 MB (50860308 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:ee3ece82fed24d30367e02d7e64c964b10ccfd15ac07fc7194004f021e5f5a72`  
		Last Modified: Wed, 17 Nov 2021 02:25:42 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:4e643b87023ec376c041e087d96526b897466ff815c93e009c21822054991b27
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.8 MB (54767270 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ef88aa400312f886965b74ff4049517b7b6ae074468393de39d842d82e0614c7`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:46 GMT
ADD file:6547698ce9c0dce5d390e739342bd015a0c3266dba4ef5828dc1b1a9eb46e826 in / 
# Wed, 17 Nov 2021 02:41:47 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:43:49 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:67eeb578521bba47ceed6d5c2de23409a33ec3c1c6fda03d83e22d74dc1cdb76`  
		Last Modified: Wed, 17 Nov 2021 02:49:53 GMT  
		Size: 54.8 MB (54767045 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:481e7fcc41eba4bf5eaf90b91e396fbbd7e3419e82dd8276f76e396ea7836e08`  
		Last Modified: Wed, 17 Nov 2021 02:53:20 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; 386

```console
$ docker pull debian@sha256:aeaa4cce1c7f6227141f7c026d4c44481a39095d516b6bee31553ecfa2b07ce1
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.8 MB (56817038 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:429df637c8c5a34b60b89f6c6e168e89daf3773f2d4a09d5dcac3dda7d89ddc9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:30 GMT
ADD file:f85ffaee07ae67dcd7f97765e4cbdd725ebb7e06d7b05a80fc7ccddb5fda6b20 in / 
# Wed, 17 Nov 2021 02:41:31 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:43:42 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:ec289c525481d0b14d52b85fde04accd67398f9603586e8bbdf75ee7b931219b`  
		Last Modified: Wed, 17 Nov 2021 02:50:49 GMT  
		Size: 56.8 MB (56816813 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:a8ff2b302b2f568b494f5434ff9f8ffed1a1c03892bd9b57cf5b3b759c7f6247`  
		Last Modified: Wed, 17 Nov 2021 02:54:42 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; mips64le

```console
$ docker pull debian@sha256:4001ef350f071ae824fd2d0617f53b72bdc03ad21c9b2deecba4548c6aeb7153
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.4 MB (54365688 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:dd50f42336e95b6becd718f780693a04e3b0bb1f118bbd4c7fa4d107ef8d4ca8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:12:23 GMT
ADD file:9e3985f5064204cf4c74f2d464977349398f8e0325083d223b22ace99f5dfa3a in / 
# Wed, 17 Nov 2021 02:12:24 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:15:33 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:b79f1dde26df3dbfdaf094cc43f4dc602b492e9dd707f7a7874e1366808028c5`  
		Last Modified: Wed, 17 Nov 2021 02:22:36 GMT  
		Size: 54.4 MB (54365459 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:64772f0e563a3f926b08d5d8e37669aa7ff21b91479ae00635a3feb9bd2f356b`  
		Last Modified: Wed, 17 Nov 2021 02:27:20 GMT  
		Size: 229.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; ppc64le

```console
$ docker pull debian@sha256:e6a53aa9bfaddca7e79497c6eb40aa1daeea834f9ca4aa6f5cf1c535f7bc9d48
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **60.0 MB (60045243 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d425b6bd2dea0e52c16acf25b9f9f40af58d410764f85ecac935b45c8bfab592`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:32:54 GMT
ADD file:33bc17dd3e3b6cd8c5ac37c8382a5d2b8e3ee8384b598b69b0d17b813dcfc767 in / 
# Wed, 17 Nov 2021 03:33:15 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 03:41:21 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:60fa303f933c621481dd7a32f72de0e35b85a3567ff116516725c45089e1cb88`  
		Last Modified: Wed, 17 Nov 2021 04:05:01 GMT  
		Size: 60.0 MB (60045015 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:e5e724ddafa6e9bd2207878f8f4c849035ecf2ae94dc3ed419e09391e7db9a43`  
		Last Modified: Wed, 17 Nov 2021 04:19:27 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy` - linux; s390x

```console
$ docker pull debian@sha256:6054ea59b35ef1d4279cd624e8d9681ef963b816a6e50612ea5119670b470c59
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.0 MB (53965892 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ca2d58825e5f75bcee91ad98bff0a56adad0030bf68f202a8b4ef3b1deda14a9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:43:42 GMT
ADD file:ba3a56dbd46c1324142a33ad1eefa66c34fda8c9c635140debf01131cb259e63 in / 
# Wed, 17 Nov 2021 02:43:45 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:45:39 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:3595b50e30d7d0d412c142596638dbf5abfc27f196fe5d138ded04b66fd2b50e`  
		Last Modified: Wed, 17 Nov 2021 02:49:48 GMT  
		Size: 54.0 MB (53965664 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:8cddf6db144344852ad11c114803127e5c012f50bcfe344b2c9492160f02fb11`  
		Last Modified: Wed, 17 Nov 2021 02:51:39 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:rc-buggy-20211115`

```console
$ docker pull debian@sha256:5bd1780862892bed142b6885eb6d84c1360277b222f7dcee521bddf40db68013
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:rc-buggy-20211115` - linux; amd64

```console
$ docker pull debian@sha256:297cc93165d2ca676dc67f86c1190c4ddc3a53bd71c622e1a6b27b9efd2c2362
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.8 MB (55758317 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:dfd5266ad852a3b4e61c0d107a89fe0e283b14dab1ee3d2823a422ccf3ac302e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:21:50 GMT
ADD file:1f62518cec36eb3320e38344c0d36779557214cfce8bc32eda000183a34a0ffa in / 
# Wed, 17 Nov 2021 02:21:51 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:23:22 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:0fef5443775fe054990b02e8f72c4c5bc7792c6a5bf6ef8df110873a81676a87`  
		Last Modified: Wed, 17 Nov 2021 02:28:03 GMT  
		Size: 55.8 MB (55758091 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:a959b47f81cc0d14fbb210c9fc123b9c76b0123bc4bf18c4c1f0695198ea50ea`  
		Last Modified: Wed, 17 Nov 2021 02:31:06 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy-20211115` - linux; arm variant v5

```console
$ docker pull debian@sha256:9b96e706cd57943194c8cfc210f4545105920d55c1b37c51f64a0e66dc376e81
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53238986 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:35d64e59a76a3dca4b08b82787aabae0f8acfe764d0619c791553c0895e11dac`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:54:00 GMT
ADD file:38a4488625c2978d1b1b906a50ef44be1d10f998ff616015c53a2da2cfb92f35 in / 
# Wed, 17 Nov 2021 02:54:01 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:58:30 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:586f1470620e32dd1fa4fe0da1865c1bfc9ac3f49aa5cc629609b8f3a6139cb3`  
		Last Modified: Wed, 17 Nov 2021 03:11:02 GMT  
		Size: 53.2 MB (53238757 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:41e71108b8bfb22a4ec1f63ec0e45c6a513b78a061de981691ed838547a8f90c`  
		Last Modified: Wed, 17 Nov 2021 03:17:17 GMT  
		Size: 229.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy-20211115` - linux; arm variant v7

```console
$ docker pull debian@sha256:b3c4fff805c799adaeb6f303688b2a506eebaeb4fa44b5acd62894106a1d5a64
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.9 MB (50860534 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:dd03e682005cb71cf59753fd8cbe2ef48938f59f40dc27f2f581c04b0e63359d`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:03:10 GMT
ADD file:df29fec66c741f67158d8ed2094810758d4a7cfb2c7cba3dbe60e5bc11ed824a in / 
# Wed, 17 Nov 2021 02:03:11 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:07:48 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:339de498e0e3324e5657ae3bd0868b26e4518664b7405a1fb321434233c56211`  
		Last Modified: Wed, 17 Nov 2021 02:19:45 GMT  
		Size: 50.9 MB (50860308 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:ee3ece82fed24d30367e02d7e64c964b10ccfd15ac07fc7194004f021e5f5a72`  
		Last Modified: Wed, 17 Nov 2021 02:25:42 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy-20211115` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:4e643b87023ec376c041e087d96526b897466ff815c93e009c21822054991b27
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.8 MB (54767270 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ef88aa400312f886965b74ff4049517b7b6ae074468393de39d842d82e0614c7`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:46 GMT
ADD file:6547698ce9c0dce5d390e739342bd015a0c3266dba4ef5828dc1b1a9eb46e826 in / 
# Wed, 17 Nov 2021 02:41:47 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:43:49 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:67eeb578521bba47ceed6d5c2de23409a33ec3c1c6fda03d83e22d74dc1cdb76`  
		Last Modified: Wed, 17 Nov 2021 02:49:53 GMT  
		Size: 54.8 MB (54767045 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:481e7fcc41eba4bf5eaf90b91e396fbbd7e3419e82dd8276f76e396ea7836e08`  
		Last Modified: Wed, 17 Nov 2021 02:53:20 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy-20211115` - linux; 386

```console
$ docker pull debian@sha256:aeaa4cce1c7f6227141f7c026d4c44481a39095d516b6bee31553ecfa2b07ce1
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.8 MB (56817038 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:429df637c8c5a34b60b89f6c6e168e89daf3773f2d4a09d5dcac3dda7d89ddc9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:30 GMT
ADD file:f85ffaee07ae67dcd7f97765e4cbdd725ebb7e06d7b05a80fc7ccddb5fda6b20 in / 
# Wed, 17 Nov 2021 02:41:31 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:43:42 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:ec289c525481d0b14d52b85fde04accd67398f9603586e8bbdf75ee7b931219b`  
		Last Modified: Wed, 17 Nov 2021 02:50:49 GMT  
		Size: 56.8 MB (56816813 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:a8ff2b302b2f568b494f5434ff9f8ffed1a1c03892bd9b57cf5b3b759c7f6247`  
		Last Modified: Wed, 17 Nov 2021 02:54:42 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy-20211115` - linux; mips64le

```console
$ docker pull debian@sha256:4001ef350f071ae824fd2d0617f53b72bdc03ad21c9b2deecba4548c6aeb7153
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.4 MB (54365688 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:dd50f42336e95b6becd718f780693a04e3b0bb1f118bbd4c7fa4d107ef8d4ca8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:12:23 GMT
ADD file:9e3985f5064204cf4c74f2d464977349398f8e0325083d223b22ace99f5dfa3a in / 
# Wed, 17 Nov 2021 02:12:24 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:15:33 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:b79f1dde26df3dbfdaf094cc43f4dc602b492e9dd707f7a7874e1366808028c5`  
		Last Modified: Wed, 17 Nov 2021 02:22:36 GMT  
		Size: 54.4 MB (54365459 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:64772f0e563a3f926b08d5d8e37669aa7ff21b91479ae00635a3feb9bd2f356b`  
		Last Modified: Wed, 17 Nov 2021 02:27:20 GMT  
		Size: 229.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy-20211115` - linux; ppc64le

```console
$ docker pull debian@sha256:e6a53aa9bfaddca7e79497c6eb40aa1daeea834f9ca4aa6f5cf1c535f7bc9d48
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **60.0 MB (60045243 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d425b6bd2dea0e52c16acf25b9f9f40af58d410764f85ecac935b45c8bfab592`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:32:54 GMT
ADD file:33bc17dd3e3b6cd8c5ac37c8382a5d2b8e3ee8384b598b69b0d17b813dcfc767 in / 
# Wed, 17 Nov 2021 03:33:15 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 03:41:21 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:60fa303f933c621481dd7a32f72de0e35b85a3567ff116516725c45089e1cb88`  
		Last Modified: Wed, 17 Nov 2021 04:05:01 GMT  
		Size: 60.0 MB (60045015 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:e5e724ddafa6e9bd2207878f8f4c849035ecf2ae94dc3ed419e09391e7db9a43`  
		Last Modified: Wed, 17 Nov 2021 04:19:27 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:rc-buggy-20211115` - linux; s390x

```console
$ docker pull debian@sha256:6054ea59b35ef1d4279cd624e8d9681ef963b816a6e50612ea5119670b470c59
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.0 MB (53965892 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ca2d58825e5f75bcee91ad98bff0a56adad0030bf68f202a8b4ef3b1deda14a9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:43:42 GMT
ADD file:ba3a56dbd46c1324142a33ad1eefa66c34fda8c9c635140debf01131cb259e63 in / 
# Wed, 17 Nov 2021 02:43:45 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:45:39 GMT
RUN echo 'deb http://deb.debian.org/debian rc-buggy main' > /etc/apt/sources.list.d/experimental.list
```

-	Layers:
	-	`sha256:3595b50e30d7d0d412c142596638dbf5abfc27f196fe5d138ded04b66fd2b50e`  
		Last Modified: Wed, 17 Nov 2021 02:49:48 GMT  
		Size: 54.0 MB (53965664 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:8cddf6db144344852ad11c114803127e5c012f50bcfe344b2c9492160f02fb11`  
		Last Modified: Wed, 17 Nov 2021 02:51:39 GMT  
		Size: 228.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:sid`

```console
$ docker pull debian@sha256:443457d19b8e3b91eff42d6d593296d260170314917b21074ece5c1654faa034
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:sid` - linux; amd64

```console
$ docker pull debian@sha256:9fc77e3e00a8bd1dc95108b60c3b3dce3ace71a17b0053d642600f82dbe83c43
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.8 MB (55758091 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:484ec39d24588e8a41861f916dc002ada3a08897635afc28342ec49654d2e53e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:21:50 GMT
ADD file:1f62518cec36eb3320e38344c0d36779557214cfce8bc32eda000183a34a0ffa in / 
# Wed, 17 Nov 2021 02:21:51 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0fef5443775fe054990b02e8f72c4c5bc7792c6a5bf6ef8df110873a81676a87`  
		Last Modified: Wed, 17 Nov 2021 02:28:03 GMT  
		Size: 55.8 MB (55758091 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; arm variant v5

```console
$ docker pull debian@sha256:8cb253de1a0437ab79f0c264fe0e432919877e1d4817561bf1e71ae42ee981a7
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53238757 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3b13ba5b02b34b57848b56d536f136e99b293908a8d01a1da3d7b84c04ee0075`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:54:00 GMT
ADD file:38a4488625c2978d1b1b906a50ef44be1d10f998ff616015c53a2da2cfb92f35 in / 
# Wed, 17 Nov 2021 02:54:01 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:586f1470620e32dd1fa4fe0da1865c1bfc9ac3f49aa5cc629609b8f3a6139cb3`  
		Last Modified: Wed, 17 Nov 2021 03:11:02 GMT  
		Size: 53.2 MB (53238757 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; arm variant v7

```console
$ docker pull debian@sha256:02d48d65e147015175298f2608a1c0e447b7fc7b55af56df74b20eb1e1a2d883
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.9 MB (50860308 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9c6fd287332e0ffc845c05b1325544b671c55a92df997da63876d772faa78a9c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:03:10 GMT
ADD file:df29fec66c741f67158d8ed2094810758d4a7cfb2c7cba3dbe60e5bc11ed824a in / 
# Wed, 17 Nov 2021 02:03:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:339de498e0e3324e5657ae3bd0868b26e4518664b7405a1fb321434233c56211`  
		Last Modified: Wed, 17 Nov 2021 02:19:45 GMT  
		Size: 50.9 MB (50860308 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:526c56b2c691579532aa34b624d025cdeb302160cc97de79d9fc3975490898a0
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.8 MB (54767045 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:afb029e966a7c39616d645b590634f68537551611fb9b450332e42c339ddece1`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:46 GMT
ADD file:6547698ce9c0dce5d390e739342bd015a0c3266dba4ef5828dc1b1a9eb46e826 in / 
# Wed, 17 Nov 2021 02:41:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:67eeb578521bba47ceed6d5c2de23409a33ec3c1c6fda03d83e22d74dc1cdb76`  
		Last Modified: Wed, 17 Nov 2021 02:49:53 GMT  
		Size: 54.8 MB (54767045 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; 386

```console
$ docker pull debian@sha256:a0281444c55276c724d03b1fa85162eee68af4c8569b246d8d981452b6bee0d3
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.8 MB (56816813 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3c6053040c6cc51fdc3d76d47e5eef8b609576f77d8e2d13ca4a5cabf6d9132f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:30 GMT
ADD file:f85ffaee07ae67dcd7f97765e4cbdd725ebb7e06d7b05a80fc7ccddb5fda6b20 in / 
# Wed, 17 Nov 2021 02:41:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ec289c525481d0b14d52b85fde04accd67398f9603586e8bbdf75ee7b931219b`  
		Last Modified: Wed, 17 Nov 2021 02:50:49 GMT  
		Size: 56.8 MB (56816813 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; mips64le

```console
$ docker pull debian@sha256:a5a88a7f1a608884d4990c3256dc55b49170b6ddf84b142132a5a05a362ec304
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.4 MB (54365459 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0765ae0a68d70d5e31a94c47bbe17c30fd29c56aa9a05657a83ffe49f7516778`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:12:23 GMT
ADD file:9e3985f5064204cf4c74f2d464977349398f8e0325083d223b22ace99f5dfa3a in / 
# Wed, 17 Nov 2021 02:12:24 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b79f1dde26df3dbfdaf094cc43f4dc602b492e9dd707f7a7874e1366808028c5`  
		Last Modified: Wed, 17 Nov 2021 02:22:36 GMT  
		Size: 54.4 MB (54365459 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; ppc64le

```console
$ docker pull debian@sha256:34b82c4e96186c458ceba7334dd6ec819b53828ea74893f1ac56e7fb22c9e182
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **60.0 MB (60045015 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:55799566733ef1ddb455270c47b9031dbca27a693f8282194488799aefc8df77`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:32:54 GMT
ADD file:33bc17dd3e3b6cd8c5ac37c8382a5d2b8e3ee8384b598b69b0d17b813dcfc767 in / 
# Wed, 17 Nov 2021 03:33:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:60fa303f933c621481dd7a32f72de0e35b85a3567ff116516725c45089e1cb88`  
		Last Modified: Wed, 17 Nov 2021 04:05:01 GMT  
		Size: 60.0 MB (60045015 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; riscv64

```console
$ docker pull debian@sha256:2929cd90ef0e52e0f7af1d1bad052ddb17625afbc7c3ae9867efdbfab5adebbd
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.5 MB (51522304 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:034a2cb250ccc14a345ca5cefe1cea75deb2f520e79b913c577e4089e9b6236b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:14:24 GMT
ADD file:9ce8adafd95fc1e31db924edf473abf22a68101cad20cd078c35db5fa719b34b in / 
# Wed, 17 Nov 2021 02:14:27 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a35675c997377ee8bf94b0a94460a6d2fd72a26b5c7fc6194b13ae99da869125`  
		Last Modified: Wed, 17 Nov 2021 02:30:42 GMT  
		Size: 51.5 MB (51522304 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid` - linux; s390x

```console
$ docker pull debian@sha256:f0a02793a77508c992d15ed2a6aad1845eb6e614048fc04d32b2b081c5a8cf2e
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.0 MB (53965664 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8b1c4eb2439909792abc5d6aa298c0ac84a59219fd9382186a49a55535b46365`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:43:42 GMT
ADD file:ba3a56dbd46c1324142a33ad1eefa66c34fda8c9c635140debf01131cb259e63 in / 
# Wed, 17 Nov 2021 02:43:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3595b50e30d7d0d412c142596638dbf5abfc27f196fe5d138ded04b66fd2b50e`  
		Last Modified: Wed, 17 Nov 2021 02:49:48 GMT  
		Size: 54.0 MB (53965664 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:sid-20211115`

```console
$ docker pull debian@sha256:443457d19b8e3b91eff42d6d593296d260170314917b21074ece5c1654faa034
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:sid-20211115` - linux; amd64

```console
$ docker pull debian@sha256:9fc77e3e00a8bd1dc95108b60c3b3dce3ace71a17b0053d642600f82dbe83c43
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.8 MB (55758091 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:484ec39d24588e8a41861f916dc002ada3a08897635afc28342ec49654d2e53e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:21:50 GMT
ADD file:1f62518cec36eb3320e38344c0d36779557214cfce8bc32eda000183a34a0ffa in / 
# Wed, 17 Nov 2021 02:21:51 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0fef5443775fe054990b02e8f72c4c5bc7792c6a5bf6ef8df110873a81676a87`  
		Last Modified: Wed, 17 Nov 2021 02:28:03 GMT  
		Size: 55.8 MB (55758091 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20211115` - linux; arm variant v5

```console
$ docker pull debian@sha256:8cb253de1a0437ab79f0c264fe0e432919877e1d4817561bf1e71ae42ee981a7
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53238757 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3b13ba5b02b34b57848b56d536f136e99b293908a8d01a1da3d7b84c04ee0075`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:54:00 GMT
ADD file:38a4488625c2978d1b1b906a50ef44be1d10f998ff616015c53a2da2cfb92f35 in / 
# Wed, 17 Nov 2021 02:54:01 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:586f1470620e32dd1fa4fe0da1865c1bfc9ac3f49aa5cc629609b8f3a6139cb3`  
		Last Modified: Wed, 17 Nov 2021 03:11:02 GMT  
		Size: 53.2 MB (53238757 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20211115` - linux; arm variant v7

```console
$ docker pull debian@sha256:02d48d65e147015175298f2608a1c0e447b7fc7b55af56df74b20eb1e1a2d883
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.9 MB (50860308 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9c6fd287332e0ffc845c05b1325544b671c55a92df997da63876d772faa78a9c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:03:10 GMT
ADD file:df29fec66c741f67158d8ed2094810758d4a7cfb2c7cba3dbe60e5bc11ed824a in / 
# Wed, 17 Nov 2021 02:03:11 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:339de498e0e3324e5657ae3bd0868b26e4518664b7405a1fb321434233c56211`  
		Last Modified: Wed, 17 Nov 2021 02:19:45 GMT  
		Size: 50.9 MB (50860308 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20211115` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:526c56b2c691579532aa34b624d025cdeb302160cc97de79d9fc3975490898a0
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.8 MB (54767045 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:afb029e966a7c39616d645b590634f68537551611fb9b450332e42c339ddece1`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:46 GMT
ADD file:6547698ce9c0dce5d390e739342bd015a0c3266dba4ef5828dc1b1a9eb46e826 in / 
# Wed, 17 Nov 2021 02:41:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:67eeb578521bba47ceed6d5c2de23409a33ec3c1c6fda03d83e22d74dc1cdb76`  
		Last Modified: Wed, 17 Nov 2021 02:49:53 GMT  
		Size: 54.8 MB (54767045 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20211115` - linux; 386

```console
$ docker pull debian@sha256:a0281444c55276c724d03b1fa85162eee68af4c8569b246d8d981452b6bee0d3
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.8 MB (56816813 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3c6053040c6cc51fdc3d76d47e5eef8b609576f77d8e2d13ca4a5cabf6d9132f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:30 GMT
ADD file:f85ffaee07ae67dcd7f97765e4cbdd725ebb7e06d7b05a80fc7ccddb5fda6b20 in / 
# Wed, 17 Nov 2021 02:41:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ec289c525481d0b14d52b85fde04accd67398f9603586e8bbdf75ee7b931219b`  
		Last Modified: Wed, 17 Nov 2021 02:50:49 GMT  
		Size: 56.8 MB (56816813 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20211115` - linux; mips64le

```console
$ docker pull debian@sha256:a5a88a7f1a608884d4990c3256dc55b49170b6ddf84b142132a5a05a362ec304
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.4 MB (54365459 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0765ae0a68d70d5e31a94c47bbe17c30fd29c56aa9a05657a83ffe49f7516778`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:12:23 GMT
ADD file:9e3985f5064204cf4c74f2d464977349398f8e0325083d223b22ace99f5dfa3a in / 
# Wed, 17 Nov 2021 02:12:24 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b79f1dde26df3dbfdaf094cc43f4dc602b492e9dd707f7a7874e1366808028c5`  
		Last Modified: Wed, 17 Nov 2021 02:22:36 GMT  
		Size: 54.4 MB (54365459 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20211115` - linux; ppc64le

```console
$ docker pull debian@sha256:34b82c4e96186c458ceba7334dd6ec819b53828ea74893f1ac56e7fb22c9e182
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **60.0 MB (60045015 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:55799566733ef1ddb455270c47b9031dbca27a693f8282194488799aefc8df77`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:32:54 GMT
ADD file:33bc17dd3e3b6cd8c5ac37c8382a5d2b8e3ee8384b598b69b0d17b813dcfc767 in / 
# Wed, 17 Nov 2021 03:33:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:60fa303f933c621481dd7a32f72de0e35b85a3567ff116516725c45089e1cb88`  
		Last Modified: Wed, 17 Nov 2021 04:05:01 GMT  
		Size: 60.0 MB (60045015 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20211115` - linux; riscv64

```console
$ docker pull debian@sha256:2929cd90ef0e52e0f7af1d1bad052ddb17625afbc7c3ae9867efdbfab5adebbd
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.5 MB (51522304 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:034a2cb250ccc14a345ca5cefe1cea75deb2f520e79b913c577e4089e9b6236b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:14:24 GMT
ADD file:9ce8adafd95fc1e31db924edf473abf22a68101cad20cd078c35db5fa719b34b in / 
# Wed, 17 Nov 2021 02:14:27 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a35675c997377ee8bf94b0a94460a6d2fd72a26b5c7fc6194b13ae99da869125`  
		Last Modified: Wed, 17 Nov 2021 02:30:42 GMT  
		Size: 51.5 MB (51522304 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20211115` - linux; s390x

```console
$ docker pull debian@sha256:f0a02793a77508c992d15ed2a6aad1845eb6e614048fc04d32b2b081c5a8cf2e
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.0 MB (53965664 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8b1c4eb2439909792abc5d6aa298c0ac84a59219fd9382186a49a55535b46365`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:43:42 GMT
ADD file:ba3a56dbd46c1324142a33ad1eefa66c34fda8c9c635140debf01131cb259e63 in / 
# Wed, 17 Nov 2021 02:43:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3595b50e30d7d0d412c142596638dbf5abfc27f196fe5d138ded04b66fd2b50e`  
		Last Modified: Wed, 17 Nov 2021 02:49:48 GMT  
		Size: 54.0 MB (53965664 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:sid-20211115-slim`

```console
$ docker pull debian@sha256:11e687c181702ab9f609bfab84519596a1acc54a57906061bdcef8b741a87637
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:sid-20211115-slim` - linux; amd64

```console
$ docker pull debian@sha256:4a1ef0b02b7fd98f40ad00919994d1f2dff0e618050398a23c54ab5c9d3e3aa7
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.9 MB (31861397 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d4d3e8adafeae528f6757d7670baadeb2e7b32a0cf408b57b863754e64e5e9f4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:21:59 GMT
ADD file:35ca78ffe4ede71d604385204c9eea0f570b5959f500ce44a523457728f4a180 in / 
# Wed, 17 Nov 2021 02:21:59 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:04679cf5fbf119a40584ce099f7734cc60227b8f675c87a17ec43d547c33471f`  
		Last Modified: Wed, 17 Nov 2021 02:28:17 GMT  
		Size: 31.9 MB (31861397 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20211115-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:326f2b47bd089aa2fe408b57f1b69ec480fbb605a6b9f0c6bf46ab333854a9eb
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.3 MB (29340931 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8c2c90717057e08876b1c3813f99ccd0f26c39aa640a7181f5cbd32b5ff92e1e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:54:21 GMT
ADD file:215c438a676c7b871090a4476ea8f75ae341f64563c5a397a3dd33553600c5e9 in / 
# Wed, 17 Nov 2021 02:54:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c03f61a5f9189e9433eeff88090ed8349d2b5bc24fd7aa4148d6b3118d724bf6`  
		Last Modified: Wed, 17 Nov 2021 03:11:33 GMT  
		Size: 29.3 MB (29340931 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20211115-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:37a1527af0ce25d2107990502426987f82b19aacee7106c2f3336331167d28c1
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.0 MB (26967299 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:afa756464f4bd10c5725d7b34ce9847ff65f720dd21efc428e10981cbdec17ef`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:03:32 GMT
ADD file:de5de93676bef46f60258f3ddf399430054c6aa7c45329247f0543427ae15f03 in / 
# Wed, 17 Nov 2021 02:03:33 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c867d9aa21d68560c825e88ee27e3dbe69d2c504e5ca360aa13a7dc021464327`  
		Last Modified: Wed, 17 Nov 2021 02:20:13 GMT  
		Size: 27.0 MB (26967299 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20211115-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:ad12341c8c774fa9bfb1cf1d2c964193bfc6b5fda1a1d7a8a3387c70fcac56a2
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.9 MB (30877691 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a8660ecd7e0037718ebe1add2156c2a15b0e0772ccf0b78765cc7dccbf754f82`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:57 GMT
ADD file:b854758dd7ed7a6e45cd0275ded71017ffe5e635d746abc50ccb2ccde01b72fe in / 
# Wed, 17 Nov 2021 02:41:57 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:dd0054daee4f80fe294039cf191b9051b3caab8c800e2a76abfae55a825361d0`  
		Last Modified: Wed, 17 Nov 2021 02:50:09 GMT  
		Size: 30.9 MB (30877691 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20211115-slim` - linux; 386

```console
$ docker pull debian@sha256:cc37a5298b26a5598819e4ef963dc7c2517944cd2b25476b144711edec65f14c
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.9 MB (32921347 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cc70c81c25ff4416b18ae66b18903a11ff9af08bed2ee30eadcae2da4ecf4d7b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:41 GMT
ADD file:67a6e5ad800f68c6727aed731577c6d0b569ac1c97817fd1d65ea5f473686b5e in / 
# Wed, 17 Nov 2021 02:41:41 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5de7309bb2a37e880c1c4e00ed924ca1e77c29332da8060c42962f0b85377b54`  
		Last Modified: Wed, 17 Nov 2021 02:51:06 GMT  
		Size: 32.9 MB (32921347 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20211115-slim` - linux; mips64le

```console
$ docker pull debian@sha256:6f422793571435cb1d0bd13cff731ab4daeb9ed58c8a9ed63b34f40ef479ae13
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.5 MB (30470555 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7c4143b24de0ff703d5035065f3474e211c66778d81aa1d42d98365b7354f0bd`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:12:45 GMT
ADD file:9bc8aaeda67d9f523e2edcb16536c65f0beac6e81b7311b7fa061932a1273ef4 in / 
# Wed, 17 Nov 2021 02:12:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2005c405f95df00e46b9bcc0c67109b5543bcc073915b9d46280e0cc6dd21837`  
		Last Modified: Wed, 17 Nov 2021 02:23:07 GMT  
		Size: 30.5 MB (30470555 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20211115-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:dea09c5c90738fc4d1669c5c12e3216a58f69f144559b7bdef9761a8e656df7d
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **36.2 MB (36152626 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4e596195337ffa611b4dea7be7323e39281146da7408979f78a2ce79d4e1e450`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:33:40 GMT
ADD file:094b4b8ef93db4cfcc04e0804b33162bb0c3321c52c336b3c1ede61730ce80ce in / 
# Wed, 17 Nov 2021 03:33:54 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:523157b838dedac06bc09fb4bfa27173b4e1fa65bc9e276d90bbf3fd0c42edd4`  
		Last Modified: Wed, 17 Nov 2021 04:06:26 GMT  
		Size: 36.2 MB (36152626 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20211115-slim` - linux; riscv64

```console
$ docker pull debian@sha256:35e8344917a5eb4797a94d2a7c90b531f9ba0525a5b17e712abe638674e6043b
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.6 MB (27636423 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e83cc7d1c13d1158089661734382fb9dccc9019bfa6581a11762e434517db67c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:16:00 GMT
ADD file:9708d42ec0ce652d2363733fb7b6a0ad792300e7b4d600b0372be14e9d4d25f5 in / 
# Wed, 17 Nov 2021 02:16:02 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a4120655cad9bd6b17587642d5d85365cd1ec359e07e9fed678fde6a248266ea`  
		Last Modified: Wed, 17 Nov 2021 02:32:15 GMT  
		Size: 27.6 MB (27636423 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-20211115-slim` - linux; s390x

```console
$ docker pull debian@sha256:74e218e67754f746ee2e345b50cfdf1b63e71accfa8d36fe0bc23e8b40201cc4
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30074875 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c9c57c5075feeb389a71e8b929ea0d9f3c6c9ee54634e228014223b6e7f7bd31`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:43:58 GMT
ADD file:d4fc84c3ce9329f6a9bb547746d999cfb812cf2e9c7f4807d765d505d9d0b655 in / 
# Wed, 17 Nov 2021 02:44:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:206d7549f208dbfc8f021bbf9b7789af3f66fdacc5b56cd3b74d6fb9a69eef38`  
		Last Modified: Wed, 17 Nov 2021 02:50:00 GMT  
		Size: 30.1 MB (30074875 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:sid-slim`

```console
$ docker pull debian@sha256:11e687c181702ab9f609bfab84519596a1acc54a57906061bdcef8b741a87637
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:sid-slim` - linux; amd64

```console
$ docker pull debian@sha256:4a1ef0b02b7fd98f40ad00919994d1f2dff0e618050398a23c54ab5c9d3e3aa7
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.9 MB (31861397 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d4d3e8adafeae528f6757d7670baadeb2e7b32a0cf408b57b863754e64e5e9f4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:21:59 GMT
ADD file:35ca78ffe4ede71d604385204c9eea0f570b5959f500ce44a523457728f4a180 in / 
# Wed, 17 Nov 2021 02:21:59 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:04679cf5fbf119a40584ce099f7734cc60227b8f675c87a17ec43d547c33471f`  
		Last Modified: Wed, 17 Nov 2021 02:28:17 GMT  
		Size: 31.9 MB (31861397 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:326f2b47bd089aa2fe408b57f1b69ec480fbb605a6b9f0c6bf46ab333854a9eb
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.3 MB (29340931 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:8c2c90717057e08876b1c3813f99ccd0f26c39aa640a7181f5cbd32b5ff92e1e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:54:21 GMT
ADD file:215c438a676c7b871090a4476ea8f75ae341f64563c5a397a3dd33553600c5e9 in / 
# Wed, 17 Nov 2021 02:54:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c03f61a5f9189e9433eeff88090ed8349d2b5bc24fd7aa4148d6b3118d724bf6`  
		Last Modified: Wed, 17 Nov 2021 03:11:33 GMT  
		Size: 29.3 MB (29340931 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:37a1527af0ce25d2107990502426987f82b19aacee7106c2f3336331167d28c1
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.0 MB (26967299 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:afa756464f4bd10c5725d7b34ce9847ff65f720dd21efc428e10981cbdec17ef`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:03:32 GMT
ADD file:de5de93676bef46f60258f3ddf399430054c6aa7c45329247f0543427ae15f03 in / 
# Wed, 17 Nov 2021 02:03:33 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c867d9aa21d68560c825e88ee27e3dbe69d2c504e5ca360aa13a7dc021464327`  
		Last Modified: Wed, 17 Nov 2021 02:20:13 GMT  
		Size: 27.0 MB (26967299 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:ad12341c8c774fa9bfb1cf1d2c964193bfc6b5fda1a1d7a8a3387c70fcac56a2
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.9 MB (30877691 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a8660ecd7e0037718ebe1add2156c2a15b0e0772ccf0b78765cc7dccbf754f82`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:57 GMT
ADD file:b854758dd7ed7a6e45cd0275ded71017ffe5e635d746abc50ccb2ccde01b72fe in / 
# Wed, 17 Nov 2021 02:41:57 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:dd0054daee4f80fe294039cf191b9051b3caab8c800e2a76abfae55a825361d0`  
		Last Modified: Wed, 17 Nov 2021 02:50:09 GMT  
		Size: 30.9 MB (30877691 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; 386

```console
$ docker pull debian@sha256:cc37a5298b26a5598819e4ef963dc7c2517944cd2b25476b144711edec65f14c
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.9 MB (32921347 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cc70c81c25ff4416b18ae66b18903a11ff9af08bed2ee30eadcae2da4ecf4d7b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:41 GMT
ADD file:67a6e5ad800f68c6727aed731577c6d0b569ac1c97817fd1d65ea5f473686b5e in / 
# Wed, 17 Nov 2021 02:41:41 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5de7309bb2a37e880c1c4e00ed924ca1e77c29332da8060c42962f0b85377b54`  
		Last Modified: Wed, 17 Nov 2021 02:51:06 GMT  
		Size: 32.9 MB (32921347 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; mips64le

```console
$ docker pull debian@sha256:6f422793571435cb1d0bd13cff731ab4daeb9ed58c8a9ed63b34f40ef479ae13
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.5 MB (30470555 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7c4143b24de0ff703d5035065f3474e211c66778d81aa1d42d98365b7354f0bd`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:12:45 GMT
ADD file:9bc8aaeda67d9f523e2edcb16536c65f0beac6e81b7311b7fa061932a1273ef4 in / 
# Wed, 17 Nov 2021 02:12:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2005c405f95df00e46b9bcc0c67109b5543bcc073915b9d46280e0cc6dd21837`  
		Last Modified: Wed, 17 Nov 2021 02:23:07 GMT  
		Size: 30.5 MB (30470555 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:dea09c5c90738fc4d1669c5c12e3216a58f69f144559b7bdef9761a8e656df7d
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **36.2 MB (36152626 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4e596195337ffa611b4dea7be7323e39281146da7408979f78a2ce79d4e1e450`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:33:40 GMT
ADD file:094b4b8ef93db4cfcc04e0804b33162bb0c3321c52c336b3c1ede61730ce80ce in / 
# Wed, 17 Nov 2021 03:33:54 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:523157b838dedac06bc09fb4bfa27173b4e1fa65bc9e276d90bbf3fd0c42edd4`  
		Last Modified: Wed, 17 Nov 2021 04:06:26 GMT  
		Size: 36.2 MB (36152626 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; riscv64

```console
$ docker pull debian@sha256:35e8344917a5eb4797a94d2a7c90b531f9ba0525a5b17e712abe638674e6043b
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.6 MB (27636423 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e83cc7d1c13d1158089661734382fb9dccc9019bfa6581a11762e434517db67c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:16:00 GMT
ADD file:9708d42ec0ce652d2363733fb7b6a0ad792300e7b4d600b0372be14e9d4d25f5 in / 
# Wed, 17 Nov 2021 02:16:02 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a4120655cad9bd6b17587642d5d85365cd1ec359e07e9fed678fde6a248266ea`  
		Last Modified: Wed, 17 Nov 2021 02:32:15 GMT  
		Size: 27.6 MB (27636423 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:sid-slim` - linux; s390x

```console
$ docker pull debian@sha256:74e218e67754f746ee2e345b50cfdf1b63e71accfa8d36fe0bc23e8b40201cc4
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30074875 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c9c57c5075feeb389a71e8b929ea0d9f3c6c9ee54634e228014223b6e7f7bd31`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:43:58 GMT
ADD file:d4fc84c3ce9329f6a9bb547746d999cfb812cf2e9c7f4807d765d505d9d0b655 in / 
# Wed, 17 Nov 2021 02:44:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:206d7549f208dbfc8f021bbf9b7789af3f66fdacc5b56cd3b74d6fb9a69eef38`  
		Last Modified: Wed, 17 Nov 2021 02:50:00 GMT  
		Size: 30.1 MB (30074875 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stable`

```console
$ docker pull debian@sha256:2d5af9ff4ea26030548d9ad24e6f9666b468694c6c0417776db8c6b6cf185f8b
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:stable` - linux; amd64

```console
$ docker pull debian@sha256:5aa2ccf34d9829752e0fdb7b972b60192b6678c52643b23b900c2fed63ba2b8e
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.9 MB (54932796 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:61cf9d218143f53d50bee7321179523e7faaff6277738ef4b2f7f9d521bcd2f9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:22:08 GMT
ADD file:7b727d1890d4b095dff203b031a2a19e353a385750824429519a9f040d8ba95a in / 
# Wed, 17 Nov 2021 02:22:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7e1d655ca55b087f26e6a4d1f91da9a4cc4fa789bfd442795a832c7df3e00d9a`  
		Last Modified: Wed, 17 Nov 2021 02:28:33 GMT  
		Size: 54.9 MB (54932796 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; arm variant v5

```console
$ docker pull debian@sha256:5dd9fc0145fcd3f402f426cd4cc868c2d8aa916739c5b722c0baca953ebd7727
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52467858 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:93d263258eb446d712289c7f63fcaa62323213458274d651b0febdef4835cb7f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:54:47 GMT
ADD file:dc0277c2d1117cc93ebc43a4f89edada90697fad31cfe1d92a3fb87f17f8a9b3 in / 
# Wed, 17 Nov 2021 02:54:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:793e32da4ddbfcd80d9f2c88ccf7f1e4b1f8f3dcbf7b21649551274d2bafa5dd`  
		Last Modified: Wed, 17 Nov 2021 03:12:16 GMT  
		Size: 52.5 MB (52467858 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; arm variant v7

```console
$ docker pull debian@sha256:7266b90cf5fef3a81a7306cebe877ff17f60649d6353c2e75706122ba753a07d
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.1 MB (50134145 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e136c687886529772f4b4202e0f6ba753009b8e7b7828b2fa9f08c432baaee88`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:03:59 GMT
ADD file:ca44c08236db78d5b80510c54cdee9150d6a9d941329c2beae1caac598751e6a in / 
# Wed, 17 Nov 2021 02:04:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:189f1a1f83bebf899831dd3d429cbad6c3a317cf7441cd397d2170d70a38bb63`  
		Last Modified: Wed, 17 Nov 2021 02:20:52 GMT  
		Size: 50.1 MB (50134145 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:a3d11fef82da36cf206e05c80749a4beec0023f796001ab654df5db0e818f007
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53619046 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:247b3cfab3fb2d0cd1c79b913c490aa80a4f3f100561965d5b01382de447bfdf`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:07 GMT
ADD file:1a630039cb75bd98ec72465cbb0b0d8d5c5dc123f40d39f719d2bdd7d3e13210 in / 
# Wed, 17 Nov 2021 02:42:08 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f9d8cc8f1c6c951399aaf87bff2af9b7abf020cb01d5d29652d4654f5bf5e5bf`  
		Last Modified: Wed, 17 Nov 2021 02:50:27 GMT  
		Size: 53.6 MB (53619046 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; 386

```console
$ docker pull debian@sha256:318d3191754c61a669bf8bd6becc48db5f46a60a6f178650ab6056a7d1b359b4
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.9 MB (55937575 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:681305b8d1d47db4bb0cf27471eb403135029d2aab4e65914e980b8a8207d8d5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:54 GMT
ADD file:38ff296db73332304b676e3a4e5824dc9ce4327981ae00635074b24912464bff in / 
# Wed, 17 Nov 2021 02:41:55 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ca2ec0b8fb28a6ea0abe69c11184f760b137b0963c71cbb1039a7fa1ad757e0f`  
		Last Modified: Wed, 17 Nov 2021 02:51:29 GMT  
		Size: 55.9 MB (55937575 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; mips64le

```console
$ docker pull debian@sha256:2e1f7c6748ac9d8106087c4c116a30893084dc172cb8da793a637621e60bfe62
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53183745 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9d5372daa27aa69296e3077679e07576f9b320538e2aaee6f94b38a7e174ccb3`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:13:11 GMT
ADD file:d9591a3b684c28012c5d38989bd97f91aea1a6c8fb10be2fcbdc4deda95400e3 in / 
# Wed, 17 Nov 2021 02:13:12 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b9eee255421b7127efa1307fe2a0eaaf2375a87446e236df18c87f8d2f7dd067`  
		Last Modified: Wed, 17 Nov 2021 02:23:49 GMT  
		Size: 53.2 MB (53183745 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; ppc64le

```console
$ docker pull debian@sha256:f826419799794c7ac2ad0a77e35f0f8c5b587c41107091b682e4eea94481a86a
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.8 MB (58819498 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2fde23417bac2c363c08de30c7d5d6ea1edee069bceba0072e2ed17e2c9d1f0f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:34:18 GMT
ADD file:bdd203d3aa82e66b1a4391f79c6433884ab123bd50edac2f11a721860eb13935 in / 
# Wed, 17 Nov 2021 03:34:27 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:984a1e73e76ac4abb413782fe27b8ab9f8d6a7b98b4ae3940a9c620b678703e7`  
		Last Modified: Wed, 17 Nov 2021 04:08:19 GMT  
		Size: 58.8 MB (58819498 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable` - linux; s390x

```console
$ docker pull debian@sha256:e26952dd2fc70b26814b10e989827cd5e0e3d6039b1b37130d81bc42a4e272e7
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53207157 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3aa95c6f10fc58d0b50f1f37b31c703cdfbbd863c43b5fe76f893e202b426748`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:44:11 GMT
ADD file:f1ea9110d656eb3478ede77049c3fea3eadf89037d13f2b6799ce8c86708778b in / 
# Wed, 17 Nov 2021 02:44:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7a62fd5c373212657c719868b2144dd6f23387e47af5bda68af8785210025ec9`  
		Last Modified: Wed, 17 Nov 2021 02:50:12 GMT  
		Size: 53.2 MB (53207157 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stable-20211115`

```console
$ docker pull debian@sha256:2d5af9ff4ea26030548d9ad24e6f9666b468694c6c0417776db8c6b6cf185f8b
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:stable-20211115` - linux; amd64

```console
$ docker pull debian@sha256:5aa2ccf34d9829752e0fdb7b972b60192b6678c52643b23b900c2fed63ba2b8e
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.9 MB (54932796 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:61cf9d218143f53d50bee7321179523e7faaff6277738ef4b2f7f9d521bcd2f9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:22:08 GMT
ADD file:7b727d1890d4b095dff203b031a2a19e353a385750824429519a9f040d8ba95a in / 
# Wed, 17 Nov 2021 02:22:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7e1d655ca55b087f26e6a4d1f91da9a4cc4fa789bfd442795a832c7df3e00d9a`  
		Last Modified: Wed, 17 Nov 2021 02:28:33 GMT  
		Size: 54.9 MB (54932796 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20211115` - linux; arm variant v5

```console
$ docker pull debian@sha256:5dd9fc0145fcd3f402f426cd4cc868c2d8aa916739c5b722c0baca953ebd7727
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52467858 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:93d263258eb446d712289c7f63fcaa62323213458274d651b0febdef4835cb7f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:54:47 GMT
ADD file:dc0277c2d1117cc93ebc43a4f89edada90697fad31cfe1d92a3fb87f17f8a9b3 in / 
# Wed, 17 Nov 2021 02:54:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:793e32da4ddbfcd80d9f2c88ccf7f1e4b1f8f3dcbf7b21649551274d2bafa5dd`  
		Last Modified: Wed, 17 Nov 2021 03:12:16 GMT  
		Size: 52.5 MB (52467858 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20211115` - linux; arm variant v7

```console
$ docker pull debian@sha256:7266b90cf5fef3a81a7306cebe877ff17f60649d6353c2e75706122ba753a07d
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.1 MB (50134145 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e136c687886529772f4b4202e0f6ba753009b8e7b7828b2fa9f08c432baaee88`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:03:59 GMT
ADD file:ca44c08236db78d5b80510c54cdee9150d6a9d941329c2beae1caac598751e6a in / 
# Wed, 17 Nov 2021 02:04:00 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:189f1a1f83bebf899831dd3d429cbad6c3a317cf7441cd397d2170d70a38bb63`  
		Last Modified: Wed, 17 Nov 2021 02:20:52 GMT  
		Size: 50.1 MB (50134145 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20211115` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:a3d11fef82da36cf206e05c80749a4beec0023f796001ab654df5db0e818f007
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53619046 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:247b3cfab3fb2d0cd1c79b913c490aa80a4f3f100561965d5b01382de447bfdf`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:07 GMT
ADD file:1a630039cb75bd98ec72465cbb0b0d8d5c5dc123f40d39f719d2bdd7d3e13210 in / 
# Wed, 17 Nov 2021 02:42:08 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f9d8cc8f1c6c951399aaf87bff2af9b7abf020cb01d5d29652d4654f5bf5e5bf`  
		Last Modified: Wed, 17 Nov 2021 02:50:27 GMT  
		Size: 53.6 MB (53619046 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20211115` - linux; 386

```console
$ docker pull debian@sha256:318d3191754c61a669bf8bd6becc48db5f46a60a6f178650ab6056a7d1b359b4
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.9 MB (55937575 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:681305b8d1d47db4bb0cf27471eb403135029d2aab4e65914e980b8a8207d8d5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:54 GMT
ADD file:38ff296db73332304b676e3a4e5824dc9ce4327981ae00635074b24912464bff in / 
# Wed, 17 Nov 2021 02:41:55 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ca2ec0b8fb28a6ea0abe69c11184f760b137b0963c71cbb1039a7fa1ad757e0f`  
		Last Modified: Wed, 17 Nov 2021 02:51:29 GMT  
		Size: 55.9 MB (55937575 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20211115` - linux; mips64le

```console
$ docker pull debian@sha256:2e1f7c6748ac9d8106087c4c116a30893084dc172cb8da793a637621e60bfe62
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53183745 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9d5372daa27aa69296e3077679e07576f9b320538e2aaee6f94b38a7e174ccb3`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:13:11 GMT
ADD file:d9591a3b684c28012c5d38989bd97f91aea1a6c8fb10be2fcbdc4deda95400e3 in / 
# Wed, 17 Nov 2021 02:13:12 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b9eee255421b7127efa1307fe2a0eaaf2375a87446e236df18c87f8d2f7dd067`  
		Last Modified: Wed, 17 Nov 2021 02:23:49 GMT  
		Size: 53.2 MB (53183745 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20211115` - linux; ppc64le

```console
$ docker pull debian@sha256:f826419799794c7ac2ad0a77e35f0f8c5b587c41107091b682e4eea94481a86a
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.8 MB (58819498 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2fde23417bac2c363c08de30c7d5d6ea1edee069bceba0072e2ed17e2c9d1f0f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:34:18 GMT
ADD file:bdd203d3aa82e66b1a4391f79c6433884ab123bd50edac2f11a721860eb13935 in / 
# Wed, 17 Nov 2021 03:34:27 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:984a1e73e76ac4abb413782fe27b8ab9f8d6a7b98b4ae3940a9c620b678703e7`  
		Last Modified: Wed, 17 Nov 2021 04:08:19 GMT  
		Size: 58.8 MB (58819498 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20211115` - linux; s390x

```console
$ docker pull debian@sha256:e26952dd2fc70b26814b10e989827cd5e0e3d6039b1b37130d81bc42a4e272e7
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53207157 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3aa95c6f10fc58d0b50f1f37b31c703cdfbbd863c43b5fe76f893e202b426748`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:44:11 GMT
ADD file:f1ea9110d656eb3478ede77049c3fea3eadf89037d13f2b6799ce8c86708778b in / 
# Wed, 17 Nov 2021 02:44:13 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:7a62fd5c373212657c719868b2144dd6f23387e47af5bda68af8785210025ec9`  
		Last Modified: Wed, 17 Nov 2021 02:50:12 GMT  
		Size: 53.2 MB (53207157 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stable-20211115-slim`

```console
$ docker pull debian@sha256:ecdf2f8d8710759f075ba30d161881b9ce7aa7f928c8194f460d78770391f852
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:stable-20211115-slim` - linux; amd64

```console
$ docker pull debian@sha256:14a2ae54e3086a3699b7b03fdad3106428a18508ce1a51ff39402a988ccd5894
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.4 MB (31370262 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d35b6367cf8fdd6af19566f069a5fc8e605371d67d5745b98095d448b28a80a9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:22:19 GMT
ADD file:a52850cd604811e39d12afbf272220cb7794a17049a0b4e6414e03dc8c632a23 in / 
# Wed, 17 Nov 2021 02:22:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:926316d23c60cb746c96ebc54245460d208f34810847693abd043260e5ce4cb3`  
		Last Modified: Wed, 17 Nov 2021 02:28:53 GMT  
		Size: 31.4 MB (31370262 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20211115-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:8c27dc2ab2cdbe1f9b210baa5ae38ede33fc1aed46295cd6653954bad0bdd637
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **28.9 MB (28910963 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:51e179fb1c7f2fc2e21494da342761b99d3999bf3f8ae7d764910d84932d8e4b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:55:19 GMT
ADD file:85f459b8e89ad3d6b603335f69ce5f419642095b076dc82a896830215ab11298 in / 
# Wed, 17 Nov 2021 02:55:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c04810969f578f3bd79096cbe2dc1dc16aa685ddc5458b0816c4ea4a8f8b0dff`  
		Last Modified: Wed, 17 Nov 2021 03:12:54 GMT  
		Size: 28.9 MB (28910963 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20211115-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:20331ff7717c103c4fd6696475f9a9633fa1999428948eca74b2b3aec49e5e0e
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.6 MB (26573135 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bfe87a59337b2ae530378df895b47363db9991f7faea4760069e94ee5f4189da`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:04:32 GMT
ADD file:ca90e95a57870c0acf9845f9e4d9c99e6a62c90a1de7acb84a25d92857dfabd1 in / 
# Wed, 17 Nov 2021 02:04:33 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8b0ed0330e95ee9108c1342726ef8fe08120e1bd9940ed5e3313bfb1fe378260`  
		Last Modified: Wed, 17 Nov 2021 02:21:26 GMT  
		Size: 26.6 MB (26573135 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20211115-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:c69c375904b8c97b411fdec4d364021b3647f31c9b0dad733e0fc8f1d531a1c1
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30056531 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d88a20c52b0d5b56be1909fd4ad70e1072176f96b435a6389bb72dbf187973e9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:23 GMT
ADD file:acb3d490321b424e8a2debb32ac32e3885c4f3a514cc3d0a9d5b29eaaf3c3b53 in / 
# Wed, 17 Nov 2021 02:42:23 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:babc46e68c552728276086eb765c80586f0d11388994ce268a654f5a7275cbed`  
		Last Modified: Wed, 17 Nov 2021 02:50:55 GMT  
		Size: 30.1 MB (30056531 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20211115-slim` - linux; 386

```console
$ docker pull debian@sha256:117c770ca2834b2b53fcfa6b4bda42f8a84b8f56ed042c568a8de70eb08eb2bf
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32380718 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b22557acd88c1aa6a6aea9706f40209ba5e525270e48b05bc660f4254ab827ad`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:10 GMT
ADD file:8b0d6c3d94b022343f86c374eea66681087a3d2c76f9a732a6d973e6184c23a2 in / 
# Wed, 17 Nov 2021 02:42:10 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:dde91c21c2824ee2e16d73c739625133b025a69ce7f5daaaf208cd823eb16e4e`  
		Last Modified: Wed, 17 Nov 2021 02:51:54 GMT  
		Size: 32.4 MB (32380718 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20211115-slim` - linux; mips64le

```console
$ docker pull debian@sha256:1aa5b83c7a12715fb5bfe7175e7659c4b5286cef5df56ea5f3e45984e8d54aee
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.6 MB (29630350 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2ca847b3cec1950cc72063aab005e91673e33e15e258fd61f046a7dc65857ba9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:13:38 GMT
ADD file:c6beead72a0a40c5ffc27d802480298bc2cb27c1b2b0961e1cd40e178638dfd9 in / 
# Wed, 17 Nov 2021 02:13:39 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0100545cbb1abaf5e8ac27cb70c2709a1bf0aa67d416410dee4337d2be2c60ca`  
		Last Modified: Wed, 17 Nov 2021 02:24:27 GMT  
		Size: 29.6 MB (29630350 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20211115-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:13b8aa8d7027c4e59134293a0998939ee8658345e94d6afa7a59335f275b93a3
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **35.3 MB (35271400 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2041aad231fe8ede91221fcf5a6bbdf749efc16efb350e9fa811c40c0bb4de85`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:35:14 GMT
ADD file:efd65507e25a160145d8fed0c44e5574ddfbe3183b7bca2cd4d2123ed5d30009 in / 
# Wed, 17 Nov 2021 03:35:24 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:52706240cba10e08061a7dc4d5fb442d2e07f70b07a406e5ae5a46838de30783`  
		Last Modified: Wed, 17 Nov 2021 04:11:05 GMT  
		Size: 35.3 MB (35271400 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-20211115-slim` - linux; s390x

```console
$ docker pull debian@sha256:8e3d2a10296a1f4f9fac697a2e5b84169e4ac0e3db0d7fd5140516545a3f6fa6
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.7 MB (29650974 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4f02c21ff01885536d9d659e577790a9d2cedde3d05d0f703485e786ddb530a5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:44:29 GMT
ADD file:7c80ce6d46566c78232e666bd96129dd3a81d391da8816117f628692aecb341a in / 
# Wed, 17 Nov 2021 02:44:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e86d4926eb0eebd71976fc0b1d79578dbfe802cc829dde77d31694bbd3fa71dc`  
		Last Modified: Wed, 17 Nov 2021 02:50:27 GMT  
		Size: 29.7 MB (29650974 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stable-backports`

```console
$ docker pull debian@sha256:5bf131da8d62070779c8bc08cdc421f10828f8bb255ac9da72ae7565db6ff8ed
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:stable-backports` - linux; amd64

```console
$ docker pull debian@sha256:a9e588d7d476d00648bf1365709b4eda3970e1e912dec00132647728f765b986
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.9 MB (54933019 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1f1b27a1422147523451dae0c7b818c307488bdb67beb78d8c60bc7ddf26f935`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:22:08 GMT
ADD file:7b727d1890d4b095dff203b031a2a19e353a385750824429519a9f040d8ba95a in / 
# Wed, 17 Nov 2021 02:22:09 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:22:12 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:7e1d655ca55b087f26e6a4d1f91da9a4cc4fa789bfd442795a832c7df3e00d9a`  
		Last Modified: Wed, 17 Nov 2021 02:28:33 GMT  
		Size: 54.9 MB (54932796 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:d008e1150512b7fa05e2d552c4bd757024deb58c5f018bec01e22d4455d8757c`  
		Last Modified: Wed, 17 Nov 2021 02:28:42 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:639a9aa0feed80dfea328cffb6e717ff1aaa0e16f0896734e94ed18bb339a889
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **52.5 MB (52468083 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a97d8e81279709a13221d6f98915ad5ad1181b2b320a24c47ee311e5f4fe3052`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:54:47 GMT
ADD file:dc0277c2d1117cc93ebc43a4f89edada90697fad31cfe1d92a3fb87f17f8a9b3 in / 
# Wed, 17 Nov 2021 02:54:48 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:55:01 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:793e32da4ddbfcd80d9f2c88ccf7f1e4b1f8f3dcbf7b21649551274d2bafa5dd`  
		Last Modified: Wed, 17 Nov 2021 03:12:16 GMT  
		Size: 52.5 MB (52467858 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:0f0aecba74d1721ca83227f246931939b0c3d0e0ab07a2e516e3c2df44f5c386`  
		Last Modified: Wed, 17 Nov 2021 03:12:28 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:a0a962ea97c7eb3e7f392816727f7ce54b15e0314cb8464dd0bad786aae7dd64
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.1 MB (50134369 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a26fc5ba15848f589fec1b4d50fdc2aebc3170114d9a80eba6699a072e4e1f54`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:03:59 GMT
ADD file:ca44c08236db78d5b80510c54cdee9150d6a9d941329c2beae1caac598751e6a in / 
# Wed, 17 Nov 2021 02:04:00 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:04:13 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:189f1a1f83bebf899831dd3d429cbad6c3a317cf7441cd397d2170d70a38bb63`  
		Last Modified: Wed, 17 Nov 2021 02:20:52 GMT  
		Size: 50.1 MB (50134145 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:64b577c408d3e309c68e7e84153c035102adc60605230ac923be81173287988e`  
		Last Modified: Wed, 17 Nov 2021 02:21:03 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:b7d77d784950050b9142e76ff2a4332f3d703ebe7d870ea351a4424430146df5
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.6 MB (53619268 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d1080748dd6feefa12216691874d26c7285642aa8a08df2e753f2e9d88a6d9d4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:07 GMT
ADD file:1a630039cb75bd98ec72465cbb0b0d8d5c5dc123f40d39f719d2bdd7d3e13210 in / 
# Wed, 17 Nov 2021 02:42:08 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:42:15 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:f9d8cc8f1c6c951399aaf87bff2af9b7abf020cb01d5d29652d4654f5bf5e5bf`  
		Last Modified: Wed, 17 Nov 2021 02:50:27 GMT  
		Size: 53.6 MB (53619046 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:27fbc4590ebb15316daaed18526114b8809eab2fa6dd2c42e1ce5811ad0500d3`  
		Last Modified: Wed, 17 Nov 2021 02:50:38 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; 386

```console
$ docker pull debian@sha256:6107cd62b5d53991f2f61753044244ad4c580e8e2ba45d1a7c3b2d155d907f59
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.9 MB (55937796 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:86ecbb7566b708aea1a420ff32e15567b664600fa67dde761660df9bded5867a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:41:54 GMT
ADD file:38ff296db73332304b676e3a4e5824dc9ce4327981ae00635074b24912464bff in / 
# Wed, 17 Nov 2021 02:41:55 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:42:01 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:ca2ec0b8fb28a6ea0abe69c11184f760b137b0963c71cbb1039a7fa1ad757e0f`  
		Last Modified: Wed, 17 Nov 2021 02:51:29 GMT  
		Size: 55.9 MB (55937575 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:ba31ead7ae256e458b038ab313a0f3015006f62508163e4d98a10c36cc5330ac`  
		Last Modified: Wed, 17 Nov 2021 02:51:40 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; mips64le

```console
$ docker pull debian@sha256:9690345c0d0d1d882c0e935beca1d71525bf5aedb48d96f0b560004bf47e4a9e
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53183968 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:95800ebf7376002f7c7d3e1fedc6a14df0d2bc1f98a6540b69087f5a24fe4712`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:13:11 GMT
ADD file:d9591a3b684c28012c5d38989bd97f91aea1a6c8fb10be2fcbdc4deda95400e3 in / 
# Wed, 17 Nov 2021 02:13:12 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:13:19 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:b9eee255421b7127efa1307fe2a0eaaf2375a87446e236df18c87f8d2f7dd067`  
		Last Modified: Wed, 17 Nov 2021 02:23:49 GMT  
		Size: 53.2 MB (53183745 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:c034ff3c389aba87742de0c03d82fd56f2677bb1da0f9774b33def1c4650411a`  
		Last Modified: Wed, 17 Nov 2021 02:23:59 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; ppc64le

```console
$ docker pull debian@sha256:093b7a7fc569488b0788cfb6e340bd6984f46a44385ce0f2f43022df9966cce3
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **58.8 MB (58819723 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c184c267bc27dee45fe273f1262a62a7f23b160dcf0bb09b1f54777696a772a7`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:34:18 GMT
ADD file:bdd203d3aa82e66b1a4391f79c6433884ab123bd50edac2f11a721860eb13935 in / 
# Wed, 17 Nov 2021 03:34:27 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 03:34:55 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:984a1e73e76ac4abb413782fe27b8ab9f8d6a7b98b4ae3940a9c620b678703e7`  
		Last Modified: Wed, 17 Nov 2021 04:08:19 GMT  
		Size: 58.8 MB (58819498 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:75f5cb40f8bb5f9ae96513693e2f5ff1c7831cdb26be3dff78f1078c5488d5f0`  
		Last Modified: Wed, 17 Nov 2021 04:08:31 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-backports` - linux; s390x

```console
$ docker pull debian@sha256:4c4742de8edf8878fd23c7cd11e2a71a07ee3230b604143237af932d6ec1ac09
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53207379 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c3e76f0a1673c1760155300519c0fa60733713fab06ff835210b7f2e77823082`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:44:11 GMT
ADD file:f1ea9110d656eb3478ede77049c3fea3eadf89037d13f2b6799ce8c86708778b in / 
# Wed, 17 Nov 2021 02:44:13 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:44:20 GMT
RUN echo 'deb http://deb.debian.org/debian stable-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:7a62fd5c373212657c719868b2144dd6f23387e47af5bda68af8785210025ec9`  
		Last Modified: Wed, 17 Nov 2021 02:50:12 GMT  
		Size: 53.2 MB (53207157 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:dc1393d13e4a191ed8615f817e0422c47b3976ea05c8ea4c2688b4492d0e5bd2`  
		Last Modified: Wed, 17 Nov 2021 02:50:19 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stable-slim`

```console
$ docker pull debian@sha256:ecdf2f8d8710759f075ba30d161881b9ce7aa7f928c8194f460d78770391f852
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:stable-slim` - linux; amd64

```console
$ docker pull debian@sha256:14a2ae54e3086a3699b7b03fdad3106428a18508ce1a51ff39402a988ccd5894
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.4 MB (31370262 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d35b6367cf8fdd6af19566f069a5fc8e605371d67d5745b98095d448b28a80a9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:22:19 GMT
ADD file:a52850cd604811e39d12afbf272220cb7794a17049a0b4e6414e03dc8c632a23 in / 
# Wed, 17 Nov 2021 02:22:19 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:926316d23c60cb746c96ebc54245460d208f34810847693abd043260e5ce4cb3`  
		Last Modified: Wed, 17 Nov 2021 02:28:53 GMT  
		Size: 31.4 MB (31370262 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:8c27dc2ab2cdbe1f9b210baa5ae38ede33fc1aed46295cd6653954bad0bdd637
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **28.9 MB (28910963 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:51e179fb1c7f2fc2e21494da342761b99d3999bf3f8ae7d764910d84932d8e4b`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:55:19 GMT
ADD file:85f459b8e89ad3d6b603335f69ce5f419642095b076dc82a896830215ab11298 in / 
# Wed, 17 Nov 2021 02:55:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c04810969f578f3bd79096cbe2dc1dc16aa685ddc5458b0816c4ea4a8f8b0dff`  
		Last Modified: Wed, 17 Nov 2021 03:12:54 GMT  
		Size: 28.9 MB (28910963 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:20331ff7717c103c4fd6696475f9a9633fa1999428948eca74b2b3aec49e5e0e
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.6 MB (26573135 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:bfe87a59337b2ae530378df895b47363db9991f7faea4760069e94ee5f4189da`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:04:32 GMT
ADD file:ca90e95a57870c0acf9845f9e4d9c99e6a62c90a1de7acb84a25d92857dfabd1 in / 
# Wed, 17 Nov 2021 02:04:33 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8b0ed0330e95ee9108c1342726ef8fe08120e1bd9940ed5e3313bfb1fe378260`  
		Last Modified: Wed, 17 Nov 2021 02:21:26 GMT  
		Size: 26.6 MB (26573135 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:c69c375904b8c97b411fdec4d364021b3647f31c9b0dad733e0fc8f1d531a1c1
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30056531 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d88a20c52b0d5b56be1909fd4ad70e1072176f96b435a6389bb72dbf187973e9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:23 GMT
ADD file:acb3d490321b424e8a2debb32ac32e3885c4f3a514cc3d0a9d5b29eaaf3c3b53 in / 
# Wed, 17 Nov 2021 02:42:23 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:babc46e68c552728276086eb765c80586f0d11388994ce268a654f5a7275cbed`  
		Last Modified: Wed, 17 Nov 2021 02:50:55 GMT  
		Size: 30.1 MB (30056531 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; 386

```console
$ docker pull debian@sha256:117c770ca2834b2b53fcfa6b4bda42f8a84b8f56ed042c568a8de70eb08eb2bf
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.4 MB (32380718 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b22557acd88c1aa6a6aea9706f40209ba5e525270e48b05bc660f4254ab827ad`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:10 GMT
ADD file:8b0d6c3d94b022343f86c374eea66681087a3d2c76f9a732a6d973e6184c23a2 in / 
# Wed, 17 Nov 2021 02:42:10 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:dde91c21c2824ee2e16d73c739625133b025a69ce7f5daaaf208cd823eb16e4e`  
		Last Modified: Wed, 17 Nov 2021 02:51:54 GMT  
		Size: 32.4 MB (32380718 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; mips64le

```console
$ docker pull debian@sha256:1aa5b83c7a12715fb5bfe7175e7659c4b5286cef5df56ea5f3e45984e8d54aee
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.6 MB (29630350 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2ca847b3cec1950cc72063aab005e91673e33e15e258fd61f046a7dc65857ba9`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:13:38 GMT
ADD file:c6beead72a0a40c5ffc27d802480298bc2cb27c1b2b0961e1cd40e178638dfd9 in / 
# Wed, 17 Nov 2021 02:13:39 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0100545cbb1abaf5e8ac27cb70c2709a1bf0aa67d416410dee4337d2be2c60ca`  
		Last Modified: Wed, 17 Nov 2021 02:24:27 GMT  
		Size: 29.6 MB (29630350 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:13b8aa8d7027c4e59134293a0998939ee8658345e94d6afa7a59335f275b93a3
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **35.3 MB (35271400 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2041aad231fe8ede91221fcf5a6bbdf749efc16efb350e9fa811c40c0bb4de85`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:35:14 GMT
ADD file:efd65507e25a160145d8fed0c44e5574ddfbe3183b7bca2cd4d2123ed5d30009 in / 
# Wed, 17 Nov 2021 03:35:24 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:52706240cba10e08061a7dc4d5fb442d2e07f70b07a406e5ae5a46838de30783`  
		Last Modified: Wed, 17 Nov 2021 04:11:05 GMT  
		Size: 35.3 MB (35271400 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stable-slim` - linux; s390x

```console
$ docker pull debian@sha256:8e3d2a10296a1f4f9fac697a2e5b84169e4ac0e3db0d7fd5140516545a3f6fa6
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.7 MB (29650974 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4f02c21ff01885536d9d659e577790a9d2cedde3d05d0f703485e786ddb530a5`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:44:29 GMT
ADD file:7c80ce6d46566c78232e666bd96129dd3a81d391da8816117f628692aecb341a in / 
# Wed, 17 Nov 2021 02:44:31 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e86d4926eb0eebd71976fc0b1d79578dbfe802cc829dde77d31694bbd3fa71dc`  
		Last Modified: Wed, 17 Nov 2021 02:50:27 GMT  
		Size: 29.7 MB (29650974 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stretch`

```console
$ docker pull debian@sha256:d8ee86bf0afeb901de293c692a540307a670306cbdb7a06e2c840f17b0c35374
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 5
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:stretch` - linux; amd64

```console
$ docker pull debian@sha256:a8eab13d0f587368d77fc953a8cb8a665a976c046c419d19be7af613554f990d
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.4 MB (45380444 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:066e7b4151cfe95cbd9c2244792a815c5b32bb380fe4ec851917569c6f750664`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:22:28 GMT
ADD file:d7365dd9cf34b10ca189f85c95c21a0c33e44092f85ffb5884d5e737fb0b9be1 in / 
# Wed, 17 Nov 2021 02:22:28 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:200cae5c9d88bb9cf1dd3fcfb831d671403f078d2310416fa3b304337d8279c0`  
		Last Modified: Wed, 17 Nov 2021 02:29:09 GMT  
		Size: 45.4 MB (45380444 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch` - linux; arm variant v5

```console
$ docker pull debian@sha256:37693dbcd033d85fbb9b15e32d711a418ae8f30f756210d8b1abb489e252225b
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **44.1 MB (44091379 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e84cd38de4fc6ca9807b62c600666c5599de965df3c10aae21c8e20049e176bd`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:55:43 GMT
ADD file:a8b0f40ff55722f4bb2e3ac65c3fefe9acaa513f89b73ed81ec04585058e58ba in / 
# Wed, 17 Nov 2021 02:55:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ec68ab8028a430064eed52d9b833f1a135dcce543b02666ee5acd73962b79070`  
		Last Modified: Wed, 17 Nov 2021 03:13:32 GMT  
		Size: 44.1 MB (44091379 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch` - linux; arm variant v7

```console
$ docker pull debian@sha256:9029da5bf6cbb6199e5f6d0eed4aaeaad998668abe02d29ea294ad2a3351e98e
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **42.1 MB (42116868 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6e2da5cf76886cdc5bdc612c501e562f2fce8be0d0aa51ec37bb1f39c4e32e5e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:04:57 GMT
ADD file:61da3fcef3aea99b4927189cfcb823a65b0e911a3f4436e44ea84a57a7919ff3 in / 
# Wed, 17 Nov 2021 02:04:58 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e4d54b576c2fcdd259e57bac14c4e1a2623f6e5dc9a0b1facaed7fe59d891e86`  
		Last Modified: Wed, 17 Nov 2021 02:22:03 GMT  
		Size: 42.1 MB (42116868 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:70f4f03a8fd72cf60c3f8023f010721fece3fe599c9e7bc6eae10024eaa6a128
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **43.2 MB (43174478 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fad7665a3afc5981cd910850721e9f91cbf791c0962e4e250e298cee3059a4e0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:33 GMT
ADD file:3225671be4edc3599e29ebd06516b95573246e68bf29649c402ed87c03c109b5 in / 
# Wed, 17 Nov 2021 02:42:34 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c64a4bafe4ad6fc05fcc6d76d2a06f168f331ffbc87e518fb4ca4ea22e75527e`  
		Last Modified: Wed, 17 Nov 2021 02:51:11 GMT  
		Size: 43.2 MB (43174478 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch` - linux; 386

```console
$ docker pull debian@sha256:baa27aa91f579ac7cb42c594b2202ff5184fa4a64d7e88f0e1486725f61b0de2
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **46.1 MB (46097202 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7b1834ace8ddd99da39204c4a6faec3b1501af684a54e103348a966587edfe1a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:21 GMT
ADD file:8e57eaeffee1cd4c93865af9d6370c68bddae095016766191b1338ae675f131c in / 
# Wed, 17 Nov 2021 02:42:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:24fc2b60c45394a399438249c8e7f92f6fa415f9c477524d5af3ab4782d51519`  
		Last Modified: Wed, 17 Nov 2021 02:52:15 GMT  
		Size: 46.1 MB (46097202 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stretch-20211115`

```console
$ docker pull debian@sha256:d8ee86bf0afeb901de293c692a540307a670306cbdb7a06e2c840f17b0c35374
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 5
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:stretch-20211115` - linux; amd64

```console
$ docker pull debian@sha256:a8eab13d0f587368d77fc953a8cb8a665a976c046c419d19be7af613554f990d
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.4 MB (45380444 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:066e7b4151cfe95cbd9c2244792a815c5b32bb380fe4ec851917569c6f750664`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:22:28 GMT
ADD file:d7365dd9cf34b10ca189f85c95c21a0c33e44092f85ffb5884d5e737fb0b9be1 in / 
# Wed, 17 Nov 2021 02:22:28 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:200cae5c9d88bb9cf1dd3fcfb831d671403f078d2310416fa3b304337d8279c0`  
		Last Modified: Wed, 17 Nov 2021 02:29:09 GMT  
		Size: 45.4 MB (45380444 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-20211115` - linux; arm variant v5

```console
$ docker pull debian@sha256:37693dbcd033d85fbb9b15e32d711a418ae8f30f756210d8b1abb489e252225b
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **44.1 MB (44091379 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e84cd38de4fc6ca9807b62c600666c5599de965df3c10aae21c8e20049e176bd`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:55:43 GMT
ADD file:a8b0f40ff55722f4bb2e3ac65c3fefe9acaa513f89b73ed81ec04585058e58ba in / 
# Wed, 17 Nov 2021 02:55:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ec68ab8028a430064eed52d9b833f1a135dcce543b02666ee5acd73962b79070`  
		Last Modified: Wed, 17 Nov 2021 03:13:32 GMT  
		Size: 44.1 MB (44091379 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-20211115` - linux; arm variant v7

```console
$ docker pull debian@sha256:9029da5bf6cbb6199e5f6d0eed4aaeaad998668abe02d29ea294ad2a3351e98e
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **42.1 MB (42116868 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6e2da5cf76886cdc5bdc612c501e562f2fce8be0d0aa51ec37bb1f39c4e32e5e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:04:57 GMT
ADD file:61da3fcef3aea99b4927189cfcb823a65b0e911a3f4436e44ea84a57a7919ff3 in / 
# Wed, 17 Nov 2021 02:04:58 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e4d54b576c2fcdd259e57bac14c4e1a2623f6e5dc9a0b1facaed7fe59d891e86`  
		Last Modified: Wed, 17 Nov 2021 02:22:03 GMT  
		Size: 42.1 MB (42116868 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-20211115` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:70f4f03a8fd72cf60c3f8023f010721fece3fe599c9e7bc6eae10024eaa6a128
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **43.2 MB (43174478 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fad7665a3afc5981cd910850721e9f91cbf791c0962e4e250e298cee3059a4e0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:33 GMT
ADD file:3225671be4edc3599e29ebd06516b95573246e68bf29649c402ed87c03c109b5 in / 
# Wed, 17 Nov 2021 02:42:34 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:c64a4bafe4ad6fc05fcc6d76d2a06f168f331ffbc87e518fb4ca4ea22e75527e`  
		Last Modified: Wed, 17 Nov 2021 02:51:11 GMT  
		Size: 43.2 MB (43174478 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-20211115` - linux; 386

```console
$ docker pull debian@sha256:baa27aa91f579ac7cb42c594b2202ff5184fa4a64d7e88f0e1486725f61b0de2
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **46.1 MB (46097202 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:7b1834ace8ddd99da39204c4a6faec3b1501af684a54e103348a966587edfe1a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:21 GMT
ADD file:8e57eaeffee1cd4c93865af9d6370c68bddae095016766191b1338ae675f131c in / 
# Wed, 17 Nov 2021 02:42:22 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:24fc2b60c45394a399438249c8e7f92f6fa415f9c477524d5af3ab4782d51519`  
		Last Modified: Wed, 17 Nov 2021 02:52:15 GMT  
		Size: 46.1 MB (46097202 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stretch-20211115-slim`

```console
$ docker pull debian@sha256:d8566545bd3750e270b8aaf703d0a048fda080cd287a249823a5a11b7c95adbf
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 5
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:stretch-20211115-slim` - linux; amd64

```console
$ docker pull debian@sha256:bd4c41c9e68cc553f0c5266f18bef07491c52a08752f81a015892c2dc5e15114
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.5 MB (22527686 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a692eb442a03726bc7790f146f97afe590a942a526ec282f4c4d1bb724a9443a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:22:38 GMT
ADD file:2868c3af63afe6b8aadac07b8776e5468a3ed135fd84ad22df15e48f0610c7ba in / 
# Wed, 17 Nov 2021 02:22:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:89166b5eeae47d8c878cb88f20cc67eda2a550a697f4be317c7b7abea566b76f`  
		Last Modified: Wed, 17 Nov 2021 02:29:32 GMT  
		Size: 22.5 MB (22527686 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-20211115-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:2aa2a15f320cd740783a33c547f4cd885d0b6e5b1ba3860024f8c8badc368c25
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **21.2 MB (21204234 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9e45f7a72e70f7c7a8193bfb2fdd770e1fa690422cb7f05e2a1332f8f8f63d60`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:56:16 GMT
ADD file:b857fb98b0880bbb3add8dd317cd1eb0f339332d4836baaedc8f2f424b5f2c98 in / 
# Wed, 17 Nov 2021 02:56:17 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ca67a5e1c16d08a9b8520dae81a39d168ef2a816d8905715eb6a328c89a2ce37`  
		Last Modified: Wed, 17 Nov 2021 03:14:12 GMT  
		Size: 21.2 MB (21204234 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-20211115-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:9a52fdfda53f199f56b7d9a7d4dcc78a655da675926c88c1f4c4af044f9a5b79
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **19.3 MB (19316397 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:eecd35162e59af35e26178b7706ea9d74dc473fef1aed9fd0545734ab75f1304`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:05:31 GMT
ADD file:f28546ff6a3759d3a957c25d4310c3065e1367dff4ae304caae1cf1dc64d061d in / 
# Wed, 17 Nov 2021 02:05:32 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:614e23553da11f1294dc969bcab416c9f981794bdbaee5641335a222730c63b2`  
		Last Modified: Wed, 17 Nov 2021 02:22:41 GMT  
		Size: 19.3 MB (19316397 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-20211115-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:b1902145e37b8fdd8c48092c245efcd8ac341091ed7ff3dbf91d7495bd293d88
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **20.4 MB (20389442 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:db5e084a09bc69d57eb2f658d7c5fa96910e232ee934ba9d2119a0d05fb7c5ed`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:48 GMT
ADD file:2adad5eee701d55a9f58c07f0706eb574d0ad6b74b0cc52a9e622f50639961c9 in / 
# Wed, 17 Nov 2021 02:42:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:34b92b588f0f390db0d125ab0ee4c85510c52367371c4bcf5af80a6ab18f0a9b`  
		Last Modified: Wed, 17 Nov 2021 02:51:38 GMT  
		Size: 20.4 MB (20389442 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-20211115-slim` - linux; 386

```console
$ docker pull debian@sha256:043ab2d585630cc218d3b5b0e09038405e13bff9585bee28cc02f5c93f9e9f86
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **23.2 MB (23156564 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:143712197327374e7944c356bf4336da6737d3df4709128da5224bfd486aa8dc`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:36 GMT
ADD file:3c9c1d0f88db0341cbe43b594cf6d2d865e39d9518b8e2b124cb9736bebe18f3 in / 
# Wed, 17 Nov 2021 02:42:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d213b39bd355350279fd232f3eedd77a11f7fb2a3d5a0017357dd45fa88342d1`  
		Last Modified: Wed, 17 Nov 2021 02:52:49 GMT  
		Size: 23.2 MB (23156564 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stretch-backports`

```console
$ docker pull debian@sha256:15cd80462228e0ddc237e2890ef3111d70ebb130d2769608f6594c503755e493
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 5
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:stretch-backports` - linux; amd64

```console
$ docker pull debian@sha256:548c41c70872c618c0b675c41b47de1fc1815ec6df921e2354605132d4f90255
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **45.4 MB (45380665 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:083e21a8f2a7c9c2e0432e1d7a259c6633a26e2d0f2d6ea28b198ac692f56dfa`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:22:28 GMT
ADD file:d7365dd9cf34b10ca189f85c95c21a0c33e44092f85ffb5884d5e737fb0b9be1 in / 
# Wed, 17 Nov 2021 02:22:28 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:22:32 GMT
RUN echo 'deb http://deb.debian.org/debian stretch-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:200cae5c9d88bb9cf1dd3fcfb831d671403f078d2310416fa3b304337d8279c0`  
		Last Modified: Wed, 17 Nov 2021 02:29:09 GMT  
		Size: 45.4 MB (45380444 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:5f54e01827d8e94188e47a454ee3cfc4c35991a75e866eda70c07e717257bce9`  
		Last Modified: Wed, 17 Nov 2021 02:29:23 GMT  
		Size: 221.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:1105e6f97d10f16d8577c28562fbdf2680651fe14d0a9271465f8da69914aad5
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **44.1 MB (44091603 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c788c320b4b1d7de55e92a031a349f458e7eee92bf6223875749f607bbc656df`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:55:43 GMT
ADD file:a8b0f40ff55722f4bb2e3ac65c3fefe9acaa513f89b73ed81ec04585058e58ba in / 
# Wed, 17 Nov 2021 02:55:45 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:55:59 GMT
RUN echo 'deb http://deb.debian.org/debian stretch-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:ec68ab8028a430064eed52d9b833f1a135dcce543b02666ee5acd73962b79070`  
		Last Modified: Wed, 17 Nov 2021 03:13:32 GMT  
		Size: 44.1 MB (44091379 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:223bbcda290abdf4a569960d4972eec20a6eea5a6fc6e34e5867907660c4c6bf`  
		Last Modified: Wed, 17 Nov 2021 03:13:51 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:a9780d74a1fd954fa25160ce85876afc392fd4b73a97255d62c73e1dc523d135
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **42.1 MB (42117091 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6c152949883b1df7964bdeda015e14838286c784556d1821f72d56d1daea0634`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:04:57 GMT
ADD file:61da3fcef3aea99b4927189cfcb823a65b0e911a3f4436e44ea84a57a7919ff3 in / 
# Wed, 17 Nov 2021 02:04:58 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:05:14 GMT
RUN echo 'deb http://deb.debian.org/debian stretch-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:e4d54b576c2fcdd259e57bac14c4e1a2623f6e5dc9a0b1facaed7fe59d891e86`  
		Last Modified: Wed, 17 Nov 2021 02:22:03 GMT  
		Size: 42.1 MB (42116868 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:e0863f9d3618f76a3e2652e9cd24aba6bd6ab604d9f02e4b392a7512536e610f`  
		Last Modified: Wed, 17 Nov 2021 02:22:22 GMT  
		Size: 223.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:af26827a27bc09d7cd71161319d6a6eba1700194c180b6cc7d81dfe4bf20cc9d
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **43.2 MB (43174702 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3f8f1701705027ba87c0a6af67888a318e98d809f6a1ec5d67fc275967f7246f`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:33 GMT
ADD file:3225671be4edc3599e29ebd06516b95573246e68bf29649c402ed87c03c109b5 in / 
# Wed, 17 Nov 2021 02:42:34 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:42:40 GMT
RUN echo 'deb http://deb.debian.org/debian stretch-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:c64a4bafe4ad6fc05fcc6d76d2a06f168f331ffbc87e518fb4ca4ea22e75527e`  
		Last Modified: Wed, 17 Nov 2021 02:51:11 GMT  
		Size: 43.2 MB (43174478 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:a690cd9aa3033cb40ab740972092e393663ef9cb10a5ba27aa278b4f24d3d8f4`  
		Last Modified: Wed, 17 Nov 2021 02:51:27 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-backports` - linux; 386

```console
$ docker pull debian@sha256:08f56a672a3be4918c737a684a245f177f5728a099de26aab0663741c84edf11
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **46.1 MB (46097424 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ff7af8d6205985b858dc9b036fb0eb8257654bd126ad51cea98724a859b29c27`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:21 GMT
ADD file:8e57eaeffee1cd4c93865af9d6370c68bddae095016766191b1338ae675f131c in / 
# Wed, 17 Nov 2021 02:42:22 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:42:29 GMT
RUN echo 'deb http://deb.debian.org/debian stretch-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:24fc2b60c45394a399438249c8e7f92f6fa415f9c477524d5af3ab4782d51519`  
		Last Modified: Wed, 17 Nov 2021 02:52:15 GMT  
		Size: 46.1 MB (46097202 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:2cafbbbfa23d42895791cf343cd8e71cc3e89946dde5df1af9beb9c2061bec6f`  
		Last Modified: Wed, 17 Nov 2021 02:52:36 GMT  
		Size: 222.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:stretch-slim`

```console
$ docker pull debian@sha256:d8566545bd3750e270b8aaf703d0a048fda080cd287a249823a5a11b7c95adbf
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 5
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386

### `debian:stretch-slim` - linux; amd64

```console
$ docker pull debian@sha256:bd4c41c9e68cc553f0c5266f18bef07491c52a08752f81a015892c2dc5e15114
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **22.5 MB (22527686 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a692eb442a03726bc7790f146f97afe590a942a526ec282f4c4d1bb724a9443a`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:22:38 GMT
ADD file:2868c3af63afe6b8aadac07b8776e5468a3ed135fd84ad22df15e48f0610c7ba in / 
# Wed, 17 Nov 2021 02:22:38 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:89166b5eeae47d8c878cb88f20cc67eda2a550a697f4be317c7b7abea566b76f`  
		Last Modified: Wed, 17 Nov 2021 02:29:32 GMT  
		Size: 22.5 MB (22527686 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:2aa2a15f320cd740783a33c547f4cd885d0b6e5b1ba3860024f8c8badc368c25
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **21.2 MB (21204234 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9e45f7a72e70f7c7a8193bfb2fdd770e1fa690422cb7f05e2a1332f8f8f63d60`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:56:16 GMT
ADD file:b857fb98b0880bbb3add8dd317cd1eb0f339332d4836baaedc8f2f424b5f2c98 in / 
# Wed, 17 Nov 2021 02:56:17 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ca67a5e1c16d08a9b8520dae81a39d168ef2a816d8905715eb6a328c89a2ce37`  
		Last Modified: Wed, 17 Nov 2021 03:14:12 GMT  
		Size: 21.2 MB (21204234 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:9a52fdfda53f199f56b7d9a7d4dcc78a655da675926c88c1f4c4af044f9a5b79
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **19.3 MB (19316397 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:eecd35162e59af35e26178b7706ea9d74dc473fef1aed9fd0545734ab75f1304`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:05:31 GMT
ADD file:f28546ff6a3759d3a957c25d4310c3065e1367dff4ae304caae1cf1dc64d061d in / 
# Wed, 17 Nov 2021 02:05:32 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:614e23553da11f1294dc969bcab416c9f981794bdbaee5641335a222730c63b2`  
		Last Modified: Wed, 17 Nov 2021 02:22:41 GMT  
		Size: 19.3 MB (19316397 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:b1902145e37b8fdd8c48092c245efcd8ac341091ed7ff3dbf91d7495bd293d88
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **20.4 MB (20389442 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:db5e084a09bc69d57eb2f658d7c5fa96910e232ee934ba9d2119a0d05fb7c5ed`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:48 GMT
ADD file:2adad5eee701d55a9f58c07f0706eb574d0ad6b74b0cc52a9e622f50639961c9 in / 
# Wed, 17 Nov 2021 02:42:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:34b92b588f0f390db0d125ab0ee4c85510c52367371c4bcf5af80a6ab18f0a9b`  
		Last Modified: Wed, 17 Nov 2021 02:51:38 GMT  
		Size: 20.4 MB (20389442 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:stretch-slim` - linux; 386

```console
$ docker pull debian@sha256:043ab2d585630cc218d3b5b0e09038405e13bff9585bee28cc02f5c93f9e9f86
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **23.2 MB (23156564 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:143712197327374e7944c356bf4336da6737d3df4709128da5224bfd486aa8dc`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:36 GMT
ADD file:3c9c1d0f88db0341cbe43b594cf6d2d865e39d9518b8e2b124cb9736bebe18f3 in / 
# Wed, 17 Nov 2021 02:42:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:d213b39bd355350279fd232f3eedd77a11f7fb2a3d5a0017357dd45fa88342d1`  
		Last Modified: Wed, 17 Nov 2021 02:52:49 GMT  
		Size: 23.2 MB (23156564 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:testing`

```console
$ docker pull debian@sha256:b6b486cf4f63908f18f39cd5bb41315c6f9c1f9c46691bc14224654cd56e6f68
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:testing` - linux; amd64

```console
$ docker pull debian@sha256:9a397dc34b6bc3fc4ce109bae07d567f064e8e4c5b803269e9f9863f92033960
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.5 MB (55457188 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:91e47690c6d78aef7e66a255c123500143c61b68cb2c700708c53cb183091fe8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:22:48 GMT
ADD file:61e992a8cfedc03feb12ae278ba2f7ab32f2845c5dea31869b10861104700c33 in / 
# Wed, 17 Nov 2021 02:22:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:99a9589829e41c216113b415d3c0c9233e7e6498d6291141cdf85cc3042e2b65`  
		Last Modified: Wed, 17 Nov 2021 02:29:54 GMT  
		Size: 55.5 MB (55457188 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; arm variant v5

```console
$ docker pull debian@sha256:056d83748337f0e70583816ef365ebd7fd56ecb9df0e946c5b4066606d237e4a
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.0 MB (52953912 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:169c0b0bd5804ef466995f279470d89e789101e568c579ba84865199ab90dc4c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:56:45 GMT
ADD file:0d14c17e0e69329fce9d7d1e61b89cf8e6c737db31e1335887a18f31372845e1 in / 
# Wed, 17 Nov 2021 02:56:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:39cfde2a5b94ace860c7e198b8278cb8182e1751cf8aea78a2142bd971fd4dc7`  
		Last Modified: Wed, 17 Nov 2021 03:15:03 GMT  
		Size: 53.0 MB (52953912 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; arm variant v7

```console
$ docker pull debian@sha256:3acd1af30fb4720506c718c605950ed96d32433b481fcac18d8fa5ace63d69e7
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50588959 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:acf0303b41d5c93cdc81b71490937c3a32a80969c91328bf1843cc527ac08774`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:06:01 GMT
ADD file:8534f2bd64383fee1357c8ed655d672d2fbef92507e2f11927991fbda800894c in / 
# Wed, 17 Nov 2021 02:06:02 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f099bc7a544c0fb4c5bd6b5b93da81e7b614c293d9032904d5b37dae9baefe43`  
		Last Modified: Wed, 17 Nov 2021 02:23:28 GMT  
		Size: 50.6 MB (50588959 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:21460e2795ce283e6ae6daa5650e104adb0f63828e9920ebd37b3734536c4dab
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.5 MB (54464394 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1a151d1b2e3d2eb6971e309cf3395fda1a45e7c149357a46c0adb4e25b402924`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:59 GMT
ADD file:c4d68bcae684e69a17ac01569a16c332bc925c4359383dbf3f27066f3abb295c in / 
# Wed, 17 Nov 2021 02:43:01 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3306c77e3ba6c6431765bbac6b0bbfb35079cfb5d567c3db405aad7687a02603`  
		Last Modified: Wed, 17 Nov 2021 02:52:02 GMT  
		Size: 54.5 MB (54464394 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; 386

```console
$ docker pull debian@sha256:cacbda39d6100fde58d428bd64b732fb46c553c7e7e90a4067bebc676a2f222e
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.5 MB (56490673 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cf77b5dd3fd8bd92ae9789e3c6b30b7d769839c6842acfb1d4cfe2d0e1af91be`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:51 GMT
ADD file:c80f770b5632d56b98b8316b44faa1f5b198563a6d15dff26def1a3ae7d54592 in / 
# Wed, 17 Nov 2021 02:42:51 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ba340dba45e23d2a978b05efb8e288ba9a536ab823eb597434206a625be512fd`  
		Last Modified: Wed, 17 Nov 2021 02:53:17 GMT  
		Size: 56.5 MB (56490673 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; mips64le

```console
$ docker pull debian@sha256:4df7840f549bff7f7f1053ba9ecaa85dbba70461987653cdd23b8bbebaa6f6fe
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.1 MB (54085678 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:03804f03f93503ed5b5a2ad6f175dfd4ad381d0a3152f9b1972d807ecb9fb974`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:14:05 GMT
ADD file:30e601ec2657da85de30169806cb5ef200f97a0d3d60df75dc47ca1630d05f2a in / 
# Wed, 17 Nov 2021 02:14:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:49bde980a154e1f7547c1e42d9a012d0d2660cce77784eb4997c24a35a6c5e2a`  
		Last Modified: Wed, 17 Nov 2021 02:25:08 GMT  
		Size: 54.1 MB (54085678 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; ppc64le

```console
$ docker pull debian@sha256:1749ea7249a055360fb0c96b4a56aabdd0f84e1bb3686bbba3b42e6158505be0
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **59.7 MB (59706108 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a35bf3898e201c03c0c4ad105aa92b8682fe140360f57e157dae032919a53991`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:35:47 GMT
ADD file:6ec33732a5ce54ae880804c7d4ce9bbc89ad76007b04d23e694d60679162abdc in / 
# Wed, 17 Nov 2021 03:36:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1c92bc8c2e6cbcb9f02ae352b98981f7632bf82483d2ab5a0394f4a3139b3c5b`  
		Last Modified: Wed, 17 Nov 2021 04:12:49 GMT  
		Size: 59.7 MB (59706108 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing` - linux; s390x

```console
$ docker pull debian@sha256:624accd10b26cb5e07fe655186edadd465550819202ff27b54d91ec3bf768e3e
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.7 MB (53669187 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1793e3f78a36a9d6a712192b3076a25c3e5407dd8d8b2b98976f3844d53a6a98`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:44:42 GMT
ADD file:6b07f3691aed718fdd31338ceb571ce22ffe4f9410f48df5c16f190258ace5ae in / 
# Wed, 17 Nov 2021 02:44:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1c02d7cb6bfcee3d40b1f3c9461f47e72e2f51d514b5df5ddb147d175da65910`  
		Last Modified: Wed, 17 Nov 2021 02:50:40 GMT  
		Size: 53.7 MB (53669187 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:testing-20211115`

```console
$ docker pull debian@sha256:b6b486cf4f63908f18f39cd5bb41315c6f9c1f9c46691bc14224654cd56e6f68
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:testing-20211115` - linux; amd64

```console
$ docker pull debian@sha256:9a397dc34b6bc3fc4ce109bae07d567f064e8e4c5b803269e9f9863f92033960
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.5 MB (55457188 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:91e47690c6d78aef7e66a255c123500143c61b68cb2c700708c53cb183091fe8`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:22:48 GMT
ADD file:61e992a8cfedc03feb12ae278ba2f7ab32f2845c5dea31869b10861104700c33 in / 
# Wed, 17 Nov 2021 02:22:48 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:99a9589829e41c216113b415d3c0c9233e7e6498d6291141cdf85cc3042e2b65`  
		Last Modified: Wed, 17 Nov 2021 02:29:54 GMT  
		Size: 55.5 MB (55457188 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20211115` - linux; arm variant v5

```console
$ docker pull debian@sha256:056d83748337f0e70583816ef365ebd7fd56ecb9df0e946c5b4066606d237e4a
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.0 MB (52953912 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:169c0b0bd5804ef466995f279470d89e789101e568c579ba84865199ab90dc4c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:56:45 GMT
ADD file:0d14c17e0e69329fce9d7d1e61b89cf8e6c737db31e1335887a18f31372845e1 in / 
# Wed, 17 Nov 2021 02:56:47 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:39cfde2a5b94ace860c7e198b8278cb8182e1751cf8aea78a2142bd971fd4dc7`  
		Last Modified: Wed, 17 Nov 2021 03:15:03 GMT  
		Size: 53.0 MB (52953912 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20211115` - linux; arm variant v7

```console
$ docker pull debian@sha256:3acd1af30fb4720506c718c605950ed96d32433b481fcac18d8fa5ace63d69e7
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50588959 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:acf0303b41d5c93cdc81b71490937c3a32a80969c91328bf1843cc527ac08774`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:06:01 GMT
ADD file:8534f2bd64383fee1357c8ed655d672d2fbef92507e2f11927991fbda800894c in / 
# Wed, 17 Nov 2021 02:06:02 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:f099bc7a544c0fb4c5bd6b5b93da81e7b614c293d9032904d5b37dae9baefe43`  
		Last Modified: Wed, 17 Nov 2021 02:23:28 GMT  
		Size: 50.6 MB (50588959 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20211115` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:21460e2795ce283e6ae6daa5650e104adb0f63828e9920ebd37b3734536c4dab
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.5 MB (54464394 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1a151d1b2e3d2eb6971e309cf3395fda1a45e7c149357a46c0adb4e25b402924`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:59 GMT
ADD file:c4d68bcae684e69a17ac01569a16c332bc925c4359383dbf3f27066f3abb295c in / 
# Wed, 17 Nov 2021 02:43:01 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:3306c77e3ba6c6431765bbac6b0bbfb35079cfb5d567c3db405aad7687a02603`  
		Last Modified: Wed, 17 Nov 2021 02:52:02 GMT  
		Size: 54.5 MB (54464394 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20211115` - linux; 386

```console
$ docker pull debian@sha256:cacbda39d6100fde58d428bd64b732fb46c553c7e7e90a4067bebc676a2f222e
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.5 MB (56490673 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cf77b5dd3fd8bd92ae9789e3c6b30b7d769839c6842acfb1d4cfe2d0e1af91be`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:51 GMT
ADD file:c80f770b5632d56b98b8316b44faa1f5b198563a6d15dff26def1a3ae7d54592 in / 
# Wed, 17 Nov 2021 02:42:51 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ba340dba45e23d2a978b05efb8e288ba9a536ab823eb597434206a625be512fd`  
		Last Modified: Wed, 17 Nov 2021 02:53:17 GMT  
		Size: 56.5 MB (56490673 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20211115` - linux; mips64le

```console
$ docker pull debian@sha256:4df7840f549bff7f7f1053ba9ecaa85dbba70461987653cdd23b8bbebaa6f6fe
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.1 MB (54085678 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:03804f03f93503ed5b5a2ad6f175dfd4ad381d0a3152f9b1972d807ecb9fb974`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:14:05 GMT
ADD file:30e601ec2657da85de30169806cb5ef200f97a0d3d60df75dc47ca1630d05f2a in / 
# Wed, 17 Nov 2021 02:14:06 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:49bde980a154e1f7547c1e42d9a012d0d2660cce77784eb4997c24a35a6c5e2a`  
		Last Modified: Wed, 17 Nov 2021 02:25:08 GMT  
		Size: 54.1 MB (54085678 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20211115` - linux; ppc64le

```console
$ docker pull debian@sha256:1749ea7249a055360fb0c96b4a56aabdd0f84e1bb3686bbba3b42e6158505be0
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **59.7 MB (59706108 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a35bf3898e201c03c0c4ad105aa92b8682fe140360f57e157dae032919a53991`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:35:47 GMT
ADD file:6ec33732a5ce54ae880804c7d4ce9bbc89ad76007b04d23e694d60679162abdc in / 
# Wed, 17 Nov 2021 03:36:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1c92bc8c2e6cbcb9f02ae352b98981f7632bf82483d2ab5a0394f4a3139b3c5b`  
		Last Modified: Wed, 17 Nov 2021 04:12:49 GMT  
		Size: 59.7 MB (59706108 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20211115` - linux; s390x

```console
$ docker pull debian@sha256:624accd10b26cb5e07fe655186edadd465550819202ff27b54d91ec3bf768e3e
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.7 MB (53669187 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:1793e3f78a36a9d6a712192b3076a25c3e5407dd8d8b2b98976f3844d53a6a98`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:44:42 GMT
ADD file:6b07f3691aed718fdd31338ceb571ce22ffe4f9410f48df5c16f190258ace5ae in / 
# Wed, 17 Nov 2021 02:44:44 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1c02d7cb6bfcee3d40b1f3c9461f47e72e2f51d514b5df5ddb147d175da65910`  
		Last Modified: Wed, 17 Nov 2021 02:50:40 GMT  
		Size: 53.7 MB (53669187 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:testing-20211115-slim`

```console
$ docker pull debian@sha256:02ca331f0e462487b34f69bc229e3c0b8d8ab6be4ed0c7bd11426da83aa06aba
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:testing-20211115-slim` - linux; amd64

```console
$ docker pull debian@sha256:9db8972ac9cbd534c5e1bd9b8bfbfeb415984ae44d2ba6a4d28069081a629250
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.8 MB (31764901 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d784224a526871c84f833d00f77ecb8c7c6a0361dd77f6f0b7f22074834657ef`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:22:58 GMT
ADD file:4aa5d26cc90f1f0f093059d716ae930aa41582f96a415acdad7741b74107ee78 in / 
# Wed, 17 Nov 2021 02:22:59 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:be34b107957341dccd77e67dd12d869e3d9f0211b3de02c8460d84ec20b7bb42`  
		Last Modified: Wed, 17 Nov 2021 02:30:16 GMT  
		Size: 31.8 MB (31764901 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20211115-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:846ad799aef4ee34a2e4340966e54051faab065209a19f59b5f6a84638da3b92
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.3 MB (29263885 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b0d0dc0d619db031db9ae4b67d6010673f52e08faa6d283a9bcb502fd23fd773`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:57:17 GMT
ADD file:f8455d04a48272073068abfb32228a25a82041419718a08795fd1b1c68f564cc in / 
# Wed, 17 Nov 2021 02:57:18 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ad12c7e3cfe9a871c5eac7f2f42ad4da1f34a3bac43ac82f28b25b56df164aaa`  
		Last Modified: Wed, 17 Nov 2021 03:15:41 GMT  
		Size: 29.3 MB (29263885 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20211115-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:c6c3ed4b1f5abb6e2d8df57e715e1b56027ce460304ed9e673ece13e6963c451
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.9 MB (26899879 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3ca46817d141b5630fda11dc768f412196d38f653a8a0d0d7fd680feb95eb7e6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:06:34 GMT
ADD file:70cd388b2c9f6b918ddc2f2d885643c55741be9ac6be4184b4f23d38decef634 in / 
# Wed, 17 Nov 2021 02:06:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:124546b9adae023b68c4476ac3e1d68d8dbfa04c9eb774e3eb2b965a555f1ede`  
		Last Modified: Wed, 17 Nov 2021 02:24:10 GMT  
		Size: 26.9 MB (26899879 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20211115-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:62d2b11ad3cf78d1f65ae7b97a45f9d41dcb0e1399d451f006d6fa53334bde95
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.8 MB (30772591 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:04c29f0dbecf61fbb880255af00d4e3ad56f8d49be3506d9f429ad1acc566839`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:43:16 GMT
ADD file:fab40bb4d8e6a00437098446a29128c40590c2acdf654ab61db5f9571896fc5e in / 
# Wed, 17 Nov 2021 02:43:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0d89119d46632c6f89f9f030dae17494d97655497bef7e7f4507ee3d62671d09`  
		Last Modified: Wed, 17 Nov 2021 02:52:26 GMT  
		Size: 30.8 MB (30772591 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20211115-slim` - linux; 386

```console
$ docker pull debian@sha256:203943a4746f968f918abb47ba683afab00e1175041bc20eaa7ea07ee281d161
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.8 MB (32801469 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a081ea265f1f65270f8b3a7051fb2c2e3fd19a9a048db82d16c900147e606299`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:43:06 GMT
ADD file:1099e1f2c957a5d8233ba185e6790ddc075486618c8017fad213beb0f97e9581 in / 
# Wed, 17 Nov 2021 02:43:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:99d6ddf8b25e8313ba05f40dafa3af9d793b378d56bd812e72a9d2abc9319461`  
		Last Modified: Wed, 17 Nov 2021 02:53:42 GMT  
		Size: 32.8 MB (32801469 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20211115-slim` - linux; mips64le

```console
$ docker pull debian@sha256:6fc2697efbdd9e535770f5f8085d03bbc28234784a7104d0a42d0165fe329951
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.4 MB (30394244 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b41eb4ac64664562f6adbe86a2f0cc0dd96dba816a6cf094cb1a3e1c9ddad8ba`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:14:32 GMT
ADD file:b5dc8524ece1652e38d98681f603a2bdee9fe57418aa5b2d982a855406ec8709 in / 
# Wed, 17 Nov 2021 02:14:33 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8965687b6023dfe66df7537d0b94db778c781081303b6ab542ee4a24408968db`  
		Last Modified: Wed, 17 Nov 2021 02:25:47 GMT  
		Size: 30.4 MB (30394244 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20211115-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:b23b1892901ceb8d6a17867a6d8ca76f639d9e9e070a9d1756ddb04673037db7
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **36.0 MB (36014958 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c58c08bb6ccbf3f4a27829c676fe9112c4c95d34dfd93c28faad99c5ad4ce9b3`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:37:58 GMT
ADD file:d9b1f8bdecc73ca980ef494764b80754e0844186cb50be3df43c0c06894085fc in / 
# Wed, 17 Nov 2021 03:38:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:779cfb5221e4cc156bfbe6ce0a113aa9a997eb6705e2c7f296457eb1ea5f38c2`  
		Last Modified: Wed, 17 Nov 2021 04:14:19 GMT  
		Size: 36.0 MB (36014958 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-20211115-slim` - linux; s390x

```console
$ docker pull debian@sha256:e889d0f89099694323c72f5d55546195a8205890c115bd2f49b7f76f59fa1f55
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.0 MB (29977819 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6bb5c6ce2b2de3440c656b047df9d84f468b0173fb3fcd8ea3fa170caa1b5372`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:45:00 GMT
ADD file:0c75c6e745f3a72c445e40c2bd2e7a1e854bce85b44a055023b16cbe60abd07c in / 
# Wed, 17 Nov 2021 02:45:01 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e4a0f640484d12b61f9f34265d5425d84583e136af9b6279863a346708691266`  
		Last Modified: Wed, 17 Nov 2021 02:50:56 GMT  
		Size: 30.0 MB (29977819 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:testing-backports`

```console
$ docker pull debian@sha256:12ee698e89865e2ca05225bcff58f2c78ba07982bf18b4c8a5a4befdf2a95fb5
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:testing-backports` - linux; amd64

```console
$ docker pull debian@sha256:1a63eab6d22d17cb3825c05c3476081cd495774b720f309c65ba7ec22ee22070
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.5 MB (55457413 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e9d2e0d561be7a2bc91bcbcbaeee1a0b01d65d5a8242ad09236304fbcb4779a3`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:22:48 GMT
ADD file:61e992a8cfedc03feb12ae278ba2f7ab32f2845c5dea31869b10861104700c33 in / 
# Wed, 17 Nov 2021 02:22:48 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:22:51 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:99a9589829e41c216113b415d3c0c9233e7e6498d6291141cdf85cc3042e2b65`  
		Last Modified: Wed, 17 Nov 2021 02:29:54 GMT  
		Size: 55.5 MB (55457188 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:7315fa08c421422153de53a1bd153deb92fc69f0b8fb7c1eaef5e13c4acaad2c`  
		Last Modified: Wed, 17 Nov 2021 02:30:04 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; arm variant v5

```console
$ docker pull debian@sha256:3fde94fd6dc2dced4b43b42df46d714af96cc8bdda648679a22bafea7cb0bcd5
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.0 MB (52954137 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:09766e7d334ea3714b61eaf6c196d60e88073508cdc0064cb9319f12b79138fb`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:56:45 GMT
ADD file:0d14c17e0e69329fce9d7d1e61b89cf8e6c737db31e1335887a18f31372845e1 in / 
# Wed, 17 Nov 2021 02:56:47 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:56:59 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:39cfde2a5b94ace860c7e198b8278cb8182e1751cf8aea78a2142bd971fd4dc7`  
		Last Modified: Wed, 17 Nov 2021 03:15:03 GMT  
		Size: 53.0 MB (52953912 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:db55d1b7540b067391595ca95e56799ce744139a66c3b46fe66619afa7006e48`  
		Last Modified: Wed, 17 Nov 2021 03:15:15 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; arm variant v7

```console
$ docker pull debian@sha256:e6c65237faddc55262028b030dafe111a6f7292720add98216df351ad6fd2063
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.6 MB (50589184 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3ae19dc1738a34453d77f7c5f911344ef46fe28b96e7e88bb63d21cbdef189dc`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:06:01 GMT
ADD file:8534f2bd64383fee1357c8ed655d672d2fbef92507e2f11927991fbda800894c in / 
# Wed, 17 Nov 2021 02:06:02 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:06:14 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:f099bc7a544c0fb4c5bd6b5b93da81e7b614c293d9032904d5b37dae9baefe43`  
		Last Modified: Wed, 17 Nov 2021 02:23:28 GMT  
		Size: 50.6 MB (50588959 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:9e5f34119a13d10b1574733baadd3a547946864926973228efe0c3cf5565655b`  
		Last Modified: Wed, 17 Nov 2021 02:23:48 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:3ff5108bb89b7d0d2f533b47dcf8b7195d9c0e6505d100485a314d8e82cb395c
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.5 MB (54464618 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f1f3c7bc1d2e2852613a7c84fde94ead968a9aaef007d68ccbd44d0e77c99b51`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:59 GMT
ADD file:c4d68bcae684e69a17ac01569a16c332bc925c4359383dbf3f27066f3abb295c in / 
# Wed, 17 Nov 2021 02:43:01 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:43:07 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:3306c77e3ba6c6431765bbac6b0bbfb35079cfb5d567c3db405aad7687a02603`  
		Last Modified: Wed, 17 Nov 2021 02:52:02 GMT  
		Size: 54.5 MB (54464394 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:c5b268af8c50dfeb7aaf05c78db001badc1728730f4d7985c3eef1fe34ec8916`  
		Last Modified: Wed, 17 Nov 2021 02:52:13 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; 386

```console
$ docker pull debian@sha256:f40babdcf3bc442fd57d17fbc2b04b64d1470cdf45c5d6360e04f969da7f8366
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.5 MB (56490897 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:0f0f239b7fee7a5411d5db505e8ff1435c80d2b25ce6c446643828f84dff48ea`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:42:51 GMT
ADD file:c80f770b5632d56b98b8316b44faa1f5b198563a6d15dff26def1a3ae7d54592 in / 
# Wed, 17 Nov 2021 02:42:51 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:42:57 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:ba340dba45e23d2a978b05efb8e288ba9a536ab823eb597434206a625be512fd`  
		Last Modified: Wed, 17 Nov 2021 02:53:17 GMT  
		Size: 56.5 MB (56490673 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:f87f1044d339656b2f1bd8d398f4242f0948cae74c923c34180332956462b643`  
		Last Modified: Wed, 17 Nov 2021 02:53:28 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; mips64le

```console
$ docker pull debian@sha256:49230c7ceaebac2485f27a7af0049f799d64982a23531fe2313acd2981d1f113
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.1 MB (54085904 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c1b5dbb3396cfbe4b0a8f55993b6abcfad4f3c3301548d99e5ddea45d64417fe`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:14:05 GMT
ADD file:30e601ec2657da85de30169806cb5ef200f97a0d3d60df75dc47ca1630d05f2a in / 
# Wed, 17 Nov 2021 02:14:06 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:14:13 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:49bde980a154e1f7547c1e42d9a012d0d2660cce77784eb4997c24a35a6c5e2a`  
		Last Modified: Wed, 17 Nov 2021 02:25:08 GMT  
		Size: 54.1 MB (54085678 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:1ddef526b2c6697b20fb1afeda1bbd423a74484190b9f2ccd98eae795ec8987e`  
		Last Modified: Wed, 17 Nov 2021 02:25:18 GMT  
		Size: 226.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; ppc64le

```console
$ docker pull debian@sha256:9e3c39a2c4432137da727787e18ae428ea12f36dfdf4c813a5dcd56e47a5a333
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **59.7 MB (59706332 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c838dc60d8eadbb19b20a5d831326dc22e973f055d3f56b223b76bee02ba532e`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:35:47 GMT
ADD file:6ec33732a5ce54ae880804c7d4ce9bbc89ad76007b04d23e694d60679162abdc in / 
# Wed, 17 Nov 2021 03:36:09 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 03:37:20 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:1c92bc8c2e6cbcb9f02ae352b98981f7632bf82483d2ab5a0394f4a3139b3c5b`  
		Last Modified: Wed, 17 Nov 2021 04:12:49 GMT  
		Size: 59.7 MB (59706108 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:f9ef0f52d5c4aaa96e76420e8169c0b31d1d9afde93e1bf873f6dedf2e749994`  
		Last Modified: Wed, 17 Nov 2021 04:13:07 GMT  
		Size: 224.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-backports` - linux; s390x

```console
$ docker pull debian@sha256:6027914093fc1280d22d7fd85f944020ee2ef5e501ca736305b88aa3c2ecc644
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.7 MB (53669412 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:05e01388f51f58cd4a3bd79c8ae54a48df7add1b34ebc3161f5f1b2353e5d434`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:44:42 GMT
ADD file:6b07f3691aed718fdd31338ceb571ce22ffe4f9410f48df5c16f190258ace5ae in / 
# Wed, 17 Nov 2021 02:44:44 GMT
CMD ["bash"]
# Wed, 17 Nov 2021 02:44:51 GMT
RUN echo 'deb http://deb.debian.org/debian testing-backports main' > /etc/apt/sources.list.d/backports.list
```

-	Layers:
	-	`sha256:1c02d7cb6bfcee3d40b1f3c9461f47e72e2f51d514b5df5ddb147d175da65910`  
		Last Modified: Wed, 17 Nov 2021 02:50:40 GMT  
		Size: 53.7 MB (53669187 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:1936d53cdd6ac9bc0483465e90743bf275dc82a32ddb19601dd4903c2cecba5a`  
		Last Modified: Wed, 17 Nov 2021 02:50:47 GMT  
		Size: 225.0 B  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:testing-slim`

```console
$ docker pull debian@sha256:02ca331f0e462487b34f69bc229e3c0b8d8ab6be4ed0c7bd11426da83aa06aba
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 8
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; s390x

### `debian:testing-slim` - linux; amd64

```console
$ docker pull debian@sha256:9db8972ac9cbd534c5e1bd9b8bfbfeb415984ae44d2ba6a4d28069081a629250
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.8 MB (31764901 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d784224a526871c84f833d00f77ecb8c7c6a0361dd77f6f0b7f22074834657ef`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:22:58 GMT
ADD file:4aa5d26cc90f1f0f093059d716ae930aa41582f96a415acdad7741b74107ee78 in / 
# Wed, 17 Nov 2021 02:22:59 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:be34b107957341dccd77e67dd12d869e3d9f0211b3de02c8460d84ec20b7bb42`  
		Last Modified: Wed, 17 Nov 2021 02:30:16 GMT  
		Size: 31.8 MB (31764901 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:846ad799aef4ee34a2e4340966e54051faab065209a19f59b5f6a84638da3b92
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.3 MB (29263885 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b0d0dc0d619db031db9ae4b67d6010673f52e08faa6d283a9bcb502fd23fd773`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:57:17 GMT
ADD file:f8455d04a48272073068abfb32228a25a82041419718a08795fd1b1c68f564cc in / 
# Wed, 17 Nov 2021 02:57:18 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:ad12c7e3cfe9a871c5eac7f2f42ad4da1f34a3bac43ac82f28b25b56df164aaa`  
		Last Modified: Wed, 17 Nov 2021 03:15:41 GMT  
		Size: 29.3 MB (29263885 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:c6c3ed4b1f5abb6e2d8df57e715e1b56027ce460304ed9e673ece13e6963c451
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **26.9 MB (26899879 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3ca46817d141b5630fda11dc768f412196d38f653a8a0d0d7fd680feb95eb7e6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:06:34 GMT
ADD file:70cd388b2c9f6b918ddc2f2d885643c55741be9ac6be4184b4f23d38decef634 in / 
# Wed, 17 Nov 2021 02:06:35 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:124546b9adae023b68c4476ac3e1d68d8dbfa04c9eb774e3eb2b965a555f1ede`  
		Last Modified: Wed, 17 Nov 2021 02:24:10 GMT  
		Size: 26.9 MB (26899879 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:62d2b11ad3cf78d1f65ae7b97a45f9d41dcb0e1399d451f006d6fa53334bde95
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.8 MB (30772591 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:04c29f0dbecf61fbb880255af00d4e3ad56f8d49be3506d9f429ad1acc566839`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:43:16 GMT
ADD file:fab40bb4d8e6a00437098446a29128c40590c2acdf654ab61db5f9571896fc5e in / 
# Wed, 17 Nov 2021 02:43:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:0d89119d46632c6f89f9f030dae17494d97655497bef7e7f4507ee3d62671d09`  
		Last Modified: Wed, 17 Nov 2021 02:52:26 GMT  
		Size: 30.8 MB (30772591 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; 386

```console
$ docker pull debian@sha256:203943a4746f968f918abb47ba683afab00e1175041bc20eaa7ea07ee281d161
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.8 MB (32801469 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a081ea265f1f65270f8b3a7051fb2c2e3fd19a9a048db82d16c900147e606299`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:43:06 GMT
ADD file:1099e1f2c957a5d8233ba185e6790ddc075486618c8017fad213beb0f97e9581 in / 
# Wed, 17 Nov 2021 02:43:07 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:99d6ddf8b25e8313ba05f40dafa3af9d793b378d56bd812e72a9d2abc9319461`  
		Last Modified: Wed, 17 Nov 2021 02:53:42 GMT  
		Size: 32.8 MB (32801469 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; mips64le

```console
$ docker pull debian@sha256:6fc2697efbdd9e535770f5f8085d03bbc28234784a7104d0a42d0165fe329951
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.4 MB (30394244 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b41eb4ac64664562f6adbe86a2f0cc0dd96dba816a6cf094cb1a3e1c9ddad8ba`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:14:32 GMT
ADD file:b5dc8524ece1652e38d98681f603a2bdee9fe57418aa5b2d982a855406ec8709 in / 
# Wed, 17 Nov 2021 02:14:33 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:8965687b6023dfe66df7537d0b94db778c781081303b6ab542ee4a24408968db`  
		Last Modified: Wed, 17 Nov 2021 02:25:47 GMT  
		Size: 30.4 MB (30394244 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:b23b1892901ceb8d6a17867a6d8ca76f639d9e9e070a9d1756ddb04673037db7
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **36.0 MB (36014958 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:c58c08bb6ccbf3f4a27829c676fe9112c4c95d34dfd93c28faad99c5ad4ce9b3`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:37:58 GMT
ADD file:d9b1f8bdecc73ca980ef494764b80754e0844186cb50be3df43c0c06894085fc in / 
# Wed, 17 Nov 2021 03:38:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:779cfb5221e4cc156bfbe6ce0a113aa9a997eb6705e2c7f296457eb1ea5f38c2`  
		Last Modified: Wed, 17 Nov 2021 04:14:19 GMT  
		Size: 36.0 MB (36014958 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:testing-slim` - linux; s390x

```console
$ docker pull debian@sha256:e889d0f89099694323c72f5d55546195a8205890c115bd2f49b7f76f59fa1f55
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.0 MB (29977819 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6bb5c6ce2b2de3440c656b047df9d84f468b0173fb3fcd8ea3fa170caa1b5372`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:45:00 GMT
ADD file:0c75c6e745f3a72c445e40c2bd2e7a1e854bce85b44a055023b16cbe60abd07c in / 
# Wed, 17 Nov 2021 02:45:01 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e4a0f640484d12b61f9f34265d5425d84583e136af9b6279863a346708691266`  
		Last Modified: Wed, 17 Nov 2021 02:50:56 GMT  
		Size: 30.0 MB (29977819 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:unstable`

```console
$ docker pull debian@sha256:47a4fff38a89f64c472f0b3d9f45d751ffa84869e98a9ad957bea221c2ba6a01
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:unstable` - linux; amd64

```console
$ docker pull debian@sha256:b65bc536a8be1cec7c979746a9f5bc064d95a12534900cfdd3f91bf1d77155a2
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.8 MB (55758105 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fa40a9573b53b202ce69f072311b01ddd07143b0c6426f1ba974f71bb4d94ef6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:23:08 GMT
ADD file:db09d197fcb0e66f610eb98b792583f6ec40f708cb0d0177e510c2a158f566fa in / 
# Wed, 17 Nov 2021 02:23:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a6543f35db5e93a9d2a1ff8c5abaf275a6bfe30ceb4f008f8047cacfd2d0f324`  
		Last Modified: Wed, 17 Nov 2021 02:30:33 GMT  
		Size: 55.8 MB (55758105 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; arm variant v5

```console
$ docker pull debian@sha256:9bea96c11fe620f3275dcc4d08027967de01386151135176cc094da05e8719f8
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53238737 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:87d01d716f51a0345e035e08e8c05e131fbbccf17173b4419d06282f678eb2c4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:57:44 GMT
ADD file:4f7ce5598f98e9f01ec7174a57364d9d28802a554530db59c008960d0a40c239 in / 
# Wed, 17 Nov 2021 02:57:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5c59a35f8148e14582e723208c2af72918650b046899af2d952e25ad86cc3dd2`  
		Last Modified: Wed, 17 Nov 2021 03:16:24 GMT  
		Size: 53.2 MB (53238737 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; arm variant v7

```console
$ docker pull debian@sha256:49c5079bec026f1962d3da01e44590245619e56c743095efb735aaced2dcd6d5
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.9 MB (50860307 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b6e2f839d6dea8032ae9237319174db4a1c7e2ea1fed71615424e986680f7e67`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:07:01 GMT
ADD file:930f169a86ba880df2aaea12f3e8598f194b73f18795a8cd70efcd27b241bda3 in / 
# Wed, 17 Nov 2021 02:07:02 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:61a45c58489b0c8e11c36c2b4e975b6bd6dd5cd0b16c91ccb452966e84c75021`  
		Last Modified: Wed, 17 Nov 2021 02:24:50 GMT  
		Size: 50.9 MB (50860307 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:7b6597251545bb909adbe64cb723a5b7d22a8b2b036bf204cb37d8b0534fae6b
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.8 MB (54767075 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ddcdb12d67815866f54942d740ac4ccca6d398da7c63af5039c45fb77d362b09`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:43:27 GMT
ADD file:44b7578caffebde5ddfd201775779df99cb1d525113ba58609f8cdf1f47092ce in / 
# Wed, 17 Nov 2021 02:43:27 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:af0aea1ea198f6a1207b9812f667f234e3ccf829e7b41959a314d3616636bc52`  
		Last Modified: Wed, 17 Nov 2021 02:52:44 GMT  
		Size: 54.8 MB (54767075 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; 386

```console
$ docker pull debian@sha256:3bef1ae42d06217e0ebbe57bf8cc33cf5083a2c0f770b43852a0a0bd1ef270d9
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.8 MB (56816838 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cba62cdea9b0d56e3a99f59a8b97dda9c7dcdecc8f078db88a706c3245da5912`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:43:20 GMT
ADD file:b1c54ba84e5f6c88c1f3a56a44deef95ed0ae707568246e0aad79bfb2ad13ff7 in / 
# Wed, 17 Nov 2021 02:43:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5ae2b303578d13cb8c2bec7380b5933eb31c2dc36e90d6745fabbb1efa42d287`  
		Last Modified: Wed, 17 Nov 2021 02:54:04 GMT  
		Size: 56.8 MB (56816838 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; mips64le

```console
$ docker pull debian@sha256:453b788ada6c924dcc9b6d882ab2c9674121c02015d302952c58f390821ccbd9
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.4 MB (54365461 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b5dfad57456d50510185dbaad6f5bfa3db4a123dc40c05a61dceac5087b06957`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:15:00 GMT
ADD file:65990c5fc1629d9c60c5eae96c8b5b16dc64c2e1e4d049da2d9d45ca8126a2b6 in / 
# Wed, 17 Nov 2021 02:15:01 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a0ac2e403b44e2769c44c8edeefcb754cc01a8aed8345df0bc46cedfb659667e`  
		Last Modified: Wed, 17 Nov 2021 02:26:29 GMT  
		Size: 54.4 MB (54365461 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; ppc64le

```console
$ docker pull debian@sha256:6117f3c4500dc062cb67d5fa056a5cfdfe79737873d7a9f12e812d79665a6a43
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **60.0 MB (60045027 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4c4e52aefd6474f08beb6b366f0553b71c2d93dd862fcff863309e37ec24bd11`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:38:38 GMT
ADD file:f98cd5c0b6d1b0eca9c66a96528f16cee04c4c39c0099aeb696452eba40f7ce5 in / 
# Wed, 17 Nov 2021 03:38:55 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:20a855c65aabb17aca96cb4aba443be5ede8aeccac91e0e21c41cf07a4ae22c2`  
		Last Modified: Wed, 17 Nov 2021 04:17:17 GMT  
		Size: 60.0 MB (60045027 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; riscv64

```console
$ docker pull debian@sha256:d64f986037c4042f19bfd08377c1da2b6e3e6ac41a8f124e08244c73ece4f4d5
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.5 MB (51522393 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:13305872cdb6a508d8abeed080e02f792061014d00f7d7c534a54673ebdb8636`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:18:02 GMT
ADD file:0ec1f46a764439430fb9f09f4839b10976be8fc64fc12afa4c027f1c62f25260 in / 
# Wed, 17 Nov 2021 02:18:04 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2bc932115d57a62ccf3e02aa3e687abcf6d81b71a70bd56a6c3eef683ff58d23`  
		Last Modified: Wed, 17 Nov 2021 02:34:09 GMT  
		Size: 51.5 MB (51522393 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable` - linux; s390x

```console
$ docker pull debian@sha256:e1baa8402343725b4371f8f00db0d53e3f4fbbe1fc7b319513e0400f9dab8305
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.0 MB (53965621 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fe0658a04896f17cea7ef9f789d7cbdc82bd927ff6c15fff2293124d3ef566e0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:45:12 GMT
ADD file:3e240d4862da4173e8dc2b043c11c59b5b8d8ea71c087729ecc0bcedf15bc5c2 in / 
# Wed, 17 Nov 2021 02:45:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e50a73e9a583b8d41392e2bc8c7f048886a3cea5c4319e2b919dfbc1866d9e9f`  
		Last Modified: Wed, 17 Nov 2021 02:51:10 GMT  
		Size: 54.0 MB (53965621 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:unstable-20211115`

```console
$ docker pull debian@sha256:47a4fff38a89f64c472f0b3d9f45d751ffa84869e98a9ad957bea221c2ba6a01
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:unstable-20211115` - linux; amd64

```console
$ docker pull debian@sha256:b65bc536a8be1cec7c979746a9f5bc064d95a12534900cfdd3f91bf1d77155a2
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **55.8 MB (55758105 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fa40a9573b53b202ce69f072311b01ddd07143b0c6426f1ba974f71bb4d94ef6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:23:08 GMT
ADD file:db09d197fcb0e66f610eb98b792583f6ec40f708cb0d0177e510c2a158f566fa in / 
# Wed, 17 Nov 2021 02:23:09 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a6543f35db5e93a9d2a1ff8c5abaf275a6bfe30ceb4f008f8047cacfd2d0f324`  
		Last Modified: Wed, 17 Nov 2021 02:30:33 GMT  
		Size: 55.8 MB (55758105 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20211115` - linux; arm variant v5

```console
$ docker pull debian@sha256:9bea96c11fe620f3275dcc4d08027967de01386151135176cc094da05e8719f8
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **53.2 MB (53238737 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:87d01d716f51a0345e035e08e8c05e131fbbccf17173b4419d06282f678eb2c4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:57:44 GMT
ADD file:4f7ce5598f98e9f01ec7174a57364d9d28802a554530db59c008960d0a40c239 in / 
# Wed, 17 Nov 2021 02:57:45 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5c59a35f8148e14582e723208c2af72918650b046899af2d952e25ad86cc3dd2`  
		Last Modified: Wed, 17 Nov 2021 03:16:24 GMT  
		Size: 53.2 MB (53238737 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20211115` - linux; arm variant v7

```console
$ docker pull debian@sha256:49c5079bec026f1962d3da01e44590245619e56c743095efb735aaced2dcd6d5
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **50.9 MB (50860307 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b6e2f839d6dea8032ae9237319174db4a1c7e2ea1fed71615424e986680f7e67`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:07:01 GMT
ADD file:930f169a86ba880df2aaea12f3e8598f194b73f18795a8cd70efcd27b241bda3 in / 
# Wed, 17 Nov 2021 02:07:02 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:61a45c58489b0c8e11c36c2b4e975b6bd6dd5cd0b16c91ccb452966e84c75021`  
		Last Modified: Wed, 17 Nov 2021 02:24:50 GMT  
		Size: 50.9 MB (50860307 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20211115` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:7b6597251545bb909adbe64cb723a5b7d22a8b2b036bf204cb37d8b0534fae6b
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.8 MB (54767075 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:ddcdb12d67815866f54942d740ac4ccca6d398da7c63af5039c45fb77d362b09`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:43:27 GMT
ADD file:44b7578caffebde5ddfd201775779df99cb1d525113ba58609f8cdf1f47092ce in / 
# Wed, 17 Nov 2021 02:43:27 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:af0aea1ea198f6a1207b9812f667f234e3ccf829e7b41959a314d3616636bc52`  
		Last Modified: Wed, 17 Nov 2021 02:52:44 GMT  
		Size: 54.8 MB (54767075 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20211115` - linux; 386

```console
$ docker pull debian@sha256:3bef1ae42d06217e0ebbe57bf8cc33cf5083a2c0f770b43852a0a0bd1ef270d9
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **56.8 MB (56816838 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:cba62cdea9b0d56e3a99f59a8b97dda9c7dcdecc8f078db88a706c3245da5912`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:43:20 GMT
ADD file:b1c54ba84e5f6c88c1f3a56a44deef95ed0ae707568246e0aad79bfb2ad13ff7 in / 
# Wed, 17 Nov 2021 02:43:20 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:5ae2b303578d13cb8c2bec7380b5933eb31c2dc36e90d6745fabbb1efa42d287`  
		Last Modified: Wed, 17 Nov 2021 02:54:04 GMT  
		Size: 56.8 MB (56816838 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20211115` - linux; mips64le

```console
$ docker pull debian@sha256:453b788ada6c924dcc9b6d882ab2c9674121c02015d302952c58f390821ccbd9
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.4 MB (54365461 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b5dfad57456d50510185dbaad6f5bfa3db4a123dc40c05a61dceac5087b06957`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:15:00 GMT
ADD file:65990c5fc1629d9c60c5eae96c8b5b16dc64c2e1e4d049da2d9d45ca8126a2b6 in / 
# Wed, 17 Nov 2021 02:15:01 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:a0ac2e403b44e2769c44c8edeefcb754cc01a8aed8345df0bc46cedfb659667e`  
		Last Modified: Wed, 17 Nov 2021 02:26:29 GMT  
		Size: 54.4 MB (54365461 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20211115` - linux; ppc64le

```console
$ docker pull debian@sha256:6117f3c4500dc062cb67d5fa056a5cfdfe79737873d7a9f12e812d79665a6a43
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **60.0 MB (60045027 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4c4e52aefd6474f08beb6b366f0553b71c2d93dd862fcff863309e37ec24bd11`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:38:38 GMT
ADD file:f98cd5c0b6d1b0eca9c66a96528f16cee04c4c39c0099aeb696452eba40f7ce5 in / 
# Wed, 17 Nov 2021 03:38:55 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:20a855c65aabb17aca96cb4aba443be5ede8aeccac91e0e21c41cf07a4ae22c2`  
		Last Modified: Wed, 17 Nov 2021 04:17:17 GMT  
		Size: 60.0 MB (60045027 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20211115` - linux; riscv64

```console
$ docker pull debian@sha256:d64f986037c4042f19bfd08377c1da2b6e3e6ac41a8f124e08244c73ece4f4d5
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **51.5 MB (51522393 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:13305872cdb6a508d8abeed080e02f792061014d00f7d7c534a54673ebdb8636`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:18:02 GMT
ADD file:0ec1f46a764439430fb9f09f4839b10976be8fc64fc12afa4c027f1c62f25260 in / 
# Wed, 17 Nov 2021 02:18:04 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:2bc932115d57a62ccf3e02aa3e687abcf6d81b71a70bd56a6c3eef683ff58d23`  
		Last Modified: Wed, 17 Nov 2021 02:34:09 GMT  
		Size: 51.5 MB (51522393 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20211115` - linux; s390x

```console
$ docker pull debian@sha256:e1baa8402343725b4371f8f00db0d53e3f4fbbe1fc7b319513e0400f9dab8305
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **54.0 MB (53965621 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:fe0658a04896f17cea7ef9f789d7cbdc82bd927ff6c15fff2293124d3ef566e0`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:45:12 GMT
ADD file:3e240d4862da4173e8dc2b043c11c59b5b8d8ea71c087729ecc0bcedf15bc5c2 in / 
# Wed, 17 Nov 2021 02:45:15 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e50a73e9a583b8d41392e2bc8c7f048886a3cea5c4319e2b919dfbc1866d9e9f`  
		Last Modified: Wed, 17 Nov 2021 02:51:10 GMT  
		Size: 54.0 MB (53965621 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:unstable-20211115-slim`

```console
$ docker pull debian@sha256:c58dfa394985cf664a2843b5fa2ae5875312facb7f5f53f18923c10aadf14bc0
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:unstable-20211115-slim` - linux; amd64

```console
$ docker pull debian@sha256:4d127543d89e77df7adefb1ab02c721ebe477aebc2c5e775cf4468508fe19282
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.9 MB (31861419 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:15e84503aae15bf442ffa628686ce29f522525a5aab22903b7cb3e59e9d638d4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:23:16 GMT
ADD file:b4ed4d765659df195ca3716dc8a2652f94859054ae635b0bcf5c658f063cc52a in / 
# Wed, 17 Nov 2021 02:23:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9569e3c54865d124493afaa67f0414fb6b317504ab4799e3bfa5bacfa286eefd`  
		Last Modified: Wed, 17 Nov 2021 02:30:47 GMT  
		Size: 31.9 MB (31861419 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20211115-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:f6b3062ca77af819926d6cc7e93598b49e9f4dca2505392ed727e645f20abac7
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.3 MB (29340959 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e25c907d24f25543967e0c8cc047ae8b56a61db576ec73a45a716be3c4f7330c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:58:04 GMT
ADD file:3a7e8203adf0dcbd9ca0ddb56acdb2e042de5d1f4bac2d1da08f86c21b753d78 in / 
# Wed, 17 Nov 2021 02:58:05 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:070dc3c6d38846bb788b75b021d394c9a6a03fdc30b8e24f949923f0d04ab299`  
		Last Modified: Wed, 17 Nov 2021 03:16:54 GMT  
		Size: 29.3 MB (29340959 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20211115-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:0ba6566c9301a5ba9effc2e13b3d7438d13bf4b628ce4c2df16859e1263931d9
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.0 MB (26967301 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2cd6bef9dc282b43eb0a36f042adae62fbfd0cb72f6fd713c1a008297d0c5773`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:07:22 GMT
ADD file:d76f71bb77d389b5bb8958ae21cfe4901fb2daa3ecb14ad55e98a7cc58e429fa in / 
# Wed, 17 Nov 2021 02:07:23 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:373dc122c20db33aa986d803b8df6b1ef50f26dfb9611342b717f1c63f649f6a`  
		Last Modified: Wed, 17 Nov 2021 02:25:19 GMT  
		Size: 27.0 MB (26967301 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20211115-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:0fef9ed9944b3fc1c0417429c56d568682029c367b86ea0b954616fd3e5b54bb
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.9 MB (30877714 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:31c3f344ba0f75926f7962b2a87c66f375718798524221ac127f45c9340ab1d1`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:43:36 GMT
ADD file:bf795394b19362c7c42bcbf0fa2d138b60bfc54ff5d1aeecfe6a91542ae5388e in / 
# Wed, 17 Nov 2021 02:43:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:96d7847b41e7c66f9d633d633de657d96a54684835e8e3dee724006aa2873f48`  
		Last Modified: Wed, 17 Nov 2021 02:53:00 GMT  
		Size: 30.9 MB (30877714 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20211115-slim` - linux; 386

```console
$ docker pull debian@sha256:6ee41bf071a8011ca12b81a075f4ed9296a465ccb7f0c543d117e6a92aeed3b3
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.9 MB (32921319 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3d8d7ab86af82a2dc5c2077fb556611c2cc3506b82f25e6c1c7c8e3d0fde543c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:43:30 GMT
ADD file:c91558179c1256223ad52dccba95ea1246a952117fec5fc4370a8d0278071a57 in / 
# Wed, 17 Nov 2021 02:43:30 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1a05d1d6eacb8561b8fe6ddcdc11bb847a35798edbb1ebf594a25749abdf85fa`  
		Last Modified: Wed, 17 Nov 2021 02:54:21 GMT  
		Size: 32.9 MB (32921319 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20211115-slim` - linux; mips64le

```console
$ docker pull debian@sha256:637eea45a80484e816de6d3f69b6973cb621ebe5a39c7c0613adb233f0b896b3
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.5 MB (30470536 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:564602c1c177081783be3a4ad308de0f43e7a0f6af4ab4b8cd52c30935354367`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:15:20 GMT
ADD file:66a0f5846ac02e6179604232c513ea1fc18f27ba753a920cda692aa113c9c0ce in / 
# Wed, 17 Nov 2021 02:15:21 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:21b4526f0d95059cb4a01a2fd7cf0c782e2c502b335534da33b169a1228f3680`  
		Last Modified: Wed, 17 Nov 2021 02:27:01 GMT  
		Size: 30.5 MB (30470536 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20211115-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:a64da95f735acd64eb5e4c2e18061d8a9095adba7347bd5fb5dd49baa40ec9f3
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **36.2 MB (36152657 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b931d91440489cbbf8399da9429be1427c0183fa32389c8d4d671ba41f5e35b1`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:39:20 GMT
ADD file:cc5439a6287d135a0230c86aa373dff0fb242a0168d57485e003a0f4faf40e17 in / 
# Wed, 17 Nov 2021 03:39:34 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b2a38940abb8e40310c01989901d553ee7daefbe600f1694f2a32e797a10e50f`  
		Last Modified: Wed, 17 Nov 2021 04:19:04 GMT  
		Size: 36.2 MB (36152657 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20211115-slim` - linux; riscv64

```console
$ docker pull debian@sha256:0e468164615d2e48aa244f28b1e55697f7faad30622acc5f4511f13e999178ad
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.6 MB (27636393 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d140b8475079ca86f026aa18f2b02605d6ee596737927e5ce05e3fb025becde6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:19:40 GMT
ADD file:db8d3d31d9e2660aa7fbf2ad5c2b6d6e911887116ff96d7a133d95b3425dab6e in / 
# Wed, 17 Nov 2021 02:19:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e27a4edd1e5549fa582dcb4b383d605456e0dff2154bbf30fa9d5e25945d06aa`  
		Last Modified: Wed, 17 Nov 2021 02:35:42 GMT  
		Size: 27.6 MB (27636393 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-20211115-slim` - linux; s390x

```console
$ docker pull debian@sha256:3b78c964d0b57877b4eda979c29db46a8dfabed2eeaf8af99c8519883cfd98f9
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30074857 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6607b7bd955726270f32f3afceddf9a7a158f4130c5a45f8e093a1ab48a67759`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:45:24 GMT
ADD file:5b9c0ac37ec3cb4e3d271065ecaf754465afd35221031bf411dc983db5beaebf in / 
# Wed, 17 Nov 2021 02:45:26 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:394ff21cc74cef2c771a1b809ab554e5ba9a1ce4db8d34d61a1fa029bd691772`  
		Last Modified: Wed, 17 Nov 2021 02:51:21 GMT  
		Size: 30.1 MB (30074857 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

## `debian:unstable-slim`

```console
$ docker pull debian@sha256:c58dfa394985cf664a2843b5fa2ae5875312facb7f5f53f18923c10aadf14bc0
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.list.v2+json`
-	Platforms: 9
	-	linux; amd64
	-	linux; arm variant v5
	-	linux; arm variant v7
	-	linux; arm64 variant v8
	-	linux; 386
	-	linux; mips64le
	-	linux; ppc64le
	-	linux; riscv64
	-	linux; s390x

### `debian:unstable-slim` - linux; amd64

```console
$ docker pull debian@sha256:4d127543d89e77df7adefb1ab02c721ebe477aebc2c5e775cf4468508fe19282
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **31.9 MB (31861419 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:15e84503aae15bf442ffa628686ce29f522525a5aab22903b7cb3e59e9d638d4`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:23:16 GMT
ADD file:b4ed4d765659df195ca3716dc8a2652f94859054ae635b0bcf5c658f063cc52a in / 
# Wed, 17 Nov 2021 02:23:16 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:9569e3c54865d124493afaa67f0414fb6b317504ab4799e3bfa5bacfa286eefd`  
		Last Modified: Wed, 17 Nov 2021 02:30:47 GMT  
		Size: 31.9 MB (31861419 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; arm variant v5

```console
$ docker pull debian@sha256:f6b3062ca77af819926d6cc7e93598b49e9f4dca2505392ed727e645f20abac7
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **29.3 MB (29340959 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:e25c907d24f25543967e0c8cc047ae8b56a61db576ec73a45a716be3c4f7330c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:58:04 GMT
ADD file:3a7e8203adf0dcbd9ca0ddb56acdb2e042de5d1f4bac2d1da08f86c21b753d78 in / 
# Wed, 17 Nov 2021 02:58:05 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:070dc3c6d38846bb788b75b021d394c9a6a03fdc30b8e24f949923f0d04ab299`  
		Last Modified: Wed, 17 Nov 2021 03:16:54 GMT  
		Size: 29.3 MB (29340959 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; arm variant v7

```console
$ docker pull debian@sha256:0ba6566c9301a5ba9effc2e13b3d7438d13bf4b628ce4c2df16859e1263931d9
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.0 MB (26967301 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2cd6bef9dc282b43eb0a36f042adae62fbfd0cb72f6fd713c1a008297d0c5773`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:07:22 GMT
ADD file:d76f71bb77d389b5bb8958ae21cfe4901fb2daa3ecb14ad55e98a7cc58e429fa in / 
# Wed, 17 Nov 2021 02:07:23 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:373dc122c20db33aa986d803b8df6b1ef50f26dfb9611342b717f1c63f649f6a`  
		Last Modified: Wed, 17 Nov 2021 02:25:19 GMT  
		Size: 27.0 MB (26967301 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; arm64 variant v8

```console
$ docker pull debian@sha256:0fef9ed9944b3fc1c0417429c56d568682029c367b86ea0b954616fd3e5b54bb
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.9 MB (30877714 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:31c3f344ba0f75926f7962b2a87c66f375718798524221ac127f45c9340ab1d1`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:43:36 GMT
ADD file:bf795394b19362c7c42bcbf0fa2d138b60bfc54ff5d1aeecfe6a91542ae5388e in / 
# Wed, 17 Nov 2021 02:43:37 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:96d7847b41e7c66f9d633d633de657d96a54684835e8e3dee724006aa2873f48`  
		Last Modified: Wed, 17 Nov 2021 02:53:00 GMT  
		Size: 30.9 MB (30877714 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; 386

```console
$ docker pull debian@sha256:6ee41bf071a8011ca12b81a075f4ed9296a465ccb7f0c543d117e6a92aeed3b3
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **32.9 MB (32921319 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:3d8d7ab86af82a2dc5c2077fb556611c2cc3506b82f25e6c1c7c8e3d0fde543c`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:43:30 GMT
ADD file:c91558179c1256223ad52dccba95ea1246a952117fec5fc4370a8d0278071a57 in / 
# Wed, 17 Nov 2021 02:43:30 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:1a05d1d6eacb8561b8fe6ddcdc11bb847a35798edbb1ebf594a25749abdf85fa`  
		Last Modified: Wed, 17 Nov 2021 02:54:21 GMT  
		Size: 32.9 MB (32921319 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; mips64le

```console
$ docker pull debian@sha256:637eea45a80484e816de6d3f69b6973cb621ebe5a39c7c0613adb233f0b896b3
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.5 MB (30470536 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:564602c1c177081783be3a4ad308de0f43e7a0f6af4ab4b8cd52c30935354367`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:15:20 GMT
ADD file:66a0f5846ac02e6179604232c513ea1fc18f27ba753a920cda692aa113c9c0ce in / 
# Wed, 17 Nov 2021 02:15:21 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:21b4526f0d95059cb4a01a2fd7cf0c782e2c502b335534da33b169a1228f3680`  
		Last Modified: Wed, 17 Nov 2021 02:27:01 GMT  
		Size: 30.5 MB (30470536 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; ppc64le

```console
$ docker pull debian@sha256:a64da95f735acd64eb5e4c2e18061d8a9095adba7347bd5fb5dd49baa40ec9f3
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **36.2 MB (36152657 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b931d91440489cbbf8399da9429be1427c0183fa32389c8d4d671ba41f5e35b1`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 03:39:20 GMT
ADD file:cc5439a6287d135a0230c86aa373dff0fb242a0168d57485e003a0f4faf40e17 in / 
# Wed, 17 Nov 2021 03:39:34 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:b2a38940abb8e40310c01989901d553ee7daefbe600f1694f2a32e797a10e50f`  
		Last Modified: Wed, 17 Nov 2021 04:19:04 GMT  
		Size: 36.2 MB (36152657 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; riscv64

```console
$ docker pull debian@sha256:0e468164615d2e48aa244f28b1e55697f7faad30622acc5f4511f13e999178ad
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **27.6 MB (27636393 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:d140b8475079ca86f026aa18f2b02605d6ee596737927e5ce05e3fb025becde6`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:19:40 GMT
ADD file:db8d3d31d9e2660aa7fbf2ad5c2b6d6e911887116ff96d7a133d95b3425dab6e in / 
# Wed, 17 Nov 2021 02:19:42 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:e27a4edd1e5549fa582dcb4b383d605456e0dff2154bbf30fa9d5e25945d06aa`  
		Last Modified: Wed, 17 Nov 2021 02:35:42 GMT  
		Size: 27.6 MB (27636393 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip

### `debian:unstable-slim` - linux; s390x

```console
$ docker pull debian@sha256:3b78c964d0b57877b4eda979c29db46a8dfabed2eeaf8af99c8519883cfd98f9
```

-	Docker Version: 20.10.7
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **30.1 MB (30074857 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6607b7bd955726270f32f3afceddf9a7a158f4130c5a45f8e093a1ab48a67759`
-	Default Command: `["bash"]`

```dockerfile
# Wed, 17 Nov 2021 02:45:24 GMT
ADD file:5b9c0ac37ec3cb4e3d271065ecaf754465afd35221031bf411dc983db5beaebf in / 
# Wed, 17 Nov 2021 02:45:26 GMT
CMD ["bash"]
```

-	Layers:
	-	`sha256:394ff21cc74cef2c771a1b809ab554e5ba9a1ce4db8d34d61a1fa029bd691772`  
		Last Modified: Wed, 17 Nov 2021 02:51:21 GMT  
		Size: 30.1 MB (30074857 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
