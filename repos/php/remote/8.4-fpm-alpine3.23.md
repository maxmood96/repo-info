...
```console
$ docker pull php@sha256:ea638ae72f3f71f10153a5b0eedf05dea5705333eb6baefc558fb7b348e1879f
```
...
```console
$ docker pull php@sha256:f3b7f4a74ebbcbb734d4dbb7e0505c9305ecf7f7230c53b57af2dd34586f5bbd
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **35.5 MB (35511750 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6dcbca1f7da322c5450a14e91319311700cd4f2152dc2b819cf4d9ffdf935d23`
-	Entrypoint: `["docker-php-entrypoint"]`
-	Default Command: `["php-fpm"]`
...
# Thu, 18 Dec 2025 16:27:43 GMT
COPY docker-php-source /usr/local/bin/ # buildkit
# Thu, 18 Dec 2025 18:25:49 GMT
RUN set -eux; 	apk add --no-cache --virtual .build-deps 		$PHPIZE_DEPS 		argon2-dev 		coreutils 		curl-dev 		gnu-libiconv-dev 		libsodium-dev 		libxml2-dev 		linux-headers 		oniguruma-dev 		openssl-dev 		readline-dev 		sqlite-dev 	; 		rm -vf /usr/include/iconv.h; 		export 		CFLAGS="$PHP_CFLAGS" 		CPPFLAGS="$PHP_CPPFLAGS" 		LDFLAGS="$PHP_LDFLAGS" 		PHP_BUILD_PROVIDER='https://github.com/docker-library/php' 		PHP_UNAME='Linux - Docker' 	; 	docker-php-source extract; 	cd /usr/src/php; 	gnuArch="$(dpkg-architecture --query DEB_BUILD_GNU_TYPE)"; 	./configure 		--build="$gnuArch" 		--with-config-file-path="$PHP_INI_DIR" 		--with-config-file-scan-dir="$PHP_INI_DIR/conf.d" 				--enable-option-checking=fatal 				--with-mhash 				--with-pic 				--enable-mbstring 		--enable-mysqlnd 		--with-password-argon2 		--with-sodium=shared 		--with-pdo-sqlite=/usr 		--with-sqlite3=/usr 				--with-curl 		--with-iconv=/usr 		--with-openssl 		--with-readline 		--with-zlib 				--disable-phpdbg 				--with-pear 						--disable-cgi 				--enable-fpm 		--with-fpm-user=www-data 		--with-fpm-group=www-data 	; 	make -j "$(nproc)"; 	find -type f -name '*.a' -delete; 	make install; 	find 		/usr/local 		-type f 		-perm '/0111' 		-exec sh -euxc ' 			strip --strip-all "$@" || : 		' -- '{}' + 	; 	make clean; 		cp -v php.ini-* "$PHP_INI_DIR/"; 		cd /; 	docker-php-source delete; 		runDeps="$( 		scanelf --needed --nobanner --format '%n#p' --recursive /usr/local 			| tr ',' '\n' 			| sort -u 			| awk 'system("[ -e /usr/local/lib/" $1 " ]") == 0 { next } { print "so:" $1 }' 	)"; 	apk add --no-cache $runDeps; 		apk del --no-network .build-deps; 		pecl update-channels; 	rm -rf /tmp/pear ~/.pearrc; 		php --version # buildkit
# Thu, 18 Dec 2025 18:25:49 GMT
COPY docker-php-ext-* docker-php-entrypoint /usr/local/bin/ # buildkit
# Thu, 18 Dec 2025 18:25:54 GMT
RUN docker-php-ext-enable opcache # buildkit
# Thu, 18 Dec 2025 18:25:58 GMT
RUN docker-php-ext-enable sodium # buildkit
# Thu, 18 Dec 2025 18:25:58 GMT
ENTRYPOINT ["docker-php-entrypoint"]
# Thu, 18 Dec 2025 18:25:58 GMT
WORKDIR /var/www/html
# Thu, 18 Dec 2025 18:25:59 GMT
RUN set -eux; 	cd /usr/local/etc; 	if [ -d php-fpm.d ]; then 		sed 's!=NONE/!=!g' php-fpm.conf.default | tee php-fpm.conf > /dev/null; 		cp php-fpm.d/www.conf.default php-fpm.d/www.conf; 	else 		mkdir php-fpm.d; 		cp php-fpm.conf.default php-fpm.d/www.conf; 		{ 			echo '[global]'; 			echo 'include=etc/php-fpm.d/*.conf'; 		} | tee php-fpm.conf; 	fi; 	{ 		echo '[global]'; 		echo 'error_log = /proc/self/fd/2'; 		echo; echo '; https://github.com/docker-library/php/pull/725#issuecomment-443540114'; echo 'log_limit = 8192'; 		echo; 		echo '[www]'; 		echo '; php-fpm closes STDOUT on startup, so sending logs to /proc/self/fd/1 does not work.'; 		echo '; https://bugs.php.net/bug.php?id=73886'; 		echo 'access.log = /proc/self/fd/2'; 		echo; 		echo 'clear_env = no'; 		echo; 		echo '; Ensure worker stdout and stderr are sent to the main error log.'; 		echo 'catch_workers_output = yes'; 		echo 'decorate_workers_output = no'; 	} | tee php-fpm.d/docker.conf; 	{ 		echo '[global]'; 		echo 'daemonize = no'; 		echo; 		echo '[www]'; 		echo 'listen = 9000'; 	} | tee php-fpm.d/zz-docker.conf; 	mkdir -p "$PHP_INI_DIR/conf.d"; 	{ 		echo '; https://github.com/docker-library/php/issues/878#issuecomment-938595965'; 		echo 'fastcgi.logging = Off'; 	} > "$PHP_INI_DIR/conf.d/docker-fpm.ini" # buildkit
# Thu, 18 Dec 2025 18:25:59 GMT
STOPSIGNAL SIGQUIT
# Thu, 18 Dec 2025 18:25:59 GMT
...
-	Layers:
	...
	-	`sha256:a7cae23b57c5f5e5d61b59604e60f51a20a0e219e30bf87f7438e8f2837ddacb`  
		...
		Size: 493.0 B  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:e6efa0ef1d2ebc7dc94c3b7a79339375b387f261b31b4edb0b7ed1fbdb765f5a`  
		Last Modified: Thu, 18 Dec 2025 18:27:03 GMT  
		Size: 14.5 MB (14451252 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:651de16edb95a3546cf2c0a5d187a401ea0c7ac0b50c6957b0aff5cf451eb43e`  
		Last Modified: Thu, 18 Dec 2025 18:26:58 GMT  
		Size: 2.5 KB (2451 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:084c39c91149489883d97538b46f31f4f3400c05acfdedd622f2347656087a21`  
		Last Modified: Thu, 18 Dec 2025 18:26:58 GMT  
		Size: 23.3 KB (23324 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:ae58e69f03a6394a8a02cf75b23b3e689c0b7f20f5fad24b0391b9adc8e281a9`  
		Last Modified: Thu, 18 Dec 2025 18:26:58 GMT  
	...
	-	`sha256:4f4fb700ef54461cfa02571ae0db9a0dc1e0cdb5577484a6d75e68dc38e8acc1`  
		...
		Size: 32.0 B  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:74344a5b102554fef0f695d9a233a07f4b38ae8e02704a557741c04c90b47261`  
		Last Modified: Thu, 18 Dec 2025 18:26:58 GMT  
...
```console
$ docker pull php@sha256:b0df970655d4da31ef74b01132e608280ab0a9be970f277e88d8bef81ed98cd4
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **329.3 KB (329292 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4c58d0f44fe03d710011e4d55401e7a845fe19f3ca91ec871fa3041e909c9705`

```dockerfile
```

-	Layers:
	-	`sha256:23bc982706afeaa0c1e90d526bf9a71f76fc9c1058dabfd3bda11b1344f54c2f`  
		Last Modified: Thu, 18 Dec 2025 21:02:41 GMT  
		Size: 278.5 KB (278540 bytes)  
		MIME: application/vnd.in-toto+json
	-	`sha256:84d7ddf8772b728663bb1e66ffa9ebccee4dce2bdf929d3a8bd38f9c1dd41312`  
		Last Modified: Thu, 18 Dec 2025 21:02:45 GMT  
