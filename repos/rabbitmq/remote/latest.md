## `rabbitmq:latest`

```console
$ docker pull rabbitmq@sha256:16df08f231b32987f75793f2c9b7f4de9b7a636acd8ea6b0e192f44140d9f293
```

-	Manifest MIME: `application/vnd.oci.image.index.v1+json`
-	Platforms: 12
	-	linux; amd64
	-	unknown; unknown
	-	linux; arm variant v7
	-	unknown; unknown
	-	linux; arm64 variant v8
	-	unknown; unknown
	-	linux; ppc64le
	-	unknown; unknown
	-	linux; riscv64
	-	unknown; unknown
	-	linux; s390x
	-	unknown; unknown

### `rabbitmq:latest` - linux; amd64

```console
$ docker pull rabbitmq@sha256:d66a27a76b1b4f5a3cb0a24349e4e0c1debd3cfe271e857802a89565a639edcf
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **116.1 MB (116117774 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:054e008dd7c906de00ed44571858b2dc740fb9ecf6005336a1f03ba69422f60b`
-	Entrypoint: `["docker-entrypoint.sh"]`
-	Default Command: `["rabbitmq-server"]`

```dockerfile
# Wed, 20 May 2026 01:37:19 GMT
ARG RELEASE
# Wed, 20 May 2026 01:37:19 GMT
ARG LAUNCHPAD_BUILD_ARCH
# Wed, 20 May 2026 01:37:19 GMT
LABEL org.opencontainers.image.version=24.04
# Wed, 20 May 2026 01:37:21 GMT
ADD file:46ac5b8ee4c64ad9ebe840abd5619f571a617ac19483764d47d0eeba7907934f in / 
# Wed, 20 May 2026 01:37:22 GMT
CMD ["/bin/bash"]
# Mon, 15 Jun 2026 23:37:38 GMT
ENV ERLANG_INSTALL_PATH_PREFIX=/opt/erlang
# Mon, 15 Jun 2026 23:37:38 GMT
ENV OPENSSL_INSTALL_PATH_PREFIX=/opt/openssl
# Mon, 15 Jun 2026 23:37:38 GMT
COPY /opt/erlang /opt/erlang # buildkit
# Mon, 15 Jun 2026 23:37:38 GMT
COPY /opt/openssl /opt/openssl # buildkit
# Mon, 15 Jun 2026 23:37:38 GMT
ENV PATH=/opt/erlang/bin:/opt/openssl/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
# Mon, 15 Jun 2026 23:37:38 GMT
ENV RABBITMQ_DATA_DIR=/var/lib/rabbitmq
# Mon, 15 Jun 2026 23:37:40 GMT
RUN set -eux; 	ln -vsf /etc/ssl/certs /etc/ssl/private "$OPENSSL_INSTALL_PATH_PREFIX/etc/ssl"; 		ldconfig; 	sed -i.ORIG -e "/\.include.*fips/ s!.*!.include $OPENSSL_INSTALL_PATH_PREFIX/etc/ssl/fipsmodule.cnf!" 		-e '/# fips =/s/.*/fips = fips_sect/' "$OPENSSL_INSTALL_PATH_PREFIX/etc/ssl/openssl.cnf"; 	sed -i.ORIG -e '/^activate/s/^/#/' "$OPENSSL_INSTALL_PATH_PREFIX/etc/ssl/fipsmodule.cnf"; 	[ "$(command -v openssl)" = "$OPENSSL_INSTALL_PATH_PREFIX/bin/openssl" ]; 	openssl version; 	openssl version -d; 		erl -noshell -eval 'ok = crypto:start(), ok = io:format("~p~n~n~p~n~n", [crypto:supports(), ssl:versions()]), init:stop().'; 		groupadd --gid 999 --system rabbitmq; 	useradd --uid 999 --system --home-dir "$RABBITMQ_DATA_DIR" --gid rabbitmq rabbitmq; 	mkdir -p "$RABBITMQ_DATA_DIR" /etc/rabbitmq /etc/rabbitmq/conf.d /tmp/rabbitmq-ssl /var/log/rabbitmq; 	chown -fR rabbitmq:rabbitmq "$RABBITMQ_DATA_DIR" /etc/rabbitmq /etc/rabbitmq/conf.d /tmp/rabbitmq-ssl /var/log/rabbitmq; 	chmod 1777 "$RABBITMQ_DATA_DIR" /etc/rabbitmq /etc/rabbitmq/conf.d /tmp/rabbitmq-ssl /var/log/rabbitmq; 	ln -sf "$RABBITMQ_DATA_DIR/.erlang.cookie" /root/.erlang.cookie # buildkit
# Mon, 15 Jun 2026 23:37:40 GMT
ENV RABBITMQ_VERSION=4.3.2
# Mon, 15 Jun 2026 23:37:40 GMT
ENV RABBITMQ_PGP_KEY_ID=0x0A9AF2115F4687BD29803A206B73A36E6026DFCA
# Mon, 15 Jun 2026 23:37:40 GMT
ENV RABBITMQ_HOME=/opt/rabbitmq
# Mon, 15 Jun 2026 23:37:40 GMT
ENV PATH=/opt/rabbitmq/sbin:/opt/erlang/bin:/opt/openssl/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
# Mon, 15 Jun 2026 23:37:55 GMT
RUN set -eux; 	export DEBIAN_FRONTEND=noninteractive; 	apt-get update; 	apt-get install --yes --no-install-recommends 		ca-certificates 		gosu 		tzdata 	; 	gosu nobody true; 		savedAptMark="$(apt-mark showmanual)"; 	apt-get install --yes --no-install-recommends 		gnupg 		wget 		xz-utils 	; 	rm -rf /var/lib/apt/lists/*; 		RABBITMQ_SOURCE_URL="https://github.com/rabbitmq/rabbitmq-server/releases/download/v$RABBITMQ_VERSION/rabbitmq-server-generic-unix-latest-toolchain-$RABBITMQ_VERSION.tar.xz"; 	RABBITMQ_PATH="/usr/local/src/rabbitmq-$RABBITMQ_VERSION"; 		wget --progress dot:giga --output-document "$RABBITMQ_PATH.tar.xz.asc" "$RABBITMQ_SOURCE_URL.asc"; 	wget --progress dot:giga --output-document "$RABBITMQ_PATH.tar.xz" "$RABBITMQ_SOURCE_URL"; 		export GNUPGHOME="$(mktemp -d)"; 	gpg --batch --keyserver hkps://keys.openpgp.org --recv-keys "$RABBITMQ_PGP_KEY_ID"; 	gpg --batch --verify "$RABBITMQ_PATH.tar.xz.asc" "$RABBITMQ_PATH.tar.xz"; 	gpgconf --kill all; 	rm -rf "$GNUPGHOME"; 		mkdir -p "$RABBITMQ_HOME"; 	tar --extract --file "$RABBITMQ_PATH.tar.xz" --directory "$RABBITMQ_HOME" --strip-components 1; 	rm -rf "$RABBITMQ_PATH"*; 	grep -qE '^SYS_PREFIX=\$\{RABBITMQ_HOME\}$' "$RABBITMQ_HOME/sbin/rabbitmq-defaults"; 	sed -i 's/^SYS_PREFIX=.*$/SYS_PREFIX=/' "$RABBITMQ_HOME/sbin/rabbitmq-defaults"; 	grep -qE '^SYS_PREFIX=$' "$RABBITMQ_HOME/sbin/rabbitmq-defaults"; 	chown -R rabbitmq:rabbitmq "$RABBITMQ_HOME"; 		apt-mark auto '.*' > /dev/null; 	apt-mark manual $savedAptMark; 	apt-get purge -y --auto-remove -o APT::AutoRemove::RecommendsImportant=false; 		[ ! -e "$RABBITMQ_DATA_DIR/.erlang.cookie" ]; 	gosu rabbitmq rabbitmqctl help; 	gosu rabbitmq rabbitmqctl list_ciphers; 	gosu rabbitmq rabbitmq-plugins list; 	rm "$RABBITMQ_DATA_DIR/.erlang.cookie" # buildkit
# Mon, 15 Jun 2026 23:37:56 GMT
RUN gosu rabbitmq rabbitmq-plugins enable --offline rabbitmq_prometheus # buildkit
# Mon, 15 Jun 2026 23:37:56 GMT
RUN ln -sf /opt/rabbitmq/plugins /plugins # buildkit
# Mon, 15 Jun 2026 23:37:56 GMT
ENV HOME=/var/lib/rabbitmq
# Mon, 15 Jun 2026 23:37:56 GMT
VOLUME [/var/lib/rabbitmq]
# Mon, 15 Jun 2026 23:37:56 GMT
ENV LANG=C.UTF-8 LANGUAGE=C.UTF-8 LC_ALL=C.UTF-8
# Mon, 15 Jun 2026 23:37:56 GMT
ENV RUNNING_UNDER_SYSTEMD=true
# Mon, 15 Jun 2026 23:37:56 GMT
COPY --chown=rabbitmq:rabbitmq 10-defaults.conf 20-management_agent.disable_metrics_collector.conf /etc/rabbitmq/conf.d/ # buildkit
# Mon, 15 Jun 2026 23:37:56 GMT
COPY docker-entrypoint.sh /usr/local/bin/ # buildkit
# Mon, 15 Jun 2026 23:37:56 GMT
ENTRYPOINT ["docker-entrypoint.sh"]
# Mon, 15 Jun 2026 23:37:56 GMT
EXPOSE map[15691/tcp:{} 15692/tcp:{} 25672/tcp:{} 4369/tcp:{} 5671/tcp:{} 5672/tcp:{}]
# Mon, 15 Jun 2026 23:37:56 GMT
CMD ["rabbitmq-server"]
```

-	Layers:
	-	`sha256:cb259a83ac3dd9fea0b394df41df2b298adf0df938fef5999475af18a751c257`  
		Last Modified: Wed, 20 May 2026 02:15:22 GMT  
		Size: 29.7 MB (29732805 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:55bd47c33cbd6400dcf010817ee162479cc932cab96155912ed97e007fa041e6`  
		Last Modified: Mon, 15 Jun 2026 23:38:21 GMT  
		Size: 46.3 MB (46300182 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:54d6a401063fbb2018dd34bf5b4c626c7b4a246f1ce3bf27310637e7d46b824b`  
		Last Modified: Mon, 15 Jun 2026 23:38:20 GMT  
		Size: 9.0 MB (8994453 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:385aaec6047432e94b753b9522b62a85ff3347d2d41a0743ea39eabf98c79c6e`  
		Last Modified: Mon, 15 Jun 2026 23:38:19 GMT  
		Size: 9.7 KB (9686 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:8cb9b36d25458a486f13bc80083d52c1de71c1b3cd75b4f437a3c80368d5af5a`  
		Last Modified: Mon, 15 Jun 2026 23:38:21 GMT  
		Size: 31.1 MB (31078898 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:ef6d48577298e2ee5b930be1d10d0857b1dfc41ad13fed4ff6652265319ef37a`  
		Last Modified: Mon, 15 Jun 2026 23:38:20 GMT  
		Size: 188.0 B  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:cc6025a68e4fa22487bd25e6b79d3565c92c5ea6ad644d3c006d7730a10d1bf2`  
		Last Modified: Mon, 15 Jun 2026 23:38:21 GMT  
		Size: 109.0 B  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:811d0b8176dd6bbfa774d8199a1d3d919eb86b137344a0d1f1513073426696fa`  
		Last Modified: Mon, 15 Jun 2026 23:38:22 GMT  
		Size: 619.0 B  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:1b9ce3e13d206fcabffce6caa748b8b623c364d406e9d63dbafd07b685497d4a`  
		Last Modified: Mon, 15 Jun 2026 23:38:22 GMT  
		Size: 834.0 B  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip

### `rabbitmq:latest` - unknown; unknown

```console
$ docker pull rabbitmq@sha256:a825352b74a7d2786146febac99a58a07410e92ef004b2536d1a32a8e989d62d
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **18.8 MB (18841051 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b81278e0742898455bf30d9903f3ad6a8c10a270e6b2a78c423aff1bfec39a63`

```dockerfile
```

-	Layers:
	-	`sha256:bc363ae59dd0d1aa12f23bf1e711a7381a907e965927ba1005b35c286e7934c0`  
		Last Modified: Mon, 15 Jun 2026 23:38:20 GMT  
		Size: 2.5 MB (2486715 bytes)  
		MIME: application/vnd.in-toto+json
	-	`sha256:b94e3fca93e4bb8bfe702523254494e29f05985c293985b2bab31e1b0349e224`  
		Last Modified: Mon, 15 Jun 2026 23:38:20 GMT  
		Size: 5.4 MB (5378535 bytes)  
		MIME: application/vnd.in-toto+json
	-	`sha256:4f6c4c3addc9382641f257edb114e13c76829fddb808d4db6592b38e0198ad21`  
		Last Modified: Mon, 15 Jun 2026 23:38:19 GMT  
		Size: 5.5 MB (5535324 bytes)  
		MIME: application/vnd.in-toto+json
	-	`sha256:dac4a664122d2f2df0d970f28f29ebd922038014ffa938d1f07088dba6022fda`  
		Last Modified: Mon, 15 Jun 2026 23:38:19 GMT  
		Size: 5.4 MB (5380277 bytes)  
		MIME: application/vnd.in-toto+json
	-	`sha256:28102685d583b7279e3dca81bcf8b5c308e8888b61aacc2352f6da5a3a69f7a5`  
		Last Modified: Mon, 15 Jun 2026 23:38:21 GMT  
		Size: 60.2 KB (60200 bytes)  
		MIME: application/vnd.in-toto+json

### `rabbitmq:latest` - linux; arm variant v7

```console
$ docker pull rabbitmq@sha256:639baf9445c8698b2499ddbc716eb3065011cc53bdc58727f47746327c900412
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **97.8 MB (97849854 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:09b549b3b06da32fc4c05620c9245539275fa6e476b099626450686cfb0a5fbc`
-	Entrypoint: `["docker-entrypoint.sh"]`
-	Default Command: `["rabbitmq-server"]`

```dockerfile
# Wed, 20 May 2026 01:36:59 GMT
ARG RELEASE
# Wed, 20 May 2026 01:36:59 GMT
ARG LAUNCHPAD_BUILD_ARCH
# Wed, 20 May 2026 01:36:59 GMT
LABEL org.opencontainers.image.version=24.04
# Wed, 20 May 2026 01:37:02 GMT
ADD file:6d117ff682b1d31146902ad551197b012e75561d62d92d029219fcbf5c493c35 in / 
# Wed, 20 May 2026 01:37:02 GMT
CMD ["/bin/bash"]
# Mon, 15 Jun 2026 23:18:23 GMT
ENV ERLANG_INSTALL_PATH_PREFIX=/opt/erlang
# Mon, 15 Jun 2026 23:18:23 GMT
ENV OPENSSL_INSTALL_PATH_PREFIX=/opt/openssl
# Mon, 15 Jun 2026 23:18:23 GMT
COPY /opt/erlang /opt/erlang # buildkit
# Mon, 15 Jun 2026 23:18:23 GMT
COPY /opt/openssl /opt/openssl # buildkit
# Mon, 15 Jun 2026 23:18:23 GMT
ENV PATH=/opt/erlang/bin:/opt/openssl/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
# Mon, 15 Jun 2026 23:18:23 GMT
ENV RABBITMQ_DATA_DIR=/var/lib/rabbitmq
# Mon, 15 Jun 2026 23:18:24 GMT
RUN set -eux; 	ln -vsf /etc/ssl/certs /etc/ssl/private "$OPENSSL_INSTALL_PATH_PREFIX/etc/ssl"; 		ldconfig; 	sed -i.ORIG -e "/\.include.*fips/ s!.*!.include $OPENSSL_INSTALL_PATH_PREFIX/etc/ssl/fipsmodule.cnf!" 		-e '/# fips =/s/.*/fips = fips_sect/' "$OPENSSL_INSTALL_PATH_PREFIX/etc/ssl/openssl.cnf"; 	sed -i.ORIG -e '/^activate/s/^/#/' "$OPENSSL_INSTALL_PATH_PREFIX/etc/ssl/fipsmodule.cnf"; 	[ "$(command -v openssl)" = "$OPENSSL_INSTALL_PATH_PREFIX/bin/openssl" ]; 	openssl version; 	openssl version -d; 		erl -noshell -eval 'ok = crypto:start(), ok = io:format("~p~n~n~p~n~n", [crypto:supports(), ssl:versions()]), init:stop().'; 		groupadd --gid 999 --system rabbitmq; 	useradd --uid 999 --system --home-dir "$RABBITMQ_DATA_DIR" --gid rabbitmq rabbitmq; 	mkdir -p "$RABBITMQ_DATA_DIR" /etc/rabbitmq /etc/rabbitmq/conf.d /tmp/rabbitmq-ssl /var/log/rabbitmq; 	chown -fR rabbitmq:rabbitmq "$RABBITMQ_DATA_DIR" /etc/rabbitmq /etc/rabbitmq/conf.d /tmp/rabbitmq-ssl /var/log/rabbitmq; 	chmod 1777 "$RABBITMQ_DATA_DIR" /etc/rabbitmq /etc/rabbitmq/conf.d /tmp/rabbitmq-ssl /var/log/rabbitmq; 	ln -sf "$RABBITMQ_DATA_DIR/.erlang.cookie" /root/.erlang.cookie # buildkit
# Mon, 15 Jun 2026 23:18:24 GMT
ENV RABBITMQ_VERSION=4.3.2
# Mon, 15 Jun 2026 23:18:24 GMT
ENV RABBITMQ_PGP_KEY_ID=0x0A9AF2115F4687BD29803A206B73A36E6026DFCA
# Mon, 15 Jun 2026 23:18:24 GMT
ENV RABBITMQ_HOME=/opt/rabbitmq
# Mon, 15 Jun 2026 23:18:24 GMT
ENV PATH=/opt/rabbitmq/sbin:/opt/erlang/bin:/opt/openssl/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
# Mon, 15 Jun 2026 23:18:43 GMT
RUN set -eux; 	export DEBIAN_FRONTEND=noninteractive; 	apt-get update; 	apt-get install --yes --no-install-recommends 		ca-certificates 		gosu 		tzdata 	; 	gosu nobody true; 		savedAptMark="$(apt-mark showmanual)"; 	apt-get install --yes --no-install-recommends 		gnupg 		wget 		xz-utils 	; 	rm -rf /var/lib/apt/lists/*; 		RABBITMQ_SOURCE_URL="https://github.com/rabbitmq/rabbitmq-server/releases/download/v$RABBITMQ_VERSION/rabbitmq-server-generic-unix-latest-toolchain-$RABBITMQ_VERSION.tar.xz"; 	RABBITMQ_PATH="/usr/local/src/rabbitmq-$RABBITMQ_VERSION"; 		wget --progress dot:giga --output-document "$RABBITMQ_PATH.tar.xz.asc" "$RABBITMQ_SOURCE_URL.asc"; 	wget --progress dot:giga --output-document "$RABBITMQ_PATH.tar.xz" "$RABBITMQ_SOURCE_URL"; 		export GNUPGHOME="$(mktemp -d)"; 	gpg --batch --keyserver hkps://keys.openpgp.org --recv-keys "$RABBITMQ_PGP_KEY_ID"; 	gpg --batch --verify "$RABBITMQ_PATH.tar.xz.asc" "$RABBITMQ_PATH.tar.xz"; 	gpgconf --kill all; 	rm -rf "$GNUPGHOME"; 		mkdir -p "$RABBITMQ_HOME"; 	tar --extract --file "$RABBITMQ_PATH.tar.xz" --directory "$RABBITMQ_HOME" --strip-components 1; 	rm -rf "$RABBITMQ_PATH"*; 	grep -qE '^SYS_PREFIX=\$\{RABBITMQ_HOME\}$' "$RABBITMQ_HOME/sbin/rabbitmq-defaults"; 	sed -i 's/^SYS_PREFIX=.*$/SYS_PREFIX=/' "$RABBITMQ_HOME/sbin/rabbitmq-defaults"; 	grep -qE '^SYS_PREFIX=$' "$RABBITMQ_HOME/sbin/rabbitmq-defaults"; 	chown -R rabbitmq:rabbitmq "$RABBITMQ_HOME"; 		apt-mark auto '.*' > /dev/null; 	apt-mark manual $savedAptMark; 	apt-get purge -y --auto-remove -o APT::AutoRemove::RecommendsImportant=false; 		[ ! -e "$RABBITMQ_DATA_DIR/.erlang.cookie" ]; 	gosu rabbitmq rabbitmqctl help; 	gosu rabbitmq rabbitmqctl list_ciphers; 	gosu rabbitmq rabbitmq-plugins list; 	rm "$RABBITMQ_DATA_DIR/.erlang.cookie" # buildkit
# Mon, 15 Jun 2026 23:18:43 GMT
RUN gosu rabbitmq rabbitmq-plugins enable --offline rabbitmq_prometheus # buildkit
# Mon, 15 Jun 2026 23:18:43 GMT
RUN ln -sf /opt/rabbitmq/plugins /plugins # buildkit
# Mon, 15 Jun 2026 23:18:43 GMT
ENV HOME=/var/lib/rabbitmq
# Mon, 15 Jun 2026 23:18:43 GMT
VOLUME [/var/lib/rabbitmq]
# Mon, 15 Jun 2026 23:18:43 GMT
ENV LANG=C.UTF-8 LANGUAGE=C.UTF-8 LC_ALL=C.UTF-8
# Mon, 15 Jun 2026 23:18:43 GMT
ENV RUNNING_UNDER_SYSTEMD=true
# Mon, 15 Jun 2026 23:18:44 GMT
COPY --chown=rabbitmq:rabbitmq 10-defaults.conf 20-management_agent.disable_metrics_collector.conf /etc/rabbitmq/conf.d/ # buildkit
# Mon, 15 Jun 2026 23:18:44 GMT
COPY docker-entrypoint.sh /usr/local/bin/ # buildkit
# Mon, 15 Jun 2026 23:18:44 GMT
ENTRYPOINT ["docker-entrypoint.sh"]
# Mon, 15 Jun 2026 23:18:44 GMT
EXPOSE map[15691/tcp:{} 15692/tcp:{} 25672/tcp:{} 4369/tcp:{} 5671/tcp:{} 5672/tcp:{}]
# Mon, 15 Jun 2026 23:18:44 GMT
CMD ["rabbitmq-server"]
```

-	Layers:
	-	`sha256:a2dede8d0e9ca179460cb274dab10c5c4b741cf1544b130df872809a4467e3e4`  
		Last Modified: Wed, 20 May 2026 02:15:37 GMT  
		Size: 26.9 MB (26859709 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:b299630d2c7579b229c58f66c189e16f9bbd2b9067a76eb89765c84cd24a601a`  
		Last Modified: Mon, 15 Jun 2026 23:19:09 GMT  
		Size: 33.3 MB (33320883 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:14e98e2b3c5691d00b5af418f2108e8d0f173642062701605706871208dd1946`  
		Last Modified: Mon, 15 Jun 2026 23:19:07 GMT  
		Size: 7.3 MB (7314898 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:4e533a260f9eb9f8d3eccbe9b894d18f65fdc4d019140d9ed12ff5b6b6eb00aa`  
		Last Modified: Mon, 15 Jun 2026 23:19:07 GMT  
		Size: 9.7 KB (9721 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:749b7e988501f49188de2b0d4d4a6246b93bd8f25f26242e7a74b8d59b8a6373`  
		Last Modified: Mon, 15 Jun 2026 23:19:09 GMT  
		Size: 30.3 MB (30342893 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:0c50f48a7777dfadc3f8b782fb72812a23b8546979e40d8aef9894ccacdbf161`  
		Last Modified: Mon, 15 Jun 2026 23:19:08 GMT  
		Size: 189.0 B  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:f6c9f144d8d05b4efaf0728f4e3105254bbc1a7e4e7703badbebfa75886a311c`  
		Last Modified: Mon, 15 Jun 2026 23:19:09 GMT  
		Size: 109.0 B  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:aa8044b951f4594689cfcb2a0b9e04c517f5860930f562a8ae136a81329906e3`  
		Last Modified: Mon, 15 Jun 2026 23:19:09 GMT  
		Size: 620.0 B  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:c35307e2fd42ceb73570f54200c74be3ea8ae2a7a3f70dbdd05a28e5b6dcac14`  
		Last Modified: Mon, 15 Jun 2026 23:19:10 GMT  
		Size: 832.0 B  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip

### `rabbitmq:latest` - unknown; unknown

```console
$ docker pull rabbitmq@sha256:0b9891b9d74bc4e93d93dfb99f9c2bc74683bf470482749393442b7986fb8105
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **18.3 MB (18295766 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:6ba34380650ead267a00a8eacc82d6557a064a1fdad4e581d69e47b61361816a`

```dockerfile
```

-	Layers:
	-	`sha256:0c0b8b747e1396a2d252e282e97de8121afaa0e6a6993b583a02d9eb38e93d85`  
		Last Modified: Mon, 15 Jun 2026 23:19:07 GMT  
		Size: 2.5 MB (2487515 bytes)  
		MIME: application/vnd.in-toto+json
	-	`sha256:4a87b711b0ea14d1224306ae238c7e1f1ecbeeca03dd0c8daf21125572db4d31`  
		Last Modified: Mon, 15 Jun 2026 23:19:07 GMT  
		Size: 5.2 MB (5197293 bytes)  
		MIME: application/vnd.in-toto+json
	-	`sha256:b2f9046d84e8de69d51bc1baa4d5d101473b630ca5520d6bd484b2dd7dd680e8`  
		Last Modified: Mon, 15 Jun 2026 23:19:07 GMT  
		Size: 5.4 MB (5351525 bytes)  
		MIME: application/vnd.in-toto+json
	-	`sha256:6837011b333a83918b7bcc2d479f6020cda8b9c0953edeac118baa7a4e30b350`  
		Last Modified: Mon, 15 Jun 2026 23:19:07 GMT  
		Size: 5.2 MB (5199035 bytes)  
		MIME: application/vnd.in-toto+json
	-	`sha256:4c162acc11151ef8892371891fc6394c3d93d73c0a387904a5104dacca83f4db`  
		Last Modified: Mon, 15 Jun 2026 23:19:08 GMT  
		Size: 60.4 KB (60398 bytes)  
		MIME: application/vnd.in-toto+json

### `rabbitmq:latest` - linux; arm64 variant v8

```console
$ docker pull rabbitmq@sha256:59fe8db91a51d87545b288057a6e8b274ed000266a0f7f7002e7b7e93710d2a7
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **113.8 MB (113797351 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b6333aacc834e768c2d9f226839decc8a5493dc82b7f70256de111c0d441eb4d`
-	Entrypoint: `["docker-entrypoint.sh"]`
-	Default Command: `["rabbitmq-server"]`

```dockerfile
# Wed, 20 May 2026 01:37:31 GMT
ARG RELEASE
# Wed, 20 May 2026 01:37:31 GMT
ARG LAUNCHPAD_BUILD_ARCH
# Wed, 20 May 2026 01:37:31 GMT
LABEL org.opencontainers.image.version=24.04
# Wed, 20 May 2026 01:37:34 GMT
ADD file:08e1f650999ca51d9b63c782d658d9485c64263966d69dc423a3b64a16449f00 in / 
# Wed, 20 May 2026 01:37:34 GMT
CMD ["/bin/bash"]
# Mon, 15 Jun 2026 23:30:17 GMT
ENV ERLANG_INSTALL_PATH_PREFIX=/opt/erlang
# Mon, 15 Jun 2026 23:30:17 GMT
ENV OPENSSL_INSTALL_PATH_PREFIX=/opt/openssl
# Mon, 15 Jun 2026 23:30:17 GMT
COPY /opt/erlang /opt/erlang # buildkit
# Mon, 15 Jun 2026 23:30:17 GMT
COPY /opt/openssl /opt/openssl # buildkit
# Mon, 15 Jun 2026 23:30:17 GMT
ENV PATH=/opt/erlang/bin:/opt/openssl/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
# Mon, 15 Jun 2026 23:30:17 GMT
ENV RABBITMQ_DATA_DIR=/var/lib/rabbitmq
# Mon, 15 Jun 2026 23:30:18 GMT
RUN set -eux; 	ln -vsf /etc/ssl/certs /etc/ssl/private "$OPENSSL_INSTALL_PATH_PREFIX/etc/ssl"; 		ldconfig; 	sed -i.ORIG -e "/\.include.*fips/ s!.*!.include $OPENSSL_INSTALL_PATH_PREFIX/etc/ssl/fipsmodule.cnf!" 		-e '/# fips =/s/.*/fips = fips_sect/' "$OPENSSL_INSTALL_PATH_PREFIX/etc/ssl/openssl.cnf"; 	sed -i.ORIG -e '/^activate/s/^/#/' "$OPENSSL_INSTALL_PATH_PREFIX/etc/ssl/fipsmodule.cnf"; 	[ "$(command -v openssl)" = "$OPENSSL_INSTALL_PATH_PREFIX/bin/openssl" ]; 	openssl version; 	openssl version -d; 		erl -noshell -eval 'ok = crypto:start(), ok = io:format("~p~n~n~p~n~n", [crypto:supports(), ssl:versions()]), init:stop().'; 		groupadd --gid 999 --system rabbitmq; 	useradd --uid 999 --system --home-dir "$RABBITMQ_DATA_DIR" --gid rabbitmq rabbitmq; 	mkdir -p "$RABBITMQ_DATA_DIR" /etc/rabbitmq /etc/rabbitmq/conf.d /tmp/rabbitmq-ssl /var/log/rabbitmq; 	chown -fR rabbitmq:rabbitmq "$RABBITMQ_DATA_DIR" /etc/rabbitmq /etc/rabbitmq/conf.d /tmp/rabbitmq-ssl /var/log/rabbitmq; 	chmod 1777 "$RABBITMQ_DATA_DIR" /etc/rabbitmq /etc/rabbitmq/conf.d /tmp/rabbitmq-ssl /var/log/rabbitmq; 	ln -sf "$RABBITMQ_DATA_DIR/.erlang.cookie" /root/.erlang.cookie # buildkit
# Mon, 15 Jun 2026 23:30:18 GMT
ENV RABBITMQ_VERSION=4.3.2
# Mon, 15 Jun 2026 23:30:18 GMT
ENV RABBITMQ_PGP_KEY_ID=0x0A9AF2115F4687BD29803A206B73A36E6026DFCA
# Mon, 15 Jun 2026 23:30:18 GMT
ENV RABBITMQ_HOME=/opt/rabbitmq
# Mon, 15 Jun 2026 23:30:18 GMT
ENV PATH=/opt/rabbitmq/sbin:/opt/erlang/bin:/opt/openssl/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
# Mon, 15 Jun 2026 23:30:34 GMT
RUN set -eux; 	export DEBIAN_FRONTEND=noninteractive; 	apt-get update; 	apt-get install --yes --no-install-recommends 		ca-certificates 		gosu 		tzdata 	; 	gosu nobody true; 		savedAptMark="$(apt-mark showmanual)"; 	apt-get install --yes --no-install-recommends 		gnupg 		wget 		xz-utils 	; 	rm -rf /var/lib/apt/lists/*; 		RABBITMQ_SOURCE_URL="https://github.com/rabbitmq/rabbitmq-server/releases/download/v$RABBITMQ_VERSION/rabbitmq-server-generic-unix-latest-toolchain-$RABBITMQ_VERSION.tar.xz"; 	RABBITMQ_PATH="/usr/local/src/rabbitmq-$RABBITMQ_VERSION"; 		wget --progress dot:giga --output-document "$RABBITMQ_PATH.tar.xz.asc" "$RABBITMQ_SOURCE_URL.asc"; 	wget --progress dot:giga --output-document "$RABBITMQ_PATH.tar.xz" "$RABBITMQ_SOURCE_URL"; 		export GNUPGHOME="$(mktemp -d)"; 	gpg --batch --keyserver hkps://keys.openpgp.org --recv-keys "$RABBITMQ_PGP_KEY_ID"; 	gpg --batch --verify "$RABBITMQ_PATH.tar.xz.asc" "$RABBITMQ_PATH.tar.xz"; 	gpgconf --kill all; 	rm -rf "$GNUPGHOME"; 		mkdir -p "$RABBITMQ_HOME"; 	tar --extract --file "$RABBITMQ_PATH.tar.xz" --directory "$RABBITMQ_HOME" --strip-components 1; 	rm -rf "$RABBITMQ_PATH"*; 	grep -qE '^SYS_PREFIX=\$\{RABBITMQ_HOME\}$' "$RABBITMQ_HOME/sbin/rabbitmq-defaults"; 	sed -i 's/^SYS_PREFIX=.*$/SYS_PREFIX=/' "$RABBITMQ_HOME/sbin/rabbitmq-defaults"; 	grep -qE '^SYS_PREFIX=$' "$RABBITMQ_HOME/sbin/rabbitmq-defaults"; 	chown -R rabbitmq:rabbitmq "$RABBITMQ_HOME"; 		apt-mark auto '.*' > /dev/null; 	apt-mark manual $savedAptMark; 	apt-get purge -y --auto-remove -o APT::AutoRemove::RecommendsImportant=false; 		[ ! -e "$RABBITMQ_DATA_DIR/.erlang.cookie" ]; 	gosu rabbitmq rabbitmqctl help; 	gosu rabbitmq rabbitmqctl list_ciphers; 	gosu rabbitmq rabbitmq-plugins list; 	rm "$RABBITMQ_DATA_DIR/.erlang.cookie" # buildkit
# Mon, 15 Jun 2026 23:30:35 GMT
RUN gosu rabbitmq rabbitmq-plugins enable --offline rabbitmq_prometheus # buildkit
# Mon, 15 Jun 2026 23:30:35 GMT
RUN ln -sf /opt/rabbitmq/plugins /plugins # buildkit
# Mon, 15 Jun 2026 23:30:35 GMT
ENV HOME=/var/lib/rabbitmq
# Mon, 15 Jun 2026 23:30:35 GMT
VOLUME [/var/lib/rabbitmq]
# Mon, 15 Jun 2026 23:30:35 GMT
ENV LANG=C.UTF-8 LANGUAGE=C.UTF-8 LC_ALL=C.UTF-8
# Mon, 15 Jun 2026 23:30:35 GMT
ENV RUNNING_UNDER_SYSTEMD=true
# Mon, 15 Jun 2026 23:30:35 GMT
COPY --chown=rabbitmq:rabbitmq 10-defaults.conf 20-management_agent.disable_metrics_collector.conf /etc/rabbitmq/conf.d/ # buildkit
# Mon, 15 Jun 2026 23:30:35 GMT
COPY docker-entrypoint.sh /usr/local/bin/ # buildkit
# Mon, 15 Jun 2026 23:30:35 GMT
ENTRYPOINT ["docker-entrypoint.sh"]
# Mon, 15 Jun 2026 23:30:35 GMT
EXPOSE map[15691/tcp:{} 15692/tcp:{} 25672/tcp:{} 4369/tcp:{} 5671/tcp:{} 5672/tcp:{}]
# Mon, 15 Jun 2026 23:30:35 GMT
CMD ["rabbitmq-server"]
```

-	Layers:
	-	`sha256:fff3795b437199a0b714aadba6fb2c251d7da853c3e257d3fed1d2c8d0f05158`  
		Last Modified: Wed, 20 May 2026 02:15:29 GMT  
		Size: 28.9 MB (28876406 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:9bfc41ac642d17a79c0db42327b4741f59a38fd80392094e2e824f6298157e83`  
		Last Modified: Mon, 15 Jun 2026 23:31:02 GMT  
		Size: 44.4 MB (44384964 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:3a0a9e05b6ed41b81f4cae3b976d8a7d4c1713b3aec2089f2628a56445be8a52`  
		Last Modified: Mon, 15 Jun 2026 23:31:00 GMT  
		Size: 9.7 MB (9722825 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:6d0e969800fc60888d29b4d2fd42d78ee4c075bf459f2f5f35bbe3fd9452b0ee`  
		Last Modified: Mon, 15 Jun 2026 23:31:00 GMT  
		Size: 9.7 KB (9665 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:f9c9514d41cf8b1c74adf08e7bebb62f4f142b1bfa24ca5ab86291c52855d286`  
		Last Modified: Mon, 15 Jun 2026 23:31:01 GMT  
		Size: 30.8 MB (30801741 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:5d403980ee69f61a3a72f46557f2630e1c69d7f8ccea2e42f949082cc66d3c2c`  
		Last Modified: Mon, 15 Jun 2026 23:31:01 GMT  
		Size: 189.0 B  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:c653a108bad677a17e1e480580fc1ce49867cdf73f2dfbbe3c48b32e1e9ac379`  
		Last Modified: Mon, 15 Jun 2026 23:31:02 GMT  
		Size: 109.0 B  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:cfad308721246e922a477d7414dbbfdf9e8cec2352097fcfc76b0e53475cabef`  
		Last Modified: Mon, 15 Jun 2026 23:31:02 GMT  
		Size: 619.0 B  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:5ce9b253f0498ac0d99816767aa638081ad7136529fe81fd5ac2aeb46cc418d1`  
		Last Modified: Mon, 15 Jun 2026 23:31:03 GMT  
		Size: 833.0 B  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip

### `rabbitmq:latest` - unknown; unknown

```console
$ docker pull rabbitmq@sha256:4c8dbcf0e84f18d587302aff4794b2278c257800cf671825013060dd3039497c
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **18.9 MB (18900020 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:f301ff82b7b495970ee7604b10d4492ecb3c2b1a6a0ced9352ab747a41a19d3d`

```dockerfile
```

-	Layers:
	-	`sha256:77cf4c0e17014e61733bfcf3981e51a7d313fd6707c366c902f91d755436ffd2`  
		Last Modified: Mon, 15 Jun 2026 23:31:00 GMT  
		Size: 2.5 MB (2487775 bytes)  
		MIME: application/vnd.in-toto+json
	-	`sha256:c9887bec48301ed095a2a9cb571e1468027f35152d7faee313a22a6ddf8f25f2`  
		Last Modified: Mon, 15 Jun 2026 23:31:00 GMT  
		Size: 5.4 MB (5397752 bytes)  
		MIME: application/vnd.in-toto+json
	-	`sha256:4c0cdca4a267a3b504ca6ce90940250962e61050459e20056e5f6a89dff68339`  
		Last Modified: Mon, 15 Jun 2026 23:31:00 GMT  
		Size: 5.6 MB (5554559 bytes)  
		MIME: application/vnd.in-toto+json
	-	`sha256:f9187dd9da9a48dffa04d5f853821a572d934e4b89615aa2d000e0ed42ecc9c2`  
		Last Modified: Mon, 15 Jun 2026 23:31:00 GMT  
		Size: 5.4 MB (5399494 bytes)  
		MIME: application/vnd.in-toto+json
	-	`sha256:8ca63185ae81fc03761494db1387f1e4c4d2883281342275991fc8993599def4`  
		Last Modified: Mon, 15 Jun 2026 23:31:01 GMT  
		Size: 60.4 KB (60440 bytes)  
		MIME: application/vnd.in-toto+json

### `rabbitmq:latest` - linux; ppc64le

```console
$ docker pull rabbitmq@sha256:4ed9aff48c07463b2cea925b6aa385ee4e5e827208d3792fb15298e202fb9be6
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **114.8 MB (114755338 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:4dff7c79a5cae001377c9a7820059bd2794eca08a84434218e12aeb92b1e6ad7`
-	Entrypoint: `["docker-entrypoint.sh"]`
-	Default Command: `["rabbitmq-server"]`

```dockerfile
# Wed, 20 May 2026 01:37:26 GMT
ARG RELEASE
# Wed, 20 May 2026 01:37:26 GMT
ARG LAUNCHPAD_BUILD_ARCH
# Wed, 20 May 2026 01:37:26 GMT
LABEL org.opencontainers.image.version=24.04
# Wed, 20 May 2026 01:37:29 GMT
ADD file:25dad72762cb0d82bbf57f17b8713b1ca4d35e813d99be37e61090f10acd5d92 in / 
# Wed, 20 May 2026 01:37:30 GMT
CMD ["/bin/bash"]
# Wed, 10 Jun 2026 20:37:04 GMT
ENV ERLANG_INSTALL_PATH_PREFIX=/opt/erlang
# Wed, 10 Jun 2026 20:37:04 GMT
ENV OPENSSL_INSTALL_PATH_PREFIX=/opt/openssl
# Wed, 10 Jun 2026 20:37:04 GMT
COPY /opt/erlang /opt/erlang # buildkit
# Wed, 10 Jun 2026 20:37:05 GMT
COPY /opt/openssl /opt/openssl # buildkit
# Wed, 10 Jun 2026 20:37:05 GMT
ENV PATH=/opt/erlang/bin:/opt/openssl/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
# Wed, 10 Jun 2026 20:37:05 GMT
ENV RABBITMQ_DATA_DIR=/var/lib/rabbitmq
# Wed, 10 Jun 2026 20:37:07 GMT
RUN set -eux; 	ln -vsf /etc/ssl/certs /etc/ssl/private "$OPENSSL_INSTALL_PATH_PREFIX/etc/ssl"; 		ldconfig; 	sed -i.ORIG -e "/\.include.*fips/ s!.*!.include $OPENSSL_INSTALL_PATH_PREFIX/etc/ssl/fipsmodule.cnf!" 		-e '/# fips =/s/.*/fips = fips_sect/' "$OPENSSL_INSTALL_PATH_PREFIX/etc/ssl/openssl.cnf"; 	sed -i.ORIG -e '/^activate/s/^/#/' "$OPENSSL_INSTALL_PATH_PREFIX/etc/ssl/fipsmodule.cnf"; 	[ "$(command -v openssl)" = "$OPENSSL_INSTALL_PATH_PREFIX/bin/openssl" ]; 	openssl version; 	openssl version -d; 		erl -noshell -eval 'ok = crypto:start(), ok = io:format("~p~n~n~p~n~n", [crypto:supports(), ssl:versions()]), init:stop().'; 		groupadd --gid 999 --system rabbitmq; 	useradd --uid 999 --system --home-dir "$RABBITMQ_DATA_DIR" --gid rabbitmq rabbitmq; 	mkdir -p "$RABBITMQ_DATA_DIR" /etc/rabbitmq /etc/rabbitmq/conf.d /tmp/rabbitmq-ssl /var/log/rabbitmq; 	chown -fR rabbitmq:rabbitmq "$RABBITMQ_DATA_DIR" /etc/rabbitmq /etc/rabbitmq/conf.d /tmp/rabbitmq-ssl /var/log/rabbitmq; 	chmod 1777 "$RABBITMQ_DATA_DIR" /etc/rabbitmq /etc/rabbitmq/conf.d /tmp/rabbitmq-ssl /var/log/rabbitmq; 	ln -sf "$RABBITMQ_DATA_DIR/.erlang.cookie" /root/.erlang.cookie # buildkit
# Wed, 10 Jun 2026 20:37:07 GMT
ENV RABBITMQ_VERSION=4.3.2
# Wed, 10 Jun 2026 20:37:07 GMT
ENV RABBITMQ_PGP_KEY_ID=0x0A9AF2115F4687BD29803A206B73A36E6026DFCA
# Wed, 10 Jun 2026 20:37:07 GMT
ENV RABBITMQ_HOME=/opt/rabbitmq
# Wed, 10 Jun 2026 20:37:07 GMT
ENV PATH=/opt/rabbitmq/sbin:/opt/erlang/bin:/opt/openssl/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
# Mon, 15 Jun 2026 23:42:59 GMT
RUN set -eux; 	export DEBIAN_FRONTEND=noninteractive; 	apt-get update; 	apt-get install --yes --no-install-recommends 		ca-certificates 		gosu 		tzdata 	; 	gosu nobody true; 		savedAptMark="$(apt-mark showmanual)"; 	apt-get install --yes --no-install-recommends 		gnupg 		wget 		xz-utils 	; 	rm -rf /var/lib/apt/lists/*; 		RABBITMQ_SOURCE_URL="https://github.com/rabbitmq/rabbitmq-server/releases/download/v$RABBITMQ_VERSION/rabbitmq-server-generic-unix-latest-toolchain-$RABBITMQ_VERSION.tar.xz"; 	RABBITMQ_PATH="/usr/local/src/rabbitmq-$RABBITMQ_VERSION"; 		wget --progress dot:giga --output-document "$RABBITMQ_PATH.tar.xz.asc" "$RABBITMQ_SOURCE_URL.asc"; 	wget --progress dot:giga --output-document "$RABBITMQ_PATH.tar.xz" "$RABBITMQ_SOURCE_URL"; 		export GNUPGHOME="$(mktemp -d)"; 	gpg --batch --keyserver hkps://keys.openpgp.org --recv-keys "$RABBITMQ_PGP_KEY_ID"; 	gpg --batch --verify "$RABBITMQ_PATH.tar.xz.asc" "$RABBITMQ_PATH.tar.xz"; 	gpgconf --kill all; 	rm -rf "$GNUPGHOME"; 		mkdir -p "$RABBITMQ_HOME"; 	tar --extract --file "$RABBITMQ_PATH.tar.xz" --directory "$RABBITMQ_HOME" --strip-components 1; 	rm -rf "$RABBITMQ_PATH"*; 	grep -qE '^SYS_PREFIX=\$\{RABBITMQ_HOME\}$' "$RABBITMQ_HOME/sbin/rabbitmq-defaults"; 	sed -i 's/^SYS_PREFIX=.*$/SYS_PREFIX=/' "$RABBITMQ_HOME/sbin/rabbitmq-defaults"; 	grep -qE '^SYS_PREFIX=$' "$RABBITMQ_HOME/sbin/rabbitmq-defaults"; 	chown -R rabbitmq:rabbitmq "$RABBITMQ_HOME"; 		apt-mark auto '.*' > /dev/null; 	apt-mark manual $savedAptMark; 	apt-get purge -y --auto-remove -o APT::AutoRemove::RecommendsImportant=false; 		[ ! -e "$RABBITMQ_DATA_DIR/.erlang.cookie" ]; 	gosu rabbitmq rabbitmqctl help; 	gosu rabbitmq rabbitmqctl list_ciphers; 	gosu rabbitmq rabbitmq-plugins list; 	rm "$RABBITMQ_DATA_DIR/.erlang.cookie" # buildkit
# Mon, 15 Jun 2026 23:43:02 GMT
RUN gosu rabbitmq rabbitmq-plugins enable --offline rabbitmq_prometheus # buildkit
# Mon, 15 Jun 2026 23:43:03 GMT
RUN ln -sf /opt/rabbitmq/plugins /plugins # buildkit
# Mon, 15 Jun 2026 23:43:03 GMT
ENV HOME=/var/lib/rabbitmq
# Mon, 15 Jun 2026 23:43:03 GMT
VOLUME [/var/lib/rabbitmq]
# Mon, 15 Jun 2026 23:43:03 GMT
ENV LANG=C.UTF-8 LANGUAGE=C.UTF-8 LC_ALL=C.UTF-8
# Mon, 15 Jun 2026 23:43:03 GMT
ENV RUNNING_UNDER_SYSTEMD=true
# Mon, 15 Jun 2026 23:43:03 GMT
COPY --chown=rabbitmq:rabbitmq 10-defaults.conf 20-management_agent.disable_metrics_collector.conf /etc/rabbitmq/conf.d/ # buildkit
# Mon, 15 Jun 2026 23:43:04 GMT
COPY docker-entrypoint.sh /usr/local/bin/ # buildkit
# Mon, 15 Jun 2026 23:43:04 GMT
ENTRYPOINT ["docker-entrypoint.sh"]
# Mon, 15 Jun 2026 23:43:04 GMT
EXPOSE map[15691/tcp:{} 15692/tcp:{} 25672/tcp:{} 4369/tcp:{} 5671/tcp:{} 5672/tcp:{}]
# Mon, 15 Jun 2026 23:43:04 GMT
CMD ["rabbitmq-server"]
```

-	Layers:
	-	`sha256:e091f822489caa06bb3d2fde38646b1d65be890bc1155c44ed55dc18ce539afc`  
		Last Modified: Wed, 20 May 2026 02:15:44 GMT  
		Size: 34.3 MB (34314099 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:33bf614c06fbc122bf0267e190edf10091febd27d0925b0fd926ce51395ecb9f`  
		Last Modified: Wed, 10 Jun 2026 20:38:31 GMT  
		Size: 39.5 MB (39526785 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:84508ebe5395fbdd1fbe1b01cb848d7621da4ef3e9426862b9175f403fae6c38`  
		Last Modified: Wed, 10 Jun 2026 20:38:30 GMT  
		Size: 9.6 MB (9606001 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:0e662f9c0642c8e8f98eb3b1245fec49a3df0459ca12b7ec4cda2eb22c0ccee2`  
		Last Modified: Wed, 10 Jun 2026 20:38:29 GMT  
		Size: 9.7 KB (9671 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:2c0140115c9512c54cd6f7bb6704892130920e229cc80c3528e0075ccfe96891`  
		Last Modified: Mon, 15 Jun 2026 23:48:43 GMT  
		Size: 31.3 MB (31297033 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:73dff97c2af9290632f6c5d1c0feeed67ddf8a3156afdfad73dc45ee26c739fd`  
		Last Modified: Mon, 15 Jun 2026 23:48:42 GMT  
		Size: 188.0 B  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:f975066390478d81a58886644a7bdec364a1a6c349dfa498e3eba1ada3391508`  
		Last Modified: Mon, 15 Jun 2026 23:48:42 GMT  
		Size: 108.0 B  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:a16f3f5c187117b45ec0f21fdf9fa215aec0169b705de5ee41aa4413a429d0ce`  
		Last Modified: Mon, 15 Jun 2026 23:48:42 GMT  
		Size: 622.0 B  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:3053c5db6da23caa5dea5f071a97baf16acf3ef0e8fc15b54719658ce7681fb6`  
		Last Modified: Mon, 15 Jun 2026 23:48:43 GMT  
		Size: 831.0 B  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip

### `rabbitmq:latest` - unknown; unknown

```console
$ docker pull rabbitmq@sha256:9a839a2913993f885ad9c3df3735c729b9cdcc57a6db850fee6c207d31f3ae39
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **18.8 MB (18755405 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:2c0418163cc3d3e8c8362d9bd89884fa67cf7804825132580ebfc532e141d117`

```dockerfile
```

-	Layers:
	-	`sha256:8d37b6ce6ec9f5f81ec748fe66fa593faf8cd592b90d170d2f9edce3c5677746`  
		Last Modified: Mon, 15 Jun 2026 23:48:42 GMT  
		Size: 2.5 MB (2491168 bytes)  
		MIME: application/vnd.in-toto+json
	-	`sha256:0ba836d557c0314f0f583d56239a83f87672f5b81f6cf59ce90917d7cdb94b93`  
		Last Modified: Mon, 15 Jun 2026 23:48:42 GMT  
		Size: 5.3 MB (5348471 bytes)  
		MIME: application/vnd.in-toto+json
	-	`sha256:88f474cc27fa75955c25c3f5916e8acd86a832e5142bb1fb6865608716230f9e`  
		Last Modified: Mon, 15 Jun 2026 23:48:43 GMT  
		Size: 5.5 MB (5505290 bytes)  
		MIME: application/vnd.in-toto+json
	-	`sha256:af34a97b450532ed3261594316869b5ed03f1f5665b11485b804f0cd1371e077`  
		Last Modified: Mon, 15 Jun 2026 23:48:42 GMT  
		Size: 5.4 MB (5350213 bytes)  
		MIME: application/vnd.in-toto+json
	-	`sha256:7a721edf24a46d9bf2d9a0b80e0f6332c962dd51c7ba7c3d2c57f8c3f64b04ad`  
		Last Modified: Mon, 15 Jun 2026 23:48:43 GMT  
		Size: 60.3 KB (60263 bytes)  
		MIME: application/vnd.in-toto+json

### `rabbitmq:latest` - linux; riscv64

```console
$ docker pull rabbitmq@sha256:69db79fa160ffa2104241b99c332c67e98661e71ecd6c87f62c2f7df83deb0ed
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **107.6 MB (107595085 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:a4c81f050519a3e41d18d012d243a302eca907e404589977a50c8712e5471aa5`
-	Entrypoint: `["docker-entrypoint.sh"]`
-	Default Command: `["rabbitmq-server"]`

```dockerfile
# Wed, 20 May 2026 02:06:08 GMT
ARG RELEASE
# Wed, 20 May 2026 02:06:08 GMT
ARG LAUNCHPAD_BUILD_ARCH
# Wed, 20 May 2026 02:06:09 GMT
LABEL org.opencontainers.image.version=24.04
# Wed, 20 May 2026 02:06:59 GMT
ADD file:f1fd7ee282731834f2f36b17e9b538e569ade4ce8b89924b635551ff3a45c706 in / 
# Wed, 20 May 2026 02:07:03 GMT
CMD ["/bin/bash"]
# Thu, 11 Jun 2026 19:52:50 GMT
ENV ERLANG_INSTALL_PATH_PREFIX=/opt/erlang
# Thu, 11 Jun 2026 19:52:50 GMT
ENV OPENSSL_INSTALL_PATH_PREFIX=/opt/openssl
# Thu, 11 Jun 2026 19:52:50 GMT
COPY /opt/erlang /opt/erlang # buildkit
# Thu, 11 Jun 2026 19:52:51 GMT
COPY /opt/openssl /opt/openssl # buildkit
# Thu, 11 Jun 2026 19:52:51 GMT
ENV PATH=/opt/erlang/bin:/opt/openssl/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
# Thu, 11 Jun 2026 19:52:51 GMT
ENV RABBITMQ_DATA_DIR=/var/lib/rabbitmq
# Thu, 11 Jun 2026 19:52:55 GMT
RUN set -eux; 	ln -vsf /etc/ssl/certs /etc/ssl/private "$OPENSSL_INSTALL_PATH_PREFIX/etc/ssl"; 		ldconfig; 	sed -i.ORIG -e "/\.include.*fips/ s!.*!.include $OPENSSL_INSTALL_PATH_PREFIX/etc/ssl/fipsmodule.cnf!" 		-e '/# fips =/s/.*/fips = fips_sect/' "$OPENSSL_INSTALL_PATH_PREFIX/etc/ssl/openssl.cnf"; 	sed -i.ORIG -e '/^activate/s/^/#/' "$OPENSSL_INSTALL_PATH_PREFIX/etc/ssl/fipsmodule.cnf"; 	[ "$(command -v openssl)" = "$OPENSSL_INSTALL_PATH_PREFIX/bin/openssl" ]; 	openssl version; 	openssl version -d; 		erl -noshell -eval 'ok = crypto:start(), ok = io:format("~p~n~n~p~n~n", [crypto:supports(), ssl:versions()]), init:stop().'; 		groupadd --gid 999 --system rabbitmq; 	useradd --uid 999 --system --home-dir "$RABBITMQ_DATA_DIR" --gid rabbitmq rabbitmq; 	mkdir -p "$RABBITMQ_DATA_DIR" /etc/rabbitmq /etc/rabbitmq/conf.d /tmp/rabbitmq-ssl /var/log/rabbitmq; 	chown -fR rabbitmq:rabbitmq "$RABBITMQ_DATA_DIR" /etc/rabbitmq /etc/rabbitmq/conf.d /tmp/rabbitmq-ssl /var/log/rabbitmq; 	chmod 1777 "$RABBITMQ_DATA_DIR" /etc/rabbitmq /etc/rabbitmq/conf.d /tmp/rabbitmq-ssl /var/log/rabbitmq; 	ln -sf "$RABBITMQ_DATA_DIR/.erlang.cookie" /root/.erlang.cookie # buildkit
# Thu, 11 Jun 2026 19:52:55 GMT
ENV RABBITMQ_VERSION=4.3.1
# Thu, 11 Jun 2026 19:52:55 GMT
ENV RABBITMQ_PGP_KEY_ID=0x0A9AF2115F4687BD29803A206B73A36E6026DFCA
# Thu, 11 Jun 2026 19:52:55 GMT
ENV RABBITMQ_HOME=/opt/rabbitmq
# Thu, 11 Jun 2026 19:52:55 GMT
ENV PATH=/opt/rabbitmq/sbin:/opt/erlang/bin:/opt/openssl/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
# Thu, 11 Jun 2026 19:55:09 GMT
RUN set -eux; 	export DEBIAN_FRONTEND=noninteractive; 	apt-get update; 	apt-get install --yes --no-install-recommends 		ca-certificates 		gosu 		tzdata 	; 	gosu nobody true; 		savedAptMark="$(apt-mark showmanual)"; 	apt-get install --yes --no-install-recommends 		gnupg 		wget 		xz-utils 	; 	rm -rf /var/lib/apt/lists/*; 		RABBITMQ_SOURCE_URL="https://github.com/rabbitmq/rabbitmq-server/releases/download/v$RABBITMQ_VERSION/rabbitmq-server-generic-unix-latest-toolchain-$RABBITMQ_VERSION.tar.xz"; 	RABBITMQ_PATH="/usr/local/src/rabbitmq-$RABBITMQ_VERSION"; 		wget --progress dot:giga --output-document "$RABBITMQ_PATH.tar.xz.asc" "$RABBITMQ_SOURCE_URL.asc"; 	wget --progress dot:giga --output-document "$RABBITMQ_PATH.tar.xz" "$RABBITMQ_SOURCE_URL"; 		export GNUPGHOME="$(mktemp -d)"; 	gpg --batch --keyserver hkps://keys.openpgp.org --recv-keys "$RABBITMQ_PGP_KEY_ID"; 	gpg --batch --verify "$RABBITMQ_PATH.tar.xz.asc" "$RABBITMQ_PATH.tar.xz"; 	gpgconf --kill all; 	rm -rf "$GNUPGHOME"; 		mkdir -p "$RABBITMQ_HOME"; 	tar --extract --file "$RABBITMQ_PATH.tar.xz" --directory "$RABBITMQ_HOME" --strip-components 1; 	rm -rf "$RABBITMQ_PATH"*; 	grep -qE '^SYS_PREFIX=\$\{RABBITMQ_HOME\}$' "$RABBITMQ_HOME/sbin/rabbitmq-defaults"; 	sed -i 's/^SYS_PREFIX=.*$/SYS_PREFIX=/' "$RABBITMQ_HOME/sbin/rabbitmq-defaults"; 	grep -qE '^SYS_PREFIX=$' "$RABBITMQ_HOME/sbin/rabbitmq-defaults"; 	chown -R rabbitmq:rabbitmq "$RABBITMQ_HOME"; 		apt-mark auto '.*' > /dev/null; 	apt-mark manual $savedAptMark; 	apt-get purge -y --auto-remove -o APT::AutoRemove::RecommendsImportant=false; 		[ ! -e "$RABBITMQ_DATA_DIR/.erlang.cookie" ]; 	gosu rabbitmq rabbitmqctl help; 	gosu rabbitmq rabbitmqctl list_ciphers; 	gosu rabbitmq rabbitmq-plugins list; 	rm "$RABBITMQ_DATA_DIR/.erlang.cookie" # buildkit
# Thu, 11 Jun 2026 19:55:18 GMT
RUN gosu rabbitmq rabbitmq-plugins enable --offline rabbitmq_prometheus # buildkit
# Thu, 11 Jun 2026 19:55:19 GMT
RUN ln -sf /opt/rabbitmq/plugins /plugins # buildkit
# Thu, 11 Jun 2026 19:55:19 GMT
ENV HOME=/var/lib/rabbitmq
# Thu, 11 Jun 2026 19:55:19 GMT
VOLUME [/var/lib/rabbitmq]
# Thu, 11 Jun 2026 19:55:19 GMT
ENV LANG=C.UTF-8 LANGUAGE=C.UTF-8 LC_ALL=C.UTF-8
# Thu, 11 Jun 2026 19:55:19 GMT
ENV RUNNING_UNDER_SYSTEMD=true
# Thu, 11 Jun 2026 19:55:19 GMT
COPY --chown=rabbitmq:rabbitmq 10-defaults.conf 20-management_agent.disable_metrics_collector.conf /etc/rabbitmq/conf.d/ # buildkit
# Thu, 11 Jun 2026 19:55:19 GMT
COPY docker-entrypoint.sh /usr/local/bin/ # buildkit
# Thu, 11 Jun 2026 19:55:19 GMT
ENTRYPOINT ["docker-entrypoint.sh"]
# Thu, 11 Jun 2026 19:55:19 GMT
EXPOSE map[15691/tcp:{} 15692/tcp:{} 25672/tcp:{} 4369/tcp:{} 5671/tcp:{} 5672/tcp:{}]
# Thu, 11 Jun 2026 19:55:19 GMT
CMD ["rabbitmq-server"]
```

-	Layers:
	-	`sha256:924f9a731915e06f77b3487378ddf9426f8422fa1d96461bef1d0e0a35c36743`  
		Last Modified: Wed, 20 May 2026 02:15:52 GMT  
		Size: 31.0 MB (30965919 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:364f651034a232609cbea91ad41b4ff50e7dc5b6cc856d0bf0e10e256dc86d9d`  
		Last Modified: Thu, 11 Jun 2026 20:01:41 GMT  
		Size: 35.2 MB (35179929 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:0d19cf499478a36604bb25679e65cdb45042180b48fcb2fb26738277c2a3110c`  
		Last Modified: Thu, 11 Jun 2026 20:01:34 GMT  
		Size: 10.8 MB (10842287 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:2873716d6b7b677351bf94c5315afc728683322bb70407f341ddbce93a09b206`  
		Last Modified: Thu, 11 Jun 2026 20:01:27 GMT  
		Size: 9.7 KB (9678 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:4cfb78363cce8251384dd8d6e4a06c83944dadc0c456c3d05e07665c47b7afe7`  
		Last Modified: Thu, 11 Jun 2026 20:01:41 GMT  
		Size: 30.6 MB (30595519 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:df4b9de52ecc3c5e5999dfca1e8b1303f75b94a4217f6527cf360981e6a09985`  
		Last Modified: Thu, 11 Jun 2026 20:01:31 GMT  
		Size: 189.0 B  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:513e91fa9c08d0b85fc7c55b9767a2a483b678d984f728c876a96a753dc847e4`  
		Last Modified: Thu, 11 Jun 2026 20:01:34 GMT  
		Size: 109.0 B  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:7e0ddc4e607c71087c630ec6bf0663d223ebef25fece4647d211f04aab960f0a`  
		Last Modified: Thu, 11 Jun 2026 20:01:36 GMT  
		Size: 622.0 B  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:c586d23be735766dec7d2088dc2c54bb26e2726b3ab8f353809a24bfc4c884c7`  
		Last Modified: Thu, 11 Jun 2026 20:01:36 GMT  
		Size: 833.0 B  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip

### `rabbitmq:latest` - unknown; unknown

```console
$ docker pull rabbitmq@sha256:7bd88307c9f5ed78a571549704d7bd460f84aaca5f865f38cb545fe831e2219e
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **18.7 MB (18724000 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:b2621a3e073d89158825f1cb6cf2ca6574f3af3f6ee24213dfefeb8779ce00c9`

```dockerfile
```

-	Layers:
	-	`sha256:f57b254bd1e3ef2b5b9feee934bd7512236b71825e6cb64756b10f879769f687`  
		Last Modified: Thu, 11 Jun 2026 20:01:29 GMT  
		Size: 2.5 MB (2479082 bytes)  
		MIME: application/vnd.in-toto+json
	-	`sha256:d40bc94c6ece3cd6d36c68826a01e805c2124f167dc160cd282368adcdf5e46b`  
		Last Modified: Thu, 11 Jun 2026 20:01:32 GMT  
		Size: 5.3 MB (5342892 bytes)  
		MIME: application/vnd.in-toto+json
	-	`sha256:b464291100e58f1c14b35395bb3a8952fd80fbfc22837b049d9c7505f29bef8a`  
		Last Modified: Thu, 11 Jun 2026 20:01:32 GMT  
		Size: 5.5 MB (5497122 bytes)  
		MIME: application/vnd.in-toto+json
	-	`sha256:f65375e2c30465ec52f1d0f08a9afa865aaa7733e5c8d8acdcdd9267332aa311`  
		Last Modified: Thu, 11 Jun 2026 20:01:33 GMT  
		Size: 5.3 MB (5344634 bytes)  
		MIME: application/vnd.in-toto+json
	-	`sha256:4488e0d7d183b7be4e7a4e75150df7d0ab24294fb352d8fb2d0f41952914db6c`  
		Last Modified: Thu, 11 Jun 2026 20:01:33 GMT  
		Size: 60.3 KB (60270 bytes)  
		MIME: application/vnd.in-toto+json

### `rabbitmq:latest` - linux; s390x

```console
$ docker pull rabbitmq@sha256:6db9ea97d84a5d0f06be0df52dfe4e05ebd5bc6c1580f331db1f852353a9e6e1
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **107.9 MB (107924286 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:473e255a9157b4b34da20913ef81564ad25e038f3362363957e0390403c51b06`
-	Entrypoint: `["docker-entrypoint.sh"]`
-	Default Command: `["rabbitmq-server"]`

```dockerfile
# Wed, 20 May 2026 01:37:09 GMT
ARG RELEASE
# Wed, 20 May 2026 01:37:09 GMT
ARG LAUNCHPAD_BUILD_ARCH
# Wed, 20 May 2026 01:37:09 GMT
LABEL org.opencontainers.image.version=24.04
# Wed, 20 May 2026 01:37:11 GMT
ADD file:b574b1e436c2db4e4d66f69c75e47a9aebf0da1ad375147eb2c0b7ff76c6ab7e in / 
# Wed, 20 May 2026 01:37:11 GMT
CMD ["/bin/bash"]
# Wed, 10 Jun 2026 20:46:01 GMT
ENV ERLANG_INSTALL_PATH_PREFIX=/opt/erlang
# Wed, 10 Jun 2026 20:46:01 GMT
ENV OPENSSL_INSTALL_PATH_PREFIX=/opt/openssl
# Wed, 10 Jun 2026 20:46:01 GMT
COPY /opt/erlang /opt/erlang # buildkit
# Wed, 10 Jun 2026 20:46:01 GMT
COPY /opt/openssl /opt/openssl # buildkit
# Wed, 10 Jun 2026 20:46:01 GMT
ENV PATH=/opt/erlang/bin:/opt/openssl/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
# Wed, 10 Jun 2026 20:46:01 GMT
ENV RABBITMQ_DATA_DIR=/var/lib/rabbitmq
# Wed, 10 Jun 2026 20:46:03 GMT
RUN set -eux; 	ln -vsf /etc/ssl/certs /etc/ssl/private "$OPENSSL_INSTALL_PATH_PREFIX/etc/ssl"; 		ldconfig; 	sed -i.ORIG -e "/\.include.*fips/ s!.*!.include $OPENSSL_INSTALL_PATH_PREFIX/etc/ssl/fipsmodule.cnf!" 		-e '/# fips =/s/.*/fips = fips_sect/' "$OPENSSL_INSTALL_PATH_PREFIX/etc/ssl/openssl.cnf"; 	sed -i.ORIG -e '/^activate/s/^/#/' "$OPENSSL_INSTALL_PATH_PREFIX/etc/ssl/fipsmodule.cnf"; 	[ "$(command -v openssl)" = "$OPENSSL_INSTALL_PATH_PREFIX/bin/openssl" ]; 	openssl version; 	openssl version -d; 		erl -noshell -eval 'ok = crypto:start(), ok = io:format("~p~n~n~p~n~n", [crypto:supports(), ssl:versions()]), init:stop().'; 		groupadd --gid 999 --system rabbitmq; 	useradd --uid 999 --system --home-dir "$RABBITMQ_DATA_DIR" --gid rabbitmq rabbitmq; 	mkdir -p "$RABBITMQ_DATA_DIR" /etc/rabbitmq /etc/rabbitmq/conf.d /tmp/rabbitmq-ssl /var/log/rabbitmq; 	chown -fR rabbitmq:rabbitmq "$RABBITMQ_DATA_DIR" /etc/rabbitmq /etc/rabbitmq/conf.d /tmp/rabbitmq-ssl /var/log/rabbitmq; 	chmod 1777 "$RABBITMQ_DATA_DIR" /etc/rabbitmq /etc/rabbitmq/conf.d /tmp/rabbitmq-ssl /var/log/rabbitmq; 	ln -sf "$RABBITMQ_DATA_DIR/.erlang.cookie" /root/.erlang.cookie # buildkit
# Wed, 10 Jun 2026 20:46:03 GMT
ENV RABBITMQ_VERSION=4.3.2
# Wed, 10 Jun 2026 20:46:03 GMT
ENV RABBITMQ_PGP_KEY_ID=0x0A9AF2115F4687BD29803A206B73A36E6026DFCA
# Wed, 10 Jun 2026 20:46:03 GMT
ENV RABBITMQ_HOME=/opt/rabbitmq
# Wed, 10 Jun 2026 20:46:03 GMT
ENV PATH=/opt/rabbitmq/sbin:/opt/erlang/bin:/opt/openssl/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
# Mon, 15 Jun 2026 23:26:42 GMT
RUN set -eux; 	export DEBIAN_FRONTEND=noninteractive; 	apt-get update; 	apt-get install --yes --no-install-recommends 		ca-certificates 		gosu 		tzdata 	; 	gosu nobody true; 		savedAptMark="$(apt-mark showmanual)"; 	apt-get install --yes --no-install-recommends 		gnupg 		wget 		xz-utils 	; 	rm -rf /var/lib/apt/lists/*; 		RABBITMQ_SOURCE_URL="https://github.com/rabbitmq/rabbitmq-server/releases/download/v$RABBITMQ_VERSION/rabbitmq-server-generic-unix-latest-toolchain-$RABBITMQ_VERSION.tar.xz"; 	RABBITMQ_PATH="/usr/local/src/rabbitmq-$RABBITMQ_VERSION"; 		wget --progress dot:giga --output-document "$RABBITMQ_PATH.tar.xz.asc" "$RABBITMQ_SOURCE_URL.asc"; 	wget --progress dot:giga --output-document "$RABBITMQ_PATH.tar.xz" "$RABBITMQ_SOURCE_URL"; 		export GNUPGHOME="$(mktemp -d)"; 	gpg --batch --keyserver hkps://keys.openpgp.org --recv-keys "$RABBITMQ_PGP_KEY_ID"; 	gpg --batch --verify "$RABBITMQ_PATH.tar.xz.asc" "$RABBITMQ_PATH.tar.xz"; 	gpgconf --kill all; 	rm -rf "$GNUPGHOME"; 		mkdir -p "$RABBITMQ_HOME"; 	tar --extract --file "$RABBITMQ_PATH.tar.xz" --directory "$RABBITMQ_HOME" --strip-components 1; 	rm -rf "$RABBITMQ_PATH"*; 	grep -qE '^SYS_PREFIX=\$\{RABBITMQ_HOME\}$' "$RABBITMQ_HOME/sbin/rabbitmq-defaults"; 	sed -i 's/^SYS_PREFIX=.*$/SYS_PREFIX=/' "$RABBITMQ_HOME/sbin/rabbitmq-defaults"; 	grep -qE '^SYS_PREFIX=$' "$RABBITMQ_HOME/sbin/rabbitmq-defaults"; 	chown -R rabbitmq:rabbitmq "$RABBITMQ_HOME"; 		apt-mark auto '.*' > /dev/null; 	apt-mark manual $savedAptMark; 	apt-get purge -y --auto-remove -o APT::AutoRemove::RecommendsImportant=false; 		[ ! -e "$RABBITMQ_DATA_DIR/.erlang.cookie" ]; 	gosu rabbitmq rabbitmqctl help; 	gosu rabbitmq rabbitmqctl list_ciphers; 	gosu rabbitmq rabbitmq-plugins list; 	rm "$RABBITMQ_DATA_DIR/.erlang.cookie" # buildkit
# Mon, 15 Jun 2026 23:26:43 GMT
RUN gosu rabbitmq rabbitmq-plugins enable --offline rabbitmq_prometheus # buildkit
# Mon, 15 Jun 2026 23:26:43 GMT
RUN ln -sf /opt/rabbitmq/plugins /plugins # buildkit
# Mon, 15 Jun 2026 23:26:43 GMT
ENV HOME=/var/lib/rabbitmq
# Mon, 15 Jun 2026 23:26:43 GMT
VOLUME [/var/lib/rabbitmq]
# Mon, 15 Jun 2026 23:26:43 GMT
ENV LANG=C.UTF-8 LANGUAGE=C.UTF-8 LC_ALL=C.UTF-8
# Mon, 15 Jun 2026 23:26:43 GMT
ENV RUNNING_UNDER_SYSTEMD=true
# Mon, 15 Jun 2026 23:26:43 GMT
COPY --chown=rabbitmq:rabbitmq 10-defaults.conf 20-management_agent.disable_metrics_collector.conf /etc/rabbitmq/conf.d/ # buildkit
# Mon, 15 Jun 2026 23:26:43 GMT
COPY docker-entrypoint.sh /usr/local/bin/ # buildkit
# Mon, 15 Jun 2026 23:26:43 GMT
ENTRYPOINT ["docker-entrypoint.sh"]
# Mon, 15 Jun 2026 23:26:43 GMT
EXPOSE map[15691/tcp:{} 15692/tcp:{} 25672/tcp:{} 4369/tcp:{} 5671/tcp:{} 5672/tcp:{}]
# Mon, 15 Jun 2026 23:26:43 GMT
CMD ["rabbitmq-server"]
```

-	Layers:
	-	`sha256:c8ebd0a624851e8502e41ee64db2b6a45537554969784d82ebbc91c905cbc2ef`  
		Last Modified: Wed, 20 May 2026 02:16:00 GMT  
		Size: 29.9 MB (29912835 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:7bfaa6666bbc5e849451dd751034923302135005a9d0d2503672c64c09398443`  
		Last Modified: Wed, 10 Jun 2026 20:47:31 GMT  
		Size: 38.6 MB (38618097 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:4a4566ec9083f520f89df3be54e6f8c5d59c867318de832ca73e46301cf30582`  
		Last Modified: Wed, 10 Jun 2026 20:47:31 GMT  
		Size: 8.6 MB (8623462 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:b0cd5e698d7f8a4cffa989650cebc9806ae0e3b1673a3d4ea46591d09fccb089`  
		Last Modified: Wed, 10 Jun 2026 20:47:30 GMT  
		Size: 9.8 KB (9813 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:e913268d19bc80beca9655e33fe536209b3b70b5688c3aaed9aec6653ee38e5b`  
		Last Modified: Mon, 15 Jun 2026 23:34:02 GMT  
		Size: 30.8 MB (30758339 bytes)  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:8628cca1f8436272a95f85fe0de7d0c874b51222796be5dc1bbdc6b7b2a8003d`  
		Last Modified: Mon, 15 Jun 2026 23:34:01 GMT  
		Size: 188.0 B  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:508dac3e569d760711de9d5570be9dd0e7d3c0347433e076992c26e99442dc1d`  
		Last Modified: Mon, 15 Jun 2026 23:34:01 GMT  
		Size: 109.0 B  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:d972a843d407bed46c89d403332395acc5a906a5ade549aa73c3db1a8372966f`  
		Last Modified: Mon, 15 Jun 2026 23:34:02 GMT  
		Size: 614.0 B  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip
	-	`sha256:aa9d226cf2604e1e7e05f02726fcbd2203c3cae82729da0a2eb0f799685b2bc8`  
		Last Modified: Mon, 15 Jun 2026 23:34:03 GMT  
		Size: 829.0 B  
		MIME: application/vnd.oci.image.layer.v1.tar+gzip

### `rabbitmq:latest` - unknown; unknown

```console
$ docker pull rabbitmq@sha256:6fd8864c0a0517c68e6d2a33b43ed1d4fd26cf110149283aa54e2e6934746e39
```

-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **18.4 MB (18381144 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:9c91b841a4fb455d6f722b10967a8fc35b1d8dc3dffbe98fa2a6b6f66f7da37d`

```dockerfile
```

-	Layers:
	-	`sha256:27ab25cb5cd0549f4962a22398c260145b3f723aa0e496f7b81211bf50a51f46`  
		Last Modified: Mon, 15 Jun 2026 23:34:02 GMT  
		Size: 2.5 MB (2488825 bytes)  
		MIME: application/vnd.in-toto+json
	-	`sha256:6c9b9a6ec019165065db39ef609a7bb021e7016634e05ed0a943cabea5404d80`  
		Last Modified: Mon, 15 Jun 2026 23:34:02 GMT  
		Size: 5.2 MB (5224964 bytes)  
		MIME: application/vnd.in-toto+json
	-	`sha256:33d7ebcc5a912eb8ba96dec3ac11a2d1d8d1d809762497b9008ece9dc9d9e2e1`  
		Last Modified: Mon, 15 Jun 2026 23:34:02 GMT  
		Size: 5.4 MB (5380448 bytes)  
		MIME: application/vnd.in-toto+json
	-	`sha256:f060f1f61735399c2ee3830da710bdf5907dcadb1a50c895bf8405362907b038`  
		Last Modified: Mon, 15 Jun 2026 23:34:02 GMT  
		Size: 5.2 MB (5226706 bytes)  
		MIME: application/vnd.in-toto+json
	-	`sha256:e05c544d659dbe6ab21bb589e24c42ae6b451ba5d19930e690a73be756c4f3bf`  
		Last Modified: Mon, 15 Jun 2026 23:34:03 GMT  
		Size: 60.2 KB (60201 bytes)  
		MIME: application/vnd.in-toto+json
